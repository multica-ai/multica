...
```ini
MULTICA_DINGTALK_AUTH_CLIENT_ID=dingxxxxxxxx
MULTICA_DINGTALK_AUTH_CLIENT_SECRET=<app-secret>
...
The `MULTICA_DINGTALK_AUTH_` prefix separates these login credentials from the
DingTalk bot integration's `MULTICA_DINGTALK_*` variables (a different feature).
`docker-compose.selfhost.yml` passes both `MULTICA_DINGTALK_AUTH_*` variables through to
