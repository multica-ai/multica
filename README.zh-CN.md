...
```bash
cs-workflow setup          # 连接 Multica Cloud，登录，启动 daemon
```
...
```bash
cs-workflow setup           # 配置、认证、启动 daemon（一条命令搞定）
```
...
在看板上创建一个 Issue（或通过 `cs-workflow issue create` 命令创建），然后将其分配给你的新 Agent。Agent 会自动接手任务、在你的 Runtime 上执行、并实时汇报进度——就像一个真正的队友一样。
