# Wallts installer for Windows — one command to get started.
#
# Install CLI (default): connects to wallts.ai
#   irm https://raw.githubusercontent.com/dwickyfp/wallts/main/scripts/install.ps1 | iex
#
# Self-host: starts a local Wallts server + installs CLI + configures
#   $env:WALLTS_MODE="local"; irm https://raw.githubusercontent.com/dwickyfp/wallts/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/dwickyfp/wallts.git"
$RepoWebUrl    = "https://github.com/dwickyfp/wallts"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".wallts\server"
$InstallDir    = if ($env:WALLTS_INSTALL_DIR) { $env:WALLTS_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/dwickyfp/wallts/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:WALLTS_SELFHOST_REF) {
        return $env:WALLTS_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = wallts version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Wallts CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Wallts requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/dwickyfp/wallts/releases/download/$latest/wallts-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "wallts-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "wallts.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/dwickyfp/wallts/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "wallts.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "wallts-cli-$version-windows-$arch.zip"
        $legacyAsset = "wallts_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "wallts.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".wallts\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "wallts.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "wallts.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "wallts.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "wallts.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Wallts CLI installed to $binDir\wallts.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "wallts") {
        ...
        if ($isUpToDate) {
            Write-Ok "Wallts CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Wallts CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Wallts CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "wallts")) {
        Write-Fail "CLI installed but 'wallts' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Wallts self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:WALLTS_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up Wallts server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Wallts repository..."
    ...
    Write-Info "Pulling official Wallts images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting Wallts services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Wallts server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Wallts - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Wallts CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     wallts setup               " -NoNewline; Write-Host "# Connect to Wallts Cloud (wallts.ai)" -ForegroundColor DarkGray
    Write-Host "     wallts setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:WALLTS_MODE="with-server"; irm https://raw.githubusercontent.com/dwickyfp/wallts/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Wallts - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Wallts server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     wallts setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:WALLTS_MODE="stop"; irm https://raw.githubusercontent.com/dwickyfp/wallts/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Wallts services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Wallts installation found at $InstallDir"
    }

    if (Test-CommandExists "wallts") {
        try {
            wallts daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:WALLTS_MODE) { $env:WALLTS_MODE.ToLower() } else { "default" }
