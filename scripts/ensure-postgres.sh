...
if is_local; then
  ...
  (cd "$REPO_ROOT" && bash "$COMPOSE_SH" up -d postgres)

  echo "==> Waiting for PostgreSQL to be ready..."
  until (cd "$REPO_ROOT" && bash "$COMPOSE_SH" exec -T postgres pg_isready -U "$POSTGRES_USER" -d postgres > /dev/null 2>&1); do
    sleep 1
  done

  echo "==> Ensuring database '$POSTGRES_DB' exists..."
  db_exists="$(cd "$REPO_ROOT" && bash "$COMPOSE_SH" exec -T postgres \
    psql -U "$POSTGRES_USER" -d postgres -Atqc "SELECT 1 FROM pg_database WHERE datname = '$POSTGRES_DB'")"

  if [ "$db_exists" != "1" ]; then
    cd "$REPO_ROOT" && bash "$COMPOSE_SH" exec -T postgres \
      psql -U "$POSTGRES_USER" -d postgres -v ON_ERROR_STOP=1 \
      -c "CREATE DATABASE \"$POSTGRES_DB\"" \
      > /dev/null
  fi

  echo "✓ PostgreSQL ready (local containers). Database: $POSTGRES_DB"
else
  # ---------- Remote: skip Docker, verify connectivity ----------
  echo "==> Remote database detected (host: $db_host). Skipping local Compose / Postgres container."
