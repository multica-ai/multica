...
run_with_server() {
  ...
  # CEREBRO-PATCH(no-master-code): cerebro removed 888888; show the docker logs path explicitly.
  printf "  or read the verification code from the backend container log:\n"
  printf "     ${CYAN}docker compose -f docker-compose.selfhost.yml logs backend${RESET}\n"
  printf "\n"
