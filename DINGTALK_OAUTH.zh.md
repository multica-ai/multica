...
MULTICA_DINGTALK_AUTH_CLIENT_SECRET=<应用密钥>
...
`MULTICA_DINGTALK_AUTH_` 前缀用于和钉钉机器人集成的 `MULTICA_DINGTALK_*` 变量区分（那是另一个功能）。`docker-compose.selfhost.yml` 会把这两个 `MULTICA_DINGTALK_AUTH_*` 变量透传给后端。改完重启生效：
