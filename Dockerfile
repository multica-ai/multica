...
RUN cd server && CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=${VERSION} -X main.commit=${COMMIT}" -o bin/cs-workflow ./cmd/cs-workflow
...
COPY --from=builder /src/server/bin/cs-workflow .
