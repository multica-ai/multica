...
- **如无必要，勿真实体**：优先用 view、字段、筛选、摘要、建议或 issue 扩展表达能力；只有这些都不能清晰承载时，才引入新持久实体。

## Entity Discipline

- 所有 workflow 设计必须先写运行时对象矩阵，再写页面、API 和数据库。
- 新表如果能被用户开始、完成、跳过、排序、复盘、分配，或能作为执行启动点，默认判定为新的可执行对象，必须升级到 product decision。
- 稳定核心和实验表面必须拆 spec、拆迁移、拆提交。
- active OpenSpec 只保留当前方向；暂停路线必须删除，或明确标记为不可实现入口。
- 设计时先问：这个能力能不能只是 issue 的视图、字段、筛选、摘要或建议？只有答案明确是否定，才考虑新对象。
