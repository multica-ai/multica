Purpose: Verify that DeepSeek TUI backend integration works as an agent runtime, using the native thread protocol instead of ACP.
...
User flow: Navigate to the Agents page. Create a new agent or edit an existing one. In the runtime picker, select a DeepSeek-TUI runtime. Save the agent configuration. Create an issue and @mention this agent. Wait for the agent to execute.
