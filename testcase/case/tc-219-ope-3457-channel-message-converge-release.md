...
- **收敛（主消息 → 某消息的回复）**: 在主消息区长按某条消息后拖动到另一条主消息上，被拖消息加入目标消息的回复线程。
- **释放（回复 → 主消息区）**: 在回复区长按某条回复后拖动到主消息区域，被拖回复回到主消息区。
...
- **`created_at` 保持不变，`order_at` 同步为 `created_at`**：列表按 `order_at ASC` 排序，UI 显示 `created_at`；收敛/释放后两者对齐，消息按原始发送时间插入新位置，不会显示旧时间却排在最新。
