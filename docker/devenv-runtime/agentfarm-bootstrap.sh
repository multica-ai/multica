...
# Trim devenv- prefix: namespace is "devenv-<slug>", device name must be "devenv-<slug>"
# so the slug variable holds just the slug portion for reuse.
SLUG="${WORKSPACE_SLUG#devenv-}"
DEVICE_NAME="devenv-${SLUG}"
...
#    stable so re-runs reuse the same daemon row rather than create duplicates.
#    The daemon stays in background; opencode serve (exec'd by entrypoint.sh) is PID 1.
multica daemon start --device-name "${DEVICE_NAME}" &

# ── 3. Wait for the claude runtime to register (≤60s). ───────────────────────
CLAUDE_RUNTIME_ID=""
