 ...
 | 版本 | 日期 | 说明 |
 | --- | --- | --- |
 | v1 | 2026-06-26 | 首次编写，基于现有 spec.md（运行时解绑）及所有 squad 运行时 selector 历史文档合并精简 |
 ...
 在此框架下，`spec.md` 已规定 issue 创建/分配时若 assignee 为 agent，前端应展现运行时选择器，让用户从自己拥有的兼容运行时中选一个。
 
 **本 spec 补全 squad 分配路径**：当 assignee 为 squad（小队）时，后端将任务路由到 squad 的 leader agent。前端需要展现运行时选择器——让当前用户选择用哪个兼容运行时执行 leader agent 的任务。
 
 ## 问题
 
 创建 issue 时，若 assignee 选为 squad，前端不会展现运行时选择器。对比：若 assignee 直接选 agent，前端会展现运行时下拉框，让用户从自己拥有的、与该 agent 兼容的在线本地运行时中选一个——这与 `spec.md` 中"用户 B 触发分配时使用 B 的运行时"的设计一致。squad 路径缺失了这一环节。
 ...
 以下问题已在之前迭代中修复，本 spec 基于修复后的状态：
 
 - `ListSquadLeaderCompatibleRuntimes` handler 改为查询请求用户自身的兼容运行时（而非 leader agent owner 的运行时）
 - `resolveRuntimeForTask` 的 explicit choice 路径校验运行时属于请求者，而非 agent owner
 - Chat/Autopilot 分支加载 project resources
 - `daemon_id` 不匹配导致 local_directory 不可用
 ...
 4. 若有兼容运行时，展现运行时下拉选择器（与 agent assignee 路径视觉一致）——多个时默认选中排序后的第一个
 ...
 **无需改动。** 现有链路已完整支持：
 ...
 - `server/internal/service/issue.go`：`enqueueSquadLeaderTask` → `EnqueueTaskForSquadLeaderByRequesterWithRuntime` 已接收 `selectedRuntimeID`
 - `server/internal/service/task.go`：`resolveRuntimeForTask` 覆盖 owner 校验和兼容性匹配，explicit choice 路径校验运行时属于请求者
 - `server/internal/handler/squad.go`：`ListSquadLeaderCompatibleRuntimes` 已改为查询请求者自身的兼容运行时
 ...
 | # | 改动 | 说明 |
 | --- | --- | --- |
 | 1 | 引入 `squadListOptions` | 在 `ManualCreatePanel` 中查询 squad 列表 |
 | 2 | 新增 `selectedAgentForRuntime` useMemo | agent 路径：按 `assigneeId` 查 agents；squad 路径：按 `leader_id` 查 leader agent |
 | 3 | 修改 `compatibleRuntimes` deps | 依赖从 `selectedAgent` 改为 `selectedAgentForRuntime` |
 | 4 | 修改 `selectedRuntimeId` 清空条件 | `assigneeType !== "agent" && assigneeType !== "squad"` 才清空 |
 | 5 | 提交时 `runtime_id` 写入条件 | `assigneeType === "agent" || assigneeType === "squad"` |
 | 6 | 运行时选择器渲染条件 | `assigneeType === "agent" || assigneeType === "squad"` |
 | 7 | `manualCreateSubmitDisabled` | squad 路径下也要求 `selectedRuntimeId` 非空 |
 | 8 | 提交 payload 中 squad 标识 | squad 路径下传 `squad_id` 而非 `agent_id` |
 
 #### `packages/core/types/api.ts`
 
 | # | 改动 | 说明 |
 | --- | --- | --- |
 | 1 | `CreateIssueRequest.runtime_id` 注释扩展 | "Per-run runtime selection for agent / squad assignees (squad path resolves via leader agent)" |
 | 2 | `UpdateIssueRequest.runtime_id` 注释扩展 | 同上 |
 
 #### `packages/views/modals/create-issue.test.tsx`
 
 | # | 改动 | 说明 |
 | --- | --- | --- |
 | 1 | 新增 `mockSquadsData` hoisted 数据 | `squad-1` 包含 `leader_id: "agent-1"` |
 | 2 | `vi.mock` 中注入 `squadListOptions` | mock 返回 `mockSquadsData.list` |
 | 3 | "forwards the picked squad when switching to agent mode" 测试 | 验证 squad → agent 切换时保留 squad_id |
 | 4 | `describe("squad runtime selector")` 测试块 | 4 个测试用例 |
