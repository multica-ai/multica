...
let startErrorDetail: string | null = null;
...
/**
 * Reads the last few lines of the daemon log file to surface startup failure
 * context. Used when the daemon was spawned but never reached /health — the
 * log is the only source of diagnostic info (the process exited too fast for
 * any other signal). Returns a truncated string suitable for UI display, or
 * null when the log doesn't exist or is empty.
 */
async function readDaemonLogTail(
  ...
  try {
    ...
    try {
      ...
      const startAt = stats.size - readSize;
      const buf = Buffer.alloc(readSize);
      const { bytesRead } = await handle.read(buf, 0, readSize, startAt);
      const text = buf.subarray(0, bytesRead).toString("utf-8");
      // Return last meaningful lines (skip trailing blank lines).
...
async function fetchHealth(): Promise<DaemonStatus> {
  ...
  if (!data || data.status !== "running") {
    ...
    // If we've been in "starting" past the auth probe window and the daemon
    // still isn't serving /health, it likely crashed on startup. Surface a
    // "start_error" state with the last lines of the log so the user can see
    // what went wrong (e.g. missing config, network error, process crash).
    if (
      currentState === "starting" &&
      authProbeDone &&
      startingSince !== null &&
      Date.now() - startingSince >= AUTH_PROBE_GRACE_MS + 5_000 &&
      !startErrorDetail
    ) {
      const logTail = await readDaemonLogTail(active.name);
      if (logTail) {
        startErrorDetail = logTail;
        currentState = "start_error";
      }
    }
    if (currentState === "start_error") {
      return {
        state: "start_error",
        profile: active.name,
        startError: startErrorDetail ?? undefined,
      };
    }
    return {
