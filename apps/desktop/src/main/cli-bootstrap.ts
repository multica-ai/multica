...
// Desktop prefers the bundled `wallts` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/dwickyfp/wallts/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "wallts.exe" : "wallts";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `wallts-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `wallts` binary. If one is already present at
