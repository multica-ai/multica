...
// wallts CLI can find agent binaries like claude/codex/opencode. Must run
...
function handleDeepLink(url: string): void {
  try {
    ...
    // wallts://auth/callback?token=<jwt>
...
function createWindow(): void {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      plugins: true,
      additionalArguments: [`--wallts-locale=${systemLocale}`],
    },
  });
...
// single-worktree case still lands at "Wallts Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Wallts Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "Wallts Canary";
...
if (is.dev) {
  ...
  // to "Wallts", but anchoring it here makes WM_CLASS ↔ StartupWMClass
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev ? "ai.wallts.desktop.dev" : "ai.wallts.desktop",
    );
