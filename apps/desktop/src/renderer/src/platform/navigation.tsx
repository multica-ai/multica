 ...
 * foreground tab. Returns `true` if the navigation was redirected (caller
 * should NOT proceed). Pathname-only changes (search / hash) are allowed
 * through so pinned filter / drawer / form-state interactions still work.
 * Dedupe is preserved: `openTab` activates an existing same-path tab if one
 * exists, otherwise creates a new one.
 */
function tryRouteToPinnedNewTab(path: string): boolean {
  const store = useTabStore.getState();
  const active = getActiveTab(store) as
    | { pinned?: boolean; path: string; router?: { state: { location: { pathname: string } } } }
    | null;
  if (!active?.pinned) return false;

  // Prefer the live router pathname (query-only navigations only sync the
  // pathname back to the store) and fall back to the stored tab path.
  const currentPathname =
    active.router?.state.location.pathname ?? active.path.split("?")[0].split("#")[0];
