    ...
    entries: [
      {
        ...
        changes: [],
        features: [
          "Quick Capture 成为创建 Issue 的默认入口 —— 支持连续创建、文件 / 图片上传，并能根据粘贴的 URL 自动丰富标题和描述",
          "Markdown 内联渲染 Mermaid 图表，复杂图支持全屏 lightbox 查看",
          "项目资源系统升级：typed project resources 直接注入 Agent runtime，项目级 `github_repo` 资源会覆盖 workspace 的默认 repo",
          "Agent / 评论 / Runtime / Skill 全面接入权限感知 UI，没有权限的操作不再展示",
          "Agent Presence v4 —— 状态 Pill、失败气泡、实时计时一应俱全",
          "Runtimes 页新增「远程机器 / AWS EC2」接入向导",
          "Inbox 新增一键 Done 按钮，并支持按事件类型自定义通知偏好，降低噪音",
        ],
        improvements: [
          "编辑器复制 / 剪切时保留 Markdown 源码，跨 Issue 粘贴不再丢格式",
          "Daemon `/tasks/claim` 轮询走 Redis 空认领 fast-path，空闲态 DB 压力显著下降",
          "长期 open 的 Issue 自动回收磁盘空间，cancelled 状态判断修正",
          "并发任务槽位数量可通过 Daemon 环境变量配置",
          "Multica Agent 的 Git 提交自动追加 `Co-authored-by` trailer，归属更清晰",
          "Quick Create 在 Daemon CLI 版本不达标时前后端双闸门拦截",
          "Create-project 与 Feedback 弹窗自动保存草稿，重新打开不丢内容",
          "Desktop 拦截 Cmd+R / Ctrl+R / F5 防止意外刷新，开发模式显示 git-described 版本，Updates 设置展示当前版本号",
          "Workspace 表格列宽可拖动；移动端 Issue 详情布局稳定，输入聚焦不再触发缩放",
        ],
        fixes: [
          "Quick Create 不再凭空脑补需求，仅依据用户输入生成；创建后会自动把发起人订阅到 Issue",
          "Inbox 点击通知后立即跳到目标评论；从 Issue 详情页 Mark as Done 时自动归档对应 Inbox 条目",
          "「Mark as Done」按钮仅在 Inbox 页显示",
          "Task rerun 启动全新 session，跳过被污染的 resume 状态",
          "Batch-update 在 no-op 时正确返回 `updated=0`，不再报陈旧数量",
          "受邀成员登录后路由到所在 workspace，不再被强制带去 `/onboarding`",
          "默认模型显示不再硬编码模型名",
          "Issue 详情页 Details 与 Token Usage 改为稳定的网格布局；breadcrumb 去掉重复的 issue identifier",
          "恢复前端 CI 检查（修复回归）",
        ],
      },
      {
