    ...
    entries: [
      {
        version: "0.5.0",
        date: "2026-09-08",
        title: "评论线程内联执行状态、按线程独立排队、运行更可靠、搜索更少超时",
        changes: [],
        features: [
          "智能体的排队、运行步骤、停止重试和完整日志，都显示在对应的评论线程里。",
          "每个根评论线程各自排队，不同讨论串不会再合并或丢回复。",
          "评论线程的导航移到右侧，长标题也能看全。",
        ],
        improvements: [
          "快速创建发起的任务不再占用 Issue 的活动记录，仍能在执行历史里查到。",
          "复杂搜索不会再提前失败，搜评论也更快了。",
        ],
        fixes: [
          "在钉钉里引用某条消息再发 /new、/clear，引用内容和指令不会再混在一起。",
          "飞书私聊的绑定卡片发不出去时，群里会直接说明。",
          "Telegram 不会再把同一条回复重复发一遍。",
          "中途停下的 Codex 运行，用掉的 token 仍会记录下来。",
          "qoder、kiro、grok、kimi 等卡住的对话，下一条消息就能继续。",
          "在飞书里创建 Issue 后给出的链接，会直接打开对应工作区。",
          "Cursor 任务启动后台服务后，不会再假完成或长时间停住。",
          "Issue 分栏视图里不再出现两个侧栏开关，AI Team 标题也写对了。",
        ],
      },
      {
        version: "0.4.41",
