    ...
    entries: [
      {
        version: "0.3.19",
        date: "2026-06-09",
        title: "更可靠的智能体、附件和 Issue 讨论",
        changes: [],
        features: [
          "Issue 讨论现在可以用某一条回复来标记解决，长讨论收起后也能保留最终结论",
          "Lark 和飞书对话现在会在 Multica 准备回复时显示输入状态，并在回复发出前自动清除",
          "智能体运行现在能知道是谁发起了当前任务，交接、审计和权限敏感的处理会更准确",
          "OpenClaw 用户现在可以在本地配置里指定自定义应用位置和数据目录",
        ],
        improvements: [
          "Issue 顶部的活跃智能体状态更容易读懂，只有真正运行时才显示动效，排队时会明确显示排队状态",
          "命令行现在会更清楚地解释常见错误、登录问题和项目配置项",
        ],
        fixes: [
          "新发布的附件现在会使用稳定的私有下载链接，临时上传链接过期后图片和文件仍能正常显示",
          "由新建 Issue 触发的自动任务在智能体任务失败后会正确失败，不会继续卡在进行中",
          "从收件箱打开评论链接时，现在只会滚动 Issue 时间线，不会把桌面窗口内容顶出可见区域",
          "Cursor 和 Codex 会话现在会在得到最终结果后更干净地结束，保留完成状态和最后的遥测信息",
          "自托管设置现在会遵循已配置的服务地址，创建项目时也会返回清楚的校验错误，而不是笼统失败",
