    ...
    entries: [
      {
        version: "0.3.24",
        date: "2026-06-17",
        title: "自定义运行配置和更清楚的智能体设置",
        changes: [],
        features: [
          "团队现在可以创建自定义运行配置，让智能体在不同环境里使用合适的本地工具、模型选项和工作方式",
          "命令行创建和更新智能体时，现在可以选择思考强度，更容易让智能体的推理深度匹配任务难度",
        ],
        improvements: [
          "新增或更新的运行配置会更快出现在应用里，智能体也会优先使用最贴合当前环境的配置",
          "客户端错误和卡顿反馈会自动合并重复信息，问题报告更安静，也更容易排查",
          "Issue 评论触发预览的文案更清楚，发送前更容易看出哪些智能体或小队会开始工作",
        ],
        fixes: [
          "Office 365 邮件在首选登录方式不可用时，现在也能更稳定地完成发送",
          "GitHub 集成会更可靠地保留安装上下文，并能在 CI 仍在运行时显示等待中的检查状态",
          "Codex 运行所需的服务退出时，现在会更快失败，避免停在不清楚的等待状态",
          "运行环境负责人现在可以重新删除自修复运行环境，切换运行环境时也会清除不兼容的模型选择",
          "Issue 页面遇到未知图标时不会异常，评论里的普通文件名也不会再被误识别成外部链接",
        ],
      },
      {
        version: "0.3.23",
