    ...
    entries: [
      {
        ...
        changes: [],
        features: [
          "全栈 Docker Compose 一键自部署——PostgreSQL、后端（自动迁移）和前端通过一条 docker compose up 启动",
          "通过 ACP（Agent Communication Protocol）接入 Hermes Agent Provider——JSON-RPC 2.0 over stdio，支持会话生命周期管理、工具自动审批和用量追踪",
          "基于 Fumadocs 搭建文档站——包含快速入门、CLI 参考、Agent 指南和贡献文档，支持全文搜索",
          "移动端响应式布局——侧边栏在小屏幕上以抽屉形式展示，收件箱切换为全屏列表/详情交替视图",
          "Issue 详情侧边栏新增 Token 用量面板——展示所有任务的汇总输入/输出 Token、缓存 Token 和运行次数",
          "支持在 UI 中切换 Agent 运行时——无需重建 Agent 即可迁移到不同运行时",
          "'C' 快捷键快速创建 Issue，与 Linear 一致的操作习惯",
          "Master Agent 聊天会话历史——支持浏览和查看已归档的历史对话（只读模式）",
          "Daemon 新增 CLI 最低版本检查——跳过过低版本的 Claude Code（< 2.0.0）和 Codex（< 0.100.0）运行时，给出明确警告而非静默失败",
          "官网「Works with」新增 OpenClaw 和 OpenCode 展示",
          "make dev 一键本地开发——自动检测环境、创建配置文件、安装依赖、启动数据库、执行迁移并启动前后端",
        ],
        improvements: [
          "侧边栏重新设计——拆分为个人（收件箱、我的 Issue）和工作区分组，Settings 移至底部，全宽搜索框配 ⌘K 提示，底部显示用户头像和档案",
          "搜索排序全面优化——ILIKE 大小写无关匹配、标识符搜索（MUL-123 或纯数字）、多词 AND 匹配、活跃 Issue 优先排序",
          "搜索结果关键词高亮——标题和评论摘要中以黄色标记匹配词",
          "每日 Token 用量图表优化——Y 轴标签改为紧凑格式，单区域趋势图配分类 Tooltip 明细",
          "Master Agent 聊天支持多行输入，保留换行格式",
          "统一共享选择器组件（状态、优先级、截止日期、项目、指派人）——创建弹窗、详情侧边栏和批量操作使用一致的 API，减少约 200 行重复代码",
          "工作区级别存储隔离——所有持久化的客户端状态（草稿、视图偏好、聊天）按 workspace ID 命名空间隔离，切换工作区时自动重新加载",
          "服务端启动时对缺失的 JWT_SECRET 和 RESEND_API_KEY 给出明确警告，方便自部署用户排查配置",
          "服务端 CORS 配置更新，支持更多开发端口",
        ],
        fixes: [
          "删除子 Issue 后正确刷新父级的子 Issue 缓存——删除的子 Issue 不再残留显示直到刷新页面",
          "搜索索引兼容 RDS 上的 pg_bigm 1.2——将 ILIKE 替换为 LOWER/LIKE 以启用 GIN 索引扫描",
          "运行时缓存在 Daemon 注册/注销事件时正确刷新——创建 Agent 对话框不再错误显示「无可用运行时」",
          "修复 Claude stream-json 启动卡住的问题",
          "待执行任务唯一索引放宽为按 (issue, agent) 粒度——不同 Agent 现在可以独立为同一 Issue 排队任务",
          "退出登录时清除 workspace_id 和查询缓存，防止共享设备上残留数据",
          "拖放文件覆盖层移至外层容器，编辑器为空时拖放区域也能覆盖整个输入区",
          "Skills 导入现在检测 GitHub 仓库的默认分支而非硬编码 main，修复非 main 分支仓库的 404 问题",
          "WebSocket 端点新增 PAT（个人访问令牌）认证支持——PAT 用户现在可以使用实时推送",
          "允许在所有绑定 Agent 已归档时删除运行时，不再永久阻止删除",
        ],
      },
      {
        version: "0.1.21",
