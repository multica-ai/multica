      ...
      entries: [
        {
          ...
          changes: [],
          features: [
            "Slack에서 네이티브 /issue 슬래시 명령으로 Multica Issue를 바로 생성할 수 있습니다. 생성 알림은 명령 실행자에게만 개인 메시지로 전달되어, 채널을 어지럽히지 않습니다.",
            "같은 Slack 워크스페이스에서 이미 한 Multica 봇과 계정을 연결한 사용자는, 새로 추가된 다른 봇에서 다시 연결을 요구받지 않습니다. 기존 연결이 자동으로 재사용됩니다.",
            "Slack 채널 문맥 읽기가 두 개의 목적이 뚜렷한 명령으로 정리되었습니다. 채널 개요(최상위 메시지와 스레드 마커)와 특정 스레드 읽기로 나뉘어, 에이전트가 사용자가 가리킨 바로 그 스레드로 즉시 이동할 수 있습니다.",
            "ByteDance TRAE CLI(traecli)가 내장 에이전트 런타임에 합류했습니다. 표준 ACP 프로토콜 위에서 동작하며, 자체 모델 목록을 제공합니다.",
            "Anthropic 모델 목록에 Claude Sonnet 5가 추가되고, 도입기 가격이 사용량 계측 계층까지 반영되었습니다.",
            "셀프호스트 배포에서도 온보딩 또는 소스 보완 대화상자를 통해 «Multica를 어디서 알게 되었는지»를 익명으로 보고할 수 있습니다. 셀프호스트 커뮤니티의 유입 경로를 파악하는 데 도움이 됩니다.",
            "각 태스크 실행에서 프롬프트 캐시 히트율이 사용량 로그에 기록되어, 향후 프롬프트 캐싱 개선을 위한 실측 데이터를 얻을 수 있게 되었습니다.",
          ],
          improvements: [
            "Autopilot의 «액세스 관리» 진입점이 편집 대화상자로 이동했습니다. 대화상자 헤더의 컴팩트한 «액세스 관리» 버튼이 가벼운 팝오버를 열어 부여 / 회수 멤버 목록을 보여 주며, 기존의 별도 헤더 버튼을 대체합니다.",
            "Issue 및 댓글 에디터가 Tiptap 3.27.1로 업그레이드되었고, 인라인 `code` 단축 입력 시 앞 글자가 사라지는 드문 회귀가 수정되었습니다.",
          ],
          fixes: [
            "Slack 채팅 에이전트가 채널 히스토리 조회 과정을 서술하지 않습니다(예: «먼저 히스토리를 읽고…»). 내부에서 조용히 읽고 곧바로 답변만 회신합니다.",
            "셀프호스트의 로컬 디스크 구성에서 첨부 미리보기(PDF / HTML)가 다시 열립니다. /uploads/* 정적 라우트도 API 첨부 엔드포인트와 동일하게 완화된 frame-ancestors 정책을 사용합니다. (커뮤니티 보고)",
            "Cursor와 Kiro 런타임 완료 시 트랜스크립트가 복구됩니다. Cursor는 터미널 결과 텍스트가 첫 번째 어시스턴트 텍스트인 경우에도 실행 메시지 스트림에 실리며, Kiro는 종단부의 `multica issue comment add`가 성공한 뒤 발생하는 알려진 종료 오류를 정상 완료로 취급합니다.",
            "셀프호스트: docker-compose.selfhost.yml에서 MULTICA_SLACK_SECRET_KEY가 백엔드 컨테이너로 전달되어, .env에 값을 설정하기만 하면 Slack 통합이 활성화됩니다. (커뮤니티 보고)",
            "Issue 보드 상단의 «N개 처리 중» 칩이 에이전트가 아니라 Issue 수를 기준으로 집계됩니다. 같은 Issue를 두 에이전트가 처리해도 «1건»으로 표시됩니다.",
            "셀프호스트 익명 소스 채널 리포팅 대상 엔드포인트가 정식 Multica API로 복원되었고, 개발 중 잠시 남아 있던 스테이징 URL이 제거되었습니다.",
            "댓글 딥링크 하이라이트가 배경색만 사용하도록 통일되었습니다. 루트 댓글과 답글에서 동일하게 동작하며, 하이라이트가 활성인 상태에서도 sticky 댓글 헤더의 색 단절이 사라졌습니다.",
          ],
        },
        {
          version: "0.3.33",
