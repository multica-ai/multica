    ...
    entries: [
      {
        ...
        changes: [],
        features: [
          "Full-stack Docker Compose for one-command self-hosting — PostgreSQL, backend (auto-migration), and frontend in a single docker compose up",
          "Hermes Agent Provider via ACP (Agent Communication Protocol) — JSON-RPC 2.0 over stdio with session lifecycle, tool auto-approval, and usage tracking",
          "Documentation site powered by Fumadocs — Getting Started, CLI reference, Agents guide, and Contributing docs with full-text search",
          "Mobile-responsive layout — sidebar renders as a drawer on small screens with a trigger button; inbox switches to full-screen list/detail toggle on phones",
          "Token usage per issue — new section in the issue detail sidebar showing aggregated input/output tokens, cache tokens, and run count across all tasks",
          "Switch agent runtime from the UI — reassign an agent to a different runtime without recreating it",
          "'C' keyboard shortcut for quick issue creation, matching Linear's convention",
          "Chat session history panel — browse and view archived Master Agent conversations in read-only mode",
          "Minimum CLI version check in daemon — skips outdated Claude Code (< 2.0.0) and Codex (< 0.100.0) runtimes with a clear warning instead of failing silently",
          "OpenClaw and OpenCode added to the landing page \"Works with\" section",
          "make dev one-command local setup — auto-detects environment, creates env files, installs deps, starts DB, runs migrations, and launches backend + frontend",
        ],
        improvements: [
          "Sidebar redesign — split into Personal (Inbox, My Issues) and Workspace groups, moved Settings to footer, full-width search input with ⌘K hint, user profile with avatar in footer",
          "Search ranking overhaul — ILIKE case-insensitive matching, identifier search (MUL-123 or bare numbers), multi-word AND matching, status-based tiebreaker for active issues",
          "Search result keyword highlighting with yellow markers in both titles and comment snippets",
          "Daily token usage chart readability — explicit nice Y-axis ticks with compact formatting, cleaner single-area trend with per-category tooltip breakdown",
          "Master Agent chat supports multiline input with preserved line breaks",
          "Consolidated shared picker components (Status, Priority, DueDate, Project, Assignee) — consistent API across create modal, detail sidebar, and batch actions, removing ~200 lines of duplication",
          "Workspace-scoped storage isolation — all persisted client state (drafts, view preferences, chat) namespaced by workspace ID with auto-rehydration on workspace switch",
          "Startup warnings for missing JWT_SECRET and RESEND_API_KEY so self-host users know what to configure",
          "Server CORS configuration updated to support additional development ports",
        ],
        fixes: [
          "Sub-issue deletion now properly invalidates the parent's children cache — deleted sub-issues no longer remain visible until page refresh",
          "Search index compatibility with pg_bigm 1.2 on RDS — replaced ILIKE with LOWER/LIKE to enable GIN index scans",
          "Runtimes cache now invalidates on daemon register/deregister events — Create Agent dialog no longer shows \"No runtime available\" when runtimes exist",
          "Claude stream-json startup hangs resolved",
          "Pending task unique index relaxed to per-(issue, agent) — different agents can now independently queue tasks for the same issue",
          "Logout now clears workspace_id and TanStack Query cache, preventing stale data on shared devices",
          "Drag-drop file overlay moved to the outer container so the drop target covers the full editor area even when content is empty",
          "Skills import now detects the GitHub repo's default branch instead of hardcoding \"main\", fixing 404s on repos with other branch names",
          "PAT (personal access token) authentication support added to WebSocket endpoint — PAT holders can now use real-time streaming",
          "Runtime deletion now allowed when all bound agents are archived, instead of permanently blocking deletion",
        ],
      },
      {
        version: "0.1.21",
