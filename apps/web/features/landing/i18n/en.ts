    ...
    entries: [
      {
        ...
        changes: [],
        features: [
          "Quick Capture is now the default New Issue experience — continuous-create mode, file/image uploads, and automatic enrichment from pasted URLs",
          "Mermaid diagrams render inline in markdown, with a fullscreen lightbox for complex graphs",
          "Typed project resources are injected into the agent runtime; project-level `github_repo` resources override workspace repos",
          "Permission-aware UI lands across agents, comments, runtimes, and skills — actions you can't take are no longer offered",
          "Agent Presence v4 — status pill, failure bubble, and live elapsed timing on every task",
          "Runtimes page gains a remote machine / AWS EC2 connection wizard",
          "Inbox adds a one-click Done button and per-event-type notification preferences to cut noise",
        ],
        improvements: [
          "Editor preserves Markdown source on copy/cut, so pasting between issues keeps formatting",
          "Daemon `/tasks/claim` polling uses a Redis empty-claim fast-path, dropping idle DB load",
          "Long-open issues now reclaim disk automatically; cancelled-status check fixed",
          "Concurrent task slot count is configurable via daemon env var",
          "Multica Agent commits include a `Co-authored-by` trailer for proper Git attribution",
          "Quick Create gates on the daemon CLI version with a pre-check and server-side enforcement",
          "Create-project and feedback modals persist drafts across reopens",
          "Desktop blocks Cmd+R / Ctrl+R / F5 from reloading the app, shows the real git-described version in dev, and surfaces the current version in Updates settings",
          "Workspace table columns are now resizable; mobile issue detail layout is stable and input focus no longer triggers zoom",
        ],
        fixes: [
          "Quick Create no longer invents requirements beyond user input, and now subscribes the requester to the issue it creates",
          "Inbox jumps straight to the targeted comment, and auto-archives when the issue is marked Done from the detail page",
          "\"Mark as Done\" button is now scoped to the Inbox page only",
          "Task rerun starts a fresh session and skips poisoned resume state",
          "Batch-update no-op responses now correctly report `updated=0` instead of stale counts",
          "Invitees land on their workspace after sign-in instead of being forced through `/onboarding`",
          "Default-model display no longer shows a hardcoded model name",
          "Issue detail wraps Details and Token Usage in a stable grid; breadcrumb no longer repeats the issue identifier",
          "Frontend CI checks restored after a regression",
        ],
      },
      {
        version: "0.2.20",
