      ...
      entries: [
        {
          ...
          changes: [],
          features: [
            "Slack からネイティブの /issue スラッシュ コマンドで Multica Issue を作成できるようになりました。作成通知はコマンド送信者のみに私信で返るので、チャンネルを賑わせません。",
            "同じ Slack ワークスペースで既に一方の Multica ボットに連携済みのユーザーは、後から追加された別のボットに対して再度の連携プロンプトが出なくなりました。既存の連携がそのまま引き継がれます。",
            "Slack チャンネル向けの文脈読み込みが 2 つのコマンドに整理されました。チャンネル概要（トップレベル メッセージ + 各スレッドの目印）と、特定スレッドの読み取りに分かれ、ユーザーが指した該当スレッドへ直接飛べます。",
            "ByteDance TRAE CLI（traecli）が組み込みのエージェント ランタイムに加わり、標準の ACP プロトコル上で動作し、独自のモデル一覧を持ちます。",
            "Anthropic のモデル一覧に Claude Sonnet 5 を追加し、導入期価格が使用量計測レイヤーまで反映されました。",
            "セルフホスト環境でも、オンボーディングまたはソース補完ダイアログから匿名で「Multica をどこで知ったか」を報告できるようになりました。セルフホスト コミュニティの流入源を把握するのに役立ちます。",
            "タスクの各実行で prompt cache のヒット率が使用量ログに記録されるようになり、今後の prompt キャッシュ最適化に向けた測定基盤ができました。",
          ],
          improvements: [
            "Autopilot の「アクセス管理」入口を編集ダイアログへ移設しました。ダイアログ ヘッダーのコンパクトな「アクセス管理」ボタンから軽量なポップオーバーが開き、付与 / 取り消しのメンバー リストが表示され、独立していた従来のヘッダー ボタンを置き換えます。",
            "Issue とコメントのエディターが Tiptap 3.27.1 に更新され、インライン `code` ショートカットの入力時に直前の文字が消えてしまうまれな不具合が修正されました。",
          ],
          fixes: [
            "Slack のチャット エージェントが履歴取得の作業を毎回説明しなくなりました（例：「まず履歴を読んでから…」）。内部で静かに読み込みを行い、回答だけを送るようになりました。",
            "セルフホストのローカル ディスク構成で、添付ファイル（PDF / HTML）のプレビューが再び開くようになりました。/uploads/* の静的ルートも、API 添付エンドポイントと同じ緩和された frame-ancestors ポリシーを使用します。（コミュニティ報告）",
            "Cursor と Kiro ランタイムの完了時トランスクリプトを取り戻しました。Cursor はターミナル結果が最初のアシスタント テキストであっても実行メッセージに流し込み、Kiro は終端の `multica issue comment add` 成功後に既知のクローズ エラーが発生してもタスクを完了扱いとします。",
            "セルフホスト：docker-compose.selfhost.yml で MULTICA_SLACK_SECRET_KEY をバックエンド コンテナに引き渡すようになり、.env に設定すれば Slack 統合がそのまま有効になります。（コミュニティ報告）",
            "Issue 一覧上部の「N 件処理中」チップが、エージェントではなく Issue 数を数えるようになり、同じ Issue に 2 体のエージェントが取り組んでいても「1 件」と表示されます。",
            "セルフホストの匿名ソース チャネル送信先を正式な Multica API エンドポイントに戻し、開発中に紛れ込んだ暫定ステージング URL を取り除きました。",
            "コメント ディープリンクのハイライトは背景色のみになり、ルート コメントと返信で挙動が統一されました。ハイライト中でも sticky コメント ヘッダーの色ズレが出なくなりました。",
          ],
        },
        {
          version: "0.3.33",
