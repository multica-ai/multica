 ...
 *   wallts-desktop-{version}-mac-{arch}.{dmg|zip}
 *   wallts-desktop-{version}-windows-{arch}.exe
 *   wallts-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^wallts-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
