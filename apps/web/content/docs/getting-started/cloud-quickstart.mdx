...
## Optional: Use Multica from Claude Code

If you use Claude Code, install the Multica MCP server so Claude can read issues, comment, and update status as native tools:

```bash
curl -fsSL https://raw.githubusercontent.com/firtal-group/firtal-cerebro/main/scripts/install-mcp.sh | bash
```

See [MCP for Claude Code](/cli/mcp) for details.
