...
function LoginPageContent() {
  ...
  const handleSendCode = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("failedSendCode"));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleVerifyCode = useCallback(
    async (value: string) => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : t("failedVerifyCode"));
        setCode("");
        setSubmitting(false);
      }
    },
      ...
      <div className="flex min-h-screen items-center justify-center">
        <Card className="w-full max-w-sm">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl">{t("cliTitle")}</CardTitle>
            <CardDescription>
              {t("cliDescription")}{" "}
          ...
          <CardContent className="flex flex-col gap-3">
            <Button onClick={handleCliAuthorize} disabled={submitting} className="w-full" size="lg">
              {submitting ? t("authorizing") : t("authorize")}
            </Button>
            <Button
              variant="ghost"
              className="w-full"
              onClick={() => { setExistingUser(null); setStep("email"); }}
            >
    ...
    <div className="flex min-h-screen items-center justify-center">
      <Card className="w-full max-w-sm">
        ...
        <CardFooter className="flex flex-col gap-3">
          <Button type="submit" form="login-form" disabled={submitting} className="w-full" size="lg">
