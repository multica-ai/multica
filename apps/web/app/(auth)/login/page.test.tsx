...
import enCommon from "@wallts/views/locales/en/common.json";
import enAuth from "@wallts/views/locales/en/auth.json";
import enSettings from "@wallts/views/locales/en/settings.json";
...
describe("LoginPage", () => {
  ...
  it("renders login form with email input and continue button", () => {
    ...
    expect(screen.getByText("Sign in to Wallts")).toBeInTheDocument();
  ...
  // the wallts:// deep link, not silently redirect to the workspace page.
    ...
    try {
      ...
      await waitFor(() => {
        expect(hrefSetter).toHaveBeenCalledWith(
          "wallts://auth/callback?token=handoff-jwt",
        );
      });
      expect(
        await screen.findByRole("button", { name: "Open Wallts Desktop" }),
      ).toBeInTheDocument();
    } finally {
      Object.defineProperty(window, "location", {
        configurable: true,
