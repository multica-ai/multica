...
test("killing app A leaves app B healthy on an internal-only network", { timeout: 60_000 }, async (t) => {
  const image = process.env.CEREBRO_APPS_WORKER_IMAGE;
  if (!image) {
    t.skip("CEREBRO_APPS_WORKER_IMAGE is not set; build the worker image before running this integration test");
    return;
  }
  try {
  ...
  t.after(async () => {
    await exec("docker", ["rm", "-f", appAContainer, appBContainer, bundleServer]).catch(() => {});
  ...
  await exec("docker", ["kill", appAContainer]);
  assert.equal(await probe({ image, network, hostname: "app-b" }), "200");

  const { stdout: internal } = await exec("docker", ["network", "inspect", network, "--format", "{{.Internal}}"]).catch(() => ({ stdout: "" }));
  assert.equal(internal.trim(), "true");
  const { stdout: ports } = await exec("docker", ["inspect", appBContainer, "--format", "{{json .NetworkSettings.Ports}}"]).catch(() => ({ stdout: "" }));
