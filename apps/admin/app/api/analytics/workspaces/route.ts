...
export async function GET(request: NextRequest) {
  ...
  if (to.getTime() - from.getTime() > MAX_BREAKDOWN_HOURS * 3_600_000) {
    return NextResponse.json({ error: `breakdown span must not exceed ${MAX_BREAKDOWN_HOURS} hours` }, { status: 400 });
  }
