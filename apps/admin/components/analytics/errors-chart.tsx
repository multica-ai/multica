...
export function ErrorsChart({
  ...
  return (
    <ChartContainer
      ...
      role="group"
      aria-label="Errors chart. Activate a bar segment to see its workspace breakdown."
    >
        ...
        <ChartTooltip
          content={
            <ChartTooltipContent
              ...
              footer="Activate a segment for its workspace breakdown."
            />
          }
        />
        {classes.map((c, i) => (
          <Bar
            ...
            shape={(props) => <InteractiveBarSegment {...props} label={`${labelOf(failureClassChartConfig, c)} errors`} onActivate={onSegmentClick ? (bucketStart) => onSegmentClick(bucketStart, c) : undefined} />}
          />
        ))}
      </BarChart>
    </ChartContainer>
