...
| **ForgeCode** | 오픈소스 | ✅ (`--cid <id>`) | ❌ | `.forge/skills/` | 정적 (환경 변수) |
...
오픈소스 CLI 코딩 하네스입니다([forgecode.dev](https://forgecode.dev/)). 다른 지원 도구들과 달리, ForgeCode의 원샷 모드(`forge -p`)는 구조화된 이벤트 스트림을 **내보내지 않습니다** — 도구 호출, 추론, 본문이 뒤섞인 스타일이 적용된 Markdown을 stdout으로 렌더링합니다. Multica는 이를 2단계 방식으로 우회합니다: 라이브 stdout을 버퍼링하고(liveness heartbeat만 전달), 실행 후에 `forge conversation dump <id>`에서 구조화된 메시지 목록(도구 호출, 도구 결과, 추론, 토큰 사용량)을 재생합니다. **세션 재개가 동작합니다** — 고정된 `--cid <id>`를 통해서이며, conversation id를 재사용해 이전 대화를 이어갑니다. **모델 선택 방식이 특이합니다** — `--model` flag가 존재하지 않으며, 모델은 `FORGE_SESSION__PROVIDER_ID` / `FORGE_SESSION__MODEL_ID` 환경 변수로 설정됩니다. Multica는 `provider/model` 값(예: `anthropic/claude-sonnet-4-20250514`)에서 이를 도출합니다. MCP 구성은 소비하지 않습니다. 스킬은 `.forge/skills/`로 복사됩니다.
...
**14개 도구 중 `mcp_config`를 실제로 소비하는 것은 10개입니다: Claude Code, CodeBuddy, Codex, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder**. 나머지 4개는 이 필드를 받아들이지만 **무시합니다** — 오류도, 경고도 없으며, 구성이 그저 효과를 내지 못합니다.
