...
관리형 Prime Agent는 공유 `~/.prime/agent` 디렉터리를 사용하지 않습니다. Multica는 runtime/agent별로 sessions, settings, authentication, user skills를 위한 비공개 영구 디렉터리를 할당합니다. 각 managed agent에서 인증을 별도로 구성하거나 명시적으로 범위가 제한된 환경 자격 증명을 사용하세요. tenant 간에 공유되는 MCP settings나 secrets를 Prime project state에 저장하지 마세요.
