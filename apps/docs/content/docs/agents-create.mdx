...
| **Queue wait timeout** | How long a queued task may wait before being marked `timeout`. New agents default to **120 minutes**; set it blank or `0` to fall back to the system default. See [Tasks](/tasks#what-happens-when-a-task-times-out). |
