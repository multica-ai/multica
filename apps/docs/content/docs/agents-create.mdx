...
**Queue wait timeout** controls how long a task is allowed to sit in `queued` before Multica marks it failed with `timeout`. For new agents, the create dialog defaults to **120 minutes**.

- Leave it as-is to give the agent its own 120-minute wait budget
- Set a different positive integer number of minutes to override that budget per agent
- Leave it blank or set it to `0` to fall back to the system default instead

This setting only affects **waiting in queue**. Once the daemon has picked the task up and the AI coding tool has started, the normal dispatch/runtime watchdogs take over. See [Tasks](/tasks#what-happens-when-a-task-times-out).
