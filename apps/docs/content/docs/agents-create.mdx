...
| **Queue wait timeout** | The minimum time a queued task waits before it may be failed as `queued_expired`. New agents default to **120 minutes**; set it blank or `0` to fall back to the system default. The task still expires only if its runtime has also been silent past the reconnect grace. See [Tasks](/tasks). |
