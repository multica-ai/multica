...
Editing the instructions later starts a new session on every issue the agent is already working on. Its next run there begins a fresh conversation, because resuming one would replay the previous instructions the agent has already read and keep following them. Files in the working directory are kept, and the run is told its earlier context was not carried over. Chat conversations are unaffected.
