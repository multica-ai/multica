...
**MCP configuration** (`mcp_config`) lets an agent connect Claude Code to Model Context Protocol servers. Add it from the agent detail page's **MCP** tab as a JSON object:
...
Only Claude Code currently consumes this field. Other runtimes accept the saved config but ignore it at execution time, so use a Claude Code runtime when the agent depends on MCP servers. See [AI coding tools comparison → MCP configuration](/providers#mcp-configuration-only-claude-code-actually-reads-it).
