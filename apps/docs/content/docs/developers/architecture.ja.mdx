...
| `packages/views/` | ブラウザホストで共有するビジネスページとコンポーネント | React |
...
Web と Tag は次の 3 層を共有します。

1. `packages/core` が API、キャッシュ、権限、プラットフォーム非依存の状態を扱います。
2. `packages/ui` が基礎コンポーネントを提供します。
3. `packages/views` がビジネスページを構成します。

ルーティングや cookie などのプラットフォーム機能はアプリケーション層に残します。共有ページは `NavigationAdapter` を通じて遷移し、フレームワーク固有のルーティング API を直接インポートしません。

たとえばブラウザホストで共有するイシュー機能は、通常次の場所にまたがります。
...
API レスポンスは `packages/core/api/` の境界で zod schema により解析します。クライアントとサーバーは異なる時点でデプロイされる場合があるため、ネットワーク JSON を TypeScript 型へ直接キャストしてはいけません。
