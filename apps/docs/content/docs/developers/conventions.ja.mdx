...
| `apps/mobile/` | `@multica/core` の型と純粋関数 | ブラウザホストの React ページ、store、プラットフォーム実装 |
...
- クライアントとサーバーは異なる時点でデプロイされる場合があるため、常に同じバージョンだと仮定しない
...
インタラクション全体を単一ホストが所有する場合を除き、共有コピーは最上位に置きます。
