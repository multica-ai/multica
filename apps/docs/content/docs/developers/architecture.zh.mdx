...
| `packages/views/` | 浏览器 Host 共用的业务页面和组件 | React |
...
路由和 cookie 等平台能力留在应用层。共享页面通过 `NavigationAdapter` 导航，不直接导入框架专用的路由 API。

例如，一项浏览器 Host 共用的 issue 功能通常会涉及：
...
API 返回值在 `packages/core/api/` 边界通过 zod schema 解析。客户端与服务端可能在不同时间发布，因此不能直接把网络 JSON 强制转换成 TypeScript 类型。
