...
provider 适配层统一了不同 AI 编程工具的启动、流式事件、取消和用量数据，但本地目录与会话仍由守护进程管理。`local_directory` 始终携带持久项目路径，`execution_mode` 独立决定守护进程是获取路径互斥锁（`in_place`）、为明确按文件拆分的工作跳过互斥锁（`shared`），还是创建隔离 checkout（`worktree`）。
