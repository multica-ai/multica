...
| `apps/mobile/` | types and pure functions from `@multica/core` | Browser-host React pages, stores, and platform implementations |
...
- Clients and servers may be deployed at different times; never assume that frontend and backend versions always match.
...
- Host-only: a section named for that host

Keep shared copy at the top level unless a host genuinely owns the entire interaction.
