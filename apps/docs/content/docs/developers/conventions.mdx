...
- **Everyday noun** — the word a user would say out loud for it. **Translate fully**, whether or not it is a database entity: `issue` → 工作项, `workspace` → 工作区, `project` → 项目.
...
### `issue` and `task` are different nouns — translate both in Chinese

`issue` is the English product name for the thing a user files and an agent works on. `task` is one agent execution run of that issue. Both are user-visible, and they are **not** the same thing — an issue can carry many runs. Never let one locale spell both with the same word.

| Entity | en | zh-Hans | ja | ko |
| --- | --- | --- | --- | --- |
| the filed unit of work (`issue`) | Issue | **工作项** | **タスク** | **태스크** |
| one agent execution run (`task`) | Task | **工作任务** | **作業** | **작업** |

Chinese used to render `issue` as 任务 and leave `task` in English. That pair collides for Chinese users: 任务 is also how people read "Task", so the board entity and the agent run look like the same word. **工作项** / **工作任务** keeps both as everyday Chinese while remaining visually distinct.
...
- **`issue` in the "problem" sense** (runtime health) is a plain noun, not the entity: `{{count}} issues` on a machine card is `{{count}} 个异常`, not `工作项`.

**Why `issue` is translated while `skill` is not**: users file and read issues all day, and "issue" has no meaning in Chinese, Japanese, or Korean outside of dev jargon. Chinese names the filed unit of work 工作项 and the run 工作任务. `skill` is a Multica-specific concept where no local word carries the meaning.
...
- "Create new issue" → "新建工作项"（`工作项` is Chinese, so no space)
...
// zh-Hans/issues.json
{
  "issue_count_other": "{{count}} 个工作项"
}
```

Common count formats:

- `{{count}} issues` → `{{count}} 个工作项`
- `{{count}} tasks` → `{{count}} 个工作任务`
...
- **Placeholders**: example-style. "输入工作项标题...".
