...
- **日常词** —— 用户会直接说出口的词。**完整翻译成中文**，是不是数据库实体都一样：`issue` → 工作项、`workspace` → 工作区、`project` → 项目。
- **Multica 专有词** —— 中文里没有对应词的概念（`skill`），或用户可能要照着敲、照着比对的 schema 标识符（`todo`、`in_progress`、`task_id`）。**用小写英文**呈现，读起来像类型名。

其余规则以 `apps/docs/content/docs/*.zh.mdx` 为准 —— docs 是已经实战 20+ 篇的 CN voice 标准。但 docs 正文里的 `issue` 大多仍保留英文，正在单独清理；这两个词以下面的表格为准，不要参考那些页面。

### `issue` 和 `task` 不是同一个词，中文里都要翻译，而且要分开

`issue` 是这个「用户提出、智能体去做的事」的英文产品名。`task` 是智能体对这件事的一次执行。这两个词用户都看得到，而且**不是一回事** —— 一个工作项下面可以跑很多次执行。任何一种语言都不能把两者写成同一个词。

| 实体 | en | zh-Hans | ja | ko |
| --- | --- | --- | --- | --- |
| 提交出来的一件事（`issue`） | Issue | **工作项** | **タスク** | **태스크** |
| 智能体的一次执行（`task`） | Task | **工作任务** | **作業** | **작업** |

中文以前把 `issue` 译成「任务」，`task` 留英文。这对中文用户会撞车：人们看到 Task 也会读成「任务」，看板上的实体和智能体的一次执行看起来像同一个词。**工作项** / **工作任务** 都是日常中文，同时又分得开。
...
- **表示「问题」的 issue** 不是这个实体：机器卡片上的 `{{count}} issues` 是 `{{count}} 个异常`，不是 `工作项`。

**为什么 `issue` 要译，而 `skill` 不译**：用户每天都在提工作项、看工作项，而 "issue" 这个词在中文、日文、韩文里除了开发圈黑话之外没有含义。中文用「工作项」称呼提交出来的那件事，用「工作任务」称呼一次执行。`skill` 则是 Multica 特有概念，没有哪个中文词能承载它的意思。
...
- "Create new issue" → "新建工作项"（`工作项` 是中文，不加空格）
