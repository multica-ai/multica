...
| `apps/mobile/` | `@multica/core` 的类型和纯函数 | 浏览器 Host 的 React 页面、store 和平台实现 |

多个 Host 共用的逻辑，**必须**抽到共享包。"小段重复"也不算例外。Mobile 拥有独立的 UI、数据层和发布流程，只共享类型与纯函数。
...
- Host 专属：放以该 Host 命名的段

除非整段交互只属于某个 Host，否则文案应保持在顶层共享。
