...
| `apps/mobile/` | `@multica/core`의 유형과 순수 함수 | 브라우저 호스트의 React 페이지, store, 플랫폼 구현 |
...
- 클라이언트와 서버는 서로 다른 시점에 배포될 수 있으므로 항상 같은 버전이라고 가정하지 않음
...
- 호스트 전용: 해당 호스트 이름의 섹션

전체 상호작용을 하나의 호스트가 소유하는 경우가 아니라면 공유 카피는 최상위에 둡니다.
