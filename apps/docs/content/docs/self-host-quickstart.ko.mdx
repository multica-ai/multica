...
Docker Compose 설치를 업그레이드하는 방법은 두 가지이며, 둘은 **동등합니다** — `make selfhost`가 바로 아래 명령들을 감싼 래퍼이기 때문입니다(`Makefile:79-107`). 편한 쪽을 쓰세요.
...
`make selfhost`는 `.env` 파일이 **없을 때만** 생성합니다(`Makefile:80-95`). 기존 설치에서 다시 실행해도 `JWT_SECRET`, Postgres 비밀번호, 이메일 설정, `FRONTEND_ORIGIN`은 그대로 유지됩니다.
...
Helm은 자체 업그레이드 경로가 있습니다 — values 파일에 `images.backend.tag` / `images.frontend.tag`를 고정하고 `helm upgrade`를 실행하거나, `kubectl -n multica rollout restart`로 부동 태그를 다시 받아옵니다. 아래 [Kubernetes 배포(대체 방안)](#kubernetes-배포대체-방안)를 참고하세요.
