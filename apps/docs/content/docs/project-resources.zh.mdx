...
浏览器不能选择电脑上的文件夹。请在运行守护进程的电脑上，通过 CLI 或 API 添加本地目录资源。
...
1. 确认目标守护进程在线。
2. 通过 CLI 或 API，用绝对路径和守护进程 ID 创建资源。
3. 按“任务如何共享这个目录”的说明，把 `execution_mode` 设为 `in_place` 或 `worktree`。
...
本地目录有两种执行模式，通过每个资源的 `execution_mode` 设置。
