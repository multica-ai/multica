...
The Wallts server scans for due triggers every **30 seconds** — **the actual fire time can lag by up to 30 seconds**, not down to the second. If the server is restarted across a fire time, it catches up missed triggers on startup (nothing is lost, but they fire right away).
...
on the autopilot detail page; Wallts generates a unique URL of the shape:

```
https://<your-wallts-host>/api/webhooks/autopilots/awt_…
```

POST any JSON to that URL — Wallts records a run with `source = webhook`,
...
…or any JSON object/array. Wallts normalizes it into an internal envelope:
...
When you don't provide an `event` field, Wallts infers it from common
...
comma-separated **actions** list. Wallts allows a webhook if **any**
...
Wallts derives the `event` name and `action` from the inbound request
...
**2. Headers.** When no body envelope is present, Wallts reads the
...
present, Wallts falls back to top-level body string fields in this
...
**Action candidates, in full.** Once the event is determined, Wallts
...
- **Rotate it if it leaks** — click "Rotate URL" on the trigger row, or run
  `wallts autopilot trigger-rotate-url <autopilot-id> <trigger-id>`. The
...
Wallts returns `200 OK` with a `status` field for normal no-op outcomes so
...
When `WALLTS_PUBLIC_URL` is set on the server (e.g. `https://wallts.example.com`),
...
custom self-hosted reverse proxies. Wallts deliberately does not derive
