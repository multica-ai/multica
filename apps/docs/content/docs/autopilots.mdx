...
- **Create issue mode** (`create_issue`) — default, **recommended**. Each trigger first creates an issue in the workspace (the title supports two placeholders: `{{date}}` interpolates to the UTC date in `YYYY-MM-DD` format and `{{trigger_time}}` interpolates to the run's UTC instant in `YYYY-MM-DD HH:MM UTC` format; any other `{{...}}` token is rejected at create-time so a typo cannot silently land as the literal string in your issue titles), then assigns the issue to the agent through the normal assignment flow. All work lands on the issue board with the same history, comments, and status as a manually assigned issue.
