...
4. **队长记录 evaluation：** `cs-workflow squad activity <issue-id> action --reason "..."`。这一行会写进 issue 的 activity 时间线，方便人类回溯队长确实评估过这一次触发。
...
- **Squad Instructions（小队自定义指令）** —— 你为这个小队写的私货（在详情页里编辑，或用 `cs-workflow squad update --instructions`）。用来写路由规则（"DB 相关派给 Alice，前端派给 Bob"）、上报策略，或者任何 issue 本身不会有的背景。
...
**归档小队**（`cs-workflow squad delete <id>`，或详情页的 Archive 按钮）：
...
| `cs-workflow squad get <id>` | 查看小队的名字、队长、描述、instructions |
| `cs-workflow squad create --name "..." --leader <agent>` | 创建小队（owner / admin）|
| `cs-workflow squad update <id> [--name X] [--description X] [--instructions X] [--leader Y] [--avatar-url Z]` | 修改一个或多个字段 |
| `cs-workflow squad delete <id>` | 归档（软删除）——同时把当前分配给小队的 issue 转给队长 |
| `cs-workflow squad member list <id>` | 列出小队成员 |
| `cs-workflow squad member add <id> --member-id <uuid> --type agent\|member [--role "..."]` | 加成员（owner / admin）|
| `cs-workflow squad member remove <id> --member-id <uuid> --type agent\|member` | 移除成员（**不能移除队长**——先换队长）|
| `cs-workflow squad activity <issue-id> <action\|no_action\|failed> --reason "..."` | 队长每次结束前由它自己调用 |

`--leader` 接受智能体名字或 UUID；其它 ID 从 `cs-workflow agent list --output json`、`cs-workflow workspace member list --output json`、`cs-workflow squad list --output json` 拿。
