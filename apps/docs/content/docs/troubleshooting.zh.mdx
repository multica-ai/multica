...
如果某款编程工具在终端里运行正常，但它的 task 报缺少环境变量或未登录错误，说明守护进程运行时的环境和你的 shell 不同。

从 Dock、访达或 `open` 启动 Multica Desktop 时，操作系统通过桌面启动器以最小环境启动它。Desktop 会运行一次登录 shell 来恢复 `PATH`，所以守护进程仍然找得到 `claude` 或 `codex` 可执行文件，但它不会导入其他变量。只在 `~/.zshrc` 或 `~/.bashrc` 里导出的内容，对守护进程和它启动的 task 都不可见。
...
如果这条命令失败，而直接运行 `claude -p "hi"` 成功，说明凭据来自 shell 配置，守护进程看不到它。

改用智能体的自定义环境变量来设置凭据。这些变量保存在服务端，执行时下发给运行时，因此无论应用以哪种方式启动都能送达工具。从已经有该变量的终端重启守护进程同样有效，但只在 Desktop 下次自行启动守护进程之前有效。
