...
**症状**：[`cs-workflow daemon`](/cli) 的 `status` 命令显示 `offline` 或 `connection refused`；服务器日志里没有 `/api/daemon/register` 或 `/api/daemon/heartbeat` 的请求。守护进程机制详见 [守护进程与运行时](/daemon-runtimes)。
...
3. **Token 过期或无效** —— 你从来没跑过 `cs-workflow login`，或 PAT 被撤销
...
```bash
cs-workflow daemon logs --lines 100    # 看 daemon 侧错误
...
cs-workflow workspace list            # 确认你是目标工作区成员
```

**怎么修**：按上面原因对症处理。最常见的两个是**改 `MULTICA_SERVER_URL` 重启 daemon**（`cs-workflow daemon restart`）和**重新登录**（`cs-workflow logout && cs-workflow login`）。

## 任务一直卡在 queued

**症状**：把 issue 分给 agent 后，issue 状态立刻变 `in_progress`，但过了很久页面没有 agent 执行的迹象；`cs-workflow daemon status` 显示 daemon `online`。
...
```bash
cs-workflow daemon status --output json       # runtime 列表 + last_seen_at
cs-workflow agent list                         # 查 agent 的 archived 状态
cs-workflow issue show <issue-id>             # 看 task 历史
```
...
- 并发打满 → 等现有任务跑完，或 `cs-workflow agent update <id> --max-concurrent-tasks 10` 提升上限
- 同 issue 串行 → 等前一个任务结束，或改分给不同 agent
- Agent 被 archive → `cs-workflow agent restore <id>`
- Runtime 未注册 → `cs-workflow daemon restart`，daemon 会重新注册
...
**症状**：`multica server` 或 `cs-workflow daemon start` 启动失败，报 `address already in use`。
...
| 组件 | 位置 | 命令 |
|---|---|---|
| **守护进程** | `~/.multica/daemon.log`（后台模式）或前台 stdout | `cs-workflow daemon logs -f --lines 100` |
...
需要更详细的 daemon 日志，把它从后台挪到前台跑：`cs-workflow daemon stop && cs-workflow daemon start --foreground`。
