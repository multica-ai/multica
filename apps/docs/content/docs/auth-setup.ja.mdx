...
有効な保留中のワークスペース招待は、そのメールアドレスに対する暗黙のサインアップ許可になります。`ALLOW_SIGNUP=false` の場合:

- 既存ユーザーは引き続きログインできます。
- 新規ユーザーは、保留中かつ有効期限内の招待に記載されたメールアドレスでログインコードを要求し、アカウントを作成できます。
- 招待されていないユーザー、および招待が期限切れ、受諾済み、辞退済み、または取り消し済みのユーザーは引き続きブロックされます。

通常の招待ユーザーを `ALLOWED_EMAILS` に追加したり、サーバーを再起動したりする必要はありません。アカウント作成後も、通常の招待フローで招待を受諾する必要があります。
