...
工具链在本地的结果：**AI 工具自己的凭据、你的代码目录、以及工具本身**都只在本地使用；Multica 服务器都看不到，无论自部署还是用 Cloud。一个刻意的例外：你选择存进 Multica 本身的 secret——某个智能体的 [自定义环境变量](/agents-create#自定义环境变量custom_env)——会存在服务器上，好让它把这些值交给运行时；把它们当作服务器侧的 secret 来对待，相应地控制权限范围。
...
有一件事平台**基本上**不做：它不会强行把你的 issue 标记成完成。issue 状态的变化（`in_progress`、`in_review`、`done`）由智能体在工作过程中自己完成——一次 run 完成只代表"这次执行结束了"，不代表"issue 做完了"。唯一的例外：当一个带 close 意图（`Closes MUL-123`）的关联 GitHub PR 被合并时，issue 会自动转到 `done`。生命周期惯例见 [Issues](/issues)。
