...
| Running too long | **2.5 hours** by default; override with `MULTICA_TASK_RUNNING_TIMEOUT` |

Both timeouts use failure reason `timeout` and **retry automatically** (next section). `MULTICA_TASK_RUNNING_TIMEOUT` accepts a Go duration string such as `8h` or `30m`. For the related runtime-missing check, see [Daemon and runtimes → When a runtime is marked offline](/daemon-runtimes#when-a-runtime-is-marked-offline).
