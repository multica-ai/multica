...
For a sub-issue in the current stage, the same terminal reconciliation also wakes the parent coordinator when the child remains non-terminal with no active task. The recovery notice points to review, unblock, or rerun; it never completes or cancels the child automatically. A configurable periodic scan catches reconciliation events missed during downtime and suppresses duplicate notices across restarts.
