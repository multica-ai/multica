...
- **Waiting** (`waiting_local_directory`) — the task needs a local project directory that isn't available on the claiming machine yet; it holds until the directory is ready
...
There is one more state you may spot in internals: **`deferred`** — a comment arrived for an agent that was already mid-run, so instead of starting a parallel run, Multica holds it and folds it into a single follow-up run when the current one finishes. See [Mentioning agents](/mentioning-agents#what-happens-when-you--multiple-agents).
