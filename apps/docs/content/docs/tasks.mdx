...
| `queued_expired` | The runtime stopped heartbeating for longer than the reconnect grace, and the task had also been queued for at least that long (or the agent's shorter **Queue wait timeout** override) | Confirm the runtime is online, then retry |
...
| `queued` | Waiting for a runtime to claim it | Waits as long as its runtime keeps heartbeating, and in every case for at least one reconnect grace after being queued. An agent may set a shorter **Queue wait timeout**, but the task still fails only after both its own queue window and the runtime-offline window have passed; not retried automatically |
