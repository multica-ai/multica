...
| `queued_expired` | Queued longer than the system default (or the agent's own **Queue wait timeout** override) with no runtime claiming it | Confirm the runtime is online, then retry |
...
| `queued` | Waiting for a runtime to claim it | Fails after **2 hours** unclaimed, or sooner if the agent sets a shorter **Queue wait timeout**; not retried automatically |
