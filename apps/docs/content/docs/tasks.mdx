...
- Continue a finished task in chat. The new conversation inherits that run's agent session and working directory, so you can ask follow-up questions with the run's context instead of starting cold. Only finished tasks offer this — a live run still owns its session and directory. When the run left no resumable session, the chat still opens in its working directory and tells you the agent starts without that context.
