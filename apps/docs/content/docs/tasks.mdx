...
| Situation | Timeout |
|---|---|
| Still waiting in `queued` | System default, or the agent's own **Queue wait timeout** override |
| Dispatched but never started (daemon picked it up but didn't launch the AI tool) | **5 minutes** |
| Running too long | **2.5 hours** |

All three use failure reason `timeout` and **retry automatically** (next section). For the related runtime-missing check, see [Daemon and runtimes → When a runtime is marked offline](/daemon-runtimes#when-a-runtime-is-marked-offline).

The queued timeout is the only one you can tune per agent: in the agent settings, **Queue wait timeout** overrides how long that agent's tasks may wait before a daemon picks them up. Leave it blank or `0` to keep using the system default. Once the task leaves `queued`, this setting no longer matters.
