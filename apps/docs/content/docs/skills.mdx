...
Some AI coding tools also discover **project-level skills committed inside a repository**, such as `.claude/skills/`, `.cursor/skills/`, `.opencode/skills/`, or `.agents/skills/`. When a Multica task checks out that repository, those files remain in the workdir and the underlying tool may load them through its native discovery rules. Multica does **not** import those repo files into the workspace skill registry automatically; they are repo-scoped behavior from the selected tool.
