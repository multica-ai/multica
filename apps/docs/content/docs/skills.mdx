...
With the CLI, you can also import from a `.skill`, `.zip`, `.tar`, or `.tar.gz` file:
...
## Sharing or backing up a skill

Use **Export** from a skill's row menu or detail page to download its `SKILL.md` and supporting files as a portable `.tar.gz` archive. The CLI provides the same operation:

```bash
multica skill export <skill-id>
multica skill export <skill-id> --output ./review-helper.tar.gz
```

Import the resulting archive in another workspace or with `multica skill import --file <path>`. Binary supporting files are included in the download, but the importer skips binary assets, so they are not restored on re-import.
