...
**설정 → Desktop**을 엽니다. **기본 창을 닫을 때**에서 **시스템 트레이로 최소화**(기본값), **작업 표시줄로 최소화** 또는 **애플리케이션 종료**를 선택합니다. 처음 두 옵션은 Multica와 실행 중인 작업 및 연결을 계속 유지합니다. 트레이 모드는 창을 숨기며 Multica 트레이 아이콘에서 다시 열 수 있습니다. 데스크톱 앱을 중지하려면 종료를 선택하세요.
