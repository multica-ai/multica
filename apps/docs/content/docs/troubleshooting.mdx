...
description: Common issues when self-hosting Wallts — symptoms, causes, how to diagnose, how to fix.
---

import { Callout } from "fumadocs-ui/components/callout";

Look up issues by symptom. Each entry gives you **symptom / likely causes / how to diagnose / how to fix**. If your situation isn't listed, open an issue on [GitHub](https://github.com/wallts-ai/wallts/issues).

## Daemon can't connect to the server

**Symptom**: [`wallts daemon`](/cli)'s `status` command shows `offline` or `connection refused`; the server logs show no `/api/daemon/register` or `/api/daemon/heartbeat` requests. For how the daemon mechanism works, see [Daemon and runtimes](/daemon-runtimes).

**Likely causes**:

1. **`WALLTS_SERVER_URL` points at the wrong address** — default is `ws://localhost:8080/ws`; self-host must change it to your server address
2. **Network / firewall blocking** — the daemon and server aren't on the same network, or outbound traffic is blocked
3. **Token expired or invalid** — you never ran `wallts login`, or the PAT was revoked
...
```bash
wallts daemon logs --lines 100    # look for daemon-side errors
echo $WALLTS_SERVER_URL          # confirm the address is set
...
cat ~/.wallts/config.json        # verify api_token exists
wallts workspace list            # confirm you're a member of the target workspace
```

**How to fix**: address each cause above. The two most common fixes are **changing `WALLTS_SERVER_URL` and restarting the daemon** (`wallts daemon restart`) and **signing in again** (`wallts logout && wallts login`).

## Tasks stuck in `queued`

**Symptom**: after assigning an issue to an agent, the issue status flips to `in_progress` immediately, but a long time passes with no sign of agent execution on the page; `wallts daemon status` shows the daemon `online`.
...
```bash
wallts daemon status --output json       # runtime list + last_seen_at
wallts agent list                         # check agent archived state
wallts issue show <issue-id>             # inspect task history
```
...
- Concurrency full → wait for running tasks to finish, or `wallts agent update <id> --max-concurrent-tasks 10` to raise the ceiling
- Same-issue serialization → wait for the previous task to finish, or reassign to a different agent
- Agent archived → `wallts agent restore <id>`
- Runtime not registered → `wallts daemon restart`, and the daemon will re-register
...
- Wrong origin → set `FRONTEND_ORIGIN=https://wallts.yourdomain.com` in the server's `.env` (or comma-separated `CORS_ALLOWED_ORIGINS`) and restart the server
...
| `smtp RCPT TO <addr>: 550 5.7.1 Unable to relay` | The relay's receive connector doesn't allow your subnet to relay to external recipients (most common for anonymous internal relays talking to outside domains) | Either restrict invites to internal recipients, or add the Wallts host's subnet to the Exchange "Anonymous Users → Relay" permission list |
...
- `Unable to relay` → either restrict to internal recipients or grant the Wallts host's IP relay permission on the Exchange receive connector
...
1. **`WALLTS_DEV_VERIFICATION_CODE` is empty** — fixed codes are disabled by default
...
```bash
cat .env | grep -E 'APP_ENV|WALLTS_DEV_VERIFICATION_CODE'
docker exec <container> env | grep -E 'APP_ENV|WALLTS_DEV_VERIFICATION_CODE'
```
...
- In production, leave `WALLTS_DEV_VERIFICATION_CODE` empty — configure Resend and use real codes
- For local development or internal testing, either copy the generated code from server logs or set `APP_ENV=development` plus `WALLTS_DEV_VERIFICATION_CODE=888888` — never enable a fixed code on a public instance (see [Sign-in and signup configuration → Fixed local testing codes](/auth-setup#fixed-local-testing-codes))
...
2. **`pg_cron` is installed but pointing at the wrong database** — `pg_cron.database_name` defaults to `postgres`; if your Wallts database has a different name, the scheduled job never sees `rollup_task_usage_hourly()`.
...
**Symptom**: `wallts server` or `wallts daemon start` fails with `address already in use`.
...
|---|---|---|
| **Daemon** | `~/.wallts/daemon.log` (background mode) or foreground stdout | `wallts daemon logs -f --lines 100` |
| **Server (Docker)** | Container stdout | `docker logs -f <container>` |
| **Server (systemd)** | journal | `journalctl -u wallts-server -f` |
| **Frontend (dev)** | Terminal running `pnpm dev` | Read directly |
| **Frontend (browser)** | DevTools → Console | Press `F12` |

For more detailed daemon logs, move it from background to foreground: `wallts daemon stop && wallts daemon start --foreground`.
