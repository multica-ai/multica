...
**Symptom**: [`cs-workflow daemon`](/cli)'s `status` command shows `offline` or `connection refused`; the server logs show no `/api/daemon/register` or `/api/daemon/heartbeat` requests. For how the daemon mechanism works, see [Daemon and runtimes](/daemon-runtimes).
...
3. **Token expired or invalid** — you never ran `cs-workflow login`, or the PAT was revoked
...
```bash
cs-workflow daemon logs --lines 100    # look for daemon-side errors
...
cs-workflow workspace list            # confirm you're a member of the target workspace
```

**How to fix**: address each cause above. The two most common fixes are **changing `MULTICA_SERVER_URL` and restarting the daemon** (`cs-workflow daemon restart`) and **signing in again** (`cs-workflow logout && cs-workflow login`).

## Tasks stuck in `queued`

**Symptom**: after assigning an issue to an agent, the issue status flips to `in_progress` immediately, but a long time passes with no sign of agent execution on the page; `cs-workflow daemon status` shows the daemon `online`.
...
```bash
cs-workflow daemon status --output json       # runtime list + last_seen_at
cs-workflow agent list                         # check agent archived state
cs-workflow issue show <issue-id>             # inspect task history
```
...
- Concurrency full → wait for running tasks to finish, or `cs-workflow agent update <id> --max-concurrent-tasks 10` to raise the ceiling
- Same-issue serialization → wait for the previous task to finish, or reassign to a different agent
- Agent archived → `cs-workflow agent restore <id>`
- Runtime not registered → `cs-workflow daemon restart`, and the daemon will re-register
...
**Symptom**: `multica server` or `cs-workflow daemon start` fails with `address already in use`.
...
|---|---|---|
| **Daemon** | `~/.multica/daemon.log` (background mode) or foreground stdout | `cs-workflow daemon logs -f --lines 100` |
...
For more detailed daemon logs, move it from background to foreground: `cs-workflow daemon stop && cs-workflow daemon start --foreground`.
