...
A plain `curl` against `/ws` returning HTTP 400 is expected: the endpoint requires workspace-scoped query parameters and rejects a bare handshake. A 400/401 recorded in the backend logs proves the request traversed the whole proxy chain — the link itself works.
