...
<Callout type="info">
Multica 서버는 이 도구들이나 그것들이 인증에 사용하는 자격 증명을 결코 보지 못합니다. 아래의 모든 것 — 설치, 인증, 모델 접근 — 은 사용자의 로컬 기기에 존재합니다. 무언가 실패한다면 거의 항상 로컬 문제입니다. (별개로: 여러분이 의도적으로 에이전트의 [사용자 지정 환경 변수](/agents-create#custom-environment-variables-custom_env)에 저장하는 secret은 서버에 저장됩니다 — 이는 도구 자체의 로컬 로그인과는 다른 경로입니다.)
</Callout>
