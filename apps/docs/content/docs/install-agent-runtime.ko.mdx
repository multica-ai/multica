...
오픈소스 CLI 코딩 하네스입니다([forgecode.dev](https://forgecode.dev/)). 데몬은 원샷 모드로 `forge -C <workdir> --cid <conversation-id> -p <prompt>`로 실행합니다. 세션 재개는 고정된 conversation id를 재사용하는 방식으로 동작합니다. ForgeCode에는 `--model` flag가 없습니다 — 모델은 `FORGE_SESSION__PROVIDER_ID` / `FORGE_SESSION__MODEL_ID` 환경 변수로 선택됩니다(`MULTICA_FORGE_MODEL=provider/model`로 설정, 예: `anthropic/claude-sonnet-4-20250514`). ForgeCode의 원샷 모드는 구조화된 이벤트 스트림이 아니라 Markdown을 렌더링하므로, 데몬은 실행 후 `forge conversation dump <id>`에서 구조화된 메시지 목록을 재생합니다. 스킬은 `.forge/skills/`로 복사됩니다.

| | |
|---|---|
| 데몬이 찾는 이름 | `forge` |
| 설치 | 공식 문서 [forgecode.dev](https://forgecode.dev/)를 참고하세요. |
| 인증 | 공급사 문서에 따릅니다(구성된 제공자의 API 키). |
