---
title: 聊天集成
description: 把 Multica 智能体接入飞书、Lark、Slack、钉钉、企业微信、Telegram 或 Mattermost，在团队已有的聊天工具中使用。
---
...
| | 飞书 / Lark | Slack | 钉钉 | 企业微信 | Telegram | Mattermost |
|---|---|---|---|---|---|---|
| 安装方式 | 在 Multica 中生成二维码，用飞书扫码授权 | 在 Slack 创建 app，再把两个 token 填入 Multica | 创建企业内部应用和 Stream 模式机器人，再把 AppKey 与 AppSecret 填入 Multica | 在企业微信管理后台创建智能机器人并开启长连接，再把 Bot ID 与 Secret 填入 Multica | 用 @BotFather 创建 Bot，再把 token 填入 Multica | 在 Mattermost 系统控制台创建机器人账号，再把服务器地址和访问令牌填入 Multica |
| 私聊智能体 | 支持 | 支持 | 支持 | 支持 | 支持 | 支持 |
| 群聊或频道 | @ Bot 后触发 | @ Bot 后触发 | @ Bot 后触发 | @ Bot 后触发 | @ Bot 或回复 Bot 后触发 | @ 机器人，或在机器人发起的话题串中回复后触发 |
| 创建 issue | `/issue` 消息命令，按输入直接创建 | `/issue` slash 命令，智能体整理描述后创建 | `/issue` 消息命令，按输入直接创建 | `/issue` 消息命令，按输入直接创建 | `/issue` 消息命令，按输入直接创建 | `/issue` 消息命令，按输入直接创建 |
...
| 连接方式 | 平台长连接 | Socket Mode | Stream 模式 | 平台长连接 | `getUpdates` 长轮询 | WebSocket |
...
- [Mattermost Bot 接入](/mattermost-bot-integration)
