...
- 个人微信按扫码者的私聊区分会话；当前版本只接收扫码者本人的私聊消息。
...
MULTICA_WEIXIN_SECRET_KEY=<base64 编码的 32 字节密钥>
```
...
个人微信使用腾讯 iLink Bot API。工作区管理员在 Agent 的“集成”页面点击“连接个人微信”，再用个人微信扫码确认。二维码确认返回的 Bot Token 会加密保存。当前版本仅支持扫码者本人私聊和文本消息，群聊与媒体消息会被忽略。

个人微信当前也要求后端只部署单个副本。接收长轮询和回复所需的会话上下文保存在持有连接的进程中；多副本可能使 Agent 的回复落到其他进程而无法发送。
