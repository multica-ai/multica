...
**設定 → Desktop** を開きます。**メインウィンドウを閉じるとき**で、**システムトレイに最小化**（既定）、**タスクバーに最小化**、または**アプリケーションを終了**を選びます。最初の 2 つでは Multica、実行中の処理、接続が動作し続けます。トレイモードではウィンドウが非表示になり、Multica のトレイアイコンから再表示できます。デスクトップアプリを停止する場合は終了を選んでください。
