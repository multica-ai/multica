...
图片、文件、视频、语音和音频消息会作为同一对话的附件交给智能体，caption 作为消息正文。Telegram 只允许 Bot 下载 20 MB 以内的文件，更大的媒体只会以占位文字到达。智能体生成的文件会在回复之后作为单独的消息发回聊天：10 MB 以内的图片内联显示，其余 50 MB 以内的文件以文件形式到达。这两个方向都需要服务端配置对象存储（见下文自托管部分）；未配置时，媒体只会以占位文字到达，智能体也会被告知无法发送文件。贴纸、位置、投票等非文件消息在私聊中以及群里明确 @ Bot 时会收到不支持提示，群里未 @ 的此类消息保持静默。
