...
管理対象の Prime Agent は共有 `~/.prime/agent` を使用しません。Multica は runtime/agent ごとに sessions、settings、authentication、user skills 用の非公開永続ディレクトリを割り当てます。認証は各 managed agent で個別に設定するか、明示的にスコープを限定した環境変数を使用してください。tenant 間で共有する MCP settings や secrets を Prime の project state に保存しないでください。
