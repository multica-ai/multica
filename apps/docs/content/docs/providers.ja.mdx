...
DeepSeek Harness にもモデル一覧を取得するコマンドがないため、一覧は空になります。DeepSeek のモデル ID（例: `deepseek-chat`）を手入力するか、空のままにして DeepSeek Harness 側で設定されたモデルを使用してください。
...
DeepSeek Harness を除き、上記のツールはすべて、後続の実行で既存のセッションを続行できます（`dsh --profile headless` はタスクごとに新しいセッションを開始します）。ただし、元のセッションが残っており、そのセッションへアクセスできるランタイムでタスクが実行される必要があります。
