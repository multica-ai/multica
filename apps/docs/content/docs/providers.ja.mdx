...
| **ForgeCode** | オープンソース | ✅ (`--cid <id>`) | ❌ | `.forge/skills/` | 静的（環境変数） |
...
オープンソースの CLI コーディングハーネスです（[forgecode.dev](https://forgecode.dev/)）。他のサポート対象ツールとは異なり、ForgeCode のワンショットモード（`forge -p`）は構造化されたイベントストリームを**出力しません** — ツール呼び出し、推論、本文が混ざったスタイル付き Markdown を stdout へレンダリングします。Multica はこれを 2 段階のアプローチで回避します: ライブの stdout をバッファリングし（liveness heartbeat のみ転送）、実行後に `forge conversation dump <id>` から構造化されたメッセージリスト（ツール呼び出し、ツール結果、推論、トークン使用量）を再生します。**セッション再開は動作します** — 固定の `--cid <id>` を通じて行われ、conversation id を再利用して以前の会話を続行します。**モデル選択の方式が独特です** — `--model` flag は存在せず、モデルは `FORGE_SESSION__PROVIDER_ID` / `FORGE_SESSION__MODEL_ID` 環境変数で設定されます。Multica は `provider/model` 値（例: `anthropic/claude-sonnet-4-20250514`）からこれらを導出します。MCP 構成は消費されません。スキルは `.forge/skills/` にコピーされます。
...
**14 個のツールのうち、`mcp_config` を実際に消費するのは 10 個です: Claude Code、CodeBuddy、Codex、Cursor、Hermes、Kimi、Kiro CLI、OpenCode、OpenClaw、Qoder**。残りの 4 個はこのフィールドを受け取りますが、**無視します** — エラーも警告もなく、構成はただ効果を発揮しません。
