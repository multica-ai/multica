...
**現在の互換性状態:** Prime Agent v0.7.2 は catalog に表示されますが、実行 admission は無効です。公式 daemon は Multica が検査する前に永続 schedules/heartbeats を起動でき、それらを coding tools を維持したまま startup 時に hard-disable する公式 mechanism がありません。起動後の cancel は競合するため Multica は fail closed します。

管理対象の Prime Agent は共有 `~/.prime/agent` を使用しません。Multica は workspace/runtime/agent ごとに sessions、settings、authentication、user skills 用の非公開永続ディレクトリを割り当てます。認証は各 managed agent で個別に設定するか、明示的にスコープを限定した環境変数を使用してください。tenant 間で共有する MCP settings や secrets を Prime の project state に保存しないでください。
