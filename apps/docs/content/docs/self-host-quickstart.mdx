...
4. Wait until the backend's `/readyz` endpoint is ready

`/health` only proves the backend process is listening. `/readyz` and `/healthz`
also check PostgreSQL and migration state, so use them for Compose healthchecks,
production readiness probes, and startup diagnostics that should fail on
database or migration problems.

The backend container **runs database migrations automatically** on startup (`docker/entrypoint.sh` runs `./migrate up` before the server starts) — you'll see the migration output in the backend logs. Version upgrades are handled the same way for single-host Docker Compose. For Kubernetes or multi-replica releases, run migrations once through a Job or controlled preflight before rolling backend pods; keep readiness on `/readyz` or `/healthz` and stop the rollout if migrations fail.
...
- **Backend won't start**: check container logs with `docker compose -f docker-compose.selfhost.yml logs backend`; common causes are a bad `JWT_SECRET`, an external `DATABASE_URL` that the container cannot reach, or an old upload volume whose files are still owned by a previous root-running image
