...
multica.example.com {
    ...
    # needs a 200 — the frontend does not serve it
    handle /health {
        reverse_proxy 127.0.0.1:8080
    }

    # Browser and daemon WebSockets — straight to the backend
    handle /ws {
        reverse_proxy 127.0.0.1:8080 {
            flush_interval -1
        }
...
The last command prints `daemon_server_url`, the URL daemons use to reach the API (`MULTICA_PUBLIC_URL` when set, otherwise `MULTICA_APP_URL`; empty output means neither is set). If it prints a `localhost` address, `.env` still carries local defaults: `FRONTEND_ORIGIN` and `MULTICA_APP_URL` do not follow `FRONTEND_PORT` automatically on Docker Compose releases older than 2.24 (the `${FRONTEND_PORT}` references copied from `.env.example` stay literal there), so set them explicitly.
...
The command first checks `<server-url>/health`, then opens a browser to complete login. After login it stores local credentials and starts the daemon. A `Server not reachable` error at this step almost always means `/health` returned 404 through the proxy — see [Troubleshooting](/troubleshooting#multica-setup-reports-the-server-is-not-reachable).
