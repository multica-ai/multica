...
4. Wait until backend `/readyz` and the frontend page are ready

`/health` is liveness only. Startup and production probes that should fail on
database or migration problems should use `/readyz` or `/healthz`.
...
That points the CLI at `http://localhost:8080` (backend) and `http://localhost:3000` (frontend), takes you through browser login, stores the PAT locally, and **starts the daemon automatically**. Server startup remains separate: `make selfhost`, the installer, or `./start.sh` brings up Docker services; `multica setup self-host` configures the CLI and daemon.

### 5b. Cross-machine: front with a reverse proxy

Because the compose stack listens on `127.0.0.1` by default, a daemon on a different machine cannot reach `http://<server-ip>:8080` directly — and you normally do not want it to, since server secrets would otherwise be reachable from the open internet. Put a reverse proxy on the server that terminates TLS and forwards to `127.0.0.1:8080` (backend) and `127.0.0.1:3000` (frontend), then point the CLI at the public HTTPS URL:
