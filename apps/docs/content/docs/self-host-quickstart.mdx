...
Two ways to upgrade a Docker Compose install. They are **equivalent** — `make selfhost` is a wrapper around exactly these commands (`Makefile:79-107`), so pick whichever you prefer:
...
`make selfhost` only generates `.env` when the file is **missing** (`Makefile:80-95`). Re-running it on an existing install leaves your `JWT_SECRET`, Postgres password, email settings, and `FRONTEND_ORIGIN` exactly as they were.
...
Helm has its own upgrade path — `helm upgrade` with `images.backend.tag` / `images.frontend.tag` pinned in your values file, or `kubectl -n multica rollout restart` to re-pull a floating tag. See [Kubernetes deployment](#kubernetes-deployment-alternative) below.
