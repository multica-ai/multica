...
去 [GitHub Releases](https://github.com/kanfashidoufu/multica/releases/latest) 拿对应平台的安装包：
...
如果 `desktop.json` 存在但内容无效，Desktop 会 fail closed，显示阻塞式配置错误，而不是悄悄回退到 Cloud。开发构建里，`electron-vite dev` 仍然优先使用 `VITE_API_URL` / `VITE_WS_URL` / `VITE_APP_URL`。
</Callout>
