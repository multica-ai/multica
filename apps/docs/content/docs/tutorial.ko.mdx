...
사이드바 아래쪽의 **설정 → 런타임**을 열고 **컴퓨터 추가**를 선택합니다. 안내에 따라 Multica CLI를 설치하고 데몬을 시작한 뒤 이 컴퓨터가 온라인인지 확인합니다.
...
런타임 페이지에서 **컴퓨터 추가**를 클릭하고 안내에 따라 명령줄로 설치하면 데몬이 백그라운드에 상주하며 실행됩니다. 자세한 내용은 [데몬과 런타임](/daemon-runtimes)을 참고하세요.
