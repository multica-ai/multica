...
<Callout type="info">
**用的是桌面应用？** 桌面应用启动时**自动拉起守护进程**，不需要手动跑 `cs-workflow setup`。见 [桌面应用](/desktop-app)。
</Callout>
...
```bash
cs-workflow issue create --title "给 README 加一段 ASCII 架构图"
```
...
`--to` 后面填智能体或成员的**名字**，子串就行——如果智能体叫 `my-code-reviewer`，填 `reviewer` 也能命中。如果工作区里名字相互重叠或冲突，改用 `--to-id <uuid>`（与 `--to` 互斥）；UUID 来自 `cs-workflow agent list --output json` 或 `cs-workflow workspace member list --output json`。
