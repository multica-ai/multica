...
- 개인 Weixin은 QR 코드를 스캔한 계정의 DM을 별도 대화로 유지합니다. 초기 버전은 해당 계정의 DM만 받습니다.
...
MULTICA_WEIXIN_SECRET_KEY=<base64로 인코딩한 32바이트 키>
```
...
개인 Weixin은 Tencent iLink Bot API를 사용합니다. Agent 연동 페이지에서 QR 코드를 스캔하고 Weixin에서 확인하세요. 초기 버전은 스캔한 본인의 DM과 텍스트 메시지를 지원하며 그룹과 미디어 메시지는 무시합니다. 응답에 필요한 대화 컨텍스트를 long-poll 프로세스가 보유하므로 현재는 백엔드를 단일 레플리카로 운영해야 합니다.
