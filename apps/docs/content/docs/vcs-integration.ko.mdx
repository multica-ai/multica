...
Multica connects to a self-hosted Git forge per workspace: **Forgejo**, **Gitea**, or **GitLab**. Once connected, any pull request (GitLab: merge request) whose branch name, title, or body contains an issue identifier (for example `MUL-123`) is **auto-linked** to that [issue](/issues), appears under **Pull requests** in the issue sidebar, and — when merged with a closing keyword — moves the issue to **Done**. CI for the head commit shows as a checks bar on the card.

These forges run alongside GitHub: a workspace can use any combination.

<Callout type="info">
Unlike GitHub, these forges have no "App" model. Each workspace stores its own instance URL plus an access token, and registers a webhook on its repository or organisation. The token and webhook secret are encrypted at rest.
</Callout>
...
1. Create an access token in your forge with read access to the repositories you want mirrored:
   - **Forgejo / Gitea**: Settings → Applications.
   - **GitLab**: a personal (or group/project) access token with the `read_api` scope.
2. In Multica, open **Settings → Integrations → Git forges**.
...
- **Pull / merge requests** — open, closed, merged, and draft state, author, branch, and (where the forge provides them) diff stats.
...
Creating PRs needs no forge configuration in Multica. Agents check out repositories in the runtime and push branches and open PRs using the runtime host's own Git credentials. To let agents work against a forge, ensure the daemon host can authenticate to it — for example an SSH deploy key, or a token in the host's Git credential helper — and add the repository URL as usual. Repository checkout works with any Git URL, so no provider-specific wiring is required.
