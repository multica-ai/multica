...
There is **no schema migration**, no new sqlc query, no new endpoint. The same `project_resource` table and the same three CRUD calls handle every type.
