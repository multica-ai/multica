...
Today two resource types ship: [`github_repo`](#resource-type-github_repo) (clone-per-task into an isolated worktree) and [`local_directory`](#resource-type-local_directory) (use a bound folder on a specific daemon, either in place or through per-issue linked worktrees).
...
For repos that can't reasonably be re-cloned per task — multi-gigabyte game checkouts, large monorepos, or any project where clone-per-task is painful — a project can instead point at an **existing directory on a specific [daemon](/daemon-runtimes)'s machine**. It can run directly in that folder (`in_place`) or create one linked worktree per issue (`worktree`), without cloning the repository again.
...
`mode` defaults to `in_place`, preserving the original behavior: tasks sharing the resolved directory serialize and edit the user's checkout. `worktree` requires a Git working tree, creates a persistent branch and worktree per issue under the daemon's workspace root, serializes tasks on the same issue, and allows different issues to run in parallel. A definite non-Git path falls back to `in_place`; Git inspection or worktree creation errors fail the task.

### When to pick `local_directory` over `github_repo`

| Concern | `github_repo` (worktree) | `local_directory` |
| --- | --- | --- |
| Checkout cost per task | Fresh clone + worktree | No clone; in place or linked worktree |
| Concurrency on the same repo | Many tasks in parallel | Serial in `in_place`; parallel per issue in `worktree` |
| Branch / dirty state | Each task gets a fresh branch from the default | Live checkout in `in_place`; persistent issue branch in `worktree` |
| Where it can run | Any daemon | Exactly one daemon (the one bound) |
| Disk footprint | One worktree per task | Zero in `in_place`; one persistent worktree per issue in `worktree` |
...
Choose `in_place` when the live checkout and its dirty state are intentional. Choose `worktree` when the path is a Git repository and cross-issue parallelism matters. Neither mode provides file-level locking; isolation in `worktree` mode comes from separate issue worktrees.
...
2. In `in_place`, acquires a lock keyed on the symlink-resolved path. In `worktree`, acquires a per-issue lock and creates or validates that issue's linked worktree while a per-repository Git lock protects add/remove operations.
3. Writes the agent's runtime context into the selected workdir: the user's directory for `in_place`, or the issue worktree for `worktree`.
4. Keeps Multica's runtime artefacts (`output/`, `logs/`, `.gc_meta.json`) in a separate envRoot **outside** the user's directory.

In `in_place`, a second task for the same directory waits. In `worktree`, only another task on the same issue waits; different issues use different worktrees and may run concurrently. Waiting tasks use the **Waiting for local directory** status.
...
- **Will never physically delete the user's bound directory.** In `worktree` mode, garbage collection may remove a clean daemon-owned issue worktree after the issue has been terminal past the configured TTL. Dirty worktrees are preserved.
...
- **No automatic branch switching in `in_place`.** The agent runs on the branch you have checked out. `worktree` creates or reuses its deterministic per-issue branch.
- **No dirty-tree protection or auto-commit.** In `in_place`, existing changes are visible and may be modified. In `worktree`, changes persist in the issue worktree; GC refuses to remove it while dirty.
