...
Browsers cannot pick folders on your computer. Add local-directory resources through the CLI or API on the machine running the daemon.
...
1. Make sure the target daemon is online.
2. Create the resource through the CLI or API with its absolute path and daemon ID.
3. Set `execution_mode` to `in_place` or `worktree`, described under "How tasks share the directory".
