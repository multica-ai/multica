...
cs-workflow project create \
  --title "Agent UX 2026" \
  --repo https://github.com/multica-ai/multica

# Manage resources later
cs-workflow project resource list <project-id>
cs-workflow project resource add  <project-id> --type github_repo --url <url>
cs-workflow project resource remove <project-id> <resource-id>

# Generic escape hatch for any resource_type the server understands —
# no CLI change needed when a new type ships:
cs-workflow project resource add <project-id> \
  --type notion_page \
  --ref '{"page_id":"…","title":"…"}'
```
...
`github_repo` resources, use `cs-workflow repo checkout <url>` to fetch the code.
```
...
- **Caching / sync.** `github_repo` is just metadata — checkout still happens via `cs-workflow repo checkout` on demand. Cached document text for Notion / Google Docs will arrive with those types.
