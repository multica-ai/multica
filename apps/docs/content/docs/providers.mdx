...
From xAI. CLI binary name is `grok` (Grok Build CLI, often installed to `~/.grok/bin/grok`). Multica drives it over **ACP** via `grok agent --always-approve stdio`, reusing the same transport as Trae / Kimi / Kiro / Qoder. **Session resumption works** through ACP `session/load`. **Model selection works** via the catalog returned from `session/new` and `session/set_model`. **Reasoning effort** is exposed through Multica’s thinking picker and passed as `--reasoning-effort` on the `agent` command. **MCP is supported** — config is passed through ACP `mcpServers` (filtered to transports advertised by `initialize`). Skills land in `.grok/skills/` (project) and `~/.grok/skills` (user); the CLI also scans `.agents/skills/`. Tool calls stream as ACP `tool_call` / `tool_call_update` events.
...
**Of the 15 tools, twelve consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, Trae, and Grok**. The other three (Antigravity, Copilot, Pi) accept the field but **ignore it** — no error, no warning, the config just has no effect.
