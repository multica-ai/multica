...
From Google, powered by AGY / Antigravity CLI. The model picker follows AGY's current catalog: Gemini 3.5 Flash, Gemini 3.1 Pro, Claude Sonnet/Opus 4.6 Thinking, and GPT-OSS 120B. **No session resumption and no MCP** — suitable for one-shot tasks that don't need long context memory.
