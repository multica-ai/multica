...
- **Authentication** uses Trae's own provider API-key environment variables (e.g. `DOUBAO_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`) — configure them on the runtime like any other CLI's credentials.
