...
| **Qwen Code** | Alibaba | ✅ (`--resume`) | ❌ (native settings only) | `.qwen/skills/` | Static default + `--model` |
...
From Alibaba. CLI binary name is `qwen`. Multica launches Qwen Code with `qwen -p <prompt> --output-format stream-json --yolo`, parses its native JSONL text, thinking, tool-use, tool-result, and terminal-result events, and resumes sessions with `--resume <session-id>`. The selected model is passed through `--model`; Qwen Code also reports the effective model in its system stream event. Skills are injected into `.qwen/skills/`, and the runtime brief is written to `QWEN.md`, both native Qwen Code locations. **Managed `mcp_config` is deliberately rejected** rather than silently ignored: Qwen Code has no CLI MCP-config flag. Leave the field unset to inherit your own Qwen Code MCP settings from `.qwen/settings.json`.
...
**Of the 17 tools, twelve consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, Trae, and Grok**. Antigravity, Copilot, DevEco Code, and Pi accept the field but **ignore it** — no error, no warning, the config has no effect. Qwen Code instead rejects a managed `mcp_config` at launch because it has no equivalent CLI flag; omit the field to inherit Qwen native settings.
...
If you set `mcp_config` in an agent configuration but pick a tool not marked ✅ in the MCP column, MCP is provider-specific: most unsupported tools ignore it, while Qwen Code rejects the run rather than silently dropping it.
</Callout>
