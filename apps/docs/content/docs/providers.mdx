...
## CodeBuddy ACP execution

CodeBuddy runs through `codebuddy --acp`, using the shared ACP client for
streaming text, reasoning, tool events, permission requests, and usage.
Multica creates sessions with `session/new` and restores them with
`session/load`. Model selection and reasoning effort use session RPCs;
Multica-managed MCP servers are passed when creating or loading the session.
CodeBuddy must already be signed in on the runtime computer.

When CodeBuddy omits token counters from a resumed ACP stream, Multica
reconciles usage with the CLI's local session record for that exact request.
Earlier turns and duplicate messages are excluded. Keep CodeBuddy session
persistence enabled to retain this metering source; unavailable counters are
not estimated, and CodeBuddy credits are not treated as a USD cost.

Each run still owns its CLI process and task credentials. ACP session
resumption does not keep the process alive between runs or guarantee faster
startup. File and terminal tools continue to run inside CodeBuddy; Multica
does not advertise client-side file or terminal proxy capabilities for this
provider. CodeBuddy-specific slash-command and Agent Teams interfaces are
not exposed by this integration.

References: [CodeBuddy ACP](https://www.codebuddy.cn/docs/cli/acp) and
