...
| **Grok Build** | xAI | ✅ | ✅ | `.grok/skills/` | Dynamic discovery (`grok models`) |
...
From xAI. An agentic coding CLI (binary `grok`). Uses the ACP protocol over stdio via `grok agent stdio` (shares a transport with Hermes, Kimi, Kiro CLI, and Qoder), with one extra step: the daemon performs an ACP `authenticate` round-trip after `initialize`. Session resumption works through ACP `session/resume`, MCP config is passed through ACP `mcpServers`, model selection is discovered dynamically (`grok models`), and skills are copied into `.grok/skills/` for native discovery. Grok is also Claude Code-compatible, so it additionally reads `CLAUDE.md` / `AGENTS.md` and `.claude/skills/`.

**Authenticating the runtime host.** Grok accepts two credential paths and Multica supports both:

- **Browser login (`grok login`)** — a one-time OAuth that works with a SuperGrok subscription and persists a cached token on the host. Do this once on any runtime host that has a browser. Grok normally opens a browser on first launch, but the daemon authenticates non-interactively (it never pops a browser mid-task), so the token must already exist before a task runs.
- **API key (`XAI_API_KEY`)** — set this in the runtime's environment for headless / CI hosts with no browser.

When `XAI_API_KEY` is present Multica prefers it; otherwise it falls back to the cached `grok login` token. If neither is available the task fails with a hint to run `grok login` or set `XAI_API_KEY` rather than hanging.
...
**Of the 14 tools, eleven consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Grok Build, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, and Qoder**. The other three accept the field but **ignore it** — no error, no warning, the config just has no effect.

The runtime paths are provider-specific: Claude Code and CodeBuddy receive it through `--mcp-config` paired with `--strict-mcp-config`; Codex writes a daemon-managed `mcp_servers` block into the per-task `$CODEX_HOME/config.toml`; Cursor writes `.cursor/mcp.json` plus per-task project approvals under `CURSOR_DATA_DIR`; Grok Build, Hermes, Kimi, Kiro CLI, and Qoder receive ACP `mcpServers`; OpenCode receives inline config through `OPENCODE_CONFIG_CONTENT`; OpenClaw receives `mcp.servers` through Multica's per-task config wrapper. OpenCode's path does **not** rewrite the project's `opencode.json`.
...
| Grok Build | `.grok/skills/` | ✅ Native (also reads `.claude/skills/` via Claude Code compatibility) |
