...
From xAI. An agentic coding CLI (binary `grok`). Uses the ACP protocol over stdio via `grok agent stdio` (shares a transport with Hermes, Kimi, Kiro CLI, and Qoder), with one extra step: the daemon performs an ACP `authenticate` round-trip after `initialize`, preferring the `xai.api_key` method when `XAI_API_KEY` is set and otherwise the `cached_token` written by `grok login`. Session resumption works through ACP `session/resume`, MCP config is passed through ACP `mcpServers`, model selection is discovered dynamically (`grok models`), and skills are copied into `.grok/skills/` for native discovery. Grok is also Claude Code-compatible, so it additionally reads `CLAUDE.md` / `AGENTS.md` and `.claude/skills/`.
