...
| **Qwen Code** | Alibaba | ✅ (`--resume <id>`) | ❌ | `.qwen/skills/` | Static + flag |
...
**Of the 17 tools, twelve consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, Trae, and Grok**. Antigravity, Copilot, DevEco Code, and Pi accept the field but ignore it. **Qwen Code rejects managed `mcp_config` explicitly**, because its CLI exposes no per-process MCP configuration channel; configure persistent servers with `qwen mcp add`.
...
If you set `mcp_config` in an agent configuration but pick a tool not marked ✅ in the MCP column, your MCP servers have no effect on Antigravity, Copilot, DevEco Code, or Pi. For Qwen Code, task start instead fails with an actionable configuration error; configure its persistent MCP servers with `qwen mcp add`.
</Callout>
