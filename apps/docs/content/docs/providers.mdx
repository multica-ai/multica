...
| **OMP** | Oh My Pi | ✅ (ACP `session/load`) | ✅ | `.omp/skills/` | Dynamic discovery |
...
From [oh-my-pi](https://pi.dev/) — a Pi-derived coding agent. It is **not** wire-compatible with the upstream `pi` CLI's bespoke JSON stream mode, so Multica drives it over the standard ACP JSON-RPC transport via `omp acp --yolo`, exactly like Kiro, Qoder, and Trae.

- **Session resumption works** through ACP `session/load` (omp advertises `loadSession: true` from `initialize`).
- **Model selection is dynamic**: the model catalog is discovered from the `session/new` `configOptions` model selector (OMP does not emit a `models` block) and switched per task via `session/set_config_option`. Set the daemon-wide default with `MULTICA_OMP_MODEL`.
- **MCP is supported** — config is passed through ACP `mcpServers`; unsupported remote transports are filtered out before `session/new`.
- **Skills** are written to `.omp/skills/` (project) for native discovery; global skills live in `~/.omp/agent/skills`.
- omp reads `AGENTS.md` natively, so the runtime brief is **not** inlined into the prompt (unlike Trae).
...
**Of the 18 tools, fourteen consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, Qwen Code, Trae, Grok, and OMP**. The other four (Antigravity, Copilot, DevEco Code, Pi) accept the field but **ignore it** — no error, no warning, the config just has no effect.

The runtime paths are provider-specific: Claude Code, CodeBuddy, and Qwen Code receive it through daemon-owned temporary JSON files passed with `--mcp-config` (mode `0600` on POSIX systems because MCP configs may embed API tokens); Claude Code and CodeBuddy additionally use `--strict-mcp-config`; Codex writes a daemon-managed `mcp_servers` block into the per-task `$CODEX_HOME/config.toml`; Cursor writes `.cursor/mcp.json` plus per-task project approvals under `CURSOR_DATA_DIR`; Hermes, Kimi, Kiro CLI, Qoder, Trae, Grok, and OMP receive ACP `mcpServers`; OpenCode receives inline config through `OPENCODE_CONFIG_CONTENT`; OpenClaw receives `mcp.servers` through Multica's per-task config wrapper. OpenCode's path does **not** rewrite the project's `opencode.json`.
