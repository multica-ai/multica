...
From xAI (binary `grok`). Uses ACP over stdio (`grok agent stdio`) — the same transport as Hermes, Kimi, Kiro CLI, and Qoder, with one wrinkle: an extra `authenticate` round-trip after `initialize`. Session resumption goes through `session/resume`, MCP config through `mcpServers`, models are discovered with `grok models`, and skills land in `.grok/skills/`. It's Claude Code–compatible, so it also reads `CLAUDE.md` / `AGENTS.md` and `.claude/skills/`. Authenticate the host once — `grok login` (works with a SuperGrok subscription) or `XAI_API_KEY` on headless hosts; the daemon never opens a browser, so the credential has to exist before a task runs.
