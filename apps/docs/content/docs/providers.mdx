...
| **Devin** | Cognition AI | ✅ (via ACP `session/load`) | ❌ (managed via Devin's own `devin mcp`) | `.devin/skills/` | Driven by Devin's own config (daemon-side selection unsupported) |
...
From Cognition AI. Uses ACP over stdio via `devin acp` (same protocol family as Hermes / Kimi / Kiro). Session resumption works through ACP `session/load`. Skills are auto-discovered from `.devin/skills/`, mirroring Devin for Terminal's native convention. **Model selection happens entirely on Devin's side** — Devin's ACP server doesn't implement `session/set_model`, so the daemon-side model dropdown is hidden; configure your model with `devin --model` or `~/.config/devin/config.json`. Tool auto-approval works through ACP's `session/request_permission`, which the daemon answers automatically per task.
...
|---|---|---|
| ✅ Really works | Claude Code, Copilot, Devin, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Pi | Pass the resume id and it continues from the previous context |
