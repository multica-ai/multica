...
QwenPaw, MiniMax Code, and Prime Agent are exceptions: they do not accept a model chosen from Multica, so the picker is inert and shows "Managed by runtime". Choose the model in the tool's own configuration instead. An empty model list is expected there and is not a sign that the runtime is offline.
...
Prime Agent is the exception: it hosts exactly one session per run and has no resume mechanism at all, so every run starts a fresh session regardless of prior history on the same issue.

Retry and session-resumption rules are in [Tasks](/tasks#manual-retry).

## MCP configuration

Antigravity, GitHub Copilot CLI, DevEco Code, Pi, and Prime Agent do not read the MCP configuration on an agent yet, so the field is not shown when creating agents for them.
