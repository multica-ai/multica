...
DeepSeek Harness has no model-list command either, so its list is empty: type a DeepSeek model id (for example `deepseek-chat`) manually, or leave it empty to use the model configured in DeepSeek Harness itself.
...
All of these tools support continuing an existing session in a later run, except DeepSeek Harness: `dsh --profile headless` starts a fresh session for every task. The prerequisites are that the original session still exists and the task is executed by a runtime that can reach it.
