...
QwenPaw and ZCode are the exceptions: neither accepts a model chosen from Multica, so the picker is inert and shows "Managed by runtime". Choose the model in the tool's own configuration instead (QwenPaw config, or `~/.zcode/cli/config.json` `model.main` for ZCode). For ZCode, the model list is read from that config for display only; an empty list there is expected when the config is missing and is not a sign that the runtime is offline.
