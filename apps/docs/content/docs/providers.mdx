...
Agents run commands on the runtime machine as the account that runs the Multica daemon, and tasks see that account's normal environment. Tools you have already set up on the machine — git, GitHub CLI, cloud CLIs, and anything configured in your home directory — keep working inside tasks with their existing configuration and credentials. On Windows, if you have configured Codex's native sandbox, Multica keeps it enabled.

<Callout type="warning">
A task can read and write whatever the daemon account can, and it has network access. To limit what tasks can reach, run the daemon inside a VM, a container, or a dedicated OS account.
</Callout>
