...
| **Warp Oz CLI** | Warp | ✅ | ✅ | `.agents/skills/` | Dynamic discovery |
...
From Warp. Uses `oz agent run` for headless agent execution. Session resumption works through Oz conversation IDs, MCP is passed through to Oz, and skills are discovered from `.agents/skills/`. For macOS daemon use, prefer Warp's standalone Oz CLI package over the CLI bundled inside `Warp.app` so background runs do not activate the desktop app.
...
|---|---|---|
| ✅ Really works | Antigravity, Claude Code, Copilot, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Pi, Warp Oz CLI | Pass the resume id and it continues from the previous context |
...
Claude Code and Warp Oz CLI consume `mcp_config`. The other tools accept the field but ignore it — no error, no warning, the config just has no effect.

<Callout type="warning">
If you set `mcp_config` in an agent configuration but pick a tool other than Claude Code or Warp Oz CLI, your MCP servers have **no effect** on that agent.
</Callout>
...
| Warp Oz CLI | `.agents/skills/` | ✅ Native |
