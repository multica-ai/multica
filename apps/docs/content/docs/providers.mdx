...
Managed Prime agents do not use the shared `~/.prime/agent` directory. Multica assigns each runtime/agent pair a private persistent state directory for sessions, settings, authentication, and user-level skills; configure credentials separately for each managed agent or provide explicitly scoped environment credentials. Do not place tenant-shared MCP settings or secrets in Prime project state.
</Callout>
