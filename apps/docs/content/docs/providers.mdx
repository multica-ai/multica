...
From xAI. CLI binary name is `grok` (Grok Build CLI, often installed to `~/.grok/bin/grok`). Multica drives it over **ACP** via `grok --no-auto-update agent --always-approve stdio`, reusing the same transport as Trae / Kimi / Kiro / Qoder. After `initialize` Multica completes xAI's headless **auth handshake** (`authenticate`) using a method actually advertised by the installed CLI (`xai.api_key`, `grok.com`, or the older `cached_token`) before any session operation. **Session resumption works** through ACP `session/load`. **Model selection works** via the catalog returned from `session/new` and `session/set_model`. **Reasoning effort** is exposed through Multica’s thinking picker and passed as `--effort` on the `agent` command. Because ACP does not expose per-model effort capabilities, Multica conservatively offers the Grok 4.5-compatible `low`, `medium`, and `high` levels. **MCP is supported** — config is passed through ACP `mcpServers` (filtered to transports advertised by `initialize`). Skills land in `.grok/skills/` (project); universal user skills live in `~/.agents/skills/`. Tool calls stream as ACP `tool_call` / `tool_call_update` events.
