...
1. Built-in defaults
2. Workspace `settings.transport_retry` (delivered to the daemon on register / repo refresh)
3. Runtime `metadata.transport_retry` (the `agent_runtime.metadata` blob for the bound runtime)
4. Daemon process env `MULTICA_TRANSPORT_RETRY_CONFIG`
5. Per-agent `custom_env.MULTICA_TRANSPORT_RETRY_CONFIG`

Example workspace or runtime metadata block:
    ...
    "policies": [
      {
        ...
        "delays_ms": [0, 0, 5000],
        "session_strategy": ["same_session", "same_session", "fresh_session"]
      }
    ]
  }
}
...
Disable globally with `{"enabled":false}` in any layer, or override individual policies in that JSON document.
