...
**Of the 11 tools, eight consume `mcp_config`: Claude Code, Codex, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, and OpenClaw**. The other three accept the field but **ignore it** — no error, no warning, the config just has no effect.
...
- [Install an agent runtime](/install-agent-runtime) — installation and authentication for each of the 11 supported tools
