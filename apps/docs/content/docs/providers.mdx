...
For native project-level paths, this table also explains repo-scoped skills: if the checked-out repository already contains a matching directory, the underlying tool may discover those committed skills on its own. Multica keeps those repo files intact. If a workspace skill has the same natural directory name, the daemon writes the workspace copy to a collision-free sibling such as `review-helper-multica`.
