...
| **ForgeCode** | Open source | ✅ (`--cid <id>`) | ❌ | `.forge/skills/` | Static (env vars) |
...
Open-source CLI coding harness ([forgecode.dev](https://forgecode.dev/)). Unlike the other supported tools, ForgeCode's one-shot mode (`forge -p`) does **not** emit a structured event stream — it renders styled Markdown to stdout with tool calls, reasoning, and prose interleaved. Multica works around this with a two-phase approach: it buffers the live stdout (forwarding only a liveness heartbeat) and, after the run, replays the structured message list from `forge conversation dump <id>` (tool calls, tool results, reasoning, token usage). **Session resumption works** via the pinned `--cid <id>`; the conversation id is reused to continue a prior conversation. **Model selection is unusual**: there is no `--model` flag — the model is set via the `FORGE_SESSION__PROVIDER_ID` / `FORGE_SESSION__MODEL_ID` environment variables, which Multica derives from a `provider/model` value (e.g. `anthropic/claude-sonnet-4-20250514`). MCP config is not consumed. Skills are copied into `.forge/skills/`.
...
**Of the 15 tools, eleven consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, and Trae**. The other four (Antigravity, Copilot, ForgeCode, Pi) accept the field but **ignore it** — no error, no warning, the config just has no effect.
