...
| **Qwenpaw** | Qwen (Alibaba) | ✅ | ❌ | `.qwenpaw/skills/` | Dynamic discovery |
...
From the Qwen team (Alibaba Cloud). Uses the ACP protocol over stdio via `qwenpaw acp`. Session resumption works through ACP `session/load`, model selection works through a `--model` flag, and skills are copied into `.qwenpaw/skills/` for native project-level discovery.
...
|---|---|---|
| ✅ Really works | Claude Code, Copilot, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Pi, Qwenpaw | Pass the resume id and it continues from the previous context |
