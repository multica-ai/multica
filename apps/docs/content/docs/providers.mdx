...
| **Droid** | Factory | ✅ | ❌ | `.factory/skills/` | Static catalog (`--model`) |
...
From Factory. CLI binary name is `droid`. Multica drives the headless `droid exec --output-format stream-json --auto high` path so agents can edit files, run commands, and push changes without an interactive terminal. **Session resumption works** via `--session-id <id>`; the daemon captures the UUID from the stream-json `init` / `completion` events. **Model selection works** through `--model` against Factory's documented model IDs (see [Available Models](https://docs.factory.ai/models.md)); reasoning effort is passed via `--reasoning-effort`. Skills land in `.factory/skills/` (Factory's native project skill root). MCP servers configured in the agent UI are not injected yet — configure MCP through `droid mcp` / Factory settings for now.
...
**Of the 17 tools, twelve consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, Trae, and Grok**. The other five (Antigravity, Copilot, DevEco Code, Droid, Pi) accept the field but **ignore it** — no error, no warning, the config just has no effect.
