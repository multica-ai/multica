...
| **Reasonix** | esengine | ❌ | ❌ | `.reasonix/skills/` | Launch flag |
...
From esengine, focused on DeepSeek-native coding workflows. Multica launches it through `npx reasonix acp` (or a custom `MULTICA_REASONIX_PATH` wrapper such as `npx dsnix@latest`). **Session resumption is not supported** — every task starts a fresh ACP session — but project skills are copied into `.reasonix/skills/` for native discovery and the selected model is passed via the launch-time `--model` flag.
...
| ❌ None | Gemini, Reasonix | The CLI has no usable resume mechanism |
...
| Reasonix | `.reasonix/skills/` | ✅ Native |
