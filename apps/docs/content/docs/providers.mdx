...
| **Trae CLI** | ByteDance | ❌ (stateless `trae-cli run`) | ❌ | `.agent_context/skills/` (fallback) | Static (`provider/model`) |
...
From ByteDance ([bytedance/trae-agent](https://github.com/bytedance/trae-agent)). A Python-native agentic coding CLI. **Unlike the ACP tools, Trae has no `acp`/`serve` transport** — its CLI exposes only `run`, `interactive`, `show-config`, and `tools`. Multica therefore drives Trae in its one-shot non-interactive mode, `trae-cli run "<task>" --console-type simple`, which is the closest analogue to Antigravity's `agy -p`. Trae prints plain assistant/tool text to stdout (relayed as text, no per-tool telemetry), and Multica reads the structured **trajectory file** (`--trajectory-file`) it writes to recover the final result, token usage, and a success/failure signal.

- **Session resumption is not supported.** `trae-cli run` is stateless — each invocation starts a fresh agent and writes a new trajectory — so there is no session id to resume. Every Multica turn for a Trae agent is a fresh run; follow-up comments still work, they just don't carry the previous run's in-CLI context.
- **Model selection is static**, using the `provider/model` convention (e.g. `doubao/doubao-seed-1.6`, `openai/gpt-4o`, `anthropic/claude-sonnet-4-20250514`, `openrouter/anthropic/claude-3-5-sonnet`). Multica splits the value on the first `/` into Trae's `--provider` / `--model` flags; a bare model with no `/` omits `--provider` and lets Trae's `trae_config.yaml` / environment decide. Set the daemon-wide default with `MULTICA_TRAECLI_MODEL`.
- **Requires a Trae config file.** `trae-cli run` resolves a `trae_config.yaml` (or `--config-file` / the `TRAE_CONFIG_FILE` env var) before doing anything and exits non-zero if none is found — Multica's isolated task workdir has none by default. **Set `TRAE_CONFIG_FILE` to an absolute path to your `trae_config.yaml` as an agent env var** (the file holds the provider, model parameters, and API key). If it's missing, the task fails with an actionable hint.
- **Authentication** uses Trae's own provider API-key environment variables (e.g. `DOUBAO_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`) and/or the keys in `trae_config.yaml` — configure them on the runtime/agent like any other CLI's credentials.
- **Failure detection.** `trae-cli run` exits `0` even when the agent did not finish the task (bad key, max-steps, upstream LLM error); the only reliable signal is `success: false` in the trajectory, which Multica promotes to a failed task. Startup/config errors exit non-zero with no trajectory and are reported as failures directly.
- **No `--system-prompt` flag and no AGENTS.md discovery**, so the Multica runtime brief is delivered by inlining it into the task prompt; skills land in the generic `.agent_context/skills/` fallback, which the inlined brief points the agent at for on-demand reading via Trae's file tools.

## Session resumption: who really supports it

The session resumption mechanism is covered in [Tasks](/tasks#can-a-task-continue-from-the-previous-context). **Every supported tool resumes sessions except Trae CLI** — pass the resume id and the task continues from the previous context. Trae CLI is the exception: `trae-cli run` is stateless, so each turn is a fresh run with no carried-over in-CLI context (see [Trae CLI](#trae-cli) above). The other quirk is Pi, whose resume id is a session file path on disk rather than a string id (see [Pi](#pi) above).

## MCP configuration: provider-specific support

**Of the 14 tools, ten consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, and Qoder**. The other four (Antigravity, Copilot, Pi, Trae CLI) accept the field but **ignore it** — no error, no warning, the config just has no effect.
