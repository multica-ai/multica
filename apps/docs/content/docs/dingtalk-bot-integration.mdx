...
- **Group routing** lists every group discovered by a bot and identifies the bot by its default Agent plus a short installation ID. Routes remain distinguishable when multiple bots share a group. Owners and admins can select one fixed Agent per group; all members can see the current routes.
