...
You can also connect the robot from **Squads → _your squad_ → Integrations**. A Squad target does not fan a message out to every member: Multica sends it to the Squad's **current Leader** with the Squad instructions and roster attached. The Leader decides whether to answer directly or distribute work to members. `/issue` creates an issue assigned to the Squad.

<Callout type="warning">
Both credentials must be from the **same** DingTalk app, and that app is installed only once in a Multica workspace. Connecting it as a second installation or in another workspace is refused. The installation's Agent or Squad remains the default for DMs; group routes may target other Agents or Squads without reconnecting the app.
</Callout>

<Callout type="info">
Want one robot to serve **multiple Agents or Squads**? Add it to each DingTalk group, @-mention it once so Multica discovers the group, then choose that group's target under **Settings → Integrations → DingTalk → Group routing**. Install separate DingTalk apps only when you want separate bot identities or separate DM routes.
</Callout>
...
| Surface | Behavior |
|---|---|
| **Agent / Squad → Integrations** | Owners and admins see **Connect DingTalk**; once connected it flips to a **Connected to DingTalk** badge with a **Disconnect** control. |
| **DM the bot** | A workspace member messages the bot in a 1:1 chat. An Agent target receives it directly. A Squad target sends it only to the current Leader, with the Squad instructions and roster. |
| **@-mention in a group** | Add the bot to the group and @-mention it. The first mention discovers the group and uses the robot's default Agent or Squad; an owner or admin can then select a different target under **Group routing**. Only mentioning messages are read — the bot does not listen to the whole group. |
...
| **`/issue` command** | Starting a message with `/issue <title>` creates a Multica issue assigned to the route's Agent or Squad, attributed to you, and posts the issue ID and title back into the same conversation. Put the description on following lines. Images sent in the same DingTalk message are attached to the created issue. |
...
- **Connected bots** lists every bot in the workspace and the Agent or Squad each is bound to (visible to all members).
- **Group routing** lists every group discovered by a bot. Owners and admins can select one Agent or Squad per group; all members can see the current routes.
- Every route change starts a new chat session for the selected target. Routing Agent A → Agent B → Agent A does not resume A's earlier session. A Squad Leader change also starts a fresh session on the next message. Archiving the target pauses processing with an unavailable notice until it is restored or the group is reassigned.
