...
| **Send images** | Images in a DM, or sent alongside a group @-mention, land in the conversation for the agent to see — PNG, JPEG, GIF, WebP, or BMP, up to 10 images per message and 20 MB each. Each image is copied into Multica storage, so it stays visible in the chat after DingTalk's temporary link expires. Files and voice messages aren't supported — the bot says so instead of silently ignoring them. |
| **`/issue` command** | Starting a message with `/issue <what to file>` hands your description to the agent, which authors a well-formed Multica issue (proper title + structured description) in the background, attributed to you. The bot acknowledges right away, and once the issue is ready it posts the result back into the same conversation — issue ID, title, and a link — alongside a Multica notification. You're subscribed to the issue as its creator, so follow-up comments and updates notify you too. Images sent with the command are attached to the issue; if creation fails, the bot says so in the chat and an inbox item lets you retry. |
| **`/new` command** | Starting a message with `/new <your message>` opens a **new chat** — a fresh session with no prior context, titled from your message. The old conversation stays in the agent's chat history; new messages continue in the new one, and a reply the agent was still writing in the old one is still delivered. Use it to switch topics or clear a long thread. |
...
- **Send images** — send screenshots or photos, with or without text; they show up in the conversation for the agent to work with. PNG, JPEG, GIF, WebP, or BMP; up to 10 images per message, 20 MB each.
- **File an issue** — send `/issue the login redirect is broken on Safari`; describe it in your own words (extra lines are fine, and images sent with the command are attached to the issue). The agent turns it into a well-formed issue in the background; you get an acknowledgement right away, and when it lands the bot posts the issue ID, title, and link back into the chat — plus a Multica notification.
- **Start fresh** — send `/new <your message>` to open a new chat with no prior context (the old one stays in history), then keep chatting normally. If the agent was still answering in the old conversation, that reply still arrives.
...
For self-host, DingTalk is **off until you set an at-rest encryption key**. The key encrypts each app's AppKey + AppSecret before they touch the database. BYO needs **no** OAuth client id/secret at the deployment level — each installation uses the credentials the admin pastes.
