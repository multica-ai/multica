...
Connect any [agent](/agents) to a DingTalk bot and your team can work with it from inside DingTalk — DM the bot, @-mention it in a group, or type `/issue` to file a [Multica issue](/issues) without opening the app.
...
| **`/issue` command** | Starting a message with `/issue <what to file>` hands your description to the agent, which authors a well-formed Multica issue (proper title + structured description) in the background, attributed to you. It replies with an acknowledgement — not an issue number — and you get a Multica notification when the issue is ready. It starts no agent chat turn. |
| **`/new` command** | Starting a message with `/new <your message>` opens a **new chat** — a fresh session with no prior context, titled from your message. The old conversation stays in the agent's chat history; new messages continue in the new one. Use it to switch topics or clear a long thread. |
...
- **File an issue** — send `/issue the login redirect is broken on Safari`; describe it in your own words (extra lines are fine). The agent turns it into a well-formed issue in the background; you get an acknowledgement right away and a Multica notification when it lands.
- **Start fresh** — send `/new <your message>` to open a new chat with no prior context (the old one stays in history), then keep chatting normally.
