...
The iOS app opens this same web login in the system authentication browser and returns through the registered `multica://auth/callback` scheme. Its `EXPO_PUBLIC_WEB_URL` must point to the frontend paired with the configured API.
