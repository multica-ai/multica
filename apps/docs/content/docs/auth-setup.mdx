...
By default, Multica does not accept a universal code like `888888`. Local development without Resend should use the generated code printed in server logs.

If you need deterministic local automation, set `MULTICA_DEV_VERIFICATION_CODE` to a 6-digit value such as `888888`, and keep `APP_ENV` non-production. This shortcut is ignored when `APP_ENV=production`.
</Callout>
