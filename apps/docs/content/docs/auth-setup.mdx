...
A live pending workspace invitation is an implicit per-email signup allowance. With `ALLOW_SIGNUP=false`:

- Existing users can continue to sign in.
- A new user can request a login code and create an account with the email address on a pending, unexpired invitation.
- Uninvited users and users whose invitations are expired, accepted, declined, or revoked remain blocked.

You do not need to add ordinary invitees to `ALLOWED_EMAILS` or restart the server. The invitation must still be accepted through the normal invitation flow.
