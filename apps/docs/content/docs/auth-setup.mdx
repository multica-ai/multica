...
description: Configure email codes, Google OAuth, generic OpenID Connect, signup allowlists, and local test codes.
---

import { Callout } from "fumadocs-ui/components/callout";
import { Mermaid } from "@/components/mermaid";

Multica supports **email + verification code** (default), **Google OAuth**, and one generic **OpenID Connect** provider. On successful sign-in, the server issues a JWT cookie with a 30-day lifetime. This page covers how to configure each method, how to restrict who can sign up, and the single biggest trap for self-hosted deployments.
...
By default Multica requires `email_verified=true` before it creates or links an account by email. Set `OIDC_REQUIRE_VERIFIED_EMAIL=false` only when the provider cannot emit this claim and its email ownership policy is trusted. OIDC configuration is read at runtime, so the frontend does not need to be rebuilt.
