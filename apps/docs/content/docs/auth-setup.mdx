...
## Self-hosted Gitea sign-in

Create an OAuth2 application in the Gitea instance and register the same
frontend callback URL used by Multica. Set the following on the API service:

```dotenv
GITEA_ISSUER_URL=https://git.example.com
GITEA_CLIENT_ID=...
GITEA_CLIENT_SECRET=...
GITEA_REDIRECT_URI=https://multica.example.com/auth/callback
```

After restarting the API, the login page shows a Gitea button. The issuer must
be reachable by both the user's browser and the API container. Multica requires
a verified primary Gitea email for first-time account creation, then uses the
stable Gitea user ID for subsequent sign-ins.
