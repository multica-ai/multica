...
## Usage and model prices

From **Runtimes**, open an existing runtime you can view, then select **Model prices** in its usage area. This entry stays available even when the runtime has no recorded usage.

Workspace members can view the shared prices; workspace owners and admins can save overrides. Subscription usage retains an API-equivalent estimate so you can see the API value of your plan's usage. It is not your subscription bill or an additional charge.

Price changes recalculate historical estimates using the current rates. Provider-reported costs and recorded token counts stay unchanged. See [Model pricing](/developers/model-pricing) for details on estimates, overrides, and synchronization.
