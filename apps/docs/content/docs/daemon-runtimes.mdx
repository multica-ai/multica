...
On a local computer, remote computer, or headless environment, [install the Multica CLI](/cli) first, then run:
...
If a background daemon cannot find a command your terminal can run, set an absolute path for the current computer:
