...
| Command | Purpose |
|---|---|
| `cs-workflow daemon start` | Start (background by default; add `--foreground` to run in the foreground) |
| `cs-workflow daemon stop` | Stop |
| `cs-workflow daemon restart` | Restart |
| `cs-workflow daemon status` | Show status |
| `cs-workflow daemon logs` | Show logs (add `-f` to follow) |

Full CLI reference in [CLI commands](/cli).

**The desktop app ships with a daemon.** If you use the [desktop app](/desktop-app), you don't need to run `cs-workflow daemon start` manually — it launches the daemon automatically on startup. See the [Desktop app](/desktop-app) page for which option fits your workflow.
...
1. Run `cs-workflow daemon status` — confirm the daemon is running and online
2. Run `cs-workflow daemon logs -f` — check for errors
