...
This structure is the biggest difference between Multica and Linear / Jira: **the AI tools, their own credentials, and your code directories stay on your machine** — the Multica server never sees them. (The one exception is secrets you deliberately save into an agent's [custom environment variables](/agents-create#custom-environment-variables-custom_env), which are stored server-side.) That means "my agent isn't working" is almost always a local problem — the daemon isn't running, an AI tool isn't installed, a key has expired. Check locally first; see [Troubleshooting](/troubleshooting) for a guide.
