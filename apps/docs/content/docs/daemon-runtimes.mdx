...
On macOS, Codex detection also checks the Codex Desktop bundle (`/Applications/Codex.app/Contents/Resources/codex`) when `codex` is not on `PATH` and `MULTICA_CODEX_PATH` is unset.

For Codex tasks, `MULTICA_CODEX_APP_VISIBLE=true` can create an extra Codex App-visible portal thread after the Multica task completes. This does not change the real execution workdir; it writes a small portal directory with `README.md`, `result.md`, `links.json`, and a best-effort `real-workdir` link, then creates a compact Codex thread pointing at it. Set `MULTICA_CODEX_APP_VISIBLE_CWD` to the Codex App workspace root where you want the sidebar entry to appear.
