...
re-registers. Running tasks keep the launch arguments they started with. Custom
runtime arguments and registration-error reporting require server and daemon
versions that both understand `fixed_args` launch specs and `failed_profiles`
registration reports, so upgrade the server before rolling out newer daemons in
mixed deployments.
