...
For DeepSeek Harness, install Node.js 20+ and run `npm install -g @deepseek-ai/dsh`. The first run creates its profile under `~/.dsh`; configure the DeepSeek API key (`DEEPSEEK_API_KEY`, or the dsh settings screen) before starting the daemon. Multica forwards a selected model to dsh as a `--patch` override, so leaving the model empty uses whatever is configured in dsh itself.
