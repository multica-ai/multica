...
Supports AGY's current Antigravity model catalog, including Gemini 3.5 Flash, Gemini 3.1 Pro, Claude Sonnet/Opus 4.6 Thinking, and GPT-OSS 120B. No session resumption, no MCP — suitable for one-shot tasks.

| | |
|---|---|
| Daemon looks for | `agy` |
| Install | Follow the official Antigravity CLI install instructions at [antigravity.google/download](https://antigravity.google/download). |
| Authentication | Authenticate with Antigravity or run `agy` interactively before using the daemon. `GEMINI_API_KEY` remains supported where AGY supports API-key auth. |
| Model selection | Pick a model in Multica to pass `agy -m <model>`. Leave it blank to let AGY use its configured default, `GEMINI_MODEL`, or `settings.json` `model.name`. |
| Settings | AGY stores CLI settings in `~/.gemini/antigravity-cli/settings.json`; use `/config`, `/settings`, or `/permissions` in the TUI to adjust safety and permissions. |
| Headless mode | Multica launches AGY with `--dangerously-skip-permissions` so daemon jobs do not block on interactive approvals. |
