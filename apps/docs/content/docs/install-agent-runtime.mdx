...
| Junie CLI | `junie` | [Install Junie CLI](https://junie.jetbrains.com/docs/junie-cli-usage.html) |
...
For Junie, launch `junie` once and use `/account` to choose a JetBrains Account, a `JUNIE_API_KEY`, BYOK, or a custom/local endpoint. Ollama, LM Studio, LiteLLM, and Junie Local models appear in the same model picker that Multica discovers over ACP. Start the selected local model service before running a task; Multica discovers the configuration but does not start or manage that service. Multica does not copy or store those credentials.
