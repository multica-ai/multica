...
On Windows PowerShell 5.1, always read and write agent-authored text files as UTF-8. Use `Get-Content -Raw -Encoding UTF8` when reading body files and `Set-Content -Encoding UTF8` when writing them. BOM-less UTF-8 files can otherwise be decoded with the system ANSI code page, such as cp936/GBK on zh-CN Windows, before the CLI sends the text.
