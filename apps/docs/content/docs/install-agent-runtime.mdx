...
Factory's terminal agent (`droid exec`). Multica drives the headless stream-json path with `--auto high` so agents can edit files and run development workflows without an interactive shell. Session resumption works via `--session-id`.

| | |
|---|---|
| Daemon looks for | `droid` |
| Install | Follow the official guide at [docs.factory.ai/cli/getting-started/overview](https://docs.factory.ai/cli/getting-started/overview). Typical routes: `curl -fsSL https://app.factory.ai/cli | sh` or `npm install -g droid`. |
| Authentication | Run `droid` once and follow the login flow, or set `FACTORY_API_KEY`. |
