...
The most complete integration. Session resumption works, MCP works, and it's the **only one of the 12 that actually consumes the `mcp_config` field** on agents (see the [matrix](/providers#mcp-configuration-only-claude-code-actually-reads-it)).
...
DeepSeek-native ACP agent. Multica detects it by finding `npx` on your `PATH`, then launching `npx reasonix acp` (or whatever wrapper you set in `MULTICA_REASONIX_PATH`). Session resumption is **not** supported.

| | |
|---|---|
| Daemon looks for | `npx` |
| Install | Install Node.js 22+, then either run `npx reasonix --help` once or follow the upstream guide at [github.com/esengine/DeepSeek-Reasonix](https://github.com/esengine/DeepSeek-Reasonix). You can also pin a wrapper command such as `npx dsnix@latest` via `MULTICA_REASONIX_PATH`. |
| Authentication | Run `reasonix setup`, or set `DEEPSEEK_API_KEY` in your shell environment. |
| Notes | DeepSeek-only. Skills are discovered natively from `.reasonix/skills/`. |
...
1. **Confirm the binary is on `PATH`.** Open a fresh terminal and run `which <name>` (for example `which claude`, `which cursor-agent`, `which kiro-cli`, or `which npx` for Reasonix). If it prints a path, the daemon will find it. If it prints nothing, fix your shell `PATH` first (the typical cause is a per-shell rc file that wasn't reloaded).
