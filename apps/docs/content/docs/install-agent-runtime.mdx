...
1. **The Multica daemon must be running.** Either run `cs-workflow daemon start` after installing the [Multica CLI](/cli), or use the [Multica desktop app](/desktop-app), which launches the daemon automatically. Without a running daemon there is nothing to detect tools.
...
2. **Restart the daemon.** `cs-workflow daemon restart`, or relaunch the desktop app. The daemon only scans `PATH` at startup.
...
- **The Runtimes page shows the row, but tasks fail immediately.** Check `cs-workflow daemon logs -f` while triggering a task. The daemon surfaces the tool's own error output.
