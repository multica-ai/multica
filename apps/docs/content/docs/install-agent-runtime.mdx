...
xAI's agentic coding CLI (`grok`). ACP over stdio via `grok agent stdio` — the same transport as Hermes, Kimi, Kiro CLI, and Qoder. Session resumption through `session/resume`, MCP config through `mcpServers`, models discovered with `grok models`, skills copied into `.grok/skills/`. Claude Code–compatible, so it also reads `CLAUDE.md` / `AGENTS.md` and `.claude/skills/`.

| | |
|---|---|
| Daemon looks for | `grok` |
| Install | See [docs.x.ai/build](https://docs.x.ai/build). |
| Authentication | `grok login` once on the host (browser OAuth, works with a SuperGrok subscription), or `XAI_API_KEY` for headless hosts. Multica prefers the key when both are set. |
| Notes | The daemon authenticates non-interactively and never opens a browser, so the credential must already exist on the host before a task runs. |
