...
| Notes | Minimum CLI version: `0.20.0`. Override the binary with `MULTICA_QWEN_PATH` and set a daemon-wide default model with `MULTICA_QWEN_MODEL`. Qwen has no temporary MCP-config flag, so Multica rejects agent `mcp_config`; configure persistent MCP servers with `qwen mcp add`. |
