...
The most complete integration. Session resumption works, MCP works, and it supports the richest daemon-controlled options (see the [matrix](/providers#mcp-configuration-support)).
...
Warp's headless CLI for running cloud-connected agents.

| | |
|---|---|
| Daemon looks for | `oz` |
| Install | Follow Warp's Oz CLI docs at [docs.warp.dev/reference/cli](https://docs.warp.dev/reference/cli/). On macOS, prefer the standalone Homebrew cask (`brew tap warpdotdev/warp && brew install --cask oz`) for daemon use. |
| Authentication | Run `oz login`, or configure `WARP_API_KEY` for automation. |
| Notes | The standalone package avoids launching the Warp desktop app during background daemon runs. |
