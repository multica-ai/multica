...
| Authentication | Run `grok login`, set `XAI_API_KEY`, or configure a custom model with its own `api_key`; Multica selects an auth method advertised by the installed CLI. |
