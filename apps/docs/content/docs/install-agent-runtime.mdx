...
| Prime Agent | `prime-agent` | [Prime Agent](https://github.com/PrimeIntellect-ai/prime-agent) |
...
For Prime Agent, install the official MIT-licensed **v0.7.2** release and verify `prime-agent-0.7.2.tgz` SHA-256 `bc5471f2a626d727b88a45eb745fff93b10c554a3c4fc5912f25d8c64b987f5e` (release source commit `83a0f9f9566219551fcb6ffaf7f519a815749a58`). Multica rejects every other version because the RPC protocol has no compatibility handshake. Run the daemon as non-root in an isolated container or restricted OS identity, then set `MULTICA_PRIME_AGENT_ISOLATED=1`; this is an operator attestation, not a sandbox. Prime v0.7.2 nevertheless remains admission-disabled until upstream provides a managed mode that hard-disables schedules and heartbeats while retaining coding tools.
