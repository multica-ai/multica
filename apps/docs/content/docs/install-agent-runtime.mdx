...
| WorkBuddy | — | [WorkBuddy](https://www.workbuddy.cn/) |
...
WorkBuddy has no separate install: the daemon detects the CodeBuddy CLI that the [WorkBuddy desktop app](https://www.workbuddy.cn/) bundles. The bundled CLI is launched through a Node.js runtime on every platform — WorkBuddy stages one under `~/.workbuddy/binaries/node/versions/`, and the daemon picks the newest staged version (falling back to `node` on `PATH`). Sign in to the WorkBuddy desktop app once — the bundled CLI shares its session. Set `MULTICA_WORKBUDDY_PATH` to override discovery with your own `codebuddy` install.
