...
<Callout type="info">
The Multica server never sees these tools or the credentials they authenticate with. Everything below — installation, authentication, model access — lives on your local machine. If something fails, it's almost always a local problem. (Separately: secrets you deliberately save into an agent's [custom environment variables](/agents-create#custom-environment-variables-custom_env) are stored on the server — that's a different channel from the tools' own local logins.)
</Callout>
