...
### Qwenpaw (Qwen / Alibaba Cloud)

ACP-over-stdio transport. Session resumption works through ACP `session/load`; skills are copied into `.qwenpaw/skills/`.

| | |
|---|---|
| Daemon looks for | `qwenpaw` |
| Install | Follow the official guide at the Qwen team's repository. The binary name is `qwenpaw`. |
| Authentication | Configure your Qwen API key per the vendor's docs. |
