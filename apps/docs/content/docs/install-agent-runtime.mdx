...
For Prime Agent, sign in (subscription or API key) and choose the model in Prime Agent's own configuration first. Multica cannot select a Prime Agent model at run time, and Prime Agent does not resume a prior session or read Multica-managed MCP configuration — see [AI coding tools comparison](/providers).
...
<Callout type="info">
Antigravity requires 1.1.10 or later, Claude Code requires 2.0.0 or later, Codex 0.100.0 or later, Copilot 1.0.0 or later, Grok 0.2.89 or later, Qwen Code 0.20.0 or later, MiniMax Code 0.1.2 or later, OpenCode 1.1.54 or later, and Prime Agent 0.7.1 or later. Below the minimum version, the daemon does not register the corresponding runtime.
</Callout>
