...
Alibaba Qwen Code CLI (`qwen`). Multica runs it headlessly as `qwen -p <prompt> --output-format stream-json --yolo`; session resumption uses `--resume`, and the daemon reads native text, thinking, tool, and result JSONL events. Skills are injected into `.qwen/skills/`, and the task brief goes to `QWEN.md`. Managed `mcp_config` is rejected at launch because the CLI has no matching config flag; leave it unset to inherit MCP servers from Qwen settings.

| | |
|---|---|
| Daemon looks for | `qwen` |
| Install | Install Qwen Code and ensure `qwen` is on `PATH`. |
| Authentication | Run `qwen` once interactively and complete the provider login. |
| Notes | Override the binary with `MULTICA_QWEN_PATH`, defaults with `MULTICA_QWEN_MODEL` and `MULTICA_QWEN_ARGS`. Minimum CLI version: `0.20.0`. |
