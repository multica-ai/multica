...
| 命令 | 用途 |
|---|---|
| `multica agent list` | 列出工作区的数字员工 |
| `multica agent get <slug>` | 查看数字员工配置 |
| `multica agent create ...` | 创建数字员工 |
| `multica agent update <slug> ...` | 修改智能体 |
| `multica agent archive <slug>` | 归档 |
| `multica agent restore <slug>` | 恢复归档的数字员工 |
...
| `multica squad activity <issue-id> <action\|no_action\|failed> --reason "..."` | 队长数字员工每轮结束时调用，记录 evaluation |
...
- [创建和配置数字员工](/agents-create) —— `multica agent create` 的完整选项
