...
| `log_level` | `debug` | 守护进程日志阈值：`debug`、`info`、`warn` 或 `error`。设置后会优先于该 profile 的 `LOG_LEVEL` |
...
- `log_level` 接受 `debug`、`info`、`warn` 或 `error`；传空值会清除 profile 覆盖，回到 `LOG_LEVEL` 或 `debug`。
