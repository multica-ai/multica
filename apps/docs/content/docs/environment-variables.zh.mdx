...
| `MULTICA_STAGE_RECOVERY_THRESHOLD` | `15m` | 无活动 run 的非终态阶段子任务达到该时长后，服务端重新检查并唤醒父协调器给出恢复指引；仅用于发现，不会自动完成或取消 |
| `MULTICA_STAGE_RECOVERY_BATCH_SIZE` | `100` | 每 30 秒巡检最多重算的疑似卡住阶段子任务数量 |
