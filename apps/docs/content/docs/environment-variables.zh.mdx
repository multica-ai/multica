...
Prime Agent 使用 `MULTICA_PRIME_PATH`、`MULTICA_PRIME_MODEL`（`provider/modelId`）及必需的 `MULTICA_PRIME_AGENT_ISOLATED=1`。root 或非 v0.7.2 版本会被拒绝；v0.7.2 也因上游无法在保留 coding tools 的同时强制禁用 schedules/heartbeats 而处于 admission-disabled 状态。
