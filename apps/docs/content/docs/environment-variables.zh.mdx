...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `MULTICA_CODEX_CAPACITY_RETRY_COUNT` | `6` | 当任务持久化的 Codex 原始错误与 `Selected model is at capacity. Please try a different model.` 完全一致时，立即追加的重试次数。默认值允许总计执行 7 次；`0` 关闭此专用策略。其他容量或限流错误不会被此策略重试。 |
