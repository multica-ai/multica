...
服务端 sweeper 会在任务 running 超过一个粗粒度墙钟超时后，把孤儿任务标记为失败。这是守护进程未上报失败时的兜底；真正卡死任务的主要防线仍是守护进程基于活动的空闲看门狗和工具看门狗。

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `MULTICA_TASK_RUNNING_TIMEOUT` | `2.5h` | 服务端 running 任务超时。接受 Go duration 字符串，例如 `8h` 或 `30m`。 |
