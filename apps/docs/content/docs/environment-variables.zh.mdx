...
[企业微信 Bot 接入](/wecom-bot-integration) 在设置 `MULTICA_WECOM_SECRET_KEY` 之前保持关闭。绑定链接依赖 `MULTICA_PUBLIC_URL`（用户可访问的前端地址，不是 Backend API 地址）。

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `MULTICA_WECOM_SECRET_KEY` | 空 | Base64 编码的 32 字节密钥，加密 Bot / Corp Secret 落库。生成：`openssl rand -base64 32` |
| `MULTICA_PUBLIC_URL` | 空 | 绑定页与 Bot 回复中的链接前缀，例如 `https://multica.example.com`（无尾斜杠）|

完整流程、群聊会话模型与 Docker 部署见 [企业微信 Bot 接入](/wecom-bot-integration)。
