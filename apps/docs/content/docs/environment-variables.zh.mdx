...
默认上报到 Multica 官方 Amplitude 实例。不想上报就把 `ANALYTICS_DISABLED=true`。

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `ANALYTICS_DISABLED` | `false` | 设 `true` 完全关闭后端上报 |
| `AMPLITUDE_API_KEY` | 内置默认 key | 换成你自己的 Amplitude 项目时填 |
| `AMPLITUDE_HOST` | `https://api2.amplitude.com` | EU 数据驻留用 `https://api.eu.amplitude.com` |
