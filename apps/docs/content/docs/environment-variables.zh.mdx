...
| `MULTICA_DAEMON_WS_CLAIM_POLL_INTERVAL` | `3m` | 健康 WebSocket 下 claim 安全轮询的上限；向下抖动后默认实际为 `2m30s`–`2m45s`，旧 Server 或结果不确定的 claim 仍使用普通轮询间隔 |
...
| `ws_claim_poll_interval` | `3m` | 健康 WebSocket 下 claim 安全轮询的上限；Daemon 会应用向下抖动 |
