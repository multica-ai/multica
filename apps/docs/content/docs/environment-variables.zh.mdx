...
| `DATABASE_REPLICA_URL` | 空 | 可选的 PostgreSQL 只读从库连接地址；配置后只启用健康探测，业务仍需在代码中显式选择从库读取 |
| `DATABASE_REPLICA_MAX_CONNS` | `10` | 单个 API 进程的最大从库连接数 |
| `DATABASE_REPLICA_MIN_CONNS` | `0` | 单个 API 进程保留的最小从库连接数 |
| `DATABASE_REPLICA_PROBE_INTERVAL` | `2s` | API 检查从库是否为可用只读 standby 的间隔 |
| `DATABASE_REPLICA_PROBE_TIMEOUT` | `1s` | 单次从库可用性探测访问主库与从库的总超时 |
| `DATABASE_REPLICA_MAX_REPLAY_LAG` | `5s` | 从库 WAL 位点落后主库时允许的最大回放延迟；超限后回退主库 |
...
主库与从库的连接上限彼此独立，需要按全部 API 进程的连接数总和预留 PostgreSQL 集群连接额度。从库配置是可选的非关键依赖：配置无效、不可用、已提升为主库、可写或延迟超限时，从库都不会被选中，已显式接入的读取会改走主库。
