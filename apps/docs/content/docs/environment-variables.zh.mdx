...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `MULTICA_RUNNING_TIMEOUT` | `8h` | 服务端 sweeper 对 `running` 任务的粗粒度兜底上限。它只按 `started_at` 计算，不看任务是否仍有输出；主要用于 daemon 已死亡但没有回报结果的孤儿任务。正常长任务应优先由 daemon 的 activity watchdog 判断是否卡死。设置为正的 Go duration（如 `12h`）可覆盖默认值 |
...
| `S3_KEY_PREFIX` | 空 | 可选的上传 object key 前缀，例如 `multica-attachments/dev` 会把文件存到 `multica-attachments/dev/workspaces/...` 下 |
| `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` | 空 | 静态凭证。全未设时用 AWS SDK 默认凭证链（IAM role / 环境凭证）|
| `AWS_ENDPOINT_URL` | 空 | 自定义 S3 兼容端点（例如 [MinIO](https://min.io/) 或华为 OBS） |
| `S3_FORCE_PATH_STYLE` | 设置 `AWS_ENDPOINT_URL` 时默认为 `true` | MinIO 这类 path-style 端点保持 `true`；华为 OBS 设为 `false`，让预签名 URL 使用 `https://<bucket>.obs.<region>.myhuaweicloud.com/<key>` |
| `HUAWEICLOUD_OBS_AK` / `HUAWEICLOUD_OBS_SK` / `HUAWEICLOUD_OBS_ENDPOINT` | 空 | 华为 OBS 变量别名。仅在对应 `AWS_*` 变量未设置时使用 |
...
2. 设了 `AWS_ENDPOINT_URL` 且 `S3_FORCE_PATH_STYLE=true` → `<AWS_ENDPOINT_URL>/<S3_BUCKET>/<key>`（path-style）
3. 设了 `AWS_ENDPOINT_URL` 且 `S3_FORCE_PATH_STYLE=false` → `https://<S3_BUCKET>.<AWS_ENDPOINT_URL host>/<key>`（virtual-hosted-style）
4. 默认走 AWS S3 → `https://<S3_BUCKET>.s3.<S3_REGION>.amazonaws.com/<key>`（virtual-hosted-style）。bucket 名含点时会回落到 `https://s3.<S3_REGION>.amazonaws.com/<S3_BUCKET>/<key>`（path-style），因为 AWS 通配证书无法覆盖含点 host。

如果设置了 `S3_KEY_PREFIX`，它会成为 `<key>` 的一部分。例如 `S3_KEY_PREFIX=multica-attachments/dev` 时，workspace 附件会存到 `multica-attachments/dev/workspaces/<workspace_id>/...`。
...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `GITEE_TOKEN` | 空 | Gitee API 的个人访问令牌。Skill Import 用它从 Gitee 仓库拉取 `skill.yaml` / `skill.json`。私有仓库必须配置；公开仓库不配也能用，但受更严格的限流和权限限制 |
