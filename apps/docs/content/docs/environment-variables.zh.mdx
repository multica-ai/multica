...
| `AWS_ENDPOINT_URL` | 空 | 自定义 S3 兼容端点（例如 MinIO 或阿里云 OSS）|
| `AWS_S3_FORCE_PATH_STYLE` | auto | 可选布尔值。大多数自定义 endpoint 默认 `true`，阿里云 OSS endpoint 默认 `false`，因为 OSS 会拒绝 path-style S3 请求 |
...
2. 设了 `AWS_ENDPOINT_URL` → 按 endpoint 风格生成。path-style 是 `<AWS_ENDPOINT_URL>/<S3_BUCKET>/<key>`；virtual-hosted-style 是 `https://<S3_BUCKET>.<endpoint-host>/<key>`
...
这些变量只在 server 侧使用。请放在部署 Secret 或 ConfigMap 中，不要放进前端环境变量。

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `FEISHU_NOTIFICATIONS_ENABLED` | `false` | 设为 `true` 后，将已开启飞书通知用户的收件箱通知通过飞书机器人发送 |
| `FEISHU_NOTIFICATIONS_APP_ID` | 空 | 飞书通知机器人应用 ID |
| `FEISHU_NOTIFICATIONS_APP_SECRET` | 空 | 飞书通知机器人应用 Secret |
| `FEISHU_MESSAGE_RECEIVE_ID_TYPE` | `email` | 飞书发送消息接口使用的接收者 ID 类型 |
| `FEISHU_API_BASE_URL` | `https://open.feishu.cn` | 仅在测试或专用飞书网关场景下覆盖 |
