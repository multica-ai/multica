...
| `MULTICA_TASK_DISPATCH_TIMEOUT` | `5m` | 兜底：将卡在 `dispatched` 的 task 失败化；该状态到 `running` 的转换应近乎瞬时 |
| `MULTICA_TASK_RUNNING_TIMEOUT` | `2h30m` | 卡在 `running` 的 task 的服务端墙钟兜底；只要运行时仍在心跳就绝不会被该超时终止 |
