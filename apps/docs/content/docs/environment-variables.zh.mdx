...
有三个功能会使用这一层，它们都会把私有内容发送到你配置的 endpoint：

- **对话标题自动生成** —— 发送新对话中用户的第一条消息，原文发送。附件不会包含在内。
- **后续提问建议**（智能体回复下方的按钮）—— 发送对话末尾的内容：最多 6 条消息，其中被追问的那条回复上限 3000 字符，更早的每条上限 800 字符。
- **任务评论后续操作**（最新一条可操作智能体评论下方的按钮）—— 发送任务标题、最多 1500 字符的任务描述，以及最多 4000 字符的最新智能体评论。不会包含附件和更早的评论。

API key 与 base URL 都为空时，这一层关闭，不会发出任何上游请求——上面的功能都不再发送内容。如果你的政策不允许这一层把私有内容发到部署之外，这就是受支持的配置方式：对话继续使用客户端根据第一条消息生成的标题，对话和任务评论下方的后续操作按钮不再出现，其余功能不受影响。
