...
| `MULTICA_AGENT_IDLE_WATCHDOG` | `30m` | 空闲看门狗：backend 持续无进展（无进展消息、消息队列为空、且没有工具在途）这么久就 force-stop。只有携带工作的消息（assistant 内容、工具调用、工具结果、错误）算活跃；status 心跳与 log 不算——卡在死掉的 provider 流上的 backend 仍会持续吐这两类消息。`0` = 关闭整套看门狗，包括 provider 专用阈值 |
