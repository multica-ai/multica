...
| `MULTICA_PLATFORM_CONTEXT` | `full` | Multica 上下文模式：`full` 保留原有工作流提示；`minimal` 只保留运行事实和中性的 task skill 清单，不附加强制触发规则；`off` 同时移除运行提示和服务端内置 skill。布尔值 `true`/`false` 仍分别等价于 `full`/`off` |
...
| `platform_context_mode` | 跟随环境 | 可设为 `full`、`minimal` 或 `off`；`minimal` 保持服务端 skill 可见，由 provider 自行决定是否触发；`off` 将其移除。空值清除本地覆盖 |
