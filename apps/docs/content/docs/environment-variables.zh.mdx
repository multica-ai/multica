...
| `MULTICA_DAEMON_LOG_MAX_SIZE_MB` | `20` | 当前 `daemon.log` 轮转前的最大大小（MB） |
| `MULTICA_DAEMON_LOG_MAX_BACKUPS` | `5` | 保留的 gzip 压缩 `daemon.log` 轮转备份数 |
| `MULTICA_DAEMON_LOG_MAX_AGE_DAYS` | `30` | 轮转后的 `daemon.log` 备份最长保留天数 |
