...
| `GITHUB_APP_ID` | 空 | webhook 可不配；Settings → Repositories → 从 GitHub 选择必须配置。与 `GITHUB_APP_PRIVATE_KEY` 配合完成 App 鉴权 REST 调用 |
| `GITHUB_APP_PRIVATE_KEY` | 空 | webhook 可不配；浏览仓库必须配置。App RSA 私钥的完整 PEM 块（包含 `-----BEGIN/END-----` 两行，保留换行），用于签发短效 App JWT |
