...
| 企业微信 | `MULTICA_WECOM_SECRET_KEY` | base64 编码的 32 字节凭据加密密钥；未配置则整个集成禁用 |
| 企业微信 | `MULTICA_WECOM_SOURCE_ID` | 覆盖内置的默认企业微信"调用来源"（source）id，用于创建智能机器人；Cloud 与自托管默认共用同一个值是既定设计，只在调试或更换来源时才需要设置 |
...
配置步骤见 [GitHub 集成](/github-integration#自托管配置)、[飞书 Bot](/lark-bot-integration#自托管配置)、[企业微信 Bot](/wecom-bot-integration#自托管配置)和 [Slack Bot](/slack-bot-integration#自托管配置)。
