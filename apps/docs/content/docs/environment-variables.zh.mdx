...
[GitHub PR ↔ issue 集成](/github-integration) 依赖 GitHub App 安装、JWT 和 webhook 凭据。四个都配上才会启用 Settings 里的 Connect GitHub 并接受 webhook。

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `GITHUB_APP_SLUG` | 空 | 你的 GitHub App slug（`https://github.com/apps/<slug>` 的尾部）。Settings → GitHub 里安装按钮的跳转 URL 用它拼 |
| `GITHUB_APP_ID` | 空 | GitHub App 设置页里的数字 App ID。用于签发 App JWT 并查询 installation 信息 |
| `GITHUB_APP_PRIVATE_KEY` | 空 | GitHub App private key，支持 raw PEM、转义换行 PEM 或 base64 编码 PEM。和 `GITHUB_APP_ID` 一起用于解析安装账号名 |
...
- setup callback 不会开放，直到 Multica 能签发 GitHub App JWT 并解析安装账号名
- `/api/webhooks/github` 端点单独依赖 `GITHUB_WEBHOOK_SECRET`；当这个 secret 缺失时，它会直接返回 **`503 github webhooks not configured`**，而不是把所有签名当 valid
...
- [GitHub 集成](/github-integration) —— 这些 GitHub App 环境变量怎么配置
