...
Open **Settings → Desktop**. Under **When closing the main window**, choose **Minimize to system tray** (the default), **Minimize to taskbar**, or **Quit application**. The first two keep Multica, its active runs, and its connections running; tray mode hides the window and you can restore it from the Multica tray icon. Choose Quit when you want to stop the desktop app.
