...
On Windows, installation waits while the bundled runtime is running or its status cannot be checked. Finish active runs, then open **Runtimes**, select this computer and choose **Stop** before retrying installation. Closing Desktop can leave the runtime running under its existing preferences; the update check never stops it.

If the notification says runtime status could not be checked, use **Download installer** to open the official downloads page. Finish active runs and stop this computer's runtime first, download the installer matching your architecture, quit Multica, then run that installer over the existing installation. This recovery path remains available when the automatic runtime check is unavailable.
