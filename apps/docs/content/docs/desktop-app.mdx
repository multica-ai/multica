...
**On macOS and Windows, CPU architectures use separate update channels** — install the build that matches your machine so future updates stay on the same architecture. M-series Macs use Apple Silicon (`arm64`); older Intel Macs use `x64`.
</Callout>

The macOS builds require macOS 12 Monterey or later and are signed and notarized, so you won't see an "unidentified developer" warning on first launch. The Linux build is an `.AppImage` — auto-updates rely on electron-updater, which can be flaky on some distros. **If auto-update doesn't work, download the new version manually and replace the old file.**
...
| Platform | File |
|---|---|
| macOS Apple Silicon (M-series) | `.dmg` (with `arm64` suffix) |
| macOS Intel | `.dmg` (with `x64` suffix) |
