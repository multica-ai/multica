...
<Callout type="info">
Who can use a given agent is governed by the agent's own **Access** setting — **Only me**, **Entire workspace**, or **Specific people** — not by workspace role. An "Only me" agent can be run only by its owner; workspace owners and admins get **no bypass** (they can see every agent and edit most of its configuration, but running it and changing its Access stay with the people the owner granted). See [Agents → Who can run an agent](/agents#who-can-run-an-agent).
</Callout>
