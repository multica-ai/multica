...
Sessions last 30 days of inactivity by default, not 30 days from sign-in: a session in continuous use is re-issued before it expires, so staying signed in does not require signing in again on a schedule. Self-hosting admins can adjust the lifetime with `AUTH_TOKEN_TTL`; see [Sign-in and signup configuration](/auth-setup#session-lifetime). Logging out clears the auth and CSRF cookies in the current browser.
