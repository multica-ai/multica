...
In day-to-day use you'll only touch the first two directly. The **[daemon](/daemon-runtimes) token** is created and refreshed automatically by `cs-workflow daemon login` — you don't have to think about it.
...
- **CLI**: `cs-workflow login` creates one automatically if there's no local PAT yet
...
When you run `cs-workflow auth logout` or click log out in the Web UI:
