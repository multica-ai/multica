...
| `GITHUB_APP_ID` | 비어 있음 | 웹훅에는 선택 사항이며 Settings → Repositories → GitHub에서 선택 기능에는 필수입니다. `GITHUB_APP_PRIVATE_KEY`와 함께 App 인증 REST 호출에 사용합니다 |
| `GITHUB_APP_PRIVATE_KEY` | 비어 있음 | 웹훅에는 선택 사항이며 저장소 탐색에는 필수입니다. App RSA 비공개 키의 전체 PEM 블록(`-----BEGIN/END-----` 줄 포함, 줄바꿈 유지)으로 단명 App JWT를 발급합니다 |
