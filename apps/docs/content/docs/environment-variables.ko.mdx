...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `MULTICA_CODEX_CAPACITY_RETRY_COUNT` | `6` | 작업에 저장된 Codex 원본 오류가 `Selected model is at capacity. Please try a different model.`와 정확히 일치할 때 추가로 즉시 재시도하는 횟수입니다. 기본값은 총 7회 실행을 허용하며, `0`은 이 전용 정책을 비활성화합니다. 그 밖의 용량 또는 속도 제한 오류는 이 정책으로 재시도하지 않습니다. |
