...
| `MULTICA_DAEMON_LOG_MAX_SIZE_MB` | `20` | 로테이션 전 현재 `daemon.log`의 최대 크기(MB) |
| `MULTICA_DAEMON_LOG_MAX_BACKUPS` | `5` | 보관할 압축된 `daemon.log` 로테이션 백업 수 |
| `MULTICA_DAEMON_LOG_MAX_AGE_DAYS` | `30` | 로테이션된 `daemon.log` 백업의 최대 보관 기간(일) |
