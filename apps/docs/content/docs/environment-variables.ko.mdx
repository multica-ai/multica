...
| `MULTICA_TASK_DISPATCH_TIMEOUT` | `5m` | `dispatched`에 남아 있는 태스크를 실패시키는 백스톱. `running`으로의 전환은 거의 즉시에 일어나야 하므로 백스톱 역할 |
| `MULTICA_TASK_RUNNING_TIMEOUT` | `2h30m` | `running`에 남아 있는 태스크의 서버 측 벽시계 백스톱. 런타임이 여전히 하트를 보내고 있는 태스크는 이 타임아웃으로 절대 종료되지 않음 |
