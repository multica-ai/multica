...
Prime Agent는 `MULTICA_PRIME_PATH`, `MULTICA_PRIME_MODEL`(`provider/modelId`), 필수 `MULTICA_PRIME_AGENT_ISOLATED=1`을 사용합니다. root 또는 v0.7.2 이외 버전은 거부되며, v0.7.2도 upstream이 coding tools를 유지하면서 schedules/heartbeats를 강제로 비활성화할 수 없어 admission-disabled 상태입니다.
