...
| `log_level` | `debug` | 데몬 로그 임계값: `debug`, `info`, `warn`, `error`. 설정하면 이 프로필의 `LOG_LEVEL`보다 우선함 |
...
- `log_level`은 `debug`, `info`, `warn`, `error`를 허용합니다. 빈 문자열은 프로필 재정의를 지우고 `LOG_LEVEL` 또는 `debug`로 돌아갑니다.
