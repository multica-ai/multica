...
위 네 개의 데몬 키 — `device_name`, `runtime_name`, `max_concurrent_tasks`, `poll_interval` — 는 프로파일의 `~/.multica/config.json`(또는 `~/.multica/profiles/<name>/config.json`)에 유지할 수도 있어, `daemon start` 마다 플래그를 다시 전달하거나 환경 변수를 export 할 필요가 없습니다.
...
`poll_interval`은 양수인 Go 기간(`500ms`, `10s`, `1m30s` 등)만 허용합니다. `0s`, 음수, 파싱 불가능한 문자열은 `config set` 시점에 거부됩니다. 이미 저장된 값을 지우고 환경 변수 / 내장 기본값으로 되돌리려면 `multica config set poll_interval ""`를 사용하세요.
