...
| `MULTICA_DAEMON_WS_CLAIM_POLL_INTERVAL` | `3m` | 정상 WebSocket에서 claim 안전 polling의 상한. 하향 jitter를 적용한 기본 실제 간격은 `2m30s`–`2m45s`이며, 구형 Server 또는 결과가 불확실한 claim은 일반 polling 간격을 유지합니다 |
...
| `ws_claim_poll_interval` | `3m` | 정상 WebSocket에서 claim 안전 polling의 상한. Daemon은 하향 jitter를 적용합니다 |
