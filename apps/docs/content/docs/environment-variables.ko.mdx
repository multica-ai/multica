...
| `MULTICA_PLATFORM_CONTEXT` | `full` | Multica 컨텍스트 모드입니다. `full`은 기존 워크플로 brief를 유지하고, `minimal`은 실행 환경 정보와 중립적인 task Skill 카탈로그만 유지하며 강제 실행 규칙을 추가하지 않습니다. `off`는 brief와 서버 제공 기본 Skills를 제거합니다. `true`/`false`는 `full`/`off`의 별칭입니다 |
...
| `platform_context_mode` | 환경을 따름 | `full`, `minimal`, `off` 중 하나입니다. `minimal`은 서버 제공 Skills를 표시한 채 실행 판단을 provider에 맡기고, `off`는 제거합니다. 빈 문자열은 로컬 재정의를 지웁니다 |
