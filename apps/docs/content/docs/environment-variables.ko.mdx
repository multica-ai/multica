...
| `MULTICA_DATABASE_STARTUP_TIMEOUT` | `3m` | migrator 또는 API 시작 중 일시적인 데이터베이스 장애에 대한 전체 재시도 시간. `0`으로 설정하면 한 번만 시도 |
| `MULTICA_DATABASE_CONNECT_TIMEOUT` | `5s` | 시작 중 각 데이터베이스 연결 시도의 제한 시간 |
