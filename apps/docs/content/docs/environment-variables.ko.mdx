...
| `DATABASE_REPLICA_URL` | 비어 있음 | 선택적 PostgreSQL 읽기 전용 대기 서버 연결 주소. 설정만으로는 상태 확인만 활성화되며 각 기능은 코드에서 복제본 읽기를 명시적으로 선택해야 함 |
| `DATABASE_REPLICA_MAX_CONNS` | `10` | API 프로세스당 최대 복제본 연결 수 |
| `DATABASE_REPLICA_MIN_CONNS` | `0` | API 프로세스가 유지하는 최소 복제본 연결 수 |
| `DATABASE_REPLICA_PROBE_INTERVAL` | `2s` | 복제본이 읽기에 적합한 대기 서버인지 확인하는 간격 |
| `DATABASE_REPLICA_PROBE_TIMEOUT` | `1s` | primary와 복제본을 함께 확인하는 단일 적격성 점검의 제한 시간 |
| `DATABASE_REPLICA_MAX_REPLAY_LAG` | `5s` | 복제본 WAL 위치가 primary보다 뒤처질 때 허용되는 최대 재생 지연. 초과하면 primary로 폴백 |
...
primary와 복제본의 연결 한도는 서로 독립적입니다. 모든 API 프로세스의 연결 합계가 PostgreSQL 클러스터 연결 한도 내에 있도록 설정해야 합니다. 복제본은 선택적이며 중요하지 않은 의존성입니다. 설정 오류, 연결 불가, primary 승격, 쓰기 가능 또는 지연 초과 상태에서는 선택되지 않으며, 명시적으로 연동된 읽기는 primary를 사용합니다.
