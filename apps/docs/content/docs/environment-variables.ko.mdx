...
- **이슈 댓글 후속 작업**(작업 가능한 최신 에이전트 댓글 아래의 버튼) — 이슈 제목, 설명의 최대 1500자, 최신 에이전트 댓글의 최대 4000자를 전송합니다. 첨부 파일과 이전 댓글은 포함되지 않습니다.

API key와 base URL이 모두 비어 있으면 이 레이어가 꺼지고 업스트림 요청을 전혀 보내지 않습니다. 위 기능은 모두 아무것도 전송하지 않습니다. 이 레이어가 비공개 내용을 배포 환경 밖으로 보내면 안 되는 정책이라면 이것이 지원되는 구성입니다. 세션은 클라이언트가 첫 메시지에서 만든 제목을 그대로 사용하고, 채팅 및 이슈 댓글 후속 작업 버튼은 표시되지 않으며, 나머지 기능은 영향을 받지 않습니다.
