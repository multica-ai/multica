...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `MULTICA_MAX_PREVIEW_SIZE` | `2MiB` | 인라인 텍스트 미리보기를 위해 메모리에 로드하는 첨부 파일 본문의 최대 크기입니다. 바이트 수 또는 `KB`/`MB`, `KiB`/`MiB` 접미사를 사용할 수 있으며 최대값은 `16MiB`입니다. 제한을 초과한 파일도 다운로드할 수 있습니다. |
