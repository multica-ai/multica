...
| WeCom | `MULTICA_WECOM_SECRET_KEY` | base64로 인코딩한 32바이트 자격 증명 암호화 키. 설정하지 않으면 이 연동 전체가 비활성화됩니다 |
| WeCom | `MULTICA_WECOM_SOURCE_ID` | 스마트 봇 생성에 사용하는 내장 기본 WeCom "source" ID를 덮어씁니다. Cloud와 자체 호스팅이 기본값을 공유하는 것은 의도된 설계이므로, 디버깅하거나 source를 교체할 때가 아니면 설정할 필요가 없습니다 |
...
설정 단계는 [GitHub 연동](/github-integration#자체-호스팅-설정), [Feishu Bot](/lark-bot-integration#자체-호스팅-설정), [WeCom Bot](/wecom-bot-integration#자체-호스팅-설정), [Slack Bot](/slack-bot-integration#자체-호스팅-설정)을 참고하세요.
