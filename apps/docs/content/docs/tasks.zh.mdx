...
上面这层是**服务端的粗粒度兜底**——按任务启动时间算，不看任务是否还在活动。真正区分「卡死」和「正常的长任务」的是**本地守护进程**：它不再用固定墙钟时长砍任务（`MULTICA_AGENT_TIMEOUT` 默认 `0` = 不设上限），而是看进展——只要 agent 还在持续产出**携带工作的**事件（assistant 内容、工具调用、工具结果、错误），守护进程就不会因为跑得久判它超时（服务端那条 2.5h 仍是外层上限）。status 心跳和 log 不算进展：卡在死掉的 provider 流上的 backend 照样会稳定吐这两类事件，把它们算作活跃会让看门狗永远不触发。只有真正无进展卡死时才会被**空闲看门狗**（`MULTICA_AGENT_IDLE_WATCHDOG`，默认 30 分钟）终止；OpenCode 还会用更短的 `MULTICA_OPENCODE_IDLE_WATCHDOG`（默认 10 分钟）识别首 token 未到或流式回复中断。如果是某个工具调用发出后长时间无任何输出（疑似卡死的子进程），则由更大的**工具看门狗**预算（`MULTICA_AGENT_TOOL_WATCHDOG`，默认 2 小时）兜底。两种看门狗写不同的失败原因，都和墙钟 `timeout` 区分开：无工具在途时的停摆是 `idle_watchdog`，工具在途超时是 `tool_watchdog`。区分是有意的——前者可以安全自动重试，后者不会自动重试（见下节）。各参数见 [环境变量 → 守护进程的调节参数](/environment-variables#守护进程的调节参数)。
...
- `timeout`——运行超时或派发超时
- `idle_watchdog`——被空闲看门狗判定无进展并强制终止。重试会**恢复原来的 session 继续跑**，而不是从头重做：卡死通常是 provider 连接静默死亡，换个新进程就是一条新连接

**不可重试**（任务停在失败状态）：

- `agent_error`——AI 编程工具自己报错（API 错误、超额度、内部 bug）。底层问题不重试，避免无限循环。
- `tool_watchdog`——某个工具调用超时未返回。**刻意不自动重试**：这个工具可能已经在进程外生效了（部署已经发出去、文件已经上传、订单已经提交），只是结果没回来。自动续跑会把它再执行一遍，所以这里停下来等人确认。
