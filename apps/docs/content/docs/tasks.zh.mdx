...
上面这层是**服务端的粗粒度兜底**——按任务启动时间算，不看任务是否还在活动。真正区分「卡死」和「正常的长任务」的是**本地守护进程**：它不再用固定墙钟时长砍任务（`MULTICA_AGENT_TIMEOUT` 默认 `0` = 不设上限），而是看进展——只要 agent 还在持续产出**携带工作的**事件（assistant 内容、工具调用、工具结果、错误），守护进程就不会因为跑得久判它超时（服务端那条 2.5h 仍是外层上限）。status 心跳和 log 不算进展：卡在死掉的 provider 流上的 backend 照样会稳定吐这两类事件，把它们算作活跃会让看门狗永远不触发。只有真正无进展卡死时才会被**空闲看门狗**（`MULTICA_AGENT_IDLE_WATCHDOG`，默认 30 分钟）终止；OpenCode 还会用更短的 `MULTICA_OPENCODE_IDLE_WATCHDOG`（默认 10 分钟）识别首 token 未到或流式回复中断。如果是某个工具调用发出后长时间无任何输出（疑似卡死的子进程），则由更大的**工具看门狗**预算（`MULTICA_AGENT_TOOL_WATCHDOG`，默认 2 小时）兜底。这类被看门狗终止的任务失败原因是 `idle_watchdog`，和墙钟 `timeout` 区分开。各参数见 [环境变量 → 守护进程的调节参数](/environment-variables#守护进程的调节参数)。
