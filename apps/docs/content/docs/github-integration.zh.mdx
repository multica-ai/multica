...
点 **Create GitHub App** 之后，从详情页记下这些值：

- 顶部 **public link** 的尾部即 slug。`https://github.com/apps/multica-acme` → slug = `multica-acme`
- **App ID**
- 在 **Private keys** 区域生成的 **private key**。Multica 支持 raw PEM、转义换行 PEM 或 base64 编码 PEM
...
GITHUB_APP_PRIVATE_KEY=<private key PEM、转义换行 PEM 或 base64 编码 PEM>
GITHUB_WEBHOOK_SECRET=<你刚生成的 webhook secret>
```

Settings 里的连接流程需要四个变量都配置。任何一个缺失：

- Settings 里 `Connect GitHub` 按钮会被 **disable**，并显示「not configured」提示
- setup callback 不会开放，直到 Multica 能签发 GitHub App JWT 并解析安装账号名

`/api/webhooks/github` 端点单独依赖 `GITHUB_WEBHOOK_SECRET`；如果这个 secret 缺失，它会直接返回 **`503 github webhooks not configured`**，而不是默默把所有签名都当成合法。
