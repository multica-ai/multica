...
GitHub 上 checks 明明挂了，PR 行却始终不出现 **Checks 失败** 提示。注意这一行是刻意的 fail-only 设计——通过和运行中本来就什么都不显示，所以在健康的 PR 上订阅缺失是看不出来的，判断配置是否正确要看 App 配置本身而不是 UI。出现上述症状说明 App 收得到 `pull_request`，收不到 `check_suite`——关联和 merge 转 Done 都正常，唯独 CI 数据一直空着。
...
CI 状态只对之后的事件生效——Multica 按收到的 suite 记录，不会回填历史。往一个开着的 PR 推一个 commit（或 re-run 一次它的 checks）即可记录新的 suite；只有其中出现失败时，侧栏才会显示提示。
...
- **运行中的 CI 不会显示，任何权限级别都一样。** check 还在跑的 PR 什么都不显示，和检查全部通过的 PR、根本没配 CI 的 PR 长得一模一样。没有运行中指示。
- **不显示 ≠ 通过。** 这一行是 fail-only 的：webhook 分不清「全都跑完且通过」和「跑得慢的上报方还没汇报」（比如 GitHub Actions 和 Vercel 都上报、只有一个跑完时，另一个完全不可见），所以 Multica 不渲染自己背不了书的绿色状态。这一行唯一敢断言的是红色 **Checks 失败** 提示——它一出现就可靠，re-run 或新 push 通过后自动消失。合并前想看全貌，去 GitHub 上打开 PR。
...
- **CI 状态按 check suite 聚合，不细到 job**——每个上报方（GitHub Actions、Vercel……）记录一个结果，具体 job 名称、逐 job 进度和日志仍然只在 GitHub 上。reviewer 评论和 reviewer 列表没接进 Multica
- **fail-only 显示**——侧栏只显示失败和合并冲突；通过和运行中的 check 什么都不渲染，因为只收 `completed` suite 的 webhook 为它们背不了书。细节见 [CI 状态能说明什么、不能说明什么](#ci-状态能说明什么不能说明什么)
- **CI 状态依赖 `check_suite` 订阅**——在补上这个事件之前创建的 App 永远不会出现「Checks 失败」提示。见 [PR 行不显示 CI 状态](#pr-行不显示-ci-状态)
