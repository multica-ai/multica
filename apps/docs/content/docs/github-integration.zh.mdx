...
`GITHUB_APP_ID` 和 `GITHUB_APP_PRIVATE_KEY` 对 webhook 处理来说**可选**，但使用 Settings → Repositories → 从 GitHub 选择时**必须配置**。服务端会用它们签发短效 installation token 来列出仓库，setup 回调也会在安装完成时直接取得真实组织名/用户名。不配的话，仓库浏览会被禁用，连接卡片会先显示 `已连接到 unknown`；等 GitHub 的 `installation.created` webhook 到达（通常几秒内），Multica 会刷新 row 并通过 realtime 推送让 Settings 页面无需手动刷新即可更新。私钥从 App 设置页 **Private keys → Generate a private key** 生成，把整段 PEM（含 `-----BEGIN/END RSA PRIVATE KEY-----` 两行）粘到 env 里，保留换行。
