...
- **运行中的 CI 不会显示。** check 还在跑的 PR 显示「Checks 还没有汇报」，和一个根本没配 CI 的 PR 长得一模一样。没有运行中指示。
...
<Callout type="info">
**为什么不直接申请 `checks: write`？** Checks 的写权限允许一个 App **在你的仓库上创建和修改 check run**——那是仓库写能力，不是更大范围的读。Multica 是一个只读镜像，从不向 GitHub 写入，为了渲染一个「进行中」的转圈就去要一个写 scope，这个取舍我们不替你做。如果你的团队确实想要运行中状态、也接受这个 scope，可以自己把 App 的 **Checks** 改成 **Read and write**：服务端已经会把 `requested` / `rerequested` 记为 pending，侧栏会开始显示「Checks 运行中」，不需要改任何代码。
</Callout>
...
- **看不到运行中的 CI，绿色只是下界**——只读的 Checks 权限只能收到 `completed` 事件，所以正在跑的 check 完全不可见，而且在另一个上报方还没跑完时行里就可能显示「检查通过」。细节和可选的解法见 [CI 状态能说明什么、不能说明什么](#ci-状态能说明什么不能说明什么)
