...
| **Permissions → Repository → Checks** | **Read-only**。PR 侧栏显示 CI 状态所必需——没有这个权限 GitHub 不会投递 `check_suite` |
...
- **CI 状态按 check suite 聚合，不细到 job**——每个上报方（GitHub Actions、Vercel……）折叠成一个通过 / 失败 / 进行中的结果，具体 job 名称和日志仍然只在 GitHub 上。reviewer 评论和 reviewer 列表没接进 Multica
