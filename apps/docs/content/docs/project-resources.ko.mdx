...
오늘날 두 가지 리소스 타입이 제공됩니다: [`github_repo`](#resource-type-github_repo)(작업마다 격리된 워크트리로 클론)와 [`local_directory`](#resource-type-local_directory)(특정 데몬에 바인딩된 폴더를 제자리에서 또는 issue별 linked worktree로 사용).
...
작업마다 다시 클론하기 곤란한 저장소는 **특정 [데몬](/daemon-runtimes) 기기에 있는 기존 디렉터리**에 연결할 수 있습니다. 기존 디렉터리에서 직접 실행하는 `in_place`와 issue마다 linked worktree를 만드는 `worktree` 중 하나를 선택할 수 있으며, 어느 쪽도 저장소를 다시 클론하지 않습니다.
...
`mode`의 기본값은 `in_place`입니다. 같은 실제 경로를 사용하는 작업을 직렬화하고 사용자의 checkout을 직접 수정합니다. `worktree`는 Git working tree가 필요하며 데몬의 workspace root 아래에 issue별 영구 브랜치와 worktree를 만듭니다. 같은 issue의 작업은 직렬화되고 서로 다른 issue는 병렬로 실행됩니다. 명백한 비 Git 경로는 `in_place`로 폴백하지만, Git 검사나 worktree 생성 오류는 작업을 실패시킵니다.

### `github_repo` 대신 `local_directory`를 선택할 때

| 고려 사항 | `github_repo`(워크트리) | `local_directory` |
| --- | --- | --- |
| 작업당 체크아웃 비용 | 새 클론 + 워크트리 | 클론 없음. 제자리 또는 linked worktree |
| 같은 저장소의 동시성 | 여러 작업 병렬 | `in_place`는 직렬, `worktree`는 issue별 병렬 |
| 브랜치 / 더티 상태 | 작업마다 기본 브랜치에서 새 브랜치를 받음 | `in_place`는 현재 상태, `worktree`는 영구 issue 브랜치 |
| 실행 가능한 위치 | 모든 데몬 | 정확히 하나의 데몬(바인딩된 것) |
| 디스크 사용량 | 작업당 워크트리 하나 | `in_place`는 0, `worktree`는 issue당 하나 |
...
현재 checkout과 더티 상태를 그대로 사용하려면 `in_place`, Git 저장소에서 issue 간 병렬성이 필요하면 `worktree`를 선택하세요. 어느 모드도 파일 단위 잠금을 제공하지 않으며, `worktree`의 격리는 issue별 독립 worktree에서 옵니다.
...
2. `in_place`는 symlink 해석 후 실제 경로로 잠급니다. `worktree`는 issue별로 잠그고 저장소 단위 Git 잠금으로 worktree 추가·삭제를 보호합니다.
3. 실제 작업 디렉터리에 실행 컨텍스트를 기록합니다. `in_place`에서는 사용자 디렉터리, `worktree`에서는 issue worktree입니다.
4. Multica의 런타임 산출물(`output/`, `logs/`, `.gc_meta.json`)은 사용자의 디렉터리 **바깥**의 별도 envRoot에 둡니다.

`in_place`에서는 같은 디렉터리의 후속 작업이 대기합니다. `worktree`에서는 같은 issue의 후속 작업만 대기하고 서로 다른 issue는 병렬로 실행할 수 있습니다. 대기 중에는 **로컬 디렉터리 대기 중(Waiting for local directory)** 상태가 표시됩니다.
...
- **사용자가 바인딩한 디렉터리는 절대 물리적으로 삭제하지 않습니다.** `worktree`에서는 issue가 종료 상태로 TTL을 지난 뒤 데몬 소유의 clean issue worktree를 GC가 삭제할 수 있습니다. 커밋되지 않은 변경이 있는 worktree는 보존됩니다.
...
- **`in_place`에서는 자동 브랜치 전환 없음.** `worktree`는 결정적인 issue 브랜치를 만들거나 재사용합니다.
- **자동 커밋 없음.** `in_place`에서는 기존 변경이 에이전트에게 보입니다. `worktree`의 변경은 issue worktree에 남고 dirty worktree는 GC되지 않습니다.
