...
有效且待接受的工作空间邀请会自动成为该邮箱的注册许可。设置 `ALLOW_SIGNUP=false` 时：

- 已有用户仍可正常登录；
- 新用户可使用待接受且未过期邀请中的邮箱申请登录码并创建账号；
- 未受邀请的用户，以及邀请已过期、已接受、已拒绝或已撤销的用户，仍会被拦截。

普通受邀人无需再加入 `ALLOWED_EMAILS`，也无需重启服务器。账号创建后，仍需通过正常邀请流程接受邀请。
