...
- **실행 중인 CI는 표시되지 않습니다.** 검사가 실행 중인 PR은 "체크가 아직 보고되지 않았습니다"로 표시되어 CI가 아예 없는 PR과 구분되지 않습니다. 진행 중 표시가 없습니다.
...
<Callout type="info">
**왜 `checks: write`를 요청하지 않나요?** Checks의 write 수준 접근은 앱이 **당신의 저장소에 check run을 생성하고 수정하도록** 허용합니다 — 더 넓은 읽기가 아니라 저장소 쓰기 권한입니다. Multica는 읽기 전용 미러이며 GitHub에 쓰지 않으므로, 진행 중 스피너를 그리려고 write 스코프를 요구하는 선택을 대신 내리지 않습니다. 팀이 실행 중 상태를 원하고 그 스코프를 받아들인다면, 직접 App의 **Checks**를 **Read and write**로 설정하세요. 서버는 이미 `requested` / `rerequested`를 pending으로 기록하므로 코드 변경 없이 사이드바에 "일부 체크가 완료되지 않았습니다"가 표시되기 시작합니다.
</Callout>
...
- **실행 중인 CI는 보이지 않으며 초록색 행은 하한값입니다** — read 수준 Checks 권한으로는 `completed` suite만 수신하므로 실행 중인 검사는 보이지 않고, 다른 보고 앱이 아직 실행 중인데도 행이 "검사 통과"로 표시될 수 있습니다. 자세한 내용과 선택적 해결책은 [CI 상태로 알 수 있는 것과 없는 것](#ci-상태로-알-수-있는-것과-없는-것)에 있습니다.
