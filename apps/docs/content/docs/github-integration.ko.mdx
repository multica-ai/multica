...
GitHub에서는 검사가 분명히 실패하고 있는데도 PR 행에 **검사 실패** 알림이 한 번도 나타나지 않는 경우입니다. 이 행은 의도적인 fail-only 설계라는 점에 유의하세요 — 통과 중이거나 실행 중일 때는 원래 아무것도 표시하지 않으므로, 건강한 PR에서는 구독 누락이 보이지 않습니다. 설정이 올바른지는 UI가 아니라 App 설정 자체로 확인해야 합니다. 이 증상은 App이 `pull_request`는 받고 있지만 `check_suite`는 받지 못하고 있다는 뜻입니다 — 연결과 merge → Done은 동작하고 CI 데이터만 계속 비어 있습니다.
...
CI 상태는 이후 이벤트에만 반영됩니다 — Multica는 수신한 suite를 기록할 뿐 과거 이력을 채우지 않습니다. 열린 PR에 커밋을 푸시하거나 체크를 re-run 하면 새 suite가 기록됩니다. 사이드바에 알림이 나타나는 것은 그중 실패가 있을 때뿐입니다.
...
- **실행 중인 CI는 어떤 권한 수준에서도 표시되지 않습니다.** 검사가 실행 중인 PR은 아무것도 표시하지 않아, 모든 검사가 통과한 PR이나 CI가 아예 없는 PR과 구분되지 않습니다. 진행 중 표시가 없습니다.
- **표시가 없다 ≠ 통과했다.** 이 행은 fail-only입니다. webhook만으로는 "전부 끝나고 통과했다"와 "느린 보고 앱이 아직 보고하지 않았다"를 구분할 수 없으므로(GitHub Actions와 Vercel이 모두 보고하는 저장소에서 하나만 끝났다면 나머지는 전혀 보이지 않습니다), Multica는 보증할 수 없는 초록색 상태를 그리지 않습니다. 이 행이 유일하게 단언하는 것은 빨간 **검사 실패** 알림입니다 — 나타나는 순간부터 신뢰할 수 있고, re-run이나 새 푸시가 통과하면 자동으로 사라집니다. 머지 전에 전체 상황을 보려면 GitHub에서 PR을 여세요.
...
- **CI 상태는 job 단위가 아니라 check suite 단위입니다** — 각 보고 앱(GitHub Actions, Vercel 등)마다 결과 하나를 기록합니다. 개별 job 이름, job별 진행 상황, 로그는 GitHub에만 남습니다. 리뷰 댓글과 리뷰어는 표시되지 않습니다.
- **fail-only 표시** — 사이드바에는 실패와 병합 충돌만 표시됩니다. 통과 중이거나 실행 중인 검사는 아무것도 그리지 않습니다. `completed` suite webhook만으로는 이를 보증할 수 없기 때문입니다. 자세한 내용은 [CI 상태로 알 수 있는 것과 없는 것](#ci-상태로-알-수-있는-것과-없는-것)에 있습니다.
- **CI 상태에는 `check_suite` 구독이 필요합니다** — 이 이벤트가 문서화되기 전에 만든 App은 **검사 실패** 알림이 한 번도 나타나지 않습니다. [PR 행에 CI 상태가 표시되지 않음](#pr-행에-ci-상태가-표시되지-않음)을 참고하세요.
