...
`GITHUB_APP_ID`와 `GITHUB_APP_PRIVATE_KEY`는 웹훅 처리에는 **선택 사항**이지만 Settings → Repositories → GitHub에서 선택 기능에는 **필수**입니다. 서버는 이를 사용해 짧은 수명의 installation token을 발급하고 저장소를 나열하며, setup 콜백도 설치 직후 실제 조직명/사용자명을 가져옵니다. 설정하지 않으면 저장소 탐색이 비활성화되고 연결 카드에 잠시 `Connected to unknown`이 표시됩니다. GitHub의 `installation.created` 웹훅이 도착하면(보통 몇 초 이내) Multica가 행을 갱신하고 실시간 브로드캐스트를 보내므로 열려 있는 Settings 탭이 수동 새로고침 없이 업데이트됩니다. 비공개 키는 App 설정 페이지의 **Private keys → Generate a private key**에서 생성한 뒤, PEM 블록 전체(`-----BEGIN/END RSA PRIVATE KEY-----` 줄 포함)를 줄바꿈을 유지한 채 env 값에 붙여넣으세요.
