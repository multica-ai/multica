...
If a run ends while a child in the current stage is still non-terminal, Multica keeps that child in the barrier and wakes the parent coordinator with an explicit recovery action after no active run remains. `in_review` asks for acceptance, `blocked` continues to block, and the platform never marks the child `done` or `cancelled` automatically. The periodic recovery scan replays the same reconciliation for missed events, with duplicate notifications suppressed across restarts.
