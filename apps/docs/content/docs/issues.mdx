...
Every workspace starts with seven built-in statuses, grouped into five lifecycle categories:

| Category | Built-in statuses | Meaning |
|---|---|---|
| `backlog` | `backlog` | Not starting yet. |
| `unstarted` | `todo` | Scoped and waiting to start. |
| `started` | `in_progress`, `in_review`, `blocked` | Work has started, is awaiting review, or cannot continue for now. |
| `completed` | `done` | Finished. |
| `canceled` | `cancelled` | No longer pursued; the record stays. |
...
A workspace owner or admin can add statuses of their own under **Settings → Issue Statuses** — `Code Review`, `QA`, `Rework`. Each new status belongs to one of the five lifecycle categories:

| Category | Default behavior for a new custom status |
|---|---|
| `backlog` | Parked like `backlog`: assigning an agent does not start a run. |
| `unstarted` | Queued like `todo`: assigning an agent starts a run. |
| `started` | Active like `in_progress`. |
| `completed` | Terminal like `done`: closes a sub-issue stage and counts toward parent progress. |
| `canceled` | Terminal like `cancelled`, with the record kept. |

The lifecycle category controls where a custom status appears. Its exact automation behavior is retained separately for compatibility, so existing custom statuses keep behaving as they did before the five-category model. When the platform sets a status itself — such as rollback to `todo` or `done` after a linked PR merges — it writes the concrete built-in status, never a custom one.

Four things follow from that model:

- **A status's category is fixed once the status exists.** Changing it later would regroup every issue already sitting on that status, so the editor keeps it read-only. Choose the lifecycle stage first, then name it.
- **Board columns are categories, not statuses.** Adding a status never adds a column — there is one column per category. A custom status sits in its category's column, and its cards carry a small chip with the status name, so `Code Review` and `QA` stay distinguishable inside Started.
