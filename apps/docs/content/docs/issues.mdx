...
| **Files** | Artifacts attached to the issue and its comments, listed in the sidebar. |
...
The issue's **Files** panel in the sidebar lists every artifact attached to the issue — files on the issue itself and on its comments. **View** a file to preview it in place (press `Esc` to close), or **Download** it. The same file is listed once even if it was attached to both the issue and a comment.
