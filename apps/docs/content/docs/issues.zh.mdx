...
把 issue [分配](/assigning-issues) 给某个智能体等于把这项工作交给它。智能体会**自动开工**——任务立即入队，运行时在线时通常几秒内开始（离线则在队列里等待），并在评论里汇报进展。当它交付一次成果时，惯例是把 issue 移到 `in_review`，让人来收尾——一次 run 完成并不代表 issue 就完成了（详见下面的 [状态](#状态)）。和给同事派活的区别在于：不用你提醒，它自己就开工了。

<Callout type="info">
智能体的身份、配置、运行位置详见 [智能体](/agents)。
</Callout>

能不能分配某个智能体取决于它的 **Access** 设置（Only me / Entire workspace / Specific people）——见 [智能体 → 谁能运行智能体](/agents#谁能运行智能体)。角色权限详见 [成员与权限](/members-roles)。
...
| 状态 | 含义 |
|---|---|
| `backlog` | 已停泊。在 `backlog` 状态下分配智能体**不会**启动它——只有 issue 离开 `backlog`（比如到 `todo`）时 run 才会入队 |
| `todo` | 已排期、准备开工——在这里分配智能体会立刻触发它 |
| `in_progress` | 正在做 |
| `in_review` | 有一次交付在等人来 review |
| `done` | 已完成 |
| `blocked` | 被外部因素卡住 |
| `cancelled` | 已取消 |

**智能体处理一条 issue 时状态怎么流转。** 这些流转是智能体在工作时按自己的指令自己做的，不是平台强制的——典型流程是：智能体开工时把 issue 移到 `in_progress`，交付了可供你查看的东西时移到 `in_review`。**一次 run 完成并不会自动变成 `done`**——`in_review` → `done` 的收尾由你 review 后决定。你也可以随时手动改状态。

平台自己只在两种情况下自动改状态：

- 如果某个智能体的 run **失败**了（且没有待重试、issue 上也没有其它 run 在跑），卡在 `in_progress` 的状态会退回 `todo`，好让你看到它需要再看看。
- 如果一个带关闭意图（`Closes MUL-123`）的关联 GitHub PR 被**合并**，issue 会移到 `done`。
