...
把 issue [分配](/assigning-issues) 给某个智能体等于把这项工作交给它。智能体会**自动开工**——在几秒内开始执行、在评论里汇报进展。当它交付一次成果时，惯例是把 issue 移到 `in_review`，让人来收尾——一次 run 完成并不代表 issue 就完成了（详见下面的 [状态](#状态)）。和给同事派活的区别只在于：它不下线、不需要你提醒、7×24 可用。
