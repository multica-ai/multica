...
<Callout type="info">
谁能使用某个智能体，由智能体自己的 **Access** 设置决定——**Only me**、**Entire workspace** 或 **Specific people**——而不是由工作区角色决定。"Only me" 的智能体只有它的 owner 能运行；工作区的 owner 和 admin **没有旁路**（他们能看到每一个智能体、能改它的大部分配置，但运行它、修改它的 Access 仍然只属于 owner 授权的那些人）。详见 [智能体 → 谁能运行智能体](/agents#谁能运行智能体)。
</Callout>
