...
4. バックエンドの `/readyz` エンドポイントが準備できるまで待機します

`/health` はバックエンドプロセスが待ち受けていることだけを示します。`/readyz` と `/healthz` は PostgreSQL と migration の状態も確認するため、Compose healthcheck、プロダクションの readiness probe、データベースや migration の問題で失敗すべき起動診断にはこれらを使ってください。

バックエンドのコンテナは起動時に **データベースの migration を自動で実行します**（`docker/entrypoint.sh` がサーバー起動前に `./migrate up` を実行）— バックエンドのログで migration の出力を確認できます。単一ホストの Docker Compose ではバージョンアップグレードも同じ方法で処理されます。Kubernetes または複数レプリカのリリースでは、バックエンド Pod をローリングする前に Job または管理された preflight で migration を一度だけ実行してください。readiness は `/readyz` または `/healthz` のままにし、migration が失敗したら rollout を停止します。
