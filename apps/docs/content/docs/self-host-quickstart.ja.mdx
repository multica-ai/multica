...
Docker Compose 構成のアップグレード方法は2通りあります。どちらも**等価**です — `make selfhost` はまさにこれらのコマンドのラッパーなので（`Makefile:79-107`）、好きな方を使ってください。
...
`make selfhost` が `.env` を生成するのは、ファイルが**存在しない**ときだけです（`Makefile:80-95`）。既存の構成で再実行しても、`JWT_SECRET`、Postgres のパスワード、メール設定、`FRONTEND_ORIGIN` はそのまま残ります。
...
Helm には独自のアップグレード経路があります — values ファイルで `images.backend.tag` / `images.frontend.tag` を固定して `helm upgrade` するか、`kubectl -n multica rollout restart` で浮動タグを取り直します。下記の [Kubernetes デプロイ（代替手段）](#kubernetes-デプロイ代替手段)を参照してください。
