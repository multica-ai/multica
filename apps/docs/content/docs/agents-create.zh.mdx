...
**排队等待超时**决定一个执行任务在 `queued` 状态里最多可以等多久；超过这个时间，Multica 会把它标记成 `timeout` 失败。新建智能体时，创建表单默认给的是 **120 分钟**。

- 保持默认值不动：这个智能体就用自己的 120 分钟排队等待预算
- 填一个正整数分钟：按智能体覆盖这条等待预算
- 留空或填 `0`：回退到系统默认值

这个设置只影响**在队列里等待**的时长。任务一旦被守护进程领走并真正启动 AI 编程工具，后面就由正常的派发/运行超时机制接管。详见 [执行任务](/tasks#任务超时会怎样)。
