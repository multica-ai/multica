...
<Callout type="info">
Multica サーバーがこれらのツールや、それらが認証に使う資格情報を見ることは決してありません。以下のすべて — インストール、認証、モデルアクセス — はあなたのローカルマシン上に存在します。何かが失敗する場合、それはほぼ常にローカルの問題です。（別の話として、あなたが意図的にエージェントの[カスタム環境変数](/agents-create#custom-environment-variables-custom_env)に保存したシークレットはサーバー側に保存されます — これはツール自体のローカルなログインとは別のチャネルです。）
</Callout>
