...
オープンソースの CLI コーディングハーネスです（[forgecode.dev](https://forgecode.dev/)）。デーモンはワンショットモードで `forge -C <workdir> --cid <conversation-id> -p <prompt>` として起動します。セッション再開は、固定された conversation id を再利用することで動作します。ForgeCode には `--model` flag はありません — モデルは `FORGE_SESSION__PROVIDER_ID` / `FORGE_SESSION__MODEL_ID` 環境変数で選択されます（`MULTICA_FORGE_MODEL=provider/model` を設定します、例: `anthropic/claude-sonnet-4-20250514`）。ForgeCode のワンショットモードは構造化されたイベントストリームではなく Markdown をレンダリングするため、デーモンは実行後の `forge conversation dump <id>` から構造化されたメッセージリストを再生します。スキルは `.forge/skills/` にコピーされます。

| | |
|---|---|
| デーモンが探す名前 | `forge` |
| インストール | 公式ドキュメント [forgecode.dev](https://forgecode.dev/) を参照してください。 |
| 認証 | ベンダーのドキュメントに従います（構成したプロバイダーの API キー）。 |
