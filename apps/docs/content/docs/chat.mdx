...
description: One-to-one, fully private conversation with an agent, outside any issue. The agent has no issue context by default and never auto-creates issues — but it acts with your workspace access when you explicitly ask it to.
---
...
**Chat is a one-to-one conversation between you and an [agent](/agents)** — stepping outside the [issue](/issues) board. By default the agent has **no issue context** — it isn't looking at your board and won't turn the conversation into issues on its own — and the entire conversation is **fully private** (nobody else in the [workspace](/workspaces), including admins, can see it). It fits discussing an approach with an agent, brainstorming, or asking a question that does not belong to any issue.
...
When you write from the floating chat window while an issue is open — on its page, or selected in [Inbox](/inbox) — each message you send carries a reference to that issue, so the agent knows what "this issue" means. The issue appears above the input; click **×** on it to send without it. The agent receives only the reference: it reads the issue when it needs to and doesn't act on it unless you ask.
