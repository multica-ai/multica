...
- Call `cs-workflow` CLI commands that do not need issue context (for example, querying basic workspace info)

**Cannot do:**

- **See any issue** — the prompt the agent receives has no issue IDs, and commands like `cs-workflow issue list` return empty
