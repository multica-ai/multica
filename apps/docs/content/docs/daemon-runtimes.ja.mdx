...
profile のコマンドや引数の変更は、デーモンが再登録された後に新しく取得するタスクへ適用されます。実行中のタスクは開始時の引数を使い続けます。カスタムランタイム引数と登録エラー表示には、server と daemon の両方が `fixed_args` launch spec と `failed_profiles` 登録レポートを理解するバージョンである必要があります。混在デプロイでは、先に server をアップグレードしてから daemon を順次更新してください。
