...
For advanced use cases, you can also isolate an instance with an explicit config path:

```bash
# Start a cloud-connected daemon using the default instance
multica daemon start

# Configure a second instance backed by a separate config file
multica --config ~/.multica/instances/local/config.json setup self-host --port 8080 --frontend-port 3000

# Start a second daemon bound to that config instance
multica --config ~/.multica/instances/local/config.json daemon start
```

When `--config` is used, Multica isolates the config file, daemon pid/log files, health port, and workspace root as one instance.
...
```bash
multica config show
multica --config ~/.multica/instances/local/config.json config show
```
...
multica --config ~/.multica/instances/local/config.json config set server_url http://localhost:8080
```
