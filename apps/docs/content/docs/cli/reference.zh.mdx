...
### Create an Agent with Fallback Runtimes

```bash
multica agent create \
  --name "Lambda" \
  --runtime-id <primary-runtime-id> \
  --fallback-runtime-ids <fallback-1>,<fallback-2>
```

`--fallback-runtime-ids` accepts an optional prioritized comma-separated list and can also be repeated. Multica skips offline entries and uses the first eligible online runtime.

### Update or Clear Fallback Runtimes

```bash
multica agent update <agent-id> --fallback-runtime-ids <fallback-1>,<fallback-2>
multica agent update <agent-id> --fallback-runtime-ids ''
```

Pass an empty value to clear the fallback chain. A fallback runtime must belong to the same workspace, cannot equal the primary runtime, and cannot appear twice.
