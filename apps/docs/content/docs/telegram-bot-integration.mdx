...
Photos, files, video, voice, and audio messages reach the agent as attachments on the same conversation, with the caption as the message text. Telegram lets a Bot download files up to 20 MB; larger media arrives as a placeholder only. Files the agent produces are sent back into the chat as separate messages after its reply: photos up to 10 MB render inline, everything else up to 50 MB arrives as a file. Both directions need object storage on the server (see the self-hosting section below); without it, media arrives as placeholder text and the agent is told it cannot send files. Stickers, locations, polls, and other non-file messages receive an unsupported-message response in private chats and when addressed in groups; unaddressed group messages of those kinds remain silent.
