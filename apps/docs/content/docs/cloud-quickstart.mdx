...
Sign in on the web ([multica.ai](https://multica.ai)). Email verification codes and Google are both supported. Connect a computer from the command line in the next step.
...
- Click **Add a computer** in the top right, then run the two commands from the dialog in a terminal on the target computer:
...
Open **Workspace → Agents** in the sidebar, click **New agent**, and fill in the form. Only a name is strictly required — confirm the runtime and AI coding tool, then create.
