...
| `DATABASE_REPLICA_URL` | empty | Optional PostgreSQL read-only standby connection string; configuring it only enables health probing until a business explicitly opts into replica reads in code |
...
| `DATABASE_REPLICA_PROBE_INTERVAL` | `2s` | How often the API checks whether the replica is an eligible read-only standby |
| `DATABASE_REPLICA_PROBE_TIMEOUT` | `1s` | Timeout for one replica eligibility probe across primary and replica |
| `DATABASE_REPLICA_MAX_REPLAY_LAG` | `5s` | Maximum replay age while the replica WAL position is behind primary; over-budget replicas fall back to primary |
...
The primary and replica connection limits are independent. Budget their sum across every API process against the PostgreSQL cluster's connection ceiling. Replica configuration is optional and non-critical: an invalid, unavailable, promoted, writable, or over-lag replica stays ineligible, and opted-in reads use primary instead.
