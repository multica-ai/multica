...
| `MULTICA_TASK_DISPATCH_TIMEOUT` | `5m` | Backstop that fails tasks stuck in `dispatched`; the transition to `running` should be near-instant |
| `MULTICA_TASK_RUNNING_TIMEOUT` | `2h30m` | Server-side wall-clock backstop for tasks stuck in `running`; never kills a task whose runtime is still heartbeating |
