...
| `MULTICA_TASK_MEMORY_HIGH` | `0` | Per-run cgroup v2 soft memory limit; accepts bytes or `K`/`M`/`G`/`T` suffixes, and `0` disables the limit |
| `MULTICA_TASK_MEMORY_MAX` | `0` | Per-run cgroup v2 hard memory limit; accepts bytes or `K`/`M`/`G`/`T` suffixes, and `0` disables the limit |
| `MULTICA_TASK_SWAP_MAX` | `unlimited` | Per-run cgroup v2 swap limit; accepts the same sizes, `0` disables swap, and `unlimited` keeps the host default |
...
Linux daemons use a transient systemd slice and scope per run when cgroup v2 and the user systemd manager are available. A task OOM kills only that scope and is stored as `resource_exhausted.memory` with peak memory/swap, PSI, victim cgroup, and launch command; it is not automatically retried under the same profile. If systemd isolation is unavailable, the daemon records a `process_group` fallback and its reason. Cancellation cleanup still applies, but memory limits and exact OOM attribution are unavailable in that mode.
