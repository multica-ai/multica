...
| `AWS_ENDPOINT_URL` | empty | Custom S3-compatible endpoint (for example MinIO or Aliyun OSS). |
| `AWS_S3_FORCE_PATH_STYLE` | auto | Optional boolean. Defaults to `true` for most custom endpoints and `false` for Aliyun OSS endpoints, because OSS rejects path-style S3 requests. |
...
2. Custom endpoint URL if `AWS_ENDPOINT_URL` is set. Path-style endpoints store `<AWS_ENDPOINT_URL>/<S3_BUCKET>/<key>`; virtual-hosted-style endpoints store `https://<S3_BUCKET>.<endpoint-host>/<key>`.
...
These variables are server-side only. Put them in your deployment secret or ConfigMap, not in frontend environment.

| Variable | Default | Description |
|---|---|---|
| `FEISHU_NOTIFICATIONS_ENABLED` | `false` | Set `true` to send enabled users' inbox notifications through the Feishu bot |
| `FEISHU_NOTIFICATIONS_APP_ID` | empty | Feishu notification bot app ID |
| `FEISHU_NOTIFICATIONS_APP_SECRET` | empty | Feishu notification bot app secret |
| `FEISHU_MESSAGE_RECEIVE_ID_TYPE` | `email` | Recipient identifier type for the Feishu message API |
| `FEISHU_API_BASE_URL` | `https://open.feishu.cn` | Override only for tests or a dedicated Feishu gateway |
