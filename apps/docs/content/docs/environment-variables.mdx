...
- **Issue comment follow-up actions** (the buttons under the latest actionable agent comment) — the issue title, up to 1500 characters of its description, and up to 4000 characters of the latest agent comment. Attachments and earlier comments are never included.

When both the API key and the base URL are empty, this layer is disabled and makes no upstream request at all — none of the features above sends anything. That is the supported configuration when your policy does not allow this layer to send private content off the deployment: chat sessions keep the title the client derives from the first message, chat and issue follow-up buttons do not appear, and everything else is unaffected.
