...
| `MULTICA_TASK_CONFIG_PROVIDER_REF_PREFIXES` | empty (disabled) | Comma-separated allowlist of control-plane provider-reference prefixes for project `task_config` resources. Only workspace owners/admins can manage bindings; an empty value keeps the feature disabled |
