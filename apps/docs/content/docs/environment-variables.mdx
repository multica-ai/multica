...
| `MULTICA_DAEMON_LOG_MAX_SIZE_MB` | `20` | Maximum active `daemon.log` size in MB before rotation |
| `MULTICA_DAEMON_LOG_MAX_BACKUPS` | `5` | Number of compressed rotated `daemon.log` backups to retain |
| `MULTICA_DAEMON_LOG_MAX_AGE_DAYS` | `30` | Maximum age in days for rotated `daemon.log` backups |
