...
| WeCom | `MULTICA_WECOM_SECRET_KEY` | Base64-encoded 32-byte credential encryption key; the integration is entirely disabled without it |
| WeCom | `MULTICA_WECOM_SOURCE_ID` | Overrides the built-in default WeCom "source" ID used to create smart bots; Cloud and self-hosted deployments share the default by design, so leave unset unless debugging or migrating to a new source |
...
For setup steps, see [GitHub integration](/github-integration#self-hosting-setup), [Lark bot](/lark-bot-integration#self-hosting-setup), [WeCom bot](/wecom-bot-integration#self-hosting-setup), and [Slack bot](/slack-bot-integration#self-hosting-setup).
