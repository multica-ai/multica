...
| `MULTICA_DAEMON_WS_CLAIM_POLL_INTERVAL` | `3m` | Upper bound for healthy WebSocket claim safety polls; downward jitter makes the normal default `2m30s`–`2m45s`, while old servers and uncertain claims retain the ordinary poll interval |
...
| `ws_claim_poll_interval` | `3m` | Upper bound for healthy WebSocket claim safety polls; the daemon applies downward-only jitter |
