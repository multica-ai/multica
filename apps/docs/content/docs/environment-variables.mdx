...
| `S3_KEY_PREFIX` | empty | Optional object key prefix for all uploads, for example `multica-attachments/dev` stores files under `multica-attachments/dev/workspaces/...` |
...
| `AWS_ENDPOINT_URL` | empty | Custom S3-compatible endpoint (for example [MinIO](https://min.io/) or Huawei OBS) |
| `S3_FORCE_PATH_STYLE` | `true` when `AWS_ENDPOINT_URL` is set | Keep `true` for MinIO-style endpoints; set `false` for Huawei OBS so presigned URLs use `https://<bucket>.obs.<region>.myhuaweicloud.com/<key>` |
| `HUAWEICLOUD_OBS_AK` / `HUAWEICLOUD_OBS_SK` / `HUAWEICLOUD_OBS_ENDPOINT` | empty | Huawei OBS aliases. Used only when the matching `AWS_*` variables are unset |
...
2. `<AWS_ENDPOINT_URL>/<S3_BUCKET>/<key>` (path-style) if `AWS_ENDPOINT_URL` is set and `S3_FORCE_PATH_STYLE` is true.
3. `https://<S3_BUCKET>.<AWS_ENDPOINT_URL host>/<key>` (virtual-hosted-style) if `AWS_ENDPOINT_URL` is set and `S3_FORCE_PATH_STYLE=false`.
4. `https://<S3_BUCKET>.s3.<S3_REGION>.amazonaws.com/<key>` (virtual-hosted-style). When `S3_BUCKET` contains dots, the server falls back to `https://s3.<S3_REGION>.amazonaws.com/<S3_BUCKET>/<key>` (path-style) because the AWS-issued wildcard TLS certificate does not validate dotted bucket hosts.

When `S3_KEY_PREFIX` is set, it becomes part of `<key>`. For example, `S3_KEY_PREFIX=multica-attachments/dev` stores workspace attachments under `multica-attachments/dev/workspaces/<workspace_id>/...`.
...
| Variable | Default | Description |
|---|---|---|
| `GITEE_TOKEN` | empty | Personal access token for the Gitee API. Used by Skill Import to fetch `skill.yaml` / `skill.json` from Gitee repositories. Required for private repos; public repos work without it but are subject to stricter rate-limits |
