...
ZCode supports `MULTICA_ZCODE_PATH` (the `zcode-acp-server` bridge executable) and `MULTICA_ZCODE_MODEL` (a model id from the bridge's advertised catalog, e.g. `glm-5.3`).
