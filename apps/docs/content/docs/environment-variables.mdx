...
| `MULTICA_CODEX_APP_VISIBLE` | `false` | When `true`, Codex tasks create an extra short Codex App-visible portal thread after completion |
| `MULTICA_CODEX_APP_PORTAL_ROOT` | `~/Documents/Codex/Multica` | Directory for Codex App visible portal files |
| `MULTICA_CODEX_APP_VISIBLE_CWD` | portal root | Working directory used for the visible Codex thread; set it to a Codex App workspace root when you want entries grouped there |
