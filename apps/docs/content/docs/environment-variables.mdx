...
The server-side sweeper marks orphaned `running` tasks as failed after a coarse wall-clock timeout. This is the backstop that catches tasks whose daemon disappeared without reporting failure; the daemon's activity-based idle and tool watchdogs remain the primary defense against stuck tasks.

| Variable | Default | Description |
|---|---|---|
| `MULTICA_TASK_RUNNING_TIMEOUT` | `2.5h` | Server-side running-task timeout. Accepts a Go duration string such as `8h` or `30m`. |
