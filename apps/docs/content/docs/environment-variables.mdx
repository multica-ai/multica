...
| `MULTICA_CURSOR_DASHBOARD_USAGE` | empty | Set to `1` to reconcile cursor-agent costs from the local Cursor Dashboard session |
...
Cursor Dashboard reconciliation is opt-in because it reads Cursor's local `state.vscdb` credential and calls an undocumented Dashboard API. The credential stays on the daemon machine; Multica receives only the matched cost and opaque hashes used to prevent duplicate attribution. Each task is queried at most three times. If the credential or API is unavailable, Multica keeps the normal static cost estimate.
