...
Project spaces are durable project-level files, distinct from chat attachments.

| Variable | Default | Description |
|---|---|---|
| `MULTICA_PROJECT_SPACE_ROOT` | `./data/project-spaces` | Root for per-workspace, per-project files. Configure the same shared path on the backend and daemons that run project agents |
| `MULTICA_PROJECT_IMPORT_STAGING_ROOT` | `./data/project-imports` | Backend-only private staging for persistent file and folder imports |

Relative values are normalized to absolute paths from each process working
directory. Use an absolute shared path in production. If a custom project-space
root is configured but is not writable or cannot perform an atomic rename, the
feature fails closed instead of falling back to the default. See [Project
spaces and folder uploads](/project-spaces).
