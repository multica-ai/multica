...
By default, the server reports to Multica's official Amplitude instance. To opt out, set `ANALYTICS_DISABLED=true`.

| Variable | Default | Description |
|---|---|---|
| `ANALYTICS_DISABLED` | `false` | Set `true` to disable backend analytics entirely |
| `AMPLITUDE_API_KEY` | built-in default key | Set when pointing at your own Amplitude project |
| `AMPLITUDE_HOST` | `https://api2.amplitude.com` | Change to your own host or use `https://api.eu.amplitude.com` for EU data residency |
