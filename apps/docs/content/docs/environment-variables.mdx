...
| `MULTICA_AGENT_TEMP_BASE` | `/tmp` (Linux/macOS) | Parent directory for private per-task temp dirs; must be an existing absolute directory, and an invalid value fails task startup. Pick a short path — child tools may bind AF_UNIX sockets under it (`sun_path` is 108 bytes on Linux, 104 on macOS) |
