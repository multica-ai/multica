...
| `MULTICA_DATABASE_STARTUP_TIMEOUT` | `3m` | Total retry budget for transient database failures while the migrator or API starts; set to `0` for one fail-fast attempt |
| `MULTICA_DATABASE_CONNECT_TIMEOUT` | `5s` | Timeout for each database connection attempt during startup |
