...
The four daemon knobs — `device_name`, `runtime_name`, `max_concurrent_tasks`, `poll_interval` — can also be persisted in the profile's `~/.multica/config.json` (or `~/.multica/profiles/<name>/config.json`) so you don't have to re-pass flags or export env vars on every `daemon start`. Set them with:
...
`poll_interval` accepts any positive Go duration (`500ms`, `10s`, `1m30s`, ...). Values ≤ 0 (`0s`, `-5s`) and un-parseable strings are rejected at `config set` time. To clear a persisted value and fall back to the env / built-in default, use `multica config set poll_interval ""`.
