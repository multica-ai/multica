...
| `GITHUB_APP_ID` | empty | Optional for webhooks; required for Settings → Repositories → Choose from GitHub. Numeric App ID used with `GITHUB_APP_PRIVATE_KEY` for App-authenticated REST calls |
| `GITHUB_APP_PRIVATE_KEY` | empty | Optional for webhooks; required for repository browsing. Full App RSA private-key PEM block (including `-----BEGIN/END-----` lines, newlines preserved), used to mint short-lived App JWTs |
