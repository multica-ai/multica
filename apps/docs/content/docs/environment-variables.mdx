...
| `MULTICA_AGENT_IDLE_WATCHDOG` | `30m` | Global no-progress watchdog for runs with no tool in flight. Only messages that carry work (assistant content, tool calls, tool results, errors) count as activity; status pings and log lines do not, because a backend hung on a dead provider stream keeps emitting those. `0` disables the whole watchdog, including provider-specific overrides |
