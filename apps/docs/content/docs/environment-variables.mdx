...
| `log_level` | `debug` | Daemon log threshold: `debug`, `info`, `warn`, or `error`. When set, it takes precedence over `LOG_LEVEL` for this profile |
...
- `log_level` accepts `debug`, `info`, `warn`, or `error`; an empty value clears the profile override and returns to `LOG_LEVEL` or `debug`.
