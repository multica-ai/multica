...
| Variable | Default | Description |
|---|---|---|
| `MULTICA_MAX_PREVIEW_SIZE` | `2MiB` | Maximum attachment body loaded into memory for inline text preview; accepts bytes or `KB`/`MB` and `KiB`/`MiB`, up to `16MiB`. Larger files remain downloadable. |
