...
| `MULTICA_STAGE_RECOVERY_THRESHOLD` | `15m` | Age after which the server rechecks non-terminal staged child issues with no active run and wakes the parent coordinator with recovery guidance; discovery only, never auto-completes or cancels |
| `MULTICA_STAGE_RECOVERY_BATCH_SIZE` | `100` | Maximum stalled staged child issues reconciled per 30-second sweep |
