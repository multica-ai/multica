...
The [GitHub PR ↔ issue integration](/github-integration) needs GitHub App install, JWT, and webhook credentials. Set all four to enable Connect GitHub in Settings and accept incoming webhooks.
...
| `GITHUB_APP_ID` | empty | The numeric App ID from the GitHub App settings page. Used to sign the App JWT for installation lookups |
| `GITHUB_APP_PRIVATE_KEY` | empty | A GitHub App private key as raw PEM, escaped-newline PEM, or base64-encoded PEM. Used with `GITHUB_APP_ID` to resolve installation account names |
...
**Behavior when any connect-flow variable is unset:**

- `Connect GitHub` in Settings → GitHub is **disabled** and shows a "not configured" hint to admins.
- The setup callback is not offered until Multica can sign a GitHub App JWT and resolve the installed account name.
- The `/api/webhooks/github` endpoint specifically requires `GITHUB_WEBHOOK_SECRET`; when that secret is missing it returns **`503 github webhooks not configured`** rather than treating every signature as valid.
...
- [GitHub integration](/github-integration) — how to set up the GitHub App that backs these variables
