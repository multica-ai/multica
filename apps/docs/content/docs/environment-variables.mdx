...
| `MULTICA_PLATFORM_CONTEXT` | `full` | Multica context mode: `full` keeps the historical workflow brief; `minimal` keeps runtime facts and a neutral catalog of task-bound skills without activation rules; `off` removes the brief and server-provided built-in skills. Boolean `true`/`false` remain aliases for `full`/`off` |
...
| `platform_context_mode` | follows environment | `full`, `minimal`, or `off`; `minimal` keeps server-provided skills visible while leaving activation to the provider, and `off` removes them. Empty clears the local override |
