...
[WeCom Bot integration](/wecom-bot-integration) stays off until `MULTICA_WECOM_SECRET_KEY` is set. Binding links require `MULTICA_PUBLIC_URL` (the user-facing frontend URL, not the backend API host).

| Variable | Default | Description |
|---|---|---|
| `MULTICA_WECOM_SECRET_KEY` | empty | Base64-encoded 32-byte key that encrypts Bot/Corp secrets at rest. Generate with `openssl rand -base64 32` |
| `MULTICA_PUBLIC_URL` | empty | Prefix for bind links in Bot replies, e.g. `https://multica.example.com` (no trailing slash) |

See [WeCom Bot integration](/wecom-bot-integration) for setup, group-chat behavior, and Docker deployment.
