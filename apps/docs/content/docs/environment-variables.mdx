...
| Channel notifications | `MULTICA_INBOX_FORWARD_CHANNELS` | Comma-separated allowlist for private forwarding of issue-backed Inbox items; empty disables it, and the initial sender supports `feishu` |
...
To forward Inbox notifications to Feishu, configure the Lark integration and opt in explicitly:

```dotenv
MULTICA_INBOX_FORWARD_CHANNELS=feishu
```

Forwarding begins from committed member Inbox items, so it does not recreate assignment, subscription, preference, or mention rules. For an issue-backed item, Multica first tries the current assignee Agent's active Bot, then active Agent subscribers in stable participation order. The recipient must be bound to that exact Bot installation. Member and `@all` mentions flow through their normal `mentioned` Inbox items; `@agent` remains an Agent execution trigger and does not create a human notification.

Items without an issue, without an eligible participating Bot, or without a same-installation member binding remain Inbox-only. Delivery is private and best-effort: queue saturation or a Channel API failure never rolls back the Inbox item. Listing multiple Channel types requests one delivery per supported type; it is explicit fan-out, not a fallback priority. The initial implementation provides only the `feishu` sender.
