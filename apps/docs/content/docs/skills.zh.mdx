...
有些 AI 编程工具还会发现**提交在仓库里的项目级 Skill**，例如 `.claude/skills/`、`.cursor/skills/`、`.opencode/skills/` 或 `.agents/skills/`。当 Multica 任务检出这个仓库时，这些文件会留在工作目录里，底层工具可能会按自己的原生发现规则加载它们。Multica 不会自动把这些仓库文件导入工作区 Skill 注册表；它们属于所选工具的 repo-scoped 行为。
