...
<Callout type="info">
Multica 维护了官方 [`multica-cli`](https://github.com/multica-ai/multica-cli) skill，供 Claude Code、Codex、Cursor 等本地 agent 使用。它会教 agent 如何安全地通过已认证的 `multica` CLI 操作 Multica，包括 issue 评论、metadata、PR、mention 和状态变更的副作用。可用 GitHub 地址 `https://github.com/multica-ai/multica-cli/tree/main/skills/multica-cli` 导入，也可以直接从该仓库安装到本地编程工具。
</Callout>
