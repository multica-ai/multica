...
Multica auto-hébergé peut aussi connecter en parallèle des instances Forgejo, Gitea ou GitLab auto-hébergées, avec la même association automatique des PR, le même passage à Terminé à la fusion et le même affichage de la CI. Le point d'entrée est **Paramètres → Code** ; voir [Fournisseurs Git auto-hébergés](/vcs-integration). Multica Cloud ne propose pas ce point d'entrée.

## Connecter GitHub

Un propriétaire ou un administrateur de l'espace de travail peut effectuer la connexion :

1. Ouvrez **Paramètres → Code**.
2. Sur la ligne **GitHub**, cliquez sur **Connecter GitHub**.
3. Sur GitHub, choisissez le compte ou l'organisation, puis autorisez tous les dépôts ou une sélection d'entre eux.
4. Revenez sur Multica une fois l'installation terminée.
...
<Callout type="info">
La **connexion GitHub** détermine de quels dépôts Multica reçoit les événements de PR ; le réglage des **dépôts de code** (section **Dépôts** de la même page) détermine les dépôts que les agents peuvent choisir au démarrage des exécutions. Ils ont des rôles différents et se configurent séparément.
</Callout>

## Interrupteurs de fonctions

La section **Pull requests et tâches** de **Paramètres → Code** comporte ces interrupteurs. **Mettre en pause les fonctionnalités GitHub**, dans le menu **⋯** de la ligne GitHub, désactive les trois interrupteurs ci-dessous sans déconnecter la GitHub App.
...
Choisir **Déconnecter** dans le menu **⋯** de la ligne GitHub, sur **Paramètres → Code**, supprime seulement la relation entre l'espace de travail Multica courant et l'installation ; cela ne désinstalle pas l'App de GitHub à votre place. Les enregistrements de PR existants sont conservés, et les nouveaux événements cessent d'arriver dans cet espace de travail.
...
Redémarrez le serveur API, puis effectuez la connexion dans **Paramètres → Code**.
