...
|---|---|---|---|---|
| インストール | Multica で QR コードを生成し、Feishu でスキャンして認証 | Multica で QR コードを生成し、WeCom でスキャンして認証 —— スキャンのたびに新しいスマート Bot が作成されます | Slack でアプリを作成し、2 つのトークンを Multica に入力 | 社内アプリと Stream モードのロボットを作成し、AppKey と AppSecret を Multica に入力 |
