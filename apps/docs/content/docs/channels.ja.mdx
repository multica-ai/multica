...
Telegram は画像、ファイル、動画、音声を添付ファイルとしてエージェントに渡し、エージェントが生成したファイルをチャットに送り返します。どちらもサーバー側にオブジェクトストレージの設定が必要です。sticker などファイル以外のメッセージにはその旨の短い返信が返り、エージェントには渡されません。
