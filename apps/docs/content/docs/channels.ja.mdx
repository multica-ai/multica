...
Qianwen/Quark S1 連携は実験的なプレビューです。Quark S1 上のプライベート Qianwen Skill からリクエストを送信し、完了結果を明示的にポーリングします。現時点ではメガネへの能動的な通知やプッシュには対応していません。
...
- [Qianwen Skill ブリッジ（実験的）](/qianwen-skill-integration)
