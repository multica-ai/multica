...
セルフホスト版 Multica では、セルフホストの Forgejo、Gitea、GitLab インスタンスも同時に接続できます。PR の自動紐づけ、マージ時のステータス変更、CI 表示を同じように利用できます。**設定 → コード**から設定してください。詳しくは[セルフホスト Git ホスティング](/vcs-integration)を参照してください。Multica Cloud にはこの項目はありません。
...
1. **設定 → コード** を開きます。
2. **GitHub** の行で **GitHub に接続**をクリックします。
3. GitHub でアカウントまたは organization を選び、すべてのリポジトリか指定したリポジトリを許可します。
4. インストールが完了したら Multica に戻ります。
...
**設定 → コード** の **プルリクエストとタスク** には次の設定があります。GitHub の行の **⋯** メニューで **GitHub 機能を一時停止** すると、下の 3 項目は機能しなくなりますが、GitHub App の接続は解除されません。
...
| PR マージ後のタスクのステータス | タスクに紐づく PR がすべてマージされたとき、タスクをあるステータス（既定は `Done`）に変更するか、変更しないかを選びます。**設定 → ステータスと遷移** で設定し、コードのページには現在の値だけが表示されます。下記の **PR マージ後のステータス変更** を参照してください。 |
...
PR がマージされても、必ずしもタスクが完了するわけではありません。そのため、マージ後に何をするかはワークスペースが決めます: **設定 → ステータスと遷移 → 自動遷移**。`Done`（新しいワークスペースの既定値）、ほかの「開始済み」「完了」カテゴリーのステータス（「回帰テスト待ち」のようなカスタムステータスも可）、または **変更しない** を選べます。この設定ができる前からあるワークスペースは、これまでの挙動を保ちます。マージしても必ずタスクが完了していたわけではない場合、初期値は **変更しない** です。
...
**設定 → コード** の GitHub の行で **⋯** メニューから**接続解除**を選ぶと、現在の Multica ワークスペースと installation の関係だけが削除されます。GitHub から App がアンインストールされることはありません。既存の PR 記録は保持されますが、新しいイベントはこのワークスペースに届かなくなります。
