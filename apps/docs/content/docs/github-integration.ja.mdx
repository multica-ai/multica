...
GitHub 上ではチェックが明らかに失敗しているのに、PR 行に **チェック失敗** アラートが一度も表示されない場合です。この行は意図的な fail-only 設計であることに注意してください — 成功中・実行中はもともと何も表示しないため、健全な PR では購読の欠落は見えません。設定が正しいかは UI ではなく App の設定自体で確認します。この症状は App が `pull_request` は受け取れているが `check_suite` を受け取れていないことを意味します — リンクと merge → Done は動作し、CI データだけが空のままになります。
...
CI ステータスは以降のイベントにのみ反映されます — Multica は受信した suite を記録するだけで、過去の履歴は埋めません。オープンな PR にコミットをプッシュするか、チェックを re-run すると新しい suite が記録されます。サイドバーにアラートが出るのは、その中に失敗があった場合のみです。
...
- **実行中の CI は、どの権限レベルでも表示されません。** チェックが実行中の PR は何も表示せず、すべてのチェックが成功した PR や CI がまったくない PR と区別がつきません。実行中を示す表示はありません。
- **表示がない ≠ 成功。** この行は fail-only です。webhook では「すべて完了して成功した」と「遅いレポート元がまだ報告していない」を区別できないため（GitHub Actions と Vercel の両方が報告する場合、片方だけが完了していると、もう片方は完全に見えません）、Multica は保証できない緑のステータスを描画しません。この行が唯一断言するのは赤い **チェック失敗** アラートです — 表示された瞬間から信頼でき、re-run や新しいプッシュで成功すると自動的に消えます。マージ前に全体像を見るには GitHub で PR を開いてください。
...
- **CI ステータスは check suite 単位で、ジョブ単位ではありません** — 各レポート元（GitHub Actions、Vercel など）につき 1 つの結果を記録します。個々のジョブ名、ジョブごとの進捗、ログは GitHub 側にのみ残ります。レビューコメントとレビュアーは表示されません。
- **fail-only 表示** — サイドバーに表示されるのは失敗とマージコンフリクトだけです。成功中・実行中のチェックは何も描画されません。`completed` suite の webhook だけではそれらを保証できないためです。詳細は [CI ステータスで分かること・分からないこと](#ci-ステータスで分かること分からないこと)にあります。
- **CI ステータスには `check_suite` の購読が必要です** — このイベントが文書化される前に作成された App では、**チェック失敗** アラートは一度も表示されません。[PR 行に CI ステータスが表示されない](#pr-行に-ci-ステータスが表示されない)を参照してください。
