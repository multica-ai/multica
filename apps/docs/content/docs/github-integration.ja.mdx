...
- **実行中の CI は表示されません。** チェックが実行中の PR は「チェックはまだ報告されていません」と表示され、CI がまったくない PR と区別がつきません。実行中を示す表示はありません。
...
<Callout type="info">
**なぜ `checks: write` を要求しないのか？** Checks の write レベルアクセスは、アプリが**あなたのリポジトリ上で check run を作成・更新する**ことを許可します — これは読み取り範囲の拡大ではなく、リポジトリへの書き込み権限です。Multica は読み取り専用のミラーであり GitHub に書き込むことはないため、実行中スピナーを表示するためだけに write スコープを要求するという判断を、こちらで勝手に行うことはしません。実行中ステータスが必要で、このスコープを受け入れられる場合は、ご自身の App の **Checks** を **Read and write** に設定してください。サーバー側はすでに `requested` / `rerequested` を pending として記録するため、コード変更なしでサイドバーに「チェックがまだ完了していません」が表示されるようになります。
</Callout>
...
- **実行中の CI は見えず、緑の行は下限値です** — read レベルの Checks 権限では `completed` の suite しか受信できないため、実行中のチェックは見えず、別のレポート元がまだ実行中でも行が「チェックに成功しました」と表示されることがあります。詳細と任意の対処は [CI ステータスで分かること・分からないこと](#ci-ステータスで分かること分からないこと)にあります。
