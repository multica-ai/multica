...
- **In-flight CI is not surfaced.** A PR whose checks are still running shows *Checks haven't reported yet*, the same as a PR with no CI at all. There is no running/pending indicator.
...
<Callout type="info">
**Why not just request `checks: write`?** Write-level access to Checks lets an app **create and update check runs on your repositories** — it is a repo-write capability, not a broader read. Multica is a read-only mirror and never writes to GitHub, so requesting a write scope purely to render an in-progress spinner is a trade we don't make on your behalf. If your team wants in-flight status and accepts that scope, set **Checks** to **Read and write** on your own App: the server already records `requested` / `rerequested` as pending and the sidebar will start showing *Some checks haven't completed yet* with no code change.
</Callout>
...
- **No in-flight CI, and a green row is a lower bound** — read-level Checks access only receives `completed` suites, so running checks are invisible and a row can read *Checks passed* while another provider is still going. Details and the opt-in fix in [what CI status can and cannot tell you](#what-ci-status-can-and-cannot-tell-you).
