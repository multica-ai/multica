...
| **CI status** | On every `check_suite` event, Multica records the suite against the PR's current head commit. The sidebar row is **fail-only**: a red **Checks failed** alert appears when an observed suite failed, and clears automatically once the current head no longer has a failed suite. Passing and in-flight checks render nothing. Requires the `check_suite` event subscription — see [setup](#1-create-the-github-app). Multica sees **completed** suites only; read [what CI status can and cannot tell you](#what-ci-status-can-and-cannot-tell-you) for why silence is not a verdict. |
...
A PR whose checks are clearly failing on GitHub never shows the **Checks failed** alert. Note that the row is fail-only by design — passing or still-running checks render nothing — so on healthy PRs a missing subscription is invisible. The reliable check is the App configuration, not the UI.
...
CI status only populates going forward — Multica records suites as they arrive and does not backfill history. Push a commit to an open PR (or re-run its checks) to record the first suites; the sidebar shows an alert only if one of them fails.
...
- **In-flight CI is not surfaced, at any permission level.** A PR whose checks are still running renders nothing, the same as a PR whose checks all passed or a PR with no CI at all. There is no running/pending indicator.
- **Silence is not a verdict.** The row is fail-only: webhooks cannot distinguish "everything finished and passed" from "the slower provider has not reported yet" (if GitHub Actions and Vercel both report and only one has finished, the other is invisible), so Multica does not render a green state it cannot stand behind. The one thing the row does assert — the red **Checks failed** alert — is reliable the moment it appears, and clears automatically when a re-run or a new push comes back clean. Open the PR on GitHub for the full picture before merging.
...
- **CI status is per check suite, not per job** — Multica records one result per reporting app (GitHub Actions, Vercel, …). Individual job names, per-job progress, and logs stay on GitHub. Review comments and reviewers are not surfaced.
- **CI status needs the `check_suite` subscription** — an App created before this event was documented will never raise the **Checks failed** alert. See [PR rows show no CI status](#pr-rows-show-no-ci-status).
- **Fail-only display** — the sidebar surfaces failures and merge conflicts only; passing and in-flight checks render nothing, because `completed`-suite webhooks cannot vouch for them. Details in [what CI status can and cannot tell you](#what-ci-status-can-and-cannot-tell-you).
