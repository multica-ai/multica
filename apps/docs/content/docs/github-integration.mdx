...
| **CI status** | On every `check_suite` event, Multica records the suite against the PR's current head commit and rolls the results up into the sidebar row: **All checks passed**, **Some checks failed**, or **Some checks haven't completed yet**, with a segmented bar showing the pass/fail/pending split. Requires the `check_suite` event subscription — see [setup](#1-create-the-github-app). |
...
| **Permissions → Repository → Checks** | **Read-only**. Required for CI status on the PR sidebar row — GitHub will not deliver `check_suite` without it. |
...
- **CI status is per check suite, not per job** — Multica rolls each reporting app (GitHub Actions, Vercel, …) up to one pass/fail/pending result. Individual job names and logs stay on GitHub. Review comments and reviewers are not surfaced.
