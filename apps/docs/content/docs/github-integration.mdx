...
After **Create GitHub App**, note these values from the App's detail page:

- The **public link** at the top — its tail is the slug. `https://github.com/apps/multica-acme` → slug = `multica-acme`.
- The **App ID**.
- A **private key** generated from the App's **Private keys** section. Store it as raw PEM, escaped-newline PEM, or base64-encoded PEM.
...
GITHUB_APP_PRIVATE_KEY=<the private key PEM, escaped-newline PEM, or base64-encoded PEM>
GITHUB_WEBHOOK_SECRET=<the webhook secret you generated>
```

All four variables are required for the Settings → GitHub connect flow. If any is missing:

- `Connect GitHub` in Settings is **disabled** and shows a "not configured" hint.
- The setup callback will not be offered until Multica can sign a GitHub App JWT and resolve the installed account name.

The `/api/webhooks/github` endpoint specifically requires `GITHUB_WEBHOOK_SECRET`; if that secret is missing, it returns **`503 github webhooks not configured`** rather than silently treating every signature as valid.
