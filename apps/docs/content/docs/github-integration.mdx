...
Self-hosted Multica can also connect self-hosted Forgejo, Gitea, or GitLab instances in parallel, with the same PR auto-linking, merge-driven status changes, and CI display. The entry point is **Settings → Code**; see [Self-hosted Git hosting](/vcs-integration). Multica Cloud does not offer this entry point.
...
2. In the **GitHub** row, click **Connect GitHub**.
3. In GitHub, pick the account or organization and authorize all repositories or a chosen subset.
4. Return to Multica after the installation completes.
...
| After PRs merge, move the issue to | What merging every PR linked to an issue does: move it to a status (`Done` by default) or leave it alone. Set on **Settings → Statuses & transitions**; the Code page shows the current choice. See [When merged PRs move an issue](#when-merged-prs-move-an-issue). |
...
**After PRs merge, move the issue to** applies to every code host, including [self-hosted Git hosting](/vcs-integration).
...
Restart the API server, then complete the connection in **Settings → Code**.
