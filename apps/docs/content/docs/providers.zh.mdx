...
| **Reasonix** | esengine | ❌ | ❌ | `.reasonix/skills/` | 启动参数 |
...
esengine 出品，偏向 DeepSeek 原生工作流。Multica 通过 `npx reasonix acp` 拉起它，也支持把 `MULTICA_REASONIX_PATH` 配成自定义 wrapper（例如 `npx dsnix@latest`）。**它不支持会话恢复**——每次任务都会新建一条 ACP 会话——但项目级 skill 会复制到 `.reasonix/skills/` 供 Reasonix 原生发现，模型通过启动时的 `--model` 参数传入。
...
| ❌ 无 | Gemini、Reasonix | CLI 没有可用的 resume 机制 |
