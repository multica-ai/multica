...
| **ForgeCode** | 开源项目 | ✅（`--cid <id>`）| ❌ | `.forge/skills/` | 静态（环境变量）|
...
开源 CLI 编程 harness（[forgecode.dev](https://forgecode.dev/))）。和其他支持的工具不同，ForgeCode 的 one-shot 模式（`forge -p`）**不**输出结构化事件流——它把工具调用、推理、正文交织在一起，渲染成 Markdown 发到 stdout。Multica 用两阶段方案绕过这个限制：运行时先缓冲 stdout（只转发存活心跳），运行结束后从 `forge conversation dump <id>` 重放结构化消息列表（工具调用、工具结果、推理、token 用量）。**会话恢复可用**，通过钉住的 `--cid <id>` 实现；会话 id 被复用来接续上一次会话。**模型选择特殊**：没有 `--model` flag——模型通过 `FORGE_SESSION__PROVIDER_ID` / `FORGE_SESSION__MODEL_ID` 环境变量设置，Multica 从 `provider/model` 值推导（如 `anthropic/claude-sonnet-4-20250514`）。MCP 配置不消费。skill 复制到 `.forge/skills/`。
...
**14 款工具里有 10 款实际消费 `mcp_config`：Claude Code、CodeBuddy、Codex、Cursor、Hermes、Kimi、Kiro CLI、OpenCode、OpenClaw、Qoder**。其他 4 款会接收这个字段但**忽略**——不报错、不警告，只是配置不生效。
...
- [安装 agent 运行时](/install-agent-runtime) —— 14 款支持工具的安装和认证
