...
| **Warp Oz CLI** | Warp | ✅ | ✅ | `.agents/skills/` | 动态发现 |
...
Anthropic 出品。**新用户首选**——功能最完整：会话恢复真用，会读取 MCP 配置，并支持 `--max-turns`、`--append-system-prompt` 等细调参数。需要一个 Anthropic API 密钥。
...
Warp 出品。通过 `oz agent run` 做 headless agent 执行。会话恢复通过 Oz conversation ID 工作，MCP 会透传给 Oz，skill 从 `.agents/skills/` 原生发现。macOS 上作为守护进程运行时，推荐安装独立的 Oz CLI 包，而不是使用 `Warp.app` 内置的 CLI，避免后台任务激活桌面 app。
...
| 状态 | 工具 | 含义 |
|---|---|---|
| ✅ 真用 | Antigravity、Claude Code、Copilot、Hermes、Kimi、Kiro CLI、OpenCode、OpenClaw、Pi、Warp Oz CLI | 传 resume id，会从上次上下文接着继续 |
...
Claude Code 和 Warp Oz CLI 会实际消费 `mcp_config`。其他工具会接收这个字段但忽略它——不报错、不警告，只是配置不生效。

<Callout type="warning">
如果你在智能体配置里设置了 `mcp_config`，但选了 Claude Code 或 Warp Oz CLI 之外的工具，你的 MCP server 对这个智能体**没有效果**。
</Callout>
