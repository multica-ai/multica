...
托管 Prime Agent 不使用共享的 `~/.prime/agent` 目录。Multica 为每个 runtime/agent 分配独立的私有持久目录，用于 sessions、settings、authentication 和 user skills。请为每个 managed agent 单独配置认证，或使用明确限定范围的环境凭据；不要在 Prime project state 中保存跨 tenant 共享的 MCP settings 或 secrets。
