...
| **Qwenpaw** | Qwen（阿里云） | ✅ | ❌ | `.qwenpaw/skills/` | 动态发现 |
...
Qwen 团队（阿里云）出品。基于 ACP 协议通过 `qwenpaw acp` stdio 传输。会话恢复走 ACP `session/load`，模型选择通过 `--model` 参数，skill 会复制到 `.qwenpaw/skills/` 实现原生项目级发现。
...
| 状态 | 工具 | 含义 |
|---|---|---|
| ✅ 真用 | Claude Code、Copilot、Hermes、Kimi、Kiro CLI、OpenCode、OpenClaw、Pi、Qwenpaw | 传 resume id，会从上次上下文接着继续 |
