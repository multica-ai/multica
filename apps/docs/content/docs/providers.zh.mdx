...
| **Devin** | Cognition AI | ✅（走 ACP `session/load`） | ❌（通过 `devin mcp` 管理） | `.devin/skills/` | 由 Devin 自身配置决定（守护进程不参与） |
...
Cognition AI 出品。通过 `devin acp` 使用 ACP stdio 协议（与 Hermes / Kimi / Kiro 同协议族）。会话恢复走 ACP `session/load`。skill 会自动从 `.devin/skills/` 读取，对齐 Devin for Terminal 的原生约定。**模型选择完全在 Devin 侧**——Devin 的 ACP 服务器不实现 `session/set_model`，所以守护进程一侧的模型下拉框会被隐藏；用 `devin --model` 或 `~/.config/devin/config.json` 配置模型。工具自动审批走 ACP 的 `session/request_permission`，由守护进程在每个任务里自动应答。
...
| 状态 | 工具 | 含义 |
|---|---|---|
| ✅ 真用 | Claude Code、Copilot、Devin、Hermes、Kimi、Kiro CLI、OpenCode、OpenClaw、Pi | 传 resume id，会从上次上下文接着继续 |
