...
description: Deploy Wallts on your own infrastructure.
---

## Architecture Overview

Wallts has three components:
...
Each user who wants to run AI agents locally also installs the **`wallts` CLI** and runs the **agent daemon** on their own machine.
...
curl -fsSL https://raw.githubusercontent.com/wallts-ai/wallts/main/scripts/install.sh | bash -s -- --with-server

# Configure CLI, authenticate, and start the daemon
wallts setup self-host
```

This installs the CLI, checks out the latest self-host assets, pulls the official Wallts images from GHCR, and configures everything for localhost. Then open http://localhost:3000 and pick a login method: configure `RESEND_API_KEY` in `.env` for email-based codes (recommended), or leave Resend unset and copy the generated code from backend logs. See [Step 2 — Log In](#step-2--log-in) for details.

<Callout>
If the self-host server is already running and you only need the CLI on a macOS/Linux machine, install it with Homebrew: `brew install wallts-ai/tap/wallts`.
</Callout>
...
<Callout>
Alternatively, configure step by step: `wallts config set server_url http://localhost:8080 && wallts config set app_url http://localhost:3000 && wallts login && wallts daemon start`
</Callout>
...
Full reference (Compose + Kubernetes templates, flag descriptions, upgrade order) lives in [`SELF_HOSTING_ADVANCED.md → Usage Dashboard Rollup`](https://github.com/wallts-ai/wallts/blob/main/SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup).
...
Wallts uses email-based magic link authentication via [Resend](https://resend.com).
...
This starts a `pgvector/pgvector:pg17` container on port 5432 with default credentials (`wallts`/`wallts`).
