...
`./stop.sh` also stops only the Compose stack by default. Source-development debugging can opt in to the old source-run daemon stop behavior with `SELFHOST_STOP_DAEMON=true ./stop.sh`, paired with `SELFHOST_START_DAEMON=true ./start.sh`.
