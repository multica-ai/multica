...
Each user who wants to run AI agents locally also installs the **`cs-workflow` CLI** and runs the **agent daemon** on their own machine.
...
<Callout>
Alternatively, configure step by step: `cs-workflow config set server_url http://localhost:8080 && cs-workflow config set app_url http://localhost:3000 && cs-workflow login && cs-workflow daemon start`
</Callout>
...
This starts a `pgvector/pgvector:pg17` container on port 5432 with default credentials (`cs-workflow`/`cs-workflow`).
