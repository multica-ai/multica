...
<Callout>
If you prefer running the Docker Compose steps manually: `cp .env.example .env`, edit `JWT_SECRET` and `POSTGRES_PASSWORD`, then `docker compose -f docker-compose.selfhost.yml pull && docker compose -f docker-compose.selfhost.yml up -d`. Leave `DATABASE_URL` unset for the bundled Compose PostgreSQL service; set it only when the backend should use an external PostgreSQL instance.
</Callout>
...
|----------|-------------|---------|
| `DATABASE_URL` | Optional external PostgreSQL connection string. Leave unset for the bundled Compose PostgreSQL service. | `postgres://user:password@postgres.example.com:5432/multica?sslmode=require` |
| `POSTGRES_PASSWORD` | **Must change from default.** Password used by the bundled Postgres container. | `openssl rand -hex 24` |
