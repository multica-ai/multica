...
| `MULTICA_DAEMON_LOG_MAX_SIZE_MB` | `20` | ローテーション前の `daemon.log` 現行ファイルの最大サイズ（MB） |
| `MULTICA_DAEMON_LOG_MAX_BACKUPS` | `5` | 保持する圧縮済み `daemon.log` ローテーションバックアップ数 |
| `MULTICA_DAEMON_LOG_MAX_AGE_DAYS` | `30` | ローテーション済み `daemon.log` バックアップの最大保持日数 |
