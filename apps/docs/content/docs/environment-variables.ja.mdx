...
| WeCom | `MULTICA_WECOM_SECRET_KEY` | base64 エンコードされた 32 バイトの資格情報暗号化キー。未設定の場合、この連携は全体が無効になります |
| WeCom | `MULTICA_WECOM_SOURCE_ID` | スマート Bot の作成に使う、組み込みのデフォルト WeCom「source」ID を上書きします。Cloud とセルフホストがデフォルトを共有するのは意図した設計のため、デバッグや source の変更時以外は設定不要です |
...
設定手順は [GitHub 連携](/github-integration#セルフホスト設定)、[Lark Bot](/lark-bot-integration#セルフホスト設定)、[WeCom Bot](/wecom-bot-integration#セルフホスト設定)、[Slack Bot](/slack-bot-integration#セルフホスト設定)を参照してください。
