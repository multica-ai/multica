...
| `MULTICA_TASK_DISPATCH_TIMEOUT` | `5m` | `dispatched` に滞留したタスクを失敗させるバックストップ。`running` への遷移はほぼ即時のはずなので防御用の時間枠です |
| `MULTICA_TASK_RUNNING_TIMEOUT` | `2h30m` | `running` に滞留したタスク向けのサーバー側ウォールクロック・バックストップ。ランタイムがまだハートビートを送っているタスクはこのタイムアウトでは決して終了させられません |
