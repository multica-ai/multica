...
Prime Agent は `MULTICA_PRIME_PATH`、`MULTICA_PRIME_MODEL`（`provider/modelId`）、必須の `MULTICA_PRIME_AGENT_ISOLATED=1` を使用します。root または v0.7.2 以外は拒否され、v0.7.2 も upstream が coding tools を維持したまま schedules/heartbeats を強制停止できないため admission-disabled です。
