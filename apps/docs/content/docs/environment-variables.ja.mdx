...
| `log_level` | `debug` | デーモンログのしきい値：`debug`、`info`、`warn`、`error`。設定時はこのプロファイルの `LOG_LEVEL` より優先 |
...
- `log_level` は `debug`、`info`、`warn`、`error` を受け付けます。空文字列でプロファイルの上書きを解除し、`LOG_LEVEL` または `debug` に戻ります。
