...
| `MULTICA_PLATFORM_CONTEXT` | `full` | Multica コンテキストモード。`full` は従来のワークフロー brief、`minimal` は実行環境の事実と中立な task Skill カタログのみを保持し、強制的な起動ルールを追加しません。`off` は brief とサーバー提供の組み込み Skills を削除します。`true`/`false` は `full`/`off` の別名です |
...
| `platform_context_mode` | 環境に従う | `full`、`minimal`、`off`。`minimal` はサーバー提供 Skills を表示したまま起動判断を provider に委ね、`off` は削除します。空文字列でローカル上書きを解除します |
