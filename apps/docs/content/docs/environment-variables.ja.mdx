...
| 変数 | デフォルト | プロダクションで必須？ |
|---|---|---|
| `DATABASE_URL` | Docker Compose では未設定。server binary は `postgres://multica:multica@localhost:5432/multica?sslmode=disable` にフォールバックします | 同梱 Compose データベースの外部では **はい** |
