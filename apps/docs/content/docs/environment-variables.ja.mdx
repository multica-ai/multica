...
| `MULTICA_DAEMON_WS_CLAIM_POLL_INTERVAL` | `3m` | 正常な WebSocket での claim 安全ポーリング上限。下方向のジッターにより既定値は実際には `2m30s`–`2m45s` となり、旧 Server または結果が不確実な claim では通常のポーリング間隔を維持します |
...
| `ws_claim_poll_interval` | `3m` | 正常な WebSocket での claim 安全ポーリング上限。Daemon は下方向のジッターを適用します |
