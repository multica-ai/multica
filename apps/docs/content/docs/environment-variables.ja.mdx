...
上記の 4 つのデーモン用キー — `device_name`、`runtime_name`、`max_concurrent_tasks`、`poll_interval` — は、プロファイルの `~/.multica/config.json`（もしくは `~/.multica/profiles/<name>/config.json`）に永続化することもできます。これにより、`daemon start` のたびにフラグを再指定したり環境変数を export したりする必要がなくなります。
...
`poll_interval` は正の Go 期間（`500ms`、`10s`、`1m30s` など）のみを受け付けます。`0s`、負値、パース不能な文字列は `config set` の時点で拒否されます。永続化した値をクリアして環境変数 / 組み込みデフォルトに戻すには `multica config set poll_interval ""` を使ってください。
