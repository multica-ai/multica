...
| `GITHUB_APP_ID` | 空 | webhook には任意。Settings → Repositories → GitHub から選択する機能には必須です。`GITHUB_APP_PRIVATE_KEY` と併せて App 認証 REST 呼び出しに使います |
| `GITHUB_APP_PRIVATE_KEY` | 空 | webhook には任意。リポジトリ参照には必須です。App の RSA 秘密鍵の完全な PEM ブロック（`-----BEGIN/END-----` 行を含み、改行を保持）で、短命の App JWT 発行に使います |
