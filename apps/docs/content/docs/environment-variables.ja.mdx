...
このレイヤーを使う機能は 3 つあり、いずれも設定した endpoint に非公開の内容を送信します。
...
- **Issue コメントのフォローアップ操作**（操作可能な最新のエージェントコメントの下に表示されるボタン） — Issue のタイトル、説明の先頭から最大 1500 文字、最新のエージェントコメントから最大 4000 文字を送信します。添付ファイルとそれ以前のコメントは含まれません。

API key と base URL の両方が空の場合、このレイヤーは無効になり、上流へのリクエストを一切行いません。上記の機能はいずれも何も送信しなくなります。このレイヤーから非公開の内容をデプロイ外へ出せないポリシーでは、これがサポートされた構成です。セッションはクライアントが最初のメッセージから生成したタイトルをそのまま使い、チャットと Issue コメントのフォローアップボタンは表示されず、他の機能への影響はありません。
