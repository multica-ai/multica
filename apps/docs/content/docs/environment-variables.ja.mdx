...
| 変数 | デフォルト | 説明 |
|---|---|---|
| `MULTICA_CODEX_CAPACITY_RETRY_COUNT` | `6` | タスクに保存された Codex の生エラーが `Selected model is at capacity. Please try a different model.` と完全に一致する場合の、追加の即時リトライ回数。デフォルトでは合計 7 回まで実行します。`0` にするとこの専用ポリシーは無効になります。その他の容量不足やレート制限のエラーは、このポリシーではリトライされません。 |
