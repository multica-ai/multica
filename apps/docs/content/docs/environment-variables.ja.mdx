...
| `MULTICA_TASK_CONFIG_PROVIDER_REF_PREFIXES` | 空（無効） | プロジェクト `task_config` リソースの control-plane provider reference プレフィックス許可リスト（カンマ区切り）。バインディングを管理できるのは workspace owner/admin のみで、空の場合は機能を無効にします |
