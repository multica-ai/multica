...
読み取り専用のディレクトリログインです。`LDAP_ENABLED=true` のときだけログインページにアカウント・パスワードのタブが出ます。未設定なら機能は表示されません。

| 変数 | デフォルト | 説明 |
|---|---|---|
| `LDAP_ENABLED` | `false` | 唯一のスイッチ。`false` の間は下の変数はいずれも効果を持ちません |
| `LDAP_URL` | 空 | `ldap://`、`ldaps://`、`ldapi://` の接続先。例 `ldaps://openldap.corp.example.com:636` |
| `LDAP_BASE_DN` | 空 | ユーザー検索を開始するサブツリー。例 `ou=people,dc=corp,dc=example,dc=com` |
| `LDAP_BIND_DN` | 空 | 本人の Bind の前にエントリーを検索するサービスアカウント。読み取り・検索権限だけで十分です |
| `LDAP_BIND_PASSWORD` | 空 | 上記サービスアカウントのパスワード。ログには記録されません |
| `LDAP_USER_FILTER` | `(uid=%s)` | `%s` は 1 つだけ。RFC 4515 でエスケープしたユーザー名が代入されます。AD は `(sAMAccountName=%s)` |
| `LDAP_EMAIL_ATTR` | `mail` | ログイン用メールアドレスを持つ属性。Multica はこれでアカウントを照合するため、属性のないエントリーはログインできません |
| `LDAP_NAME_ATTR` | `displayName` | 初回ログイン時に表示名をプレフィルする属性 |
| `LDAP_TLS_INSECURE` | `false` | 証明書検証をスキップします。`ldaps://`/`ldapi://` のみが対象で、信頼できる内部ネットワークで一時的に使う場合に限ってください |
| `LDAP_TIMEOUT` | `5s` | 接続 + 検索 + Bind 全体に与える時間。Go の duration 形式（`5s`、`1m` など） |

ディレクトリログインは `ALLOW_SIGNUP`、`ALLOWED_EMAILS`、`ALLOWED_EMAIL_DOMAINS` を参照しません。Bind が成功すればそれでアカウントを許可します。詳細は[ログインとサインアップ](/auth-setup#企業ディレクトリldapログイン)を参照してください。
...
| `RATE_LIMIT_AUTH` | `5` | 認証コード送信・Google ログイン開始・ディレクトリアカウントログインの、IP あたり毎分の回数 |
