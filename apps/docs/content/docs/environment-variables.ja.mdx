...
| `MULTICA_DATABASE_STARTUP_TIMEOUT` | `3m` | migrator または API の起動時に発生する一時的なデータベース障害に対する合計再試行時間。`0` にすると 1 回だけ試行します |
| `MULTICA_DATABASE_CONNECT_TIMEOUT` | `5s` | 起動時のデータベース接続試行ごとのタイムアウト |
