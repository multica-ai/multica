...
description: How Multica connects agents to chat platforms — one channel engine, per-platform adapters for Lark (飞书), Slack, and DingTalk (钉钉) — covering the inbound pipeline, sessions, and authorization.
---

import { Callout } from "fumadocs-ui/components/callout";
import { Mermaid } from "@/components/mermaid";

A **channel** connects a Multica [agent](/agents) to a chat platform so your team can work with it where they already talk. Today there are three channels — [Lark (飞书)](/lark-bot-integration), [Slack](/slack-bot-integration), and [DingTalk (钉钉)](/dingtalk-bot-integration) — and all run on the **same engine**: a platform-neutral core plus a thin per-platform adapter. Adding a platform is "implement the adapter," not "rebuild the pipeline."
...
Every inbound message — Lark, Slack, or DingTalk — runs through the same ordered steps in the engine `Router`. A platform adapter only supplies the per-platform pieces (the `ResolverSet`); the policy lives in the engine.

1. **Route to installation** — map the event to a `channel_installation` (→ workspace + agent). Lark routes by `app_id`; Slack routes by the app id carried on the event; DingTalk routes by its AppKey.
...
| **DingTalk / 钉钉** | the conversation id | One session per 1:1 chat or group — DingTalk has no threads, so consecutive turns in the same conversation accumulate into one transcript (multi-turn memory). |

<Callout type="info">
In a group, only messages that **@-mention the bot** are ingested. No channel reads the group's other (un-@'d) messages or scrollback today, so the agent won't see messages it wasn't addressed in. Fetching surrounding history as context is a planned enhancement.
</Callout>

## Authorization

Two independent gates protect a bot in a shared group — both enforced in the engine for every message, identically across channels:
...
<Callout type="info">
**DingTalk (钉钉) — bring your own app.** A workspace admin creates a DingTalk app, adds a Stream-mode robot, and pastes its AppKey + AppSecret into Multica. Each agent gets its own DingTalk app, so several agents can each have a distinct bot in one organization. See [DingTalk Bot integration](/dingtalk-bot-integration) for the step-by-step setup.
</Callout>
...
On Multica Cloud all are already configured. See [Environment variables](/environment-variables) for the full reference.
...
- [DingTalk Bot integration](/dingtalk-bot-integration) — bring-your-own-app setup (Stream-mode robot + AppKey/AppSecret), per-agent bots
