...
An experimental, backend-only [Qianwen Skill bridge](/qianwen-skill-integration) is also available for private Quark S1 Skills. It uses asynchronous submit and status tools instead of a long-lived Bot connection, and does not yet support proactive device push.
...
- [Qianwen Skill bridge (experimental)](/qianwen-skill-integration) — submit private Quark S1 requests and poll their results.
