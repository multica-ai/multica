...
An experimental [Qianwen Skill bridge](/qianwen-skill-integration) is also available for private Quark S1 Skills. Its Shared Web/Desktop Settings card creates a Multica installation and manages caller-relative identity pairing; the provider-side Skill and device setup remains separate. Signed Tools submit work, poll one result, or list the bound user's visible non-terminal tasks. The bridge is pull-only and still requires a real Qianwen Tool and S1 smoke test. Industry message push is a separate enterprise business-onboarding route, not a prerequisite for this polling integration, and retail S1 private-Skill eligibility is unconfirmed.
...
- [Qianwen Skill bridge (experimental)](/qianwen-skill-integration)
...
- [Qianwen Skill bridge (experimental)](/qianwen-skill-integration) — pair a private Quark S1 Skill, submit signed requests, poll results, and list current tasks; no proactive device push.
