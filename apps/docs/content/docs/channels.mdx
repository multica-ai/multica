...
- Personal Weixin isolates the scanner's direct chat. The initial release accepts only direct messages from the account that scanned the QR code.
...
MULTICA_WEIXIN_SECRET_KEY=<base64-encoded 32-byte key>
```
...
Personal Weixin uses Tencent's iLink Bot API. A workspace admin selects **Connect personal Weixin** on the Agent's Integrations tab and confirms the QR code in Weixin. The returned Bot Token is encrypted at rest. The initial release supports text-only direct messages from the scanning account; group and media messages are ignored.

Personal Weixin currently also requires a single backend replica. The long-poll holder keeps the conversation context token needed for replies in process, so a completion published on another replica cannot deliver the response.
