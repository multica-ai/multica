...
description: Connect Multica agents to Feishu, Lark, WeCom, Slack, or DingTalk and use them from the chat tools your team already uses.
---
...
Feishu/Lark, WeCom, Slack, and DingTalk are supported today. They share the same session, identity, and execution mechanics but install differently.

## Choose a platform

| | Feishu / Lark | WeCom | Slack | DingTalk |
|---|---|---|---|---|
| Installation | Generate a QR code in Multica and authorize by scanning it with Feishu | Scan a QR code in Multica with WeCom (each scan creates a new smart bot), or enter an existing bot's Bot ID and Secret by hand | Create an app in Slack, then paste two tokens into Multica | Create an internal app and Stream-mode robot, then paste its AppKey and AppSecret into Multica |
...
| Create issues | `/issue` message command; creates the issue from your input as-is | `/issue` message command; creates the issue from your input as-is | `/issue` slash command; the agent writes up the description before creating | `/issue` message command; creates the issue from your input as-is |
| Connection | Platform long connection | Platform long connection | Socket Mode | Stream mode |
...
- [WeCom Bot integration](/wecom-bot-integration)
...
- Feishu/Lark and WeCom separate sessions by chat; later messages in the same chat continue the same session.
...
Workspace owners and admins can connect or disconnect Bots; for Feishu/Lark and WeCom Bots, connecting and disconnecting is also open to the agent's owner. Regular members can view connected integrations and use the agents they have access to.

After disconnecting, the Bot stops receiving new messages. Existing Multica conversations and run records are kept. For WeCom, disconnecting only unlinks the bot from Multica — it does not delete the smart bot in WeCom itself; remove it from the WeCom admin console if you want it gone entirely.
...
- [WeCom Bot integration](/wecom-bot-integration) — connect an agent by scanning a QR code.
