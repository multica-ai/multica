...
- **Codex operational context mode.** This mode omits repository instructions, ambient skills, previous sessions, managed MCP configuration, and custom Codex arguments from the model's loaded context. It does not hide files, environment variables, AWS or Sentry credentials, shell commands, or network access from the child process. Use it to make a focused agent cheaper and more predictable, never as a permission boundary.
