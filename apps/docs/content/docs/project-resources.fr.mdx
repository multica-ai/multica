...
- **Les modifications de chaque exécution s'affichent sous sa réponse.** À la fin d'une exécution, le diff entre son point de départ et la pointe de branche qu'elle a livrée est envoyé et apparaît sous la réponse de l'exécution comme une carte **Modifications de cette exécution**. La carte ouvre un visualiseur de diff qui peut basculer sur toute la tâche : les modifications de la pull request liée, ou la branche par rapport au point de départ du travail. Au-delà de 1 Mo, seule la liste des fichiers est conservée.
