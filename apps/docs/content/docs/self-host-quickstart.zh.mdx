...
最后一条命令输出 `daemon_server_url`——daemon 连接 API 用的地址（设了 `MULTICA_PUBLIC_URL` 时用它，否则用 `MULTICA_APP_URL`；输出为空说明两者都没设）。如果输出的是 `localhost`，说明 `.env` 里还是本地默认值：不要依赖从 `.env.example` 复制来的 `${FRONTEND_PORT}` 引用，把 `FRONTEND_ORIGIN` 和 `MULTICA_APP_URL` 显式设置为公网地址，然后重建容器。
