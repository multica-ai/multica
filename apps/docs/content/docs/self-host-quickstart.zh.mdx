...
Docker Compose 部署有两种升级写法，两者**完全等价**——`make selfhost` 就是下面这两条命令的封装（`Makefile:79-107`），用哪个都行：
...
`make selfhost` 只在 `.env` **不存在**的时候才生成它（`Makefile:80-95`）。在已有部署上重复跑，你的 `JWT_SECRET`、Postgres 密码、邮件配置、`FRONTEND_ORIGIN` 都原样保留。
...
Helm 有自己的升级路径——在 values 文件里钉 `images.backend.tag` / `images.frontend.tag` 然后 `helm upgrade`，或者用 `kubectl -n multica rollout restart` 重新拉一个浮动 tag。见下面的 [Kubernetes 部署（替代方案）](#kubernetes-部署替代方案)。
