...
curl -fsS https://app.example.com/api/config | jq .daemon_server_url
```

最后一条命令应输出你的公网 HTTPS 地址。如果输出的是 `localhost`，说明 `.env` 里还是本地默认值：`FRONTEND_ORIGIN` 和 `MULTICA_APP_URL` 在 2.24 之前的 Docker Compose 上不会自动跟随 `FRONTEND_PORT`（从 `.env.example` 复制来的 `${FRONTEND_PORT}` 引用会保持字面量），需要显式设置。
...
这条命令会先检查 `<server-url>/health`，再打开浏览器完成登录。登录完成后，它会保存本机凭据并启动守护进程。这一步报 `Server not reachable` 几乎总是因为 `/health` 经代理返回了 404——见[问题排查](/troubleshooting#multica-setup-报告服务器不可达)。
