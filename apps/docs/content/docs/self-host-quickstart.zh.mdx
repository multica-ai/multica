...
`/health` 只证明 backend 进程在监听。`/readyz` 和 `/healthz` 还会检查 PostgreSQL 和 migration 状态，所以 Compose healthcheck、生产 readiness probe、以及需要在数据库或 migration 异常时失败的启动诊断都应该使用它们。

后端容器启动时会**自动跑数据库 migration**（`docker/entrypoint.sh` 在启动 server 前执行 `./migrate up`）——你会在 backend 日志里看到 migration 输出。单机 Docker Compose 的版本升级也按同样方式处理。对于 Kubernetes 或多副本 release，请在滚动 backend pod 之前通过 Job 或受控 preflight 只运行一次 migration；readiness 保持使用 `/readyz` 或 `/healthz`，migration 失败时停止 rollout。
