...
multica usage export \
  --from 2026-09-01 \
  --to 2026-09-08 \
  --timezone Asia/Shanghai \
  --group-by agent,provider,model,day
```
...
`usage export` returns authoritative workspace token usage as JSON. Date-only
bounds are local midnights in the requested IANA timezone, while RFC3339 bounds
remain exact instants. The interval is half-open: records at `--from` are
included and records at `--to` are excluded. Use repeatable `--agent` filters
with an agent ID or exact name; `--runtime` and `--project` accept UUIDs. The
allowed grouping dimensions are `agent`, `provider`, `model`, and `day`. The CLI
retrieves all cursor pages before printing, so the JSON document is not
silently truncated.
...
| `daemon`, `runtime`, `usage` | Start and stop the local daemon; view runtimes and export workspace usage |
