...
In WSL2 or another headless shell where the CLI cannot open a browser, `multica login --callback-host <host-or-ip>` prints a manual login URL. Open that URL in a browser that can reach the printed callback host; after email-code or Google sign-in, the browser redirects back to the CLI listener so the token can be saved locally.
