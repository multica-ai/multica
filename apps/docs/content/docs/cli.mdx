...
Set a profile-specific daemon log threshold with `multica config set log_level info`. It accepts `debug`, `info`, `warn`, and `error`; the profile value takes precedence over `LOG_LEVEL`.
