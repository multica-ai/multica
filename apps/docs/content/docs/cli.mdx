...
| | `resource list/add/update/remove` | Manage project resources | `--type`, `--url`, `--local-path`, `--daemon-id`, `--agent-id` (optional exact agent binding), `--execution-mode` (`in_place` / `worktree` for a local directory) |
