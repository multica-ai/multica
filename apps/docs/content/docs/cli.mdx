...
| `autopilot` | `list/get/create/update/delete` | Manage automations | `create`: `--title`, `--mode` (required), one of `--agent` or `--squad`, `--priority`, `--project`, `--subscriber` (repeatable); `update`: `--agent` or `--squad`; the assignee flags are mutually exclusive |
