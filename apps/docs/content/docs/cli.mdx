...
| | `mcp list/add/update/remove/probe` | Manage the workspace's MCP server library | Writes are owner/admin. A server added here is assigned to no agent — give it to one with `multica agent mcp add`. `list` shows names, transports, and the last daemon probe. `probe` runs initialize + tools/list on a connected daemon (`--runtime-id` when several are online). The stored entry is write-only and never returned. `--server-config-file` / `--server-config-stdin` keep tokens out of shell history |
