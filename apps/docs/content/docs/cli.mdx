...
`multica skill export` saves a portable `.tar.gz` archive. Without `--output`, it uses the filename returned by the server; the archive can be imported with `multica skill import --file <path>`.
