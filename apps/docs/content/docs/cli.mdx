...
| `autopilot` | `trigger-add/trigger-update/trigger-list` | Configure and inspect triggers | `--provider github`, `--signing-secret-file` or `--signing-secret-stdin`, `--enabled=false`; update also supports `--clear-signing-secret`. See the [signed admission contract](/autopilots#github-signed-admission-api-and-cli-contract). |
