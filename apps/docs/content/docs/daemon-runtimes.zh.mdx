...
| 命令 | 作用 |
|---|---|
| `cs-workflow daemon start` | 启动（默认后台，加 `--foreground` 前台运行）|
| `cs-workflow daemon stop` | 停止 |
| `cs-workflow daemon restart` | 重启 |
| `cs-workflow daemon status` | 查看状态 |
| `cs-workflow daemon logs` | 查看日志（加 `-f` 跟随）|

完整 CLI 参考见 [CLI 命令速查](/cli)。

**桌面应用自带守护进程。**用 [桌面应用](/desktop-app) 就不必手动 `cs-workflow daemon start`——它启动时会自动拉起守护进程。哪种方式更适合你的工作流，详见 [桌面应用](/desktop-app) 页面。
...
1. 跑 `cs-workflow daemon status`，确认守护进程在运行且在线
2. 跑 `cs-workflow daemon logs -f`，看是否有错误
