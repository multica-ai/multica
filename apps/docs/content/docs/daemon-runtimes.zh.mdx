...
修改 profile 的命令或参数后，已开始的任务仍使用启动时的参数；守护进程重新注册后，新领取的任务才会使用新配置。自定义运行时参数和注册错误展示要求 server 与 daemon 都支持 `fixed_args` launch spec 和 `failed_profiles` 注册报告；混合版本部署时，先升级 server，再逐步升级 daemon。
