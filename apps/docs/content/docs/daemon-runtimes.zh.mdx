...
这个结构带来 Multica 和 Linear / Jira 最大的差别：**AI 工具本身、它们各自的凭据、你的代码目录都留在本地**，Multica 服务器都看不到。（唯一的例外是你刻意存进某个智能体 [自定义环境变量](/agents-create#自定义环境变量custom_env) 里的 secret，它们存在服务器侧。）"我的智能体不工作"类问题几乎都是本地问题——守护进程没启动、某款 AI 工具没装、密钥过期——请先从本地查起；定位指引见 [故障排查](/troubleshooting)。
