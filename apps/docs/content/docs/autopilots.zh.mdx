...
- **先建 issue 模式**（`create_issue`）—— 默认，**推荐**。每次触发先在工作区里建一个 issue（标题支持两个占位符：`{{date}}` 会插值成 UTC 日期 `YYYY-MM-DD`，`{{trigger_time}}` 会插值成本次触发的 UTC 时刻 `YYYY-MM-DD HH:MM UTC`；其他 `{{...}}` 形式的占位符会在创建时被拒绝，避免拼错以后悄无声息地把原文当成 issue 标题），再按分配流程把 issue 派给智能体。所有工作都落在 issue 看板上，历史、评论、状态和手动分配的 issue 完全一致。
