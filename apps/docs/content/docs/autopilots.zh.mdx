---
title: Autopilots
description: 让数字员工按 cron 定时自己开工，或在 webhook 到来时被触发——也可以通过 UI / CLI 手动触发一次。
---
...
- **先建 issue 模式**（`create_issue`）—— 默认，**推荐**。每次触发先在工作区里建一个 issue（标题目前只支持一个占位符 `{{date}}`，会插值成 UTC 日期 `YYYY-MM-DD`；其他 `{{...}}` 形式的占位符会在创建时被拒绝，避免拼错以后悄无声息地把原文当成 issue 标题），再按分配流程把 issue 派给数字员工。所有工作都落在 issue 看板上，历史、评论、状态和手动分配的 issue 完全一致。
...
如果 Autopilot 很重要，要自己设计监控——例如让数字员工在成功时给自己发个评论，通过缺失评论来发现失败。
</Callout>
...
- [**分配 issue 给数字员工**](/assigning-issues) —— 一次性把 issue 指派给数字员工
- [**在评论里 @ 智能体**](/mentioning-agents) —— 评论里让数字员工看一眼
