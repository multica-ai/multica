...
profile의 명령이나 인수 변경은 데몬이 다시 등록된 뒤 새로 가져오는 작업부터 적용됩니다. 이미 실행 중인 작업은 시작할 때의 인수를 유지합니다. 사용자 지정 런타임 인수와 등록 오류 표시는 server와 daemon이 모두 `fixed_args` launch spec 및 `failed_profiles` 등록 보고를 이해하는 버전이어야 합니다. 혼합 배포에서는 server를 먼저 업그레이드한 뒤 daemon을 순차적으로 업데이트하세요.
