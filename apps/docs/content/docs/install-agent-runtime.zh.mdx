...
1. **Multica 守护进程在运行。** 装完 [Multica CLI](/zh/cli) 后跑 `cs-workflow daemon start`；或者用 [Multica 桌面端](/zh/desktop-app)，它启动时自动拉起守护进程。守护进程没起来，就没人去扫工具。
...
2. **重启守护进程。** `cs-workflow daemon restart`，或者重启桌面端。守护进程只在启动时扫一次 `PATH`。
...
- **Runtimes 页面看到行，但任务一跑就失败。** 一边触发任务一边跑 `cs-workflow daemon logs -f`。守护进程会把工具自己的报错原样吐出来。
