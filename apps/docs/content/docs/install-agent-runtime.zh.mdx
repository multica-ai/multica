...
集成最完整的一款。会话续接好用，MCP 好用，并支持最丰富的守护进程控制参数（见[矩阵](/zh/providers#mcp-配置支持)）。
...
Warp 的 headless CLI，用于运行连接云端的智能体。

| | |
|---|---|
| 守护进程扫描 | `oz` |
| 安装 | 按 Warp 的 Oz CLI 文档 [docs.warp.dev/reference/cli](https://docs.warp.dev/reference/cli/) 安装。macOS 上作为守护进程使用时，推荐独立 Homebrew cask：`brew tap warpdotdev/warp && brew install --cask oz`。 |
| 认证 | 运行 `oz login`，或为自动化配置 `WARP_API_KEY`。 |
| 备注 | 独立包可以避免后台守护进程任务拉起 Warp 桌面 app。 |
