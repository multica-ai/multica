...
开源 CLI 编程 harness（[forgecode.dev](https://forgecode.dev/))。守护进程以 one-shot 模式启动：`forge -C <workdir> --cid <conversation-id> -p <prompt>`。会话续接通过复用钉住的 conversation id 实现。ForgeCode 没有 `--model` flag——模型通过 `FORGE_SESSION__PROVIDER_ID` / `FORGE_SESSION__MODEL_ID` 环境变量选择（设 `MULTICA_FORGE_MODEL=provider/model`，如 `anthropic/claude-sonnet-4-20250514`）。因为 ForgeCode 的 one-shot 模式渲染的是 Markdown 而非结构化事件流，守护进程在运行结束后从 `forge conversation dump <id>` 重放结构化消息列表。skill 复制到 `.forge/skills/`。

| | |
|---|---|
| 守护进程扫描 | `forge` |
| 安装 | 看官方文档 [forgecode.dev](https://forgecode.dev/)。 |
| 认证 | 按厂商文档（为配置的 provider 准备 API key）。 |
