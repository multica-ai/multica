...
通过 Google Antigravity CLI 使用 AGY 当前模型目录，包括 Gemini 3.5 Flash、Gemini 3.1 Pro、Claude Sonnet/Opus 4.6 Thinking 和 GPT-OSS 120B。没有会话续接，没有 MCP —— 适合一次性、无需上下文记忆的任务。

| | |
|---|---|
| 守护进程扫描 | `agy` |
| 安装 | 看官方 Antigravity CLI 安装说明：[antigravity.google/download](https://antigravity.google/download)。 |
| 认证 | 使用守护进程前，先在 Antigravity 中登录，或交互式运行 `agy` 完成登录。AGY 支持 API key 时，`GEMINI_API_KEY` 仍可使用。 |
| 模型选择 | 在 Multica 中选择模型会传给 `agy -m <model>`；留空则让 AGY 使用自身默认值、`GEMINI_MODEL` 或 `settings.json` 里的 `model.name`。 |
| 设置 | AGY CLI 设置保存在 `~/.gemini/antigravity-cli/settings.json`；在 TUI 中用 `/config`、`/settings` 或 `/permissions` 调整安全和权限。 |
| Headless 模式 | Multica 使用 `--dangerously-skip-permissions` 启动 AGY，避免守护进程任务卡在交互式审批上。 |
