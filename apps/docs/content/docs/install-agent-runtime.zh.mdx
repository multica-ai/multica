...
偏向 DeepSeek 原生工作流的 ACP agent。Multica 通过在 `PATH` 里找到 `npx` 来识别它，然后拉起 `npx reasonix acp`（或者你在 `MULTICA_REASONIX_PATH` 里指定的 wrapper 命令）。**它不支持会话续接**。

| | |
|---|---|
| 守护进程扫描 | `npx` |
| 安装 | 安装 Node.js 22+，然后先跑一次 `npx reasonix --help`，或参考上游仓库 [github.com/esengine/DeepSeek-Reasonix](https://github.com/esengine/DeepSeek-Reasonix)。如果你想固定 wrapper，也可以把 `MULTICA_REASONIX_PATH` 设成 `npx dsnix@latest` 之类的命令。 |
| 认证 | 跑 `reasonix setup`，或在 shell 里设置 `DEEPSEEK_API_KEY`。 |
| 备注 | 仅支持 DeepSeek。Skills 会从 `.reasonix/skills/` 原生发现。 |
...
1. **确认可执行文件在 `PATH` 上。** 开一个新终端，跑 `which <名字>`（比如 `which claude`、`which cursor-agent`、`which kiro-cli`，Reasonix 则看 `which npx`）。打印出路径，守护进程就找得到；什么都不打印，先修 shell 的 `PATH`（最常见原因是 rc 文件没重新加载）。
