      ...
      <ScrollView contentContainerClassName="px-6 py-6 gap-6">
        <View className="gap-1">
          <Text className="text-xs uppercase tracking-wider text-muted-foreground">
            {t("select_workspace.signed_in_as")}
          </Text>
          ...
          {isLoading ? (
            ...
            <View className="gap-3">
              <Text className="text-sm text-destructive">
                {t("select_workspace.load_error_prefix")}{" "}
                {error instanceof Error
                  ? error.message
                  : t("select_workspace.unknown_error")}
              </Text>
              <Button variant="outline" onPress={() => refetch()}>
                <Text>{t("select_workspace.retry")}</Text>
              </Button>
            </View>
          ) : !data || data.length === 0 ? (
            <Text className="text-sm text-muted-foreground">
              {t("select_workspace.empty_state")}
            </Text>
          ) : (
            <View className="gap-3">
              {data.map((ws) => (
