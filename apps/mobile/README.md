...
发布前仍然需要确认 `app.json` 中的 `version` 是否符合当前 release 预期。当前 `runtimeVersion.policy` 是 `appVersion`，所以 app version 也会影响 OTA 更新的 runtime 分组。
...
`runtimeVersion.policy` 当前为 `appVersion`。这意味着 OTA 更新只会发送给相同 runtime 的安装包；当 `app.json` 的 `version` 改变后，新版本会进入新的 runtime 分组，旧安装包不会接收新 runtime 的 OTA 更新。
