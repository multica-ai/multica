...
vi.mock("../core/api", () => ({
  createApp: (input: unknown) => createApp(input),
  listAppFolders: () => listAppFolders(),
}));
...
describe("AppBuilderPage", () => {
  beforeEach(() => {
    ...
    listAppFolders.mockReset();
    listAppFolders.mockResolvedValue([
      { id: "collection-1", parent_id: null, name: "Operations" },
    ]);
  });
  ...
  it("creates an app and opens its detail page", async () => {
    ...
    await userEvent.selectOptions(screen.getByLabelText("Collection"), "collection-1");
