...
export function AppDetailPage({ appId, runtimeBaseUrl }: { appId: string; runtimeBaseUrl: string }) {
  ...
  useEffect(() => { if (enabled) getAppDetail(appId).then((value) => { setApp(value); if (value.workflows[0]) setWorkflow(value.workflows[0].definition); }).catch((cause) => setError(cause instanceof Error ? cause.message : "Could not load app")); }, [appId, enabled]);
  ...
  return <div className="flex h-full flex-col">
    ...
    <div className="grid flex-1 min-h-0 gap-4 overflow-y-auto p-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)] lg:p-6">
      <section className="min-h-[420px] overflow-hidden rounded-xl border bg-card">
        ...
        {version ? <AppViewFrame title={app.name} src={`${runtimeBaseUrl.replace(/\/$/, "")}/apps/${app.id}/${version}/`} /> : <div className="grid min-h-80 place-items-center text-sm text-muted-foreground">Publish the first version to open the app.</div>}
      </section>
      <section className="rounded-xl border bg-card p-4">
        <div className="mb-4 flex items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 font-semibold"><Workflow className="size-4" />Workflow</h2><p className="text-xs text-muted-foreground">Configure the same JSON contract that agents and the API use.</p></div><Button size="sm" onClick={async () => {
          ...
          try {
            ...
            if (!workflowId) workflowId = (await createWorkflow({ app_id: app.id, name: `${app.name} workflow`, version: "1.0.0", definition: workflow })).id;
            const run = await testWorkflow(workflowId, sample);
