...
export function createAppTemplate(name: string, version = "0.1.0"): AppSourceFile[] {
  ...
  return [
    ...
    { path: "frontend/index.html", media_type: "text/html; charset=utf-8", content: "<!doctype html><html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Mini App</title></head><body><main><h1>Mini App</h1><form><input id=\"value\" required><button>Run</button></form><pre id=\"result\"></pre></main><script type=\"module\" src=\"./app.js\"></script></body></html>" },
    { path: "frontend/app.js", media_type: "text/javascript; charset=utf-8", content: "document.querySelector('form').addEventListener('submit', async (event) => { event.preventDefault(); const result = await multica.workers.invoke({ value: document.querySelector('#value').value }); document.querySelector('#result').textContent = JSON.stringify(result, null, 2); });" },
