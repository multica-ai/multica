...
import type { ModelRegistryChangeRequest, MemberWithUser } from "@multica/core/types";
...
import { modelRegistryChangeRequestsOptions } from "../../core/queries";
import { useReviewModelRegistryChangeRequest } from "../../core/mutations";
...
const STATUS_LABEL: Record<ModelRegistryChangeRequest["status"], string> = {
  pending: "Pending",
  approved: "Approved",
  rejected: "Rejected",
  merged: "Merged",
...
const STATUS_CLASS: Record<ModelRegistryChangeRequest["status"], string> = {
...
export function ModelRegistryChangeRequestQueue({ members, canReview }: Props) {
  const [open, setOpen] = useState(true);
  const [reviewTarget, setReviewTarget] = useState<ModelRegistryChangeRequest | null>(null);
  const [reviewComment, setReviewComment] = useState("");
  const [reviewAction, setReviewAction] = useState<"approve" | "reject" | null>(null);

  const { data: requests = [], isLoading } = useQuery(modelRegistryChangeRequestsOptions());
  const pendingCount = requests.filter((r) => r.status === "pending").length;
  const mutation = useReviewModelRegistryChangeRequest();
  ...
  const handleReview = async () => {
    ...
    try {
      ...
      toast.success(reviewAction === "approve" ? "Change approved" : "Change rejected");
  ...
  const openReview = (req: ModelRegistryChangeRequest, action: "approve" | "reject") => {
    setReviewTarget(req);
    ...
    <div>
          ...
          {requests.map((req) => (
            <div key={req.id} className="rounded-md border bg-card">
              <div className="flex items-start gap-2 px-2.5 py-2">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    <span className="truncate text-xs font-medium">{req.title}</span>
                    <span
              ...
              <div className="border-t px-2.5 py-2">
                <pre className="max-h-48 overflow-y-auto whitespace-pre-wrap font-mono text-[11px] text-muted-foreground">
                  {req.diff || "No diff available."}
                </pre>
              </div>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {reviewAction === "approve" ? "Approve" : "Reject"} change: {reviewTarget?.title}
            </DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={mutation.isPending}
            >
              {mutation.isPending ? "Saving…" : reviewAction === "approve" ? "Approve" : "Reject"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
