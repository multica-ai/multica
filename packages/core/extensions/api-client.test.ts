...
describe("ApiClient platform Extensions", () => {
  it("uses the shared authenticated request path for list/detail/import", async () => {
    const fetchMock = vi
      .fn()
    ...
    await client.importPlatformExtension(
      bytes('{"schema_version":"platform.extension/v1"}'),
    );
    ...
    expect(new TextDecoder().decode(importInit.body as Uint8Array)).toBe(
  ...
  it("rejects documents above 5 MiB before performing a request", async () => {
  ...
  it("measures the limit in UTF-8 bytes and accepts the exact boundary", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify({ ...mapping, idempotent: false }), { status: 201 }),
    ...
    await expect(
      client.importPlatformExtension(
        bytes("你".repeat(Math.floor(PLATFORM_EXTENSION_MAX_IMPORT_BYTES / 3) + 1)),
      ),
    ).rejects.toBeInstanceOf(PlatformExtensionDocumentTooLargeError);
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it("rejects invalid UTF-8 bytes before performing a request", async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    ...
    ).rejects.toBeInstanceOf(PlatformExtensionDocumentEncodingError);
    expect(fetchMock).not.toHaveBeenCalled();
  });
});
