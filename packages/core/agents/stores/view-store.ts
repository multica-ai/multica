...
// stage plus the Fork-specific defaults view. Impure on paper, but the modes
// are mutually exclusive in practice and "mine" is the historical product
// default; archived/default ignore the ownership lens for their own datasets.
export type AgentsScope = "mine" | "all" | "default" | "archived";

export const AGENT_SCOPES: AgentsScope[] = ["mine", "all", "default", "archived"];
...
export const useAgentsViewStore = create<AgentsViewState>()(
  persist(
    (set) => ({
      ...
      setScope: (scope) =>
        set(
          scope === "mine" || scope === "default"
            ? { scope, filters: EMPTY_AGENT_FILTERS }
            : { scope },
        ),
      toggleSort: (field) =>
        set((state) =>
      ...
      toggleFilter: (key, value) =>
        set((state) => {
          ...
          const scope =
            state.scope === "mine" || state.scope === "default"
              ? "all"
              : state.scope;
