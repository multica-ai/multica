...
describe("RoundsBlock", () => {
  it("shows issue titles, schedule and starts the next round", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Start Daily ideas" }));
    expect(onStart).toHaveBeenCalledWith("round-1");
  });

  it("shows live run progress and the next-round action", () => {
    const onStart = vi.fn();
    render(<RoundsBlock statuses={[status({ active_run: { id: "run-1", round_id: "round-1", status: "ready", total_count: 4, responded_count: 3, stalled_count: 1, nudged_count: 0, started_at: "", ready_at: "", completed_at: null, created_at: "" } })]} issueTitles={{}} onStart={onStart} onSelectIssue={vi.fn()} />);
    expect(screen.getByText("3/4 ready")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Start next Daily ideas" }));
    expect(onStart).toHaveBeenCalledWith("round-1");
  });
...
describe("RoundManager", () => {
  it("creates, edits, removes members and deletes rounds", () => {
    ...
    expect(actions.onCreate).toHaveBeenCalledWith(expect.objectContaining({ name: "Weekly review" }));
