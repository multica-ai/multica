...
import { goalTypesOptions, periodsOptions, rocksOptions, settingsOptions, strategyOptions } from "../core/queries";
...
export function RocksPage() {
  ...
  const rocks = typeFilter ? allRocks.filter((rock) => rock.goal_type_id === typeFilter) : allRocks;
      ...
      <div className="mx-auto flex w-full max-w-7xl min-w-0 flex-col gap-5 p-4 sm:p-6">
        <header className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={() => setFormRock(null)} className="h-10 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground">+ New {terminology.rock}</button>
          </div>
        </header>
        ...
        <section className="overflow-hidden rounded-xl border bg-card">
          ...
          {list.isLoading ? <p className="p-8 text-sm text-muted-foreground">Loading {terminology.rocks}…</p> : list.isError ? <p role="alert" className="p-8 text-sm text-destructive">{terminology.rocks} could not be loaded.</p> : rocks.length === 0 ? <div className="p-10 text-center"><h2 className="font-semibold">No {terminology.rocks} yet</h2><p className="mt-1 text-sm text-muted-foreground">Create the first outcome now and connect execution later.</p></div> : rocks.map((rock) => (
            <button key={rock.id} type="button" aria-label={`View ${rock.title}`} onClick={() => setSelectedId(selectedId === rock.id ? undefined : rock.id)} className={`grid w-full gap-3 border-b px-4 py-4 text-left last:border-b-0 md:grid-cols-[minmax(16rem,2fr)_minmax(10rem,1fr)_minmax(10rem,1fr)_7rem_6rem] md:items-center md:gap-4 ${selectedId === rock.id ? "bg-primary/5" : "hover:bg-muted/40"}`}>
              <span className="min-w-0"><span className="flex min-w-0 items-center gap-2"><strong className="truncate text-sm">{rock.title}</strong>{rock.goal_type_name && <span className="shrink-0 rounded-full border px-2 py-0.5 text-xs" style={rock.goal_type_color ? { borderColor: rock.goal_type_color, color: rock.goal_type_color } : undefined} title={rock.goal_type_scope_label || undefined}>{rock.goal_type_name}</span>}</span><span className="mt-1 block truncate text-xs text-muted-foreground">{rock.strategy_item_title ? `↳ 1-Year Plan · ${rock.strategy_item_title}` : `No ${terminology.strategy} connection`}</span></span>
