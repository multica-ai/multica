...
describe("formatCountdown", () => {
  it("counts down minutes and seconds", () => {
    expect(formatCountdown(new Date(NOW + 90_000).toISOString(), NOW)).toBe("om 1m");
    expect(formatCountdown(new Date(NOW + 45_000).toISOString(), NOW)).toBe("om 45s");
  });
  it("counts down hours and days", () => {
...
describe("wakeupCounter", () => {
  it("uses countdown for time triggers and waiting for condition triggers", () => {
    expect(wakeupCounter(mk({ trigger_type: "time", fire_at: new Date(NOW + 60_000).toISOString() }), NOW)).toBe("om 1m");
