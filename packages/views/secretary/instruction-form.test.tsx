...
const { save } = vi.hoisted(() => ({
  save: vi.fn().mockResolvedValue({ sequence: 1, request_id: "test" }),
}));

vi.mock("@multica/core/hooks", () => ({ useWorkspaceId: () => "workspace" }));
vi.mock("@multica/core/api", () => ({
  api: { saveSecretaryInstruction: save },
}));

describe("secretary instruction editing", () => {
  it("retains the existing waiting date when only the note is edited", async () => {
    const item = SecretaryItemSchema.parse({
      key: "a",
      case_id: "c",
      title: "八月考勤",
      situation: "已查到月报",
      ...
      stage: "waiting",
      owner: "external",
      follow_up_on: "2026-09-08",
      source_updated_at: "2026-09-06T08:00:00+08:00",
    });
    renderWithI18n(
    ...
    fireEvent.change(screen.getByLabelText("等谁回复、需要什么结果"), {
      target: { value: "等负责方确认适用异常" },
    });
