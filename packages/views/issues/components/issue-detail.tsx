...
import type { IssueStatus, IssuePriority, TimelineEntry, IssueSubscriber } from "@multica/core/types";
...
export function IssueDetail({ issueId, onDelete, onDone, defaultSidebarOpen = true, layoutId = "multica_issue_detail_layout", highlightCommentId, linkSelfInBreadcrumb = false }: IssueDetailProps) {
  ...
  // CEREBRO-PATCH(issue-detail-mobile-sheet-close): Auto-close the mobile sheet on route changes
  // (JEH-601: iOS swipe-back lock).
  useEffect(() => {
  ...
  // CEREBRO-PATCH(channels-flag-gate): Channel/DM-aware rendering. Computed before any early returns so the
  ...
  const detailContent = (
    <div className="flex h-full min-w-0 flex-1 flex-col">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-auto">
                  ...
                  <>
                    ...
                    <DropdownMenuSub>
                      ...
                      <DropdownMenuSubContent>
                        ...
                        {agents.filter((a) => !a.archived_at && canAssignAgent(a, user?.id, currentUserRole ?? undefined)).map((a) => (
                          <DropdownMenuItem
                            key={a.id}
                    ...
                    {/* Create sub-issue */}
                    <DropdownMenuItem onClick={actions.openCreateSubIssue}>
                      <Plus className="h-3.5 w-3.5" />
                      Create sub-issue
                    </DropdownMenuItem>

                    {/* Add as sub-issue of another issue */}
                    <DropdownMenuItem onClick={actions.openSetParent}>
                ...
                <DropdownMenuItem onClick={actions.togglePin}>
                  {actions.isPinned ? <PinOff className="h-3.5 w-3.5" /> : <Pin className="h-3.5 w-3.5" />}
        ...
        {/* CEREBRO-PATCH(channels-flag-gate): Channel-header bar — chat-only. Sits directly under the page
        ...
        {isChat && (
          ...
          >
            ...
            <BacklogAgentHintDialog
              ...
              onMoveToTodo={() => {
                handleUpdateField({ status: "todo" });
                setBacklogHintOpen(false);
              }}
            />

            {/* CEREBRO-PATCH(issue-detail-actions-pickers): parent/sub-issue pickers handled via actions.openSetParent (modal) */}
        ...
        <div className="mx-auto w-full max-w-4xl px-3 sm:px-6 md:px-8 py-4 sm:py-6 md:py-8">
          ...
          {/* CEREBRO-PATCH(channels-flag-gate): Description editor — task-only. Channels render the
