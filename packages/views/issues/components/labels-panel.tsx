...
export function LabelsPanel() {
  ...
  const saveEdit = (id: string) => {
    ...
    if (!name) {
      ...
      setEditError(t("labels_name_required"));
      return;
    }
    setEditError("");
    update.mutate(
      { id, name, color: editColor },
      {
        ...
        onError: (err: unknown) => {
          toast.error(err instanceof Error ? err.message : t("toast_failed_update_label"));
        },
      },
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <Input
          id="label-new-name"
          placeholder={t("labels_new_name_placeholder")}
          value={newName}
          ...
          className="flex-1"
          maxLength={32}
          aria-label={t("labels_aria_new_name")}
        />
      ...
      <div className="space-y-2 max-h-[50vh] overflow-y-auto">
        {isLoading && <p className="text-sm text-muted-foreground">{t("labels_loading")}</p>}
        {!isLoading && labels.length === 0 && (
          <p className="text-sm text-muted-foreground">{t("labels_no_labels")}</p>
        )}
        {labels.map((label) => {
                ...
                {isEditing ? (
                  <>
                    ...
                    <Button
                      ...
                      aria-label={t("labels_aria_save")}
                    >
                      <Check className="h-4 w-4" />
                    </Button>
                    <Button size="sm" variant="ghost" onClick={cancelEdit} aria-label={t("labels_aria_cancel")}>
                      <X className="h-4 w-4" />
                    </Button>
                  </>
                ) : (
                  <>
                    ...
                    <Button
                      ...
                      aria-label={t("labels_aria_edit", { name: label.name })}
                    >
                    ...
                    <Button
                      ...
                      aria-label={t("labels_aria_delete", { name: label.name })}
                    >
      ...
      <AlertDialog open={!!pendingDeletion} onOpenChange={(o) => !o && setPendingDeletion(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("labels_delete_title")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("labels_delete_desc", { name: pendingDeletion?.name ?? "" })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                ...
                del.mutate(pendingDeletion.id, {
                  ...
                  onError: (err: unknown) => {
                    toast.error(err instanceof Error ? err.message : t("toast_failed_delete_label"));
                  },
                });
              }}
            >
...
function ColorPalette({
  ...
  return (
    <div className={compact ? "flex items-center" : "flex items-center gap-3"}>
      {!compact && <UILabel className="text-xs text-muted-foreground">{t("labels_color")}</UILabel>}
      <label
        ...
        aria-label={t("labels_aria_pick_color")}
        title={value}
      >
        <input
          type="color"
