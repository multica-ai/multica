    ...
    <>
      ...
      <div className="mt-4 sticky top-4 z-10">
        <SingleAgentLiveCard
          ...
          issueId={issueId}
          agentName={firstEntry.task.agent_id ? getActorName("agent", firstEntry.task.agent_id) : tAgents("agent_fallback")}
        />
      </div>
        ...
        <div className="mt-1.5 space-y-1.5">
          {restEntries.map(({ task, items }) => (
            <SingleAgentLiveCard
              ...
              items={items}
              issueId={issueId}
              agentName={task.agent_id ? getActorName("agent", task.agent_id) : tAgents("agent_fallback")}
            />
          ))}
        </div>
      )}
    ...
    <div className="rounded-lg border border-info/20 bg-info/5 backdrop-blur-sm">
      <div className="flex items-center gap-2 px-3 py-2 text-muted-foreground">
        ...
        <div className="flex items-center gap-1.5 text-xs min-w-0">
          ...
          <span className="font-medium text-foreground truncate">{t("agent_working", { agent: agentName })}</span>
          <span className="text-muted-foreground tabular-nums shrink-0">{elapsed}</span>
          {toolCount > 0 && (
            <span className="text-muted-foreground shrink-0">{t("agent_tools", { count: toolCount })}</span>
          )}
        </div>
