...
// CEREBRO-PATCH(editor-direct-path-mocks): cerebro's b787595c switched
// `issue-detail.tsx`, `comment-card.tsx`, `comment-input.tsx`, and friends
// from the `../../editor` barrel to direct sub-module imports for preview
// compile speed. The barrel mock above only intercepts the barrel — these
// re-export it through the direct paths so the test mocks still apply.
vi.mock("../../editor/title-editor", async () => {
  const barrel = await import("../../editor");
  return { TitleEditor: (barrel as { TitleEditor: unknown }).TitleEditor };
});
