  ...
  return (
    <Card className={cn("!py-0 !gap-0 overflow-clip transition-colors duration-700", isHighlighted && "ring-2 ring-brand/50 bg-brand/5")}>
      <Collapsible open={open} onOpenChange={handleOpenChange}>
        {/* Sticky header — stays at viewport top when scrolling through long threads */}
        <div className="sticky top-0 z-20 bg-card border-b border-border/50">
          {onCollapseResolved && (
            <button
              type="button"
              onClick={onCollapseResolved}
              className="flex w-full items-center justify-between px-4 py-2.5 text-left text-sm text-muted-foreground hover:bg-muted/50 transition-colors"
