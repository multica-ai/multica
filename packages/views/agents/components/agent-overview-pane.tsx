...
} from "@multica/cerebro-agent-context/views"; // CEREBRO-PATCH(agent-office-tab): FIR-1775 collapsible tab-sections + Instructions = versioned Agent Office surface
...
export function AgentOverviewPane({
  ...
  // CEREBRO-PATCH(agent-office-tab): FIR-1775 — resolve tab labels here (i18n
  // lives in the pane) and hand the flat visible-tab list to the collapsible
  // section shell, which groups them into Activity / Setup / Advanced.
  const sectionTabItems: AgentTabSectionItem[] = visibleTabs.map((tab) => ({
    id: tab.id,
    icon: tab.icon,
