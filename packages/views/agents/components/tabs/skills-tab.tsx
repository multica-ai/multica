    ...
    <div className="space-y-4">
      ...
      {agent.skills.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12">
          ...
          <p className="mt-3 text-sm text-muted-foreground">{t.skills.emptyTitle}</p>
      ...
      {showPicker && (
        <Dialog open onOpenChange={(v) => { if (!v) setShowPicker(false); }}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle className="text-sm">{t.skills.pickerTitle}</DialogTitle>
              <DialogDescription className="text-xs">
                {t.skills.pickerDescription}
              </DialogDescription>
            </DialogHeader>
