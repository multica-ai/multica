...
const RUN_STATUS_CONFIG: Record<string, { labelKey: string; color: string; icon: typeof CheckCircle2; spin?: boolean }> = {
  issue_created: { labelKey: "run_status_issue_created", color: "text-blue-500", icon: Clock },
  running: { labelKey: "run_status_running", color: "text-blue-500", icon: Loader2, spin: true },
  completed: { labelKey: "run_status_completed", color: "text-emerald-500", icon: CheckCircle2 },
  failed: { labelKey: "run_status_failed", color: "text-destructive", icon: XCircle },
};
  ...
  const content = (
    <>
      ...
      <span className="w-32 shrink-0 text-right text-xs text-muted-foreground tabular-nums">
        {formatDate(run.triggered_at || run.created_at, locale)}
      </span>
...
function TriggerRow({ trigger, autopilotId }: { trigger: AutopilotTrigger; autopilotId: string }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(t("toast_trigger_deleted"));
      setConfirmOpen(false);
    } catch {
      toast.error(t("toast_failed_delete_trigger"));
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="flex items-center gap-3 rounded-md border px-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {!trigger.enabled && (
            <span className="text-xs bg-muted px-1.5 py-0.5 rounded">{t("detail_disabled")}</span>
          )}
        </div>
        ...
        {trigger.next_run_at && (
          <div className="text-xs text-muted-foreground">
            {t("detail_next_prefix")}{formatDate(trigger.next_run_at, locale)}
          </div>
        )}
      </div>
      <Button
      ...
      <AlertDialog open={confirmOpen} onOpenChange={(v) => { if (!v && !deleting) setConfirmOpen(false); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("detail_delete_trigger")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("detail_delete_trigger_desc")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function AddTriggerDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setLabel("");
      toast.success(t("toast_trigger_added"));
    } catch {
      toast.error(t("toast_failed_add_trigger"));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm">
        <DialogTitle>{t("detail_add_trigger")}</DialogTitle>
        <div className="space-y-4 pt-2">
          <TriggerConfigSection config={config} onChange={setConfig} />
          <div>
            <label className="text-xs font-medium text-muted-foreground">{t("detail_trigger_label")}</label>
            <input
              type="text"
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              placeholder={t("detail_trigger_placeholder")}
          ...
          <div className="flex justify-end pt-1">
            <Button size="sm" onClick={handleSubmit} disabled={submitting}>
              {submitting ? t("adding") : t("detail_add_trigger")}
            </Button>
          </div>
        </div>
      </DialogContent>
...
export function AutopilotDetailPage({ autopilotId }: { autopilotId: string }) {
  ...
  const handleRunNow = async () => {
    try {
      ...
      toast.error(e?.message || t("toast_failed_trigger"));
    }
  };
    ...
    <div className="flex h-full flex-col">
      ...
      <PageHeader className="justify-between px-5">
        <div className="flex items-center gap-2">
          ...
          <div className="ml-1 flex items-center gap-1.5">
            <Switch
              ...
              aria-label={autopilot.status === "active" ? t("aria_pause") : t("aria_activate")}
            />
            <span className={cn(
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={handleRunNow} disabled={autopilot.status !== "active" || triggerAutopilot.isPending}>
            ...
            {triggerAutopilot.isPending ? t("running") : t("run_now")}
          </Button>
        </div>
      </PageHeader>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto p-6 space-y-8">
          {/* Properties */}
          <section className="space-y-4">
            <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("detail_properties")}</h2>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <label className="text-xs text-muted-foreground">{t("detail_agent")}</label>
              ...
              <div>
                <label className="text-xs text-muted-foreground">{t("detail_output_mode")}</label>
                <div className="mt-1">
                  {autopilot.execution_mode === "create_issue" ? t("mode_create_issue") : t("mode_run_only")}
                </div>
              </div>
              {autopilot.description && (
                <div className="col-span-2">
                  <label className="text-xs text-muted-foreground">{t("detail_prompt")}</label>
                  <div className="mt-1">
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("detail_triggers")}</h2>
              <Button size="sm" variant="outline" onClick={() => setTriggerDialogOpen(true)}>
                <Plus className="h-3.5 w-3.5 mr-1" />
                {t("detail_add_trigger")}
              </Button>
            </div>
            {triggers.length === 0 ? (
              <div className="rounded-md border border-dashed p-4 text-center text-sm text-muted-foreground">
                {t("no_triggers_configured")}
              </div>
            ) : (
          ...
          {/* Run History */}
          <section className="space-y-3">
            <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("detail_run_history")}</h2>
            {runsLoading ? (
              <div className="space-y-1">
                {Array.from({ length: 3 }).map((_, i) => (
                  <Skeleton key={i} className="h-10 w-full" />
          ...
          <section className="space-y-3 pt-4 border-t">
            <h2 className="text-sm font-medium text-destructive uppercase tracking-wider">{t("detail_danger_zone")}</h2>
            <Button size="sm" variant="destructive" onClick={() => setDeleteConfirmOpen(true)}>
              <Trash2 className="h-3.5 w-3.5 mr-1" />
              {t("detail_delete_autopilot")}
            </Button>
          </section>
        </div>
      </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("delete_autopilot_title")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("delete_autopilot_desc", { title: autopilot.title })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleting}>{c("cancel")}</AlertDialogCancel>
