      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b px-4 pt-4 pb-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground">
            {scope === "agent" && deletedAgentCount > 0
              ? t(($) => $.leaderboard.caption_with_deleted, {
                  count: agentRows.length - 1,
          ...
          <div className="divide-y">
            {activeRows.map((row) => {
              ...
              if (scope === "agent") {
                ...
                // The deleted-agents bucket is a synthetic row, not a real
                // agent: render a neutral placeholder (no avatar fetch / hover
                // card / UUID) and dash out Time/Tasks, which it never carries
                // (see bucketUnknownAgentRows).
                if (r.agentId === DELETED_AGENTS_ROW_ID) {
                  return (
                    <div
                      key={r.agentId}
