...
import { getRegistryPricing } from "@multica/core/runtimes/model-registry-pricing-store";
...
// CEREBRO-PATCH(model-registry-pricing-lookup): FIR-2698 pricing per million
// tokens (USD) comes from the single-source model registry
// (server/internal/cerebro/modelregistry) via getRegistryPricing — a
// synchronous read-through cache kept warm by
// useSyncModelRegistryPricing (mounted once in DashboardGuard). This used to
// be a hardcoded table maintained here in parallel with four other copies
// across the backend; that drift was the FIR-2689 root cause. Prices are now
// edited in one place (Settings → Model registry) and read everywhere,
...
function resolvePricing(model: string) {
  ...
  for (const candidate of canonicalCandidates(model)) {
    const hit = getRegistryPricing(candidate);
    if (hit) return hit;
  }
  for (const candidate of canonicalCandidates(model)) {
