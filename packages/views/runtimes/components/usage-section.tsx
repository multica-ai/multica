...
const TIME_RANGES = [
  { key: "7d" as const, days: 7 as const },
  { key: "30d" as const, days: 30 as const },
  { key: "90d" as const, days: 90 as const },
] as const;
...
export function UsageSection({ runtimeId }: { runtimeId: string }) {
  ...
  if (usage.length === 0) {
    return (
      <div className="flex flex-col items-center rounded-lg border border-dashed py-6">
        ...
        <p className="mt-2 text-xs text-muted-foreground">{t.usage.noUsage}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Summary cards */}
      <div className="grid grid-cols-4 gap-3">
        <TokenCard label={t.usage.inputLabel} value={formatTokens(totals.input)} />
        <TokenCard label={t.usage.outputLabel} value={formatTokens(totals.output)} />
        <TokenCard label={t.usage.cacheReadLabel} value={formatTokens(totals.cacheRead)} />
        <TokenCard label={t.usage.cacheWriteLabel} value={formatTokens(totals.cacheWrite)} />
      </div>
      ...
      <div className="rounded-lg border">
        <div className="grid grid-cols-[100px_1fr_80px_80px_80px_80px] gap-2 border-b px-3 py-2 text-xs font-medium text-muted-foreground">
          <div>{t.usage.tableDate}</div>
          <div>{t.usage.tableModel}</div>
          <div className="text-right">{t.usage.tableInput}</div>
          <div className="text-right">{t.usage.tableOutput}</div>
          <div className="text-right">{t.usage.tableCacheR}</div>
          <div className="text-right">{t.usage.tableCacheW}</div>
        </div>
