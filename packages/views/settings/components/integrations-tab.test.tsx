...
describe("Settings IntegrationsTab", () => {
  ...
  it("shows each channel description below its icon and title", () => {
    ...
    for (const channel of ["lark", "slack", "dingtalk", "wecom", "telegram", "mattermost"]) {
  ...
  // (#6585). Requiring a distinct shape per channel is the cheap guard against a
  // regression to that.
  it("gives every channel its own brand mark", () => {
    renderTab();

    const shapes = ["lark", "slack", "dingtalk", "wecom", "telegram", "mattermost"].map(
