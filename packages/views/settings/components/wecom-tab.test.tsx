...
// wecom-tab.test.tsx — the retained Settings → Integrations WeCom panel.
// It covers retained installations and confirm-before-disconnect behavior.
//
...
vi.mock("@multica/core/wecom", () => ({
  ...
  wecomKeys: {
    installations: (wsId: string) => ["wecom", "installations", wsId],
  },
}));
...
describe("WecomTab", () => {
  ...
  it("lists a connected installation with its agent name and a disconnect control", () => {
    installationsRef.current = {
      installations: [
        { id: "i1", agent_id: "agent-7", bot_id: "aibot_x", status: "active" },
      ],
      configured: true,
      install_supported: true,
    };
