...
export function AccountTab() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : t("settings.account.avatarUploadFailed"));
    }
  };
  ...
  const handleProfileSave = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t("settings.account.profileUpdateFailed"));
    } finally {
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-xs text-muted-foreground">
                {t("settings.account.clickUploadAvatar")}
              </div>
            </div>

            <div>
              <Label className="text-xs text-muted-foreground">{t("settings.account.nameLabel")}</Label>
              <Input
                type="search"
                value={profileName}
        ...
        <Card>
          <CardContent className="space-y-3">
            <div className="space-y-1">
              <h3 className="text-sm font-medium">{t("settings.account.languageTitle")}</h3>
            ...
            <p className="text-xs text-muted-foreground">
              {t("settings.account.languageFallbackHint")}
            </p>
          </CardContent>
        </Card>
      </section>
