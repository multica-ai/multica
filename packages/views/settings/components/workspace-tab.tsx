...
import { Eye, Loader2, Lock, LogOut } from "lucide-react";
...
function workspaceGlobalEnvKeyCount(
...
export function WorkspaceTab() {
  ...
  const [globalEnvRevealed, setGlobalEnvRevealed] = useState<
    EnvEditorEntry[] | null
  >(null);
  const [globalEnvOriginal, setGlobalEnvOriginal] = useState<
  ...
  const currentGlobalEnvMap = globalEnvRevealed
    ? entriesToEnvMap(globalEnvRevealed)
    : globalEnvOriginal;
  ...
  return (
    <SettingsTab title={t(($) => $.page.tabs.general)}>
      ...
      <SettingsSection title={t(($) => $.workspace_env.section_title)}>
        <SettingsCard>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="space-y-1">
                  <p className="flex items-center gap-2 text-sm font-medium">
                    ...
                    {globalEnvKeyCount > 0
                      ? t(($) => $.workspace_env.not_revealed_title, {
                          count: globalEnvKeyCount,
                        })
