...
import { ApiError } from "@wallts/core/api";
...
vi.mock("@wallts/ui/components/ui/sidebar", () => ({
...
vi.mock("@wallts/ui/components/ui/dropdown-menu", () => ({
...
vi.mock("@wallts/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));
...
vi.mock("@wallts/core/api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@wallts/core/api")>();
  return {
    ...actual,
    api: {
      ...actual.api,
...
vi.mock("@wallts/core/inbox/queries", () => ({ deduplicateInboxItems: (items: unknown[]) => items, inboxKeys: { list: () => ["inbox"] } }));
vi.mock("@wallts/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@wallts/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@wallts/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@wallts/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@wallts/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@wallts/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@wallts/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
vi.mock("@wallts/core/runtimes/hooks", () => ({ useMyRuntimesNeedUpdate: () => false }));
