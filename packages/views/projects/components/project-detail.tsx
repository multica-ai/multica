  ...
  const handleMoveIssue = useCallback(
    (issueId: string, newStatus: IssueStatus, newPosition?: number) => {
      ...
      updateIssueMutation.mutate(
        { id: issueId, ...updates },
        { onError: () => toast.error(t.detail.moveIssueFailed) },
      );
    },
    [updateIssueMutation, t.detail.moveIssueFailed],
  );
  ...
  if (projectIssues.length === 0) {
    return (
      <div className="flex flex-1 min-h-0 flex-col items-center justify-center gap-2 text-muted-foreground">
        ...
        <p className="text-sm">{t.detail.issuesEmptyTitle}</p>
        <p className="text-xs">{t.detail.issuesEmptyHint}</p>
      </div>
    );
  }
...
export function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  }, [project, deleteProject, router, wsPaths, t.detail.deleteSuccess]);
  ...
  if (!project) {
    return <div className="flex items-center justify-center h-full text-muted-foreground">{t.detail.notFound}</div>;
  }
                ...
                render={
                  <button type="button" className="inline-flex items-center gap-1.5 text-xs hover:text-foreground transition-colors">
                    {project.lead_type && project.lead_id ? (
                      ...
                      <span className="text-muted-foreground">{t.detail.noLead}</span>
                    )}
                  </button>
                }
              />
              <PopoverContent align="start" className="w-52 p-0">
                <div className="px-2 py-1.5 border-b">
                  <input
                    ...
                    placeholder={t.leadPopover.placeholder}
                ...
                <div className="p-1 max-h-60 overflow-y-auto">
                  ...
                  >
                    ...
                    <span className="text-muted-foreground">{t.leadPopover.noLead}</span>
                  </button>
                  {filteredMembers.length > 0 && (
                    <>
                      <div className="px-2 pt-2 pb-1 text-xs font-medium text-muted-foreground uppercase tracking-wider">{t.leadPopover.membersHeading}</div>
                  ...
                  {filteredAgents.length > 0 && (
                    <>
                      <div className="px-2 pt-2 pb-1 text-xs font-medium text-muted-foreground uppercase tracking-wider">{t.leadPopover.agentsHeading}</div>
                  ...
                  {filteredMembers.length === 0 && filteredAgents.length === 0 && leadFilter && (
                    <div className="px-2 py-3 text-center text-sm text-muted-foreground">{t.leadPopover.noResults}</div>
                  )}
                </div>
              </PopoverContent>
            </Popover>
          ...
          <PageHeader className="gap-2 bg-background text-sm">
            <div className="flex flex-1 items-center gap-1.5 min-w-0">
              <AppLink href={wsPaths.projects()} className="text-muted-foreground hover:text-foreground transition-colors shrink-0">
                {workspaceName ?? t.detail.breadcrumbFallback}
              </AppLink>
            ...
            <div className="flex items-center gap-1 shrink-0">
              <Button
                ...
                title={isPinned ? t.detail.unpinFromSidebar : t.detail.pinToSidebar}
                onClick={() => {
                  if (isPinned) {
