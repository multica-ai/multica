...
describe("WelcomeAfterOnboarding", () => {
  ...
  describe("runtime path", () => {
    it("creates a Helper agent then opens a blocking modal with starter cards", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/welcome to 3J Tracker/i)).toBeInTheDocument();
      });

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.runtime_id).toBe("rt-1");
      expect(agentArgs.name).toBe("Helper");
      expect(agentArgs.instructions).toContain("Helper");

      // 3 starter card titles come from HELPER_STARTER_PROMPTS (TS const,
      // EN under the test's en locale).
      expect(
        screen.getByText("Introduce 3J Tracker to me"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Walk me through the core features"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Show me what 3J Tracker can do for me — as slides"),
      ).toBeInTheDocument();
    });

    it("reuses an existing Helper agent instead of creating duplicates", async () => {
      mockListAgents.mockResolvedValueOnce([
        {
          id: "agent-existing",
          name: "Helper",
    ...
    it("selecting cards then clicking Assign creates one issue per pick and navigates to the first", async () => {
      ...
      // Toggle two cards.
      fireEvent.click(screen.getByText("Introduce 3J Tracker to me"));
      fireEvent.click(
        screen.getByText("Show me what 3J Tracker can do for me — as slides"),
      );
      ...
      expect(titles).toEqual([
        "Introduce 3J Tracker to me",
        "Show me what 3J Tracker can do for me — as slides",
      ]);
    ...
    it("uses Korean persisted Helper and starter issue artifacts under ko locale", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText("3J Tracker를 간단히 소개해 주세요"),
        ).toBeInTheDocument(),
      );

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.description).toContain("워크스페이스 사용 어시스턴트");
      expect(agentArgs.instructions).toContain(
        "당신은 이 워크스페이스에 내장된 AI 어시스턴트 Helper",
      );

      fireEvent.click(screen.getByText("3J Tracker를 간단히 소개해 주세요"));
      fireEvent.click(
      ...
      expect(issueArgs.title).toBe("3J Tracker를 간단히 소개해 주세요");
      expect(issueArgs.description).toContain(
        "3J Tracker를 1-2문단으로 간단히 소개해 주세요",
      );
    });
    ...
    it("uses Japanese persisted Helper and starter issue artifacts under ja locale", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText("3J Tracker を簡単に紹介してください"),
        ).toBeInTheDocument(),
      );

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.description).toContain("ワークスペースの使い方アシスタント");
      expect(agentArgs.instructions).toContain(
        "あなたは Helper、このワークスペースに組み込まれた AI アシスタント",
      );

      fireEvent.click(screen.getByText("3J Tracker を簡単に紹介してください"));
      fireEvent.click(
      ...
      await waitFor(() => expect(mockCreateIssue).toHaveBeenCalledTimes(1));
      const [issueArgs] = mockCreateIssue.mock.calls[0]!;
      expect(issueArgs.title).toBe("3J Tracker を簡単に紹介してください");
      expect(issueArgs.description).toContain(
        "3J Tracker を1〜2段落で簡単に紹介してください",
      );
    });
  });
  ...
  describe("skip path", () => {
    ...
    it("silently dismisses without showing the Modal when provisioning fails", async () => {
      ...
      expect(screen.queryByText(/Welcome to 3J Tracker/i)).not.toBeInTheDocument();
    });
    ...
    it("uses Korean persisted skip-path issue and comment artifacts under ko locale", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/3J Tracker에 오신 것을 환영합니다/i)).toBeInTheDocument();
      });
    ...
    it("uses Japanese persisted skip-path issue and comment artifacts under ja locale", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/3J Tracker へようこそ/),
        ).toBeInTheDocument();
      });
      ...
      expect(installCall![0].description).toContain("3J Tracker へようこそ。");
      expect(guideCall![0].title).toBe(
        "ステップ2 — 最初の Agent を作成する",
      );
