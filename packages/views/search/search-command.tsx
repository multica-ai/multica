...
// Static keyword/icon table — the human-readable label is injected per-render
// from the active locale so search matching remains stable while UI text
// follows the user's language.
const NAV_PAGE_META: Omit<NavPage, "label">[] = [
  { key: "inbox", icon: Inbox, keywords: ["inbox", "notifications"] },
  { key: "myIssues", icon: CircleUser, keywords: ["my", "issues", "assigned"] },
  { key: "issues", icon: ListTodo, keywords: ["issues", "tasks", "bugs"] },
  { key: "projects", icon: FolderKanban, keywords: ["projects", "kanban"] },
  { key: "agents", icon: Bot, keywords: ["agents", "bots", "ai"] },
  { key: "runtimes", icon: Monitor, keywords: ["runtimes", "environments"] },
  { key: "skills", icon: BookOpenText, keywords: ["skills", "library"] },
  { key: "settings", icon: Settings, keywords: ["settings", "config", "preferences"] },
];
...
export function SearchCommand() {
  ...
  const navPages = useMemo<NavPage[]>(
    () =>
      NAV_PAGE_META.map((meta) => ({
        ...meta,
        label: t.navPages[meta.key],
      })),
    [t],
  );
  ...
  const commands = useMemo<CommandItem[]>(() => {
    const activeThemeCheck = (value: ThemeValue) =>
      theme === value ? (
        <Check
          aria-label={t.commands.currentTheme}
    ...
    if (currentIssue) {
      ...
      items.push(
        {
          key: "copy-issue-link",
          label: t.commands.copyIssueLink,
          icon: Link2,
          ...
          onSelect: () => {
            ...
            toast.success(t.commands.linkCopied);
            setOpen(false);
          },
        },
        {
          key: "copy-issue-identifier",
          label: t.commands.copyIdentifier(identifier),
          icon: Copy,
          keywords: ["copy", "id", "identifier", identifier.toLowerCase()],
          onSelect: () => {
            void navigator.clipboard.writeText(identifier);
            toast.success(t.commands.copiedIdentifier(identifier));
            setOpen(false);
          },
        },
      );
