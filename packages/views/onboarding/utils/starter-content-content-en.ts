...
export function buildAgentGuidedSubIssues(
  ...
  const tier3: ImportStarterIssuePayload[] = [
    ...
    },
    {
      ...
      priority: "low",
      assign_to_self: true,
      title: "Ask your agent for a personal Multica slide deck",
      description: [
        `Use this when you want a quick, role-specific tour of what Multica can do for you.`,
        ``,
        `**Prompt to send**:`,
        ``,
        `Build me a single-file HTML slide deck that shows what Multica can do for me. Tailor it to my role and use case from the onboarding questionnaire. Paste the full HTML in a fenced \`\`\`html block so I can save it as \`multica-intro.html\` and open it in a browser.`,
        ``,
        `**Format**:`,
        `- One self-contained HTML file with inline CSS and JS`,
        `- 5-8 slides total`,
        `- Include the four core concepts: workspace, issue, agent, runtime`,
        `- Include 3-4 concrete scenarios tailored to my use case`,
        `- Add keyboard navigation with ArrowLeft / ArrowRight / Space`,
        ``,
        `**Design constraints**:`,
        `- Each slide must fit in one viewport without scrolling`,
        `- Use responsive \`clamp()\` sizing for type and spacing`,
