...
export function buildAgentGuidedSubIssues(
  ...
  const tier3: ImportStarterIssuePayload[] = [
    ...
    },
    {
      ...
      priority: "low",
      assign_to_self: true,
      title: "让智能体生成一份你的 Multica 入门 slides",
      description: [
        `当你想快速看到 Multica 对自己有什么用时，用这个 task 生成一份按角色定制的介绍。`,
        ``,
        `**可以发送的 prompt**：`,
        ``,
        `给我做一份单文件 HTML 演示稿，介绍 Multica 能为我做什么。根据我在 onboarding 问卷里的角色和使用场景定制。把完整 HTML 贴在 \`\`\`html 代码块里，我可以保存成 \`multica-intro.html\` 并在浏览器里打开。`,
        ``,
        `**格式**：`,
        `- 一个自包含 HTML 文件，CSS 和 JS 都 inline`,
        `- 总共 5-8 张 slide`,
        `- 包含四个核心概念：workspace、issue、agent、runtime`,
        `- 包含 3-4 个按我的使用场景定制的具体例子`,
        `- 支持 ArrowLeft / ArrowRight / Space 键切换`,
        ``,
        `**设计约束**：`,
        `- 每张 slide 必须在一个视口内完整显示，不要滚动`,
        `- 字体和间距使用响应式 \`clamp()\``,
        `- 避免泛滥的紫色渐变 AI 风格`,
        `- 兼容 \`prefers-reduced-motion: reduce\``,
      ].join("\n"),
    },
    {
      status: "backlog",
