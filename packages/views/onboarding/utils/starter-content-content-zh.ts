...
export function buildWelcomeIssueText(
  ...
  const header = [
    ...
    ``,
    `[了解 Multica 是怎么运转的 →](/docs/zh/how-multica-works)`,
    ``,
    `---`,
    ``,
  ].join("\n");
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        ...
        ``,
        `[关于把 issue 分配给智能体 →](/docs/zh/assigning-issues)`,
      ].join("\n"),
    },
    {
      status: "todo",
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        ...
        `1. 去 [github.com/furtherref/multica/releases/latest](https://github.com/furtherref/multica/releases/latest) 下载 macOS 的 \`.dmg\``,
        `2. 安装并打开`,
        ...
        ``,
        `[关于守护进程与运行时 →](/docs/zh/daemon-runtimes)`,
      ].join("\n"),
    },
    {
      status: "todo",
