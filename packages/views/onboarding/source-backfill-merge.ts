...
export function mergedQuestionnairePatch(
  ...
  return {
    // CEREBRO-PATCH(onboarding-team-size): preserve the cerebro team-size answer
    // when overlaying source so the backfill doesn't wipe it (PATCH overwrites
    // the whole JSONB column).
    team_size: coerceString(raw.team_size) as TeamSize | null,
    team_size_other: coerceString(raw.team_size_other),
