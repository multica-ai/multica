    ...
    <div className="animate-onboarding-enter grid h-full min-h-0 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_480px]">
      ...
      <div className="flex min-h-0 flex-col">
        ...
        >
          <div className="mx-auto w-full max-w-[620px] px-6 py-10 sm:px-10 md:px-14 lg:px-0 lg:py-14">
            ...
            <div className="mt-10 flex flex-col gap-7">
              <QuestionBlock
                num={1}
                question={t.questionnaire.questions.teamSize}
                ariaLabel={t.questionnaire.questions.teamSize}
              >
                <OptionCard
                  selected={answers.team_size === "solo"}
                  onSelect={() => setTeamSize("solo")}
                  label={t.questionnaire.options.teamSize.solo}
                />
                <OptionCard
                  selected={answers.team_size === "team"}
                  onSelect={() => setTeamSize("team")}
                  label={t.questionnaire.options.teamSize.team}
                />
                <OtherOptionCard
                  ...
                  placeholder={t.questionnaire.options.teamSize.otherPlaceholder}
                />
              </QuestionBlock>

              <QuestionBlock
                num={2}
                question={t.questionnaire.questions.role}
                ariaLabel={t.questionnaire.questions.role}
              >
                <OptionCard
                  selected={answers.role === "developer"}
                  onSelect={() => setRole("developer")}
                  label={t.questionnaire.options.role.developer}
                />
                <OptionCard
                  selected={answers.role === "product_lead"}
                  onSelect={() => setRole("product_lead")}
                  label={t.questionnaire.options.role.productLead}
                />
                <OptionCard
                  selected={answers.role === "writer"}
                  onSelect={() => setRole("writer")}
                  label={t.questionnaire.options.role.writer}
                />
                <OptionCard
                  selected={answers.role === "founder"}
                  onSelect={() => setRole("founder")}
                  label={t.questionnaire.options.role.founder}
                />
                <OtherOptionCard
                  ...
                  placeholder={t.questionnaire.options.role.otherPlaceholder}
                />
              </QuestionBlock>

              <QuestionBlock
                num={3}
                question={t.questionnaire.questions.useCase}
                ariaLabel={t.questionnaire.questions.useCase}
              >
                <OptionCard
                  selected={answers.use_case === "coding"}
                  onSelect={() => setUseCase("coding")}
                  label={t.questionnaire.options.useCase.coding}
                />
                <OptionCard
                  selected={answers.use_case === "planning"}
                  onSelect={() => setUseCase("planning")}
                  label={t.questionnaire.options.useCase.planning}
                />
                <OptionCard
                  selected={answers.use_case === "writing_research"}
                  onSelect={() => setUseCase("writing_research")}
                  label={t.questionnaire.options.useCase.writingResearch}
                />
                <OptionCard
                  selected={answers.use_case === "explore"}
                  onSelect={() => setUseCase("explore")}
                  label={t.questionnaire.options.useCase.explore}
                />
                <OtherOptionCard
                  ...
                  placeholder={t.questionnaire.options.useCase.otherPlaceholder}
                />
              </QuestionBlock>
            </div>
          </div>
        ...
        <footer className="flex shrink-0 items-center justify-end gap-4 bg-background px-6 py-4 sm:px-10 md:px-14 lg:px-16">
          ...
          >
            {t.questionnaire.answeredCounter(answeredCount, 3)}
          </span>
          <Button
            size="lg"
    ...
    <div className="flex max-w-[380px] flex-col gap-8">
      <section className="flex flex-col gap-4">
        <div className="text-xs font-medium uppercase tracking-[0.08em] text-muted-foreground">
          {t.questionnaire.sidePanel.whyEyebrow}
        </div>
        <h2 className="font-serif text-[22px] font-medium leading-[1.25] tracking-tight text-foreground">
          {t.questionnaire.sidePanel.whyHeadline}
        </h2>
      </section>

      <section className="flex flex-col gap-4">
        <div className="text-xs font-medium uppercase tracking-[0.08em] text-muted-foreground">
          {t.questionnaire.sidePanel.whatGetEyebrow}
        </div>
        <div className="flex flex-col gap-4">
          <UnlockItem
            icon={<PenLine className="h-4 w-4" />}
            title={t.questionnaire.sidePanel.starterTitle}
            body={t.questionnaire.sidePanel.starterBody}
          />
          <UnlockItem
            icon={<Sparkles className="h-4 w-4" />}
            title={t.questionnaire.sidePanel.headStartTitle}
            body={t.questionnaire.sidePanel.headStartBody}
          />
        </div>
      </section>
      ...
      >
        {t.questionnaire.sidePanel.learnAgents}
      </a>
    </div>
  );
}
