 ...
 * install script. Step 2 is environment-aware: on a self-hosted deployment
 * `/api/config` exposes `daemon_server_url` / `daemon_app_url` and the command
 * becomes `multica setup self-host --server-url ... --app-url ...`, matching
 * the Runtimes "Add a computer" dialog. Only when those are absent (Multica
 * Cloud, or an older server) does it fall back to the cloud `multica setup`.
 */
