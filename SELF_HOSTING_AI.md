...
Wait for the server output `✓ Multica server is running and CLI is ready!` before running `cs-workflow setup self-host`.

**Expected result:**
- Frontend at http://localhost:3000
- Backend at http://localhost:8080
- `cs-workflow` CLI installed and configured for localhost
...
The `cs-workflow setup self-host` command will:
...
2. Run `make selfhost`
3. Run `cs-workflow setup self-host --port <PORT> --frontend-port <FRONTEND_PORT>`

## Troubleshooting

- **Backend not ready:** `docker compose -f docker-compose.selfhost.yml logs backend`
- **Frontend not ready:** `docker compose -f docker-compose.selfhost.yml logs frontend`
- **Daemon issues:** `cs-workflow daemon logs`
