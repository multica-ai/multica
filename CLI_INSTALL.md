...
**If login fails:**
- If no browser is available (headless environment), the user can generate a Personal Access Token at `<your-frontend-origin>/settings` (the URL set as `FRONTEND_ORIGIN` on the server) and run: `multica login --token`
