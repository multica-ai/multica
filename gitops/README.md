...
- **Scheduling**: all pods run on EKS Auto Mode + Karpenter `devops` NodePool (`nodeSelector.eks.amazonaws.com/compute-type=auto` + `karpenter.sh/nodepool=devops`).
...
1. **Populate SSM parameters** under `/agentfarm/tools/` (AWS account `637423279283`, region `us-east-1`). All values are SecureString.
   ...
   | `/agentfarm/tools/RESEND_API_KEY` | Email provider |
   | `/agentfarm/tools/RESEND_FROM_EMAIL` | `noreply@multica.ai` |
