...
24. feat(daemon): per-task cost/token/wall-clock budget caps — `MULTICA_BUDGET_MAX_COST_USD` / `MULTICA_BUDGET_MAX_TOKENS` / `MULTICA_BUDGET_MAX_WALL_CLOCK` (e.g. `90m`), per-agent override via the agent's CustomEnv (same key names), all zero = off so default behavior is byte-identical. Wall clock wraps the run context under the existing cancel watcher; cost/token checked on every attempt boundary against the accumulated GAP-4 usage (chain stops on overrun — failover would only spend more); overrun fails the task with new taxonomy reason `agent_error.budget_exceeded`, which is off the retry allowlist, so FailTask posts the failure comment and the GAP-27 dead-letter case file automatically. Cost uses provider-reported price ticks — backends reporting no cost leave that cap inert (token + wall clock still bound them). Known ceiling: checks are per attempt, not live-per-step; a runner usage callback is the upgrade path (GAP-28, issue #23)
