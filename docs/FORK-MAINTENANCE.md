...
30. feat(service): event trigger dispatch stub (GAP-31, issue #31) — `CompleteTask` now calls `maybeEnqueueEventTriggers` after verifier: queries `ListEventAutopilotTriggersForTask` with filter `[{"event":"task.completed"}]` for completing agent's workspace, logs matched count. Ponytail stub — full enqueue via `CreateAutopilotTask`/`CreateAutopilotRun` when scheduler job lands; log-only prevents silent drop and proves containment query works. No new table/migration, ~20 lines, additive.
