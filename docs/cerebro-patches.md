...
| `cerebro-credentials-routes` | server/cmd/server/router.go | 4 | JEH-1196 — import + handler instance + a single `cerebroCredentialsHandler.Mount(r)` call under the workspace-scoped group; new routes are added inside the cerebro `credentials` package's `Mount` method, not on router.go. |
