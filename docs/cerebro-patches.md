...
| `issue-chip-multilne` | packages/views/issues/components/issue-chip.tsx | 12 | JEH-1253 — three badge UX improvements: (1) `flex-wrap` on BASE_CLASS so the identifier group and title can flow onto separate lines when the title is long; (2) identifier styled as secondary (`text-[10px] text-muted-foreground`) so the title is the visual focus; (3) active-run indicator: a small pulsing dot (`.animate-pulse bg-primary`) appears next to the status icon when any task for the issue is `queued`, `dispatched`, or `running` — driven by the existing `issueKeys.tasks` query (already kept warm by WS invalidation in ExecutionLogSection). |
