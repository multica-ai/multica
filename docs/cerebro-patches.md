...
| `issue-detail-mobile-scroll-to-start` | packages/views/issues/components/issue-detail.tsx | 1 | JEH-1002 — when entering an issue from the inbox on mobile, the auto-scroll to the highlighted comment used `block: "center"`, which left visible whitespace above the message and made the landing position feel imprecise. Switching to `block: "start"` anchors the comment's top edge to the top of the scroll viewport so the user lands exactly on the start of the message they tapped. |
| `issue-handler` | server/internal/handler/issue.go | 51 | Server handler additions |
| `issue-mention-new-tab` | packages/views/editor/readonly-content.tsx | 19 | JEH-874 — clicking an issue mention chip in a comment or description opens it in a new tab with a full https:// href (via `getShareableUrl` / `window.location.origin`), so the user keeps the current context and right-click → Copy Link returns a shareable URL on the actual Multica host (Electron's renderer origin would otherwise leak a non-https:// link). |
