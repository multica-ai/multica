...
| `firtal-gateway-anthropic-tool-schema` | server/internal/cerebro/runtime/firtal_gateway_tools_extended.go<br>server/internal/cerebro/runtime/tools_registry_test.go | 2 | JEH-1441 — define `items` on nested array tool schemas and guard all built-in schemas before sending them to Anthropic, which rejects malformed array schemas before the agent can start. |
