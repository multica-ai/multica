...
| `list-agents-group-filter` | server/internal/handler/agent.go | 5 | JEH-1009 PR 4 — `ListAgents` narrows non-admin viewers to the agent IDs granted via their group memberships. nil-invoker / admin paths skip the filter. |
| `list-agents-owner-exempt` | server/internal/handler/agent.go | 4 | JEH-1057 — owners always see their own agents in `ListAgents`, gated on `create_agent` capability. Revoking the capability also hides own agents (no ghost rows). |
