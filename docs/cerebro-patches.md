...
| `issue-chip-mobile-wrap` | packages/views/issues/components/issue-chip.tsx | 4 | JEH-1593 — on mobile the pill title clipped off-screen because `inline-flex` with `whitespace-nowrap` created a single non-breaking line. Changed outer container from `inline-flex items-center gap-1.5` to `inline box-decoration-clone` so the pill participates in the normal text line-box algorithm; removed `whitespace-nowrap` from the title span so text wraps to the next line. `box-decoration-break: clone` (Tailwind `box-decoration-clone`) repaints the border and background on each wrapped line, keeping the pill look intact. Desktop appearance is unchanged — the identifier group (flex span with shrink-0) always stays on the first line. |
