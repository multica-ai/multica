...
| `cerebro-groups-client` | packages/core/api/client.ts | ~40 | JEH-1006 — typed wrappers around `/api/cerebro/workspaces/{id}/groups` and `/api/cerebro/groups/{id}` for the Cerebro groups settings UI. The `fetch<T>` helper is private so the cerebro-only client lives here. |
