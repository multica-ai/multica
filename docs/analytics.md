...
- `[Amplitude] Page Viewed` — fired by `apps/web/components/pageview-tracker.tsx` on
  ...
  `/ → signup` step. Amplitude's auto-tracking is disabled in
  `initAnalytics` so we own the event shape.
...
- `download_page_viewed` — fired once per `/download` mount after OS
  ...
  `$setOnce` so every downstream event gains a platform dimension
  without re-emitting.
