...
| 8b | **Agent Vault login provisioning for agent-browser** | `handler/agent_browser_auth_cerebro.go` on `POST /api/cerebro/agent-browser/provision-auth` → `multica agent-browser provision-auth` → `agent-browser auth save --password-stdin`; internal-only apps use `POST /api/cerebro/agent-browser/internal-verify` → `multica agent-browser internal-verify --screenshot <path>` | **FIR-3006.** Task-token-only and exact-box gated: the owner must belong to `browser-testers`, the exact calling agent must be on that same group's agent allowlist, the group must explicitly Allow `credential.reveal` on `agentvault-vault:Shared/browser-login/<app>`, and the complete Workspace › Runtime › Agent › Group › User chain must allow the same resource for the requested host. The server reads only the requested username/password keys. The local provisioning bridge keeps both values in process memory, supplies the password over stdin, suppresses child stdout/stderr, and prints only non-sensitive result metadata. For the fixed internal app list, the backend runs the complete login check, validates the post-login markers, captures a PNG before closing the browser session, and returns it to the CLI; the CLI writes that PNG with owner-only permissions and prints only its path plus non-sensitive verification metadata. |
...
| **Local-runtime per-tool enforcement** (Claude/Codex/Cursor/Gemini) | `ResolveDaemonToolPolicy` plus provider-native before-call seams | **always enforced** | FIR-3401 — every daemon-spawned local runtime resolves built-in calls through the same canonical tool-policy chain and approval inbox as Gateway. Claude uses `PreToolUse`, Gemini uses `BeforeTool`, Cursor uses fail-closed `preToolUse`, and Codex app-server approval requests call the same resolver. Deny blocks; Ask waits for a final approval decision; resolver or transport failure blocks. Local CLI providers without a complete before-call adapter (`copilot`, `opencode`, `openclaw`, `hermes`, `pi`, `kimi`, `kiro`, `antigravity`) are rejected before spawn; they cannot run outside the engine. The former rollout switches and claim-time stage are deleted, so no local runtime can silently spawn without enforcement. Credentials, sandbox and repo floors remain independent and unchanged. |
