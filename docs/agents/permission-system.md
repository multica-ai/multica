...
| 12 | **Agent comments must carry a target** (FIR-2674) | `server/internal/cerebro/commentguard/guard.go` (`RejectComment`); enforced in `handler/comment.go` (`CreateComment`, after `ExpandIssueIdentifiers`) | **Off by default** — gated by the `cerebro_comment_target_guard` feature flag (registry.ts), resolved per workspace at request time; turn it on from the Multica feature-flags screen (no env var, no restart). When on: an **agent**-authored comment with no mention at all (member / agent / squad / issue) is rejected with 422; member comments are never gated. An issue link satisfies the rule and has no side effect, so the guard never forces an agent-trigger loop. |
