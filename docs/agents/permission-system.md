...
| 14 | **Per-tool Connection deny** (`connection:<name>` + `resource_pattern=<tool>`) | `server/internal/cerebro/toolpolicy/table_connection.go` (`DisallowedMCPTools` → `DeniedConnectionTools`, reusing the same `Table` resolution the admin UI reads) resolved at task claim in `handler/daemon.go`; the daemon applies it in `server/internal/daemon/daemon.go` | **TECH-3156 — the third thing wired live to the tool-policy chain (not behind `CEREBRO_APPROVAL_GATE_ENABLED`).** Per-tool Allow/Ask/Deny on a connection's underlying tools is stored in `cerebro_tool_policy` and resolved through the full Workspace › Runtime › Agent › Group › User chain (Base = Allow). At claim the effective **Deny** tools are computed and, for **Claude Code** runtimes, passed as `--disallowedTools mcp__<connection>__<tool>` so a denied MCP tool is never callable. **Fail-closed:** any resolve error withholds *all* workspace connections that claim; on **non-Claude** providers (no `--disallowedTools`) the daemon withholds any whole connection that has a denied tool (`daemonmcp.StripServers`). **API (`type='api'`) connections** expand to per-(method, path) rows for CRUD config, but are config-only — they are not emitted as `--disallowedTools` tokens (no REST enforcement path yet). |
