...
| 2b | **Workspace Connections MCP inventory** | `server/internal/cerebro/connections.Store.BuildMCPConfig`, merged into task claim in `handler/daemon.go` and daemon scan config in `handler.GetRuntimeMcpConfig`; scan results flow through `runtime_tools_scan_cerebro.go` → `runtimetools.RecordScan` | Enabled `workspace_connection` MCP servers are injected into runtime MCP config and included in daemon `tools/list` scans. The scan creates concrete `cerebro_runtime_tool` rows, which is what runtime/agent/group/user grants actually enforce. The Connections tab in the tool-policy UI is an admin surface over those rows; execution still depends on the live runtime-tool cascade in row 2. |
