...
> `cerebro_task_mandate` tool snapshot. Managed and local runtimes check that
> snapshot at each call only while `cerebro_task_mandate_enforcement` is on.
> Migration 9147 removes the former direct
...
> and supported local runtimes always resolve the canonical policy before a
> call and additionally enforce the immutable Task Mandate while
> `cerebro_task_mandate_enforcement` is on. Independent credential, sandbox,
...
**FIR-4076 emergency compatibility mode.** Task-token authentication still
validates the immutable token and task rows, stamps the authoritative identity
headers, and attaches `TaskScopeContext`. Resource-bound routes therefore keep
enforcing the task's issue, agent, or workspace boundary. `RequireUserScope`
temporarily passes task tokens through so the existing Multica CLI/MCP surface
works for all runtimes while platform actions receive explicit Task Mandate
capabilities. Property definition handlers and other owner-only handlers keep
...
> **FIR-3819 external-owner and failure contract.** The platform catalog's
> externally managed permissions are deliberately read-only in Settings and
> Capabilities because their real gates are membership, ownership, access-list,
> daemon-token, gateway-token, or webhook-secret checks. Both surfaces name that
> gate. `catalog_external_contract_test.go` requires every externally managed
> key to retain behavioral test proof of its actual security owner. Connection
> call-time resolution is fail-closed for a registry-confirmed Connection tool:
> a missing store, agent lookup failure, or resolver error returns a visible
> Deny and never silently grants the call. Dynamic MCP callable names are mapped
> back to their connection's bare tool resource before resolution, so namespaced
> tools cannot miss their own rule. A list-time API endpoint lookup may keep a
> tool discoverable, but its mandatory call-time check still blocks while
> resolution is unavailable. This supersedes row 2b's historical TECH-3174
> fail-open sentence.
...
| 4e | **Workflow CLI/MCP tools (`list_workflows` / `get_workflow` / `create_workflow` / `update_workflow` / `delete_workflow` / `toggle_workflow` / `activate_workflow` / `get_active_workflow`)** | `runtime/tools_registry.go` — all eight are `ToolStatusExcluded` in the matrix; the actual CLI-runtime MCP handlers live in `clitools/mcp_tools_workflow.go` and call the existing `/api/cerebro/workflows*` routes with the caller's own identity. `handler.RequireManageWorkflows` is the shared agent-mutation boundary: `PlatformActionGate` always resolves `manage_workflows`, and Task Mandate does so while `cerebro_task_mandate_enforcement` is on, before every `POST`, `PUT`, or `DELETE`; reads and member behavior remain unchanged. | FIR-2937/FIR-4196 — workflow management parity uses the existing HTTP surface without creating a second authority path. Settings → Permissions is the only authoring surface for `manage_workflows`; CLI and MCP wrappers inherit the same route decision. `ToolStatusExcluded` remains inventory/display metadata, not a native in-app gateway grant. |
...
| 4h | **Workflow Command library tools** (`list_commands` / `get_command` / `create_command` / `update_command` / `delete_command`) | The shared handlers live in `clitools/mcp_tools_command.go`, call `/api/cerebro/commands*` with the task/member's own identity, and are bridged into both local MCP and the Firtal Gateway registry. `handler.RequireManageWorkflows` gates every agent mutation through `manage_workflows`; reads retain their authenticated workspace context. | **FIR-3493/FIR-4196 — no independent authority.** The HTTP boundary always re-checks `PlatformActionGate` and additionally checks Task Mandate while `cerebro_task_mandate_enforcement` is on before an agent can change a reusable command definition. Saved workflows execute their own copied `argv`, so later command edits or deletion do not rewrite an existing workflow. |
| 4g | **Eval catalog CLI/MCP tools** (`list_evals` / `get_eval` / `create_eval` / `update_eval` / `delete_eval` / `list_eval_runs` / `record_eval_run` / `list_eval_bindings` / `bind_eval` / `unbind_eval`) | Local MCP handlers live in `server/internal/cerebro/clitools/mcp_tools_eval.go`; CLI commands live under `multica workflow eval` in `server/cmd/multica/cerebro_eval.go`. Both call the same `/api/cerebro/evals*` routes, where `handler.RequireManageWorkflows` gates every agent mutation. | **FIR-3308/FIR-4196 — no parallel permission path.** Reads use the authenticated workspace context. Every agent write always inherits the shared `manage_workflows` `PlatformActionGate` decision and also the Task Mandate decision while `cerebro_task_mandate_enforcement` is on; the wrappers grant no eval or workflow authority themselves. |
...
| 12 | **Agent comments must address a recipient** (FIR-2674 / TECH-3279 / FIR-1501) | `server/internal/cerebro/commentguard/guard.go` collects message facts; the managed `before.message.send` policy in `server/internal/cerebro/workflows/managed_policies.go` decides. | The Workflow policy rejects an **agent** comment that addresses no recipient unless an active wakeup is the persisted continuation. A recipient is a member, another agent, a squad, or @all; a bare issue link and a self-mention do not count. Member comments are never gated. |
...
| 14 | **Who can administer workspace connections** (`manage_connections`) | `server/cmd/server/router.go` (the protected connection group uses `RequireWorkspaceMemberFromURL` + `h.RequireConnectionsManagePolicy("id")`) → `handler/group_permissions_cerebro.go` (`cerebroRequireConnectionsPolicy` → `RequireConnectionsManagePolicy`) → `toolpolicy.Resolve`; catalog entry in `cerebro/platformcatalog/catalog.go` (`manage_connections`) | **TECH-3513 slice 1 — the third thing wired live to the tool-policy chain.** Gates `POST /api/workspaces/{id}/connections`, `POST .../connections/test`, `PUT .../connections/{connId}`, `DELETE .../connections/{connId}`, plus the temporary `GET .../connections/company-brain-migration-census` diagnostic. The diagnostic is also default-off behind `cerebro_company_brain_migration_census`; when enabled, each credential-backed `whoami` call is independently subject to the requesting member's effective connection-tool verdict and fails closed on Ask, Deny, missing identity, or policy errors. Replaces the old hardcoded `RequireWorkspaceRoleFromURL("owner","admin")` on connection writes. Consulted unconditionally, independent of approval-inbox availability. Workspace + Group + User layers carry signal (groups auto-resolved from the user). Base = Allow + admin bypass preserve the prior behavior 1:1 (owner/admin still pass with no stored rows); an admin tightens a group/member to restrict. Anything other than Allow (Deny **or** Ask) blocks, since these routes have no inbox-approval path yet. Ordinary connection reads (`GET .../connections`) and the runtime MCP execution endpoint (`POST .../mcp`) stay member-level. Distinct from row 2b, which gates which connection *tools* an agent may call. |
...
| **Platform actions in the tool-policy table** | no secondary rollout flag | **always visible with Unified tool permissions** | The Settings API always includes the code-owned platform catalog (`platformcatalog`, incl. `create_local_runtime`). Engine-owned rows are settable and resolve through the same declared contract as call time. Actions protected by a different security boundary are read-only, name that external owner, and never load an old advisory Allow/Ask/Deny row. Attempts to set or clear one return a conflict that names the same owner, so Settings cannot suggest that it changes live enforcement. The retired `cerebro_platform_capabilities` flag can no longer make a live permission disappear from Settings. |
...
**Loop-gate reporting tools are retired (FIR-3692).** `report_loop_check`,
`report_loop_judge` and `report_loop_human` no longer exist: the Workflow engine
now owns every gate decision, so there is no agent-facing tool that reports a
gate verdict. `open_loop_step` (row 17a) is the only loop tool the Firtal Gateway
still registers.

Human sign-off on a human-type check survives as a **route**, not a tool. The
assignee approves through `POST /api/cerebro/workflows/{id}/human-checks/{checkId}/approve`
(`workflows/handler.go` `ApproveHumanCheck`, listed in `permguard`/`platformcatalog`),
which records the decision on the matching pending check. The route grants no
issue access, reveals no credentials and creates no new work — it only updates a
check the engine already dispatched.
