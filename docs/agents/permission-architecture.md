...
Workflow, Command, and Eval operations under `/api/cerebro/workflows*`, `/api/cerebro/commands*`, and `/api/cerebro/evals*` share one agent enforcement boundary: `handler.RequireManageWorkflows` checks the exact `platformcatalog.ToolBindings` callable against the Task Mandate on every agent read and write. Writes additionally resolve `manage_workflows` through `PlatformActionGate` before mutation. Member behavior remains unchanged, and the independent `/api/cerebro/workflow-hooks*` family keeps its `hooks:*` contracts. Settings → Permissions is therefore the only authoring surface for `manage_workflows`; CLI/MCP wrappers inherit the HTTP decision.
