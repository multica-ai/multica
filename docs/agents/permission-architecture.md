...
| `validateOnBehalfOfUserID` + the Gateway's own copy | `handler/issue_on_behalf_of_cerebro.go`, `cerebro/runtime/firtal_gateway_tools_extended.go` | who an agent may name as an issue's human origin (`on_behalf_of_user_id` on create/update) — fail-closed to an **active `member` of the issue's own workspace**, so an agent id or a cross-workspace user cannot be forged (FIR-4930) | membership row; field omitted → unchanged derived behavior |
