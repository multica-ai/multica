...
**痛点**：用户全局 `~/.codex/hooks.json` 及常见的 `~/.codex/hooks/` helper scripts **未被暴露**到 per-task `CODEX_HOME`。因此 Multica app-server 路径启动的 Codex 不会加载用户配置的 hook，用户配置的 Codex hooks 无法生效；临时旁路 reporter 只能兜底，不应作为长期主链路。
...
- **不新建 reporter 旁路链路**；本 change 只负责让 native hook 链路可用，旁路 reporter 的去留由后续决定。
...
| 开发官 | 实现与单测 | 是 |
| 用户 | 隔离边界红线（不泄露用户 hook 工具链的本机私密 cookie/token/socket）、验收环境结果 | 是 |
| 编排官 | 流水线编排与阶段验收 | 是 |
...
- **hook 执行链路收敛（自托管开发测试环境验证）**：新 Codex session 仍保持 `originator=multica-agent-sdk` / `source=vscode`；用户配置的 Codex native hook 能被正常加载与执行，外部 hook 上报集成端可见对应事件链路，相关用量指标在有使用量窗口内收敛。
...
| 把用户 hook 工具链的本机私密（cookie / token / socket 等）写入 git 或 issue metadata | 低 | 高（泄密） | Requirement 4 红线；只暴露 hooks 两项资源，不 dump 私密到版本库或 metadata |
