...
- 需要页面级验证。
- 触发原因：
  1. 统计入口与操作入口需要在真实路由中分离，不能只靠组件测试证明。
  2. 时间范围切换、个人/团队视角切换、项目/标签/任务 breakdown 都依赖前后端真实集成。
  3. 统计页被定义为交付 gate，必须证明它不会退化成 `My Time` 列表页的客户端拼装结果。
- 现有基线：
  - `e2e/time-tracking.spec.ts` 已覆盖 `/my-time` 操作页和 `Team Time` 的基础路径，但补齐前没有独立验证团队统计聚合和范围切换。
- 执行时必须覆盖的用户路径：
  1. 用户进入统计入口，默认看到本周 summary、成员/项目聚合和空/加载态之一。
  2. 用户切换范围或视角后，页面刷新到对应聚合结果，而不是跳回操作页。
  3. 新增或编辑 time entry 后，统计结果会通过 invalidation 或实时同步更新。
- 关键断言：
  - 路由展示统计页 heading，而不是 `My Time` 的操作控件。
  - summary 文案、成员行、项目行来自同一个时间窗口。
  - 空状态、错误态、范围切换都能在浏览器里被观察到。
