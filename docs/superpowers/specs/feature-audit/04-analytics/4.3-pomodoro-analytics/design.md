...
- 需要页面级验证。
- 触发原因：
  1. 番茄统计与计时操作共用事实源，必须验证“完成一次番茄后统计页更新”这条真实路径。
  2. 周/月统计、目标缺失提示、完成率展示都依赖前后端真实集成，组件测试不能替代。
  3. 历史上 `/pomodoro` 已有页面层行为，新增统计入口或扩统计面时回归风险高。
- 现有基线：
  - `e2e/pomodoro.spec.ts` 已覆盖 `/pomodoro` 可达、focus-first 布局和 today summary 自增。
  - 补齐前还没有覆盖周/月统计、目标缺失提示或独立统计入口。
- 执行时必须覆盖的用户路径：
  1. 用户进入番茄统计入口，看到 summary 和日分布，而不是直接落回计时主面板。
  2. 用户完成一个 pomodoro 后，统计页刷新 today/week/month 数据。
  3. 目标契约缺失时，完成率卡片展示明确提示。
- 关键断言：
  - route heading 与统计摘要同时出现。
  - 完成一次番茄后 summary 数字增加。
  - 无目标时展示“待配置目标”类提示，不输出伪造百分比。
