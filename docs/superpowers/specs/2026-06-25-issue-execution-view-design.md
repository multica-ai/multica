...
当 Issue 关联了 Workflow（`assignee_type = 'workflow'`）时，Issue 详情页的主内容区由旧的嵌入式 `WorkflowDagViewer` 替换为 **Workflow 执行全景图**——一个类似 Workflow Panorama 的阶段泳道视图，但渲染的是该 Workflow **运行时**（`workflow_node_run`）的状态叠加在模板节点上的动态图。
...
2. 点击一个 Issue 进入详情页
3. 如果该 Issue 关联了 Workflow，主内容区展示 Workflow 执行全景图
...
当 Issue **未关联 Workflow** 时，主内容区保持现有布局不变（描述 + 子 Issue 列表 + 评论 + 活动日志）。
...
│ ∎ 需求收集                ✓ 已完成│  ← 节点名称 + 状态
│                                  │
│ 🤖 Worker: agent 小助手     ✓    │  ← Worker 行
│ 👤 Critic: human 张伟       ✓    │  ← Critic 行（有 critic 配置时）
│                                  │
│ 📎 2 个产物                      │  ← 产物徽章（artifact_count > 0）
└──────────────────────────────────┘
```
...
| Worker 行 | 类型图标（🤖/👤）+ `agent/member` 标签 + 名称 + 状态 icon | `text-[11px]`，gap-2，h-6 |
| Critic 行 | 同上，仅当 `critic_type` 非空时显示 | `text-[11px]`，gap-2，h-6 |
| 产物行 | 附件图标 + N 个产物 | `text-[11px] text-muted-foreground`，仅当 artifact_count > 0 |
...
| `runtime-node-card.test.tsx` | 各种状态映射正确色带；Worker/Critic 信息渲染；Critic 无配置时不显示 Critic 行；产物徽章显示/不显示；点击事件 |
