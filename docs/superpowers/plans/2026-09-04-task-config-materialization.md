...
3. Add failing daemon materializer/preflight tests for path traversal, symlinks, collisions, mode, atomic writes, tuple/source/ownership gates, unmanaged workdirs, reused-root cleanup, and worktree lifecycle. Endpoint integration tests require the repository PostgreSQL fixture; the handler package keeps pure provider/identity/payload contract tests for environments without that service.
