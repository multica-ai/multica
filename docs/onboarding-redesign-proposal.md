...
- ~~"公司规模"~~ → solo/team/evaluating 三分已足够；具体公司规模属于 Day 3 邮件采集范围
...
| `evaluating` | **"Add a second agent with a different role"** 置顶（强调编排核心价值） |
| `other` | 按 `evaluating` 路径处理（低承诺默认值，显示 cloud waitlist 入口、不强推 team invite）；`team_size_other` 文本存下做市场研究 |
...
- **Q1 = `evaluating`** 或 `other` → core #4 "Create a second agent" 置顶（强调编排核心价值，让评估用户尽快看到产品差异化）
...
CREATE TABLE user_onboarding (
  ...
  -- 期望结构:
  -- {
  --   "team_size": "solo" | "team" | "evaluating" | "other",         -- Q1
...
2. **Step 1（问卷）**：新建 `step-questionnaire.tsx`，合并 welcome + 3 题；抽出 `<OptionCard>` / `<OtherOptionCard>` 复用组件；删除旧的 `step-welcome.tsx`
