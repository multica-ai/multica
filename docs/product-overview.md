...
工作目录是执行 scratch space，不是交付物存储。任务如果产出代码、文档、报告、研究笔记或其他文件，agent 在把 issue 移到 `in_review` 前必须在最终评论里留下 durable evidence：repo-backed 工作给出 PR / commit / patch；repo-less 产物通过 Multica attachment 上传，或在文件足够短时把完整内容贴到评论里。需要 repo 但缺少可用 repo resource / checkout 的任务应标记为 `blocked`，而不是只把文件留在 scratch workdir。
...
- 工作目录可用于恢复同一 `(agent, issue)` 的后续运行，但不能作为用户可见的文件交付物；文件交付必须通过 PR / commit / patch、附件或完整评论内容保存
...
- `issue.metadata` — issue 级 KV scratchpad；长期会被后续运行反复读取的事实可以 pin 在这里，例如 `pr_url`、`pipeline_status`、`artifact_summary`、`deploy_url` 或当前 blocker。日志、长摘录、secret、运行次数和一次性细节不应写入 metadata。
