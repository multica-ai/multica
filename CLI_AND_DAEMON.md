...
One call returns one page. `--limit` is the page size, 1 to 100 (the server returns at most 100 issues per request), and `--offset` is how many issues to skip. A limit outside 1 to 100 or a negative offset is rejected rather than quietly clamped. In table mode a line on stderr says when you are looking at a page of a larger set (`Showing 1-50 of 2706 issues. Next page: --offset 50`), and stays silent when everything fit. With `--output json` the envelope carries `total`, `limit`, `offset` and `has_more`. `limit` is the page size the server applied, which can be smaller than the number you passed, so while `has_more` is true a script should advance `--offset` by the `limit` in that same response, never by the number it passed. An empty page always reports `has_more: false`. To walk a whole list, sort by `created_at` (`--sort created_at --direction asc`): the default `position` order is re-ranked by board drags and status changes, so pages can shift under a walk. Sorting by `created_at` keeps them still, but no offset walk is a consistent snapshot while others are writing.
