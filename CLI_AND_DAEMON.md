...
`--fields` (JSON output only) whitelists which top-level issue keys come back — pass a comma-separated list such as `--fields=id,title,status,priority`. Omit it for the full issue object, unchanged from before this flag existed; `description` alone is typically most of the payload, so requesting only the fields you need cuts token/bandwidth cost. Field names are the real API keys, not table-display labels — assignee is `assignee_type`/`assignee_id` rather than a single `assignee` field, and there is no `labels` field on an issue. An unknown name is rejected up front with the valid list rather than silently dropped. Has no effect on `--output table`.
