...
### Remote or headless setup

Open **Runtimes → Add a computer** in the Multica web app and run the generated
one-line command on the target machine. It installs the CLI, authenticates, and
starts the daemon without prompts or a browser. The embedded `mst_` setup token
expires after 30 minutes, works once, and is exchanged for the same renewable
90-day `mul_` credential used by browser login. The page reports token
redemption, daemon connectivity, and detected agent runtimes separately.

For self-hosted deployments, the generated command includes both the backend
and frontend origins. The underlying non-interactive CLI form is:

```bash
multica setup self-host --token <mst_...> \
  --server-url https://api.example.com \
  --app-url https://app.example.com
```

Use a command generated by the target workspace rather than creating or
reusing a personal access token manually.
