...
- **Codex marketplace/plugin cache reclamation** — the exact managed path `codex-home/.tmp` stays available across nearby runs and is removed only after both its directory mtime and the task's last completion metadata have been idle for `MULTICA_GC_CODEX_TEMP_TTL` (default `72h`). This avoids turning installed marketplaces into a per-run download and keeps resumed tasks usable offline. The same active-root guard, env-root reservation, ownership proof, and symlink/junction containment checks protect this path; task exit never removes it. Set the TTL to `0` to disable this cleanup independently.
