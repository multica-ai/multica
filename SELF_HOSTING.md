...
> **Exception — WeCom (企业微信) and personal Weixin must run single-replica.** Unlike Slack and Lark, whose outbound is stateless HTTP that any replica can perform, these adapters keep delivery state in the process holding the channel lease. WeCom sends through its live WebSocket; personal Weixin retains the latest iLink context token beside its long poller. If more than one backend replica is used, a completion published on another replica cannot deliver the response. Until cross-replica outbound routing lands, run the backend as a single replica whenever `MULTICA_WECOM_SECRET_KEY` or `MULTICA_WEIXIN_SECRET_KEY` is set. Everything else (including the rollup scheduler above) is multi-replica safe.
