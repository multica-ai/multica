...
- **DingTalk (钉钉):** configure `MULTICA_DINGTALK_AUTH_CLIENT_ID` / `MULTICA_DINGTALK_AUTH_CLIENT_SECRET` in `.env` to add a QR-code login button that signs members of a DingTalk organization in via their enterprise email — no email delivery needed. See [DingTalk OAuth Login](DINGTALK_OAUTH.md).
