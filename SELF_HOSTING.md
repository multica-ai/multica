...
Deploy Wallts on your own infrastructure in minutes.
...
Each user who runs AI agents locally also installs the **`wallts` CLI** and runs the **agent daemon** on their own machine.
...
This installs the `wallts` CLI, checks out the latest self-host assets, pulls the official Wallts images from GHCR, and configures everything for localhost.
...
`make selfhost-build` uses local `wallts-backend:dev` / `wallts-web:dev` tags, so it does not overwrite the pulled `:latest` images.
...
- **Deterministic local/private testing:** set `APP_ENV=development` and `WALLTS_DEV_VERIFICATION_CODE=888888` in `.env`, then restart the backend. This fixed code is ignored when `APP_ENV=production`.
...
> **Warning:** do **not** set `WALLTS_DEV_VERIFICATION_CODE` on a publicly reachable instance — anyone who knows an email address can then log in with that fixed code.
...
If you already run a Kubernetes cluster, you can deploy Wallts there instead of Docker Compose using the released OCI Helm chart at `oci://ghcr.io/dwickyfp/charts/wallts` or the source chart at [`deploy/helm/wallts/`](deploy/helm/wallts/). It targets a typical k3s / k8s setup with an Ingress controller and a default `ReadWriteOnce` StorageClass — authored against k3s + Traefik + `local-path`, and should work on any cluster with minor tweaks.

The chart creates the following resources in the target namespace:

- `wallts-postgres` — `pgvector/pgvector:pg17` backed by a 10Gi PVC
- `wallts-backend` — Go API/WS server backed by a 5Gi uploads PVC
- `wallts-frontend` — Next.js standalone server
- Two `Ingress` resources: one for the web host, one for the backend host
- `wallts-config` ConfigMap (rendered from `values.yaml`)

The `wallts-secrets` Secret is **not** managed by the chart — you create it once with `kubectl` so real values never need to land in git.

> **One release per namespace:** the prebuilt `wallts-web` image bakes `REMOTE_API_URL=http://backend:8080` at build time, so the chart ships an ExternalName Service literally named `backend`. Because that name is unprefixed, you can run only one Wallts release per namespace, and `helm install` will fail if a `Service/backend` already exists there (pass `--take-ownership`, or use a dedicated namespace). If you build a web image with a patched `REMOTE_API_URL`, set `frontend.compatibility.backendAlias: false` to drop the alias.
...
The chart defaults to `wallts.dev.lan` (web) and `api.wallts.dev.lan` (backend). Pick one of:

- **`/etc/hosts`** on every machine that needs access (developer laptops + the machine running the daemon):

  ```text
  192.168.1.206  wallts.dev.lan api.wallts.dev.lan
  ```
...
```bash
kubectl create namespace wallts
```

### Step 3 — Create the `wallts-secrets` Secret

The chart references this Secret by name. Create it once with random values:

```bash
kubectl -n wallts create secret generic wallts-secrets \
  --from-literal=JWT_SECRET="$(openssl rand -hex 32)" \
  --from-literal=POSTGRES_PASSWORD="$(openssl rand -hex 16)" \
  --from-literal=RESEND_API_KEY="" \
  --from-literal=GOOGLE_CLIENT_SECRET="" \
  --from-literal=CLOUDFRONT_PRIVATE_KEY="" \
  --from-literal=WALLTS_DEV_VERIFICATION_CODE=""
```
...
```bash
helm install wallts oci://ghcr.io/dwickyfp/charts/wallts \
...
```bash
helm show values oci://ghcr.io/dwickyfp/charts/wallts \
...
```bash
helm install wallts deploy/helm/wallts -n wallts
```

Watch the pods come up:

```bash
kubectl -n wallts get pods -w
```
...
```bash
curl -H "Host: api.wallts.dev.lan" http://<ingress-ip>/healthz
# {"status":"ok","checks":{"db":"ok","migrations":"ok"}}
```

Then open http://wallts.dev.lan in your browser.
...
- **Recommended (production):** patch the Secret with a real Resend key, then restart the backend:
  ...
  ```bash
  kubectl -n wallts patch secret wallts-secrets --type=merge \
    -p '{"stringData":{"RESEND_API_KEY":"re_xxx"}}'
  kubectl -n wallts rollout restart deploy/wallts-backend
  ```
...
- **Without email configured:** the verification code is generated server-side and printed to the backend pod logs (look for `[DEV] Verification code for ...:`). Useful for one-off testing.
  ...
  ```bash
  kubectl -n wallts logs -f deploy/wallts-backend | grep "Verification code"
  ```

- **Deterministic local/private testing:** set `backend.config.appEnv: development` in your values file and `WALLTS_DEV_VERIFICATION_CODE=888888` in the Secret, then `helm upgrade` and restart. This fixed code is ignored when `APP_ENV=production`.

  ```bash
  helm upgrade wallts oci://ghcr.io/dwickyfp/charts/wallts \
    --version <chart-version> \
    -n wallts \
    -f my-values.yaml --set backend.config.appEnv=development
  kubectl -n wallts patch secret wallts-secrets --type=merge \
    -p '{"stringData":{"WALLTS_DEV_VERIFICATION_CODE":"888888"}}'
  kubectl -n wallts rollout restart deploy/wallts-backend
  ```
...
```bash
wallts setup self-host \
  --server-url http://api.wallts.dev.lan \
  --app-url http://wallts.dev.lan
```
...
```bash
kubectl -n wallts rollout restart deploy/wallts-backend deploy/wallts-frontend
```

To upgrade to a specific Wallts release, upgrade to the matching chart version. The released chart defaults its app images to the matching Git tag:
...
```bash
# /etc/cron.d/wallts-rollup — every 5 minutes
...
```bash
curl -fsSL https://raw.githubusercontent.com/dwickyfp/wallts/main/scripts/install.sh | bash -s -- --stop
```
...
## Switching to Wallts Cloud

If you've been self-hosting and want to switch your CLI to [Wallts Cloud](https://wallts.ai):

```bash
wallts setup
```

This reconfigures the CLI for wallts.ai, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
Pin `WALLTS_IMAGE_TAG` in `.env` to an exact version like `v0.2.4` if you want to stay on a specific release. Migrations run automatically on backend startup.
...
If you prefer configuring the CLI step by step instead of `wallts setup`:
