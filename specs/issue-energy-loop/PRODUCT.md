...
2. Focus、Time Entry、Daily Review 只能记录 issue 执行过程和结果，不能把非 issue 对象升级为可执行工作项。
3. 如果后续重新引入 Plan 或计划项模型，必须先写新的 product spec 和 tech spec，明确为什么 issue 模型不足，并重新评审运行时对象矩阵。
