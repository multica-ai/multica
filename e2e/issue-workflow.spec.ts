...
async function updateStatus(
  ...
  const status = workflow.statuses.find((candidate) => candidate.legacy_status_key === legacyKey);
  if (!status) throw new Error(`Missing ${legacyKey} workflow status`);
  return request<WorkflowResponse>(api, workspaceId, `/api/issue-workflows/${workflow.workflow.id}/statuses/${status.id}`, {
    method: "PATCH",
    body: JSON.stringify({ expected_revision: workflow.workflow.revision, ...body }),
  });
}
...
async function chooseStatus(page: Page, current: string, next: string) {
  ...
  await option.click();
  await page.getByRole("dialog").getByRole("button", { name: `Enter ${next}`, exact: true }).click();
...
test.describe("project-scoped issue workflow", () => {
  ...
  test("runs agent, squad, human takeover, re-entry, and replay cases", async ({ page }, testInfo) => {
    ...
    const projectTitle = `Workflow E2E ${suffix}`;
    const issueTitle = `Workflow vertical slice ${suffix}`;
    ...
    try {
      ...
      runtimeId = (await client.query<{ id: string }>(`
        ...
        RETURNING id
      `, [workspaceId, `Workflow Runtime ${suffix}`, userId])).rows[0]?.id ?? "";
    ...
    let workflow = await request<WorkflowResponse>(api, workspaceId, `/api/projects/${projectId}/issue-workflow`, {
      method: "PUT",
      body: JSON.stringify({ mode: "custom" }),
    });
    workflow = await updateStatus(api, workspaceId, workflow, "todo", {
    ...
    const todo = workflow.statuses.find((status) => status.legacy_status_key === "todo");
    if (!todo) throw new Error("Customized Todo status is missing");

    await page.goto(`/${(await api.getWorkspaces())[0]?.slug}/projects/${projectId}?view=workflow`, { waitUntil: "domcontentloaded" });
    ...
    await capture(page, testInfo, "01-project-custom-workflow.png");
