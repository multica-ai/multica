-- Restore the migration-102 rollup window function (no cost_cents) BEFORE
-- dropping the column, so the live function never references a column that is
-- about to disappear. Body is the verbatim 102 definition.
...
ALTER TABLE task_usage_hourly DROP COLUMN IF EXISTS cost_cents;
