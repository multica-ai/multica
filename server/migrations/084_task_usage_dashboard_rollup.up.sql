...
-- CEREBRO-PATCH(migration-084-task-usage-dashboard): make upstream migration idempotent for fork deploy retries.
CREATE TABLE IF NOT EXISTS task_usage_dashboard_daily (
...
CREATE INDEX IF NOT EXISTS idx_task_usage_dashboard_daily_workspace_date
...
CREATE INDEX IF NOT EXISTS idx_task_usage_dashboard_daily_project_date
    ON task_usage_dashboard_daily (workspace_id, project_id, bucket_date DESC);

-- Per-agent reads (the "by agent" panel).
CREATE INDEX IF NOT EXISTS idx_task_usage_dashboard_daily_agent_date
    ON task_usage_dashboard_daily (workspace_id, agent_id, bucket_date DESC);

CREATE TABLE IF NOT EXISTS task_usage_dashboard_rollup_state (
...
CREATE TABLE IF NOT EXISTS task_usage_dashboard_dirty (
    bucket_date  DATE        NOT NULL,
    workspace_id UUID        NOT NULL,
    agent_id     UUID        NOT NULL,
    project_id   UUID,
...
CREATE INDEX IF NOT EXISTS idx_task_usage_dashboard_dirty_enqueued_at
