-- Cerebro-only: pause / unpause for agent_runtime. paused_at is the source
-- of truth — when it is non-NULL the runtime is paused, regardless of the
-- existing online/offline status (which is heartbeat-driven and orthogonal
-- to pause).
--
-- unpause_at is optional. When set, a periodic sweeper transitions the
-- runtime back to unpaused at that instant. NULL means "stay paused until
-- manually unpaused" (useful when the rate-limit response gave us no reset
-- hint).
--
