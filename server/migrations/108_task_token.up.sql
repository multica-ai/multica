...
-- CEREBRO-PATCH(task-token-drop-fork-059): drop the fork's JEH-324 task_token
-- (migration 059, BYTEA-keyed) so upstream's MUL-2600 schema recreates it.
-- Tokens are short-lived (~1h) so the recreate is safe in production.
