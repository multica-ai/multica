...
func TestMigration204RepairsLegacyCompositeWorkspaceConstraint(t *testing.T) {
	...
	schema := pgx.Identifier{fmt.Sprintf("migration_204_%d", time.Now().UnixNano())}.Sanitize()
	...
	`); err != nil {
		t.Fatalf("create legacy migration 203 shape: %v", err)
	}
	...
	up, err := os.ReadFile("204_issue_external_identity_workspace_guard.up.sql")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := tx.Exec(ctx, string(up)); err != nil {
		t.Fatalf("apply migration 204 up: %v", err)
	}
	...
	if broadConstraintCount != 0 {
		t.Fatalf("migration 204 left %d broad issue constraints", broadConstraintCount)
	}
	var aliasCount int
	...
	// history may include either supported shape of migration 203.
	if _, err := tx.Exec(ctx, string(up)); err != nil {
		t.Fatalf("reapply migration 204 up: %v", err)
	}
	down, err := os.ReadFile("204_issue_external_identity_workspace_guard.down.sql")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := tx.Exec(ctx, string(down)); err != nil {
		t.Fatalf("apply migration 204 down: %v", err)
	}
	_, moveErr = tx.Exec(ctx, `UPDATE issue SET workspace_id=$1 WHERE id=$2`, otherWorkspace, issueID)
	if !errors.As(moveErr, &pgErr) || pgErr.Code != "23503" || !strings.Contains(pgErr.Message, "external identity issue workspace cannot change") {
...
func TestMigration204RefusesExistingWorkspaceMismatch(t *testing.T) {
	dbURL := os.Getenv("DATABASE_URL")
	if dbURL == "" {
		t.Skip("DATABASE_URL is required for the DB-backed migration test")
	}
	...
	schema := pgx.Identifier{fmt.Sprintf("migration_204_mismatch_%d", time.Now().UnixNano())}.Sanitize()
	if _, err := tx.Exec(ctx, "CREATE SCHEMA "+schema); err != nil {
		t.Fatalf("create schema: %v", err)
	}
	if _, err := tx.Exec(ctx, "SET LOCAL search_path TO "+schema+", public"); err != nil {
