...
CREATE INDEX idx_work_session_issue ON work_session(issue_id);
...
ALTER TABLE task_message ADD COLUMN work_session_id UUID REFERENCES work_session(id) ON DELETE CASCADE;
ALTER TABLE task_message ADD CONSTRAINT task_message_one_parent
    CHECK ((task_id IS NOT NULL) != (work_session_id IS NOT NULL));
CREATE INDEX idx_task_message_work_session ON task_message(work_session_id, seq);
