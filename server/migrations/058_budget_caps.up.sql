...
CREATE TABLE IF NOT EXISTS workspace_budget_config (
...
CREATE TABLE IF NOT EXISTS user_budget_override (
...
-- Per-agent override.
CREATE TABLE IF NOT EXISTS agent_budget_override (
    workspace_id        UUID NOT NULL REFERENCES workspace(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_budget_state_window ON budget_state (workspace_id, window_type, window_start);
...
CREATE INDEX IF NOT EXISTS idx_budget_change_log_workspace ON budget_change_log (workspace_id, changed_at DESC);
