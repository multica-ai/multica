...
DO $$
BEGIN
  CREATE INDEX IF NOT EXISTS idx_issue_title_bigm ON issue USING gin (LOWER(title) gin_bigm_ops);
  CREATE INDEX IF NOT EXISTS idx_issue_description_bigm ON issue USING gin (LOWER(COALESCE(description, '')) gin_bigm_ops);
...
DO $$
BEGIN
  CREATE INDEX IF NOT EXISTS idx_comment_content_bigm ON comment USING gin (LOWER(content) gin_bigm_ops);
EXCEPTION WHEN OTHERS THEN
