...
CREATE INDEX IF NOT EXISTS idx_channel_archived ON channel(workspace_id, archived_at);
...
CREATE INDEX IF NOT EXISTS idx_channel_message_thread ON channel_message(thread_root_id);
CREATE INDEX IF NOT EXISTS idx_channel_message_status ON channel_message(status);
...
ALTER TABLE issue ADD COLUMN IF NOT EXISTS source_channel_id UUID REFERENCES channel(id);
ALTER TABLE issue ADD COLUMN IF NOT EXISTS source_thread_root_id UUID REFERENCES channel_message(id);
ALTER TABLE issue ADD COLUMN IF NOT EXISTS discussion_snapshot JSONB;
