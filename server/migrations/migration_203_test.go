...
func TestMigration203OwnsOnlyItsObjectsAndDoesNotMutateIssueConstraints(t *testing.T) {
	upBytes, err := os.ReadFile("203_issue_external_identity.up.sql")
	if err != nil {
		t.Fatal(err)
	}
	downBytes, err := os.ReadFile("203_issue_external_identity.down.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, forbidden := range []string{"uq_issue_workspace_id", "alter table issue", "unique (workspace_id, id)"} {
		if strings.Contains(up, forbidden) || strings.Contains(down, forbidden) {
			t.Fatalf("migration 203 contains destructive/pre-existing issue constraint operation %q", forbidden)
		}
	}
	for _, required := range []string{"references issue(id)", "issue_external_identity_workspace_180", "issue_external_identity_issue_workspace_180", "for share"} {
		if !strings.Contains(up, required) {
			t.Fatalf("migration 203 missing owned invariant component %q", required)
		}
	}
...
func TestMigration203RejectsMovingAnIssueAwayFromItsAliasWorkspace(t *testing.T) {
	...
	schemaName := fmt.Sprintf("migration_203_%d", time.Now().UnixNano())
	...
	up, err := os.ReadFile("203_issue_external_identity.up.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !errors.As(moveErr, &pgErr) || pgErr.Code != "23503" || !strings.Contains(pgErr.Message, "external identity issue workspace cannot change") {
		t.Fatalf("workspace move error = %v, want migration 203 same-workspace guard", moveErr)
	}
	...
	down, err := os.ReadFile("203_issue_external_identity.down.sql")
	if err != nil {
		t.Fatal(err)
	}
...
func TestMigration203SerializesAliasInsertWithIssueWorkspaceMove(t *testing.T) {
	dbURL := os.Getenv("DATABASE_URL")
	if dbURL == "" {
		t.Skip("DATABASE_URL is required for the DB-backed migration test")
	}
	...
	schema := pgx.Identifier{fmt.Sprintf("migration_203_race_%d", time.Now().UnixNano())}.Sanitize()
	...
	var pgErr *pgconn.PgError
	if !errors.As(moveErr, &pgErr) || pgErr.Code != "23503" || !strings.Contains(pgErr.Message, "external identity issue workspace cannot change") {
		t.Fatalf("concurrent workspace move error = %v, want migration 203 same-workspace guard", moveErr)
	}
