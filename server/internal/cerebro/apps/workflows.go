...
// TestWorkflow records a durable identity envelope and queues the same run
// contract consumed by Hatchet workers. No registry key is stored in the row.
func (h *Handler) TestWorkflow(w http.ResponseWriter, r *http.Request) {
	...
	err = h.pool.QueryRow(r.Context(), `
		SELECT d.version,a.id::text,a.current_version,g.scopes
		...
		WHERE d.id=$1 AND d.workspace_id=$2`, id, workspaceID).Scan(&workflowVersion, &appID, &appVersion, &rawScopes)
	if errors.Is(err, pgx.ErrNoRows) {
