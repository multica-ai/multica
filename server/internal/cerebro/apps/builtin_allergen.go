...
const multica=createMulticaApp({appId:"f1540000-0000-4154-8154-000000000001",version:"1.0.0"});
document.querySelector("form").addEventListener("submit",async(event)=>{event.preventDefault();const output=document.querySelector("#result");output.hidden=false;output.textContent="Formatting…";try{const result=await multica.workers.invoke({ingredients:document.querySelector("#ingredients").value});output.textContent=result.formatted_ingredients+"\n\nAllergens: "+result.allergens.join(", ")}catch{output.textContent="Formatting failed"}});`),
		builtinFile("backend/index.mjs", "text/javascript; charset=utf-8", `export default async ({ingredients}, multica) => { const response = await multica.connections.call("ai_gateway", "chat.completions", { model: "claude-haiku-4-5", messages: [{ role: "system", content: "Return JSON with formatted_ingredients and allergens. Uppercase regulated allergens only." }, { role: "user", content: String(ingredients || "") }], response_format: { type: "json_object" } }); const body = response.result || response; const content = body.choices && body.choices[0] && body.choices[0].message && body.choices[0].message.content; if (typeof content !== "string") throw new Error("AI gateway response contained no result"); return JSON.parse(content); }`),
	}
}
