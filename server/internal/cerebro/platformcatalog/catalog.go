...
var catalog = []Capability{
	// --- Apps -----------------------------------------------------------------
	{
		...
		Description:   "Create a workspace mini app with a named owner.",
		DescriptionZh: "创建具有指定所有者的工作区迷你应用。",
		Ops:           []string{"POST /api/cerebro/apps/"},
	},
	{
		Key:           "apps.manage",
		Title:         "Manage apps",
		Category:      CategoryApps,
		Description:   "Preview, publish, roll back, approve scopes, and manage workflows for workspace apps.",
		DescriptionZh: "预览、发布、回滚、批准权限范围并管理工作区应用的工作流。",
		Ops: []string{
			"POST /api/cerebro/apps/builtins/allergen-formatter/install",
			"POST /api/cerebro/app-folders/",
			"PATCH /api/cerebro/app-folders/{folderId}",
			"DELETE /api/cerebro/app-folders/{folderId}",
			"PUT /api/cerebro/app-folders/{folderId}/apps/{appId}",
			"POST /api/cerebro/apps/{id}/preview",
			"POST /api/cerebro/apps/{id}/publish",
			"POST /api/cerebro/apps/{id}/rollback",
			"POST /api/cerebro/apps/{id}/disable",
			"POST /api/cerebro/apps/{id}/approve-scopes",
	...
	},
	{
		...
		Title:         "Delete apps",
		Category:      CategoryApps,
		Description:   "Delete an app and all of its versions, workflows, and app-owned data.",
		DescriptionZh: "删除应用及其所有版本、工作流和应用自有数据。",
		Ops:           []string{"DELETE /api/cerebro/apps/{id}"},
	},
	// --- Issues ---------------------------------------------------------------
	{
		Key:           "create_issue",
...
var excluded = map[string]string{
	...
	"POST /api/cerebro/apps/{id}/invoke":                     "member app-use action — invokes only the current ready version and records the viewing human",
	...
	"POST /api/cerebro/apps/workflow-runs/{runId}/execute":         "pre-auth service-to-service — CEREBRO_APP_WORKFLOW_INGEST_KEY bearer token authorises the Hatchet mini-app worker; identity is loaded from the durable run envelope (FIR-3172)",
	...
	"POST /api/cerebro/apps-internal/host/registry":                "pre-auth app worker bridge — short-lived human/app/version grant plus approved Registry scope authorises one call (FIR-3172)",
	"POST /api/cerebro/apps-internal/host/connection":              "pre-auth app worker bridge — short-lived human/app/version grant plus approved Connection scope authorises one call (FIR-3172)",
	"POST /api/cerebro/app-workflow-webhooks/{workflowId}/{token}": "pre-auth signed webhook — the saved workflow token authorises the delivery (FIR-3172)",
	"POST /api/cerebro/app-workflow-triggers/data-event":           "pre-auth service-to-service — CEREBRO_APP_DATA_EVENT_KEY authorises Registry events (FIR-3172)",
	"POST /api/cerebro/app-workflow-triggers/schedule":             "pre-auth service-to-service — CEREBRO_APP_WORKFLOW_INGEST_KEY authorises the Hatchet schedule tick (FIR-3172)",
