...
// TestTable_RepoRowsExpandResourceScopedRoles proves a reusable Role is not
// limited to capability-wide rows. The same exact-repo rule the call-time store
// enforces must be visible in the per-repository read model, without leaking to
// another repository.
func TestTable_RepoRowsExpandResourceScopedRoles(t *testing.T) {
	s := newTPStore(t)
	clearAll(t, s)
	clearCaps(t, s)
	clearRepos(t, s)
	...
	const (
		repoA = "github.com/firtal-group/role-protected"
		repoB = "github.com/firtal-group/role-open"
	)
	setWorkspaceRepos(t, s, `[{"url":"`+repoA+`"},{"url":"`+repoB+`"}]`)
	agent := uuidByte(71)
	assignResourceRole(t, s, "Repo reviewer", agent, capRepoCheckout, repoA, SettingDeny)
	...
	protected, ok := findRepoRow(rows, capRepoCheckout, repoA)
	if !ok {
		t.Fatalf("repo.checkout row for %s missing", repoA)
	}
	if protected.Effective.Setting != SettingDeny {
		t.Fatalf("role-protected repo effective = %q, want deny", protected.Effective.Setting)
	}
	open, ok := findRepoRow(rows, capRepoCheckout, repoB)
	if !ok {
		t.Fatalf("repo.checkout row for %s missing", repoB)
	}
	if open.Effective.Setting != SettingAllow {
		t.Fatalf("resource-scoped role leaked to %s: effective = %q, want allow", repoB, open.Effective.Setting)
	}
}
