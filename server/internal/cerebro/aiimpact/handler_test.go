...
func TestWorkspaceEvidenceReadModelFiltersByFunction(t *testing.T) {
	workspaceID := uuid.New()
	selectedFunctionID := uuid.New()
	otherFunctionID := uuid.New()
	selectedLoopID := uuid.New()
	otherLoopID := uuid.New()
	selectedMetricID := uuid.New()
	otherMetricID := uuid.New()
	periodStart := time.Date(2026, time.July, 1, 0, 0, 0, 0, time.UTC)
	periodEnd := periodStart.Add(24 * time.Hour)
	store := &recordingObservationStore{
		functions: []Function{
			{ID: selectedFunctionID, WorkspaceID: workspaceID, Name: "Customer Service", Active: true},
			{ID: otherFunctionID, WorkspaceID: workspaceID, Name: "Finance", Active: true},
		},
		operatingLoops: []OperatingLoop{
			{ID: selectedLoopID, WorkspaceID: workspaceID, FunctionID: selectedFunctionID, Name: "Resolve customer needs", Active: true},
			{ID: otherLoopID, WorkspaceID: workspaceID, FunctionID: otherFunctionID, Name: "Close the books", Active: true},
		},
		metrics: []Metric{
			{ID: selectedMetricID, WorkspaceID: workspaceID, OperatingLoopID: selectedLoopID, Name: "Resolved needs", Family: FamilyOutcome, Unit: "needs", Direction: DirectionIncrease, Source: "support", Active: true},
			{ID: otherMetricID, WorkspaceID: workspaceID, OperatingLoopID: otherLoopID, Name: "Close duration", Family: FamilyOutcome, Unit: "hours", Direction: DirectionDecrease, Source: "ledger", Active: true},
		},
		workspaceObservations: []Observation{
			{ID: uuid.New(), MetricID: selectedMetricID, PeriodStart: periodStart, PeriodEnd: periodEnd, Value: 15, EvidenceStatus: EvidenceMeasured, Confidence: 0.9, Source: "support", Method: "audited count", CreatedAt: periodEnd},
			{ID: uuid.New(), MetricID: otherMetricID, PeriodStart: periodStart, PeriodEnd: periodEnd, Value: 6, EvidenceStatus: EvidenceMeasured, Confidence: 0.9, Source: "ledger", Method: "reconciliation", CreatedAt: periodEnd},
		},
	}
	handler := NewHandler(NewService(store))
	router := chi.NewRouter()
	...
	req := httptest.NewRequest(http.MethodGet, "/api/cerebro/ai-impact/evidence?function_id="+selectedFunctionID.String(), nil)
	ctx := middleware.SetMemberContext(req.Context(), workspaceID.String(), db.Member{
		UserID: pgtype.UUID{Bytes: [16]byte(uuid.New()), Valid: true},
		Role:   "member",
	})
	...
	if recorder.Code != http.StatusOK {
		t.Fatalf("function-filtered evidence response = %d, want 200: %s", recorder.Code, recorder.Body.String())
	}
	var response struct {
		Evidence []struct {
	...
	if err := json.NewDecoder(recorder.Body).Decode(&response); err != nil {
		t.Fatalf("decode function-filtered evidence response: %v", err)
	}
	if len(response.Evidence) != 1 || response.Evidence[0].FunctionID != selectedFunctionID || response.Evidence[0].MetricID != selectedMetricID {
		t.Fatalf("function-filtered evidence response = %+v, want only selected function evidence", response.Evidence)
	}
}

func TestWorkspaceEvidenceReadModelFiltersByOperatingLoop(t *testing.T) {
	workspaceID := uuid.New()
	functionID := uuid.New()
	selectedLoopID := uuid.New()
	otherLoopID := uuid.New()
	...
	store := &recordingObservationStore{
		...
		operatingLoops: []OperatingLoop{
			{ID: selectedLoopID, WorkspaceID: workspaceID, FunctionID: functionID, Name: "Resolve customer needs", Active: true},
			{ID: otherLoopID, WorkspaceID: workspaceID, FunctionID: functionID, Name: "Review service quality", Active: true},
		},
		metrics: []Metric{
			{ID: selectedMetricID, WorkspaceID: workspaceID, OperatingLoopID: selectedLoopID, Name: "Resolved needs", Family: FamilyOutcome, Unit: "needs", Direction: DirectionIncrease, Source: "support", Active: true},
			{ID: otherMetricID, WorkspaceID: workspaceID, OperatingLoopID: otherLoopID, Name: "Quality score", Family: FamilyQuality, Unit: "score", Direction: DirectionIncrease, Source: "review", Active: true},
		},
		workspaceObservations: []Observation{
			{ID: uuid.New(), MetricID: selectedMetricID, PeriodStart: periodStart, PeriodEnd: periodEnd, Value: 15, EvidenceStatus: EvidenceMeasured, Confidence: 0.9, Source: "support", Method: "audited count", CreatedAt: periodEnd},
			{ID: uuid.New(), MetricID: otherMetricID, PeriodStart: periodStart, PeriodEnd: periodEnd, Value: 0.8, EvidenceStatus: EvidenceMeasured, Confidence: 0.9, Source: "review", Method: "quality review", CreatedAt: periodEnd},
		},
	}
	handler := NewHandler(NewService(store))
	router := chi.NewRouter()
	...
	req := httptest.NewRequest(http.MethodGet, "/api/cerebro/ai-impact/evidence?operating_loop_id="+selectedLoopID.String(), nil)
	ctx := middleware.SetMemberContext(req.Context(), workspaceID.String(), db.Member{
		UserID: pgtype.UUID{Bytes: [16]byte(uuid.New()), Valid: true},
		Role:   "member",
	})
	...
	if recorder.Code != http.StatusOK {
		t.Fatalf("operating-loop-filtered evidence response = %d, want 200: %s", recorder.Code, recorder.Body.String())
	}
	var response struct {
		Evidence []struct {
	...
	if err := json.NewDecoder(recorder.Body).Decode(&response); err != nil {
		t.Fatalf("decode operating-loop-filtered evidence response: %v", err)
	}
	if len(response.Evidence) != 1 || response.Evidence[0].OperatingLoopID != selectedLoopID || response.Evidence[0].MetricID != selectedMetricID {
		t.Fatalf("operating-loop-filtered evidence response = %+v, want only selected operating loop evidence", response.Evidence)
	}
}

func TestWorkspaceEvidenceReadModelFiltersByMetric(t *testing.T) {
	workspaceID := uuid.New()
	functionID := uuid.New()
	operatingLoopID := uuid.New()
	selectedMetricID := uuid.New()
	...
	store := &recordingObservationStore{
		...
		metrics: []Metric{
			{ID: selectedMetricID, WorkspaceID: workspaceID, OperatingLoopID: operatingLoopID, Name: "Resolved needs", Family: FamilyOutcome, Unit: "needs", Direction: DirectionIncrease, Source: "support", Active: true},
			{ID: otherMetricID, WorkspaceID: workspaceID, OperatingLoopID: operatingLoopID, Name: "Quality score", Family: FamilyQuality, Unit: "score", Direction: DirectionIncrease, Source: "review", Active: true},
		},
		workspaceObservations: []Observation{
			{ID: uuid.New(), MetricID: selectedMetricID, PeriodStart: periodStart, PeriodEnd: periodEnd, Value: 15, EvidenceStatus: EvidenceMeasured, Confidence: 0.9, Source: "support", Method: "audited count", CreatedAt: periodEnd},
			{ID: uuid.New(), MetricID: otherMetricID, PeriodStart: periodStart, PeriodEnd: periodEnd, Value: 0.8, EvidenceStatus: EvidenceMeasured, Confidence: 0.9, Source: "review", Method: "quality review", CreatedAt: periodEnd},
	...
	req := httptest.NewRequest(http.MethodGet, "/api/cerebro/ai-impact/evidence?metric_id="+selectedMetricID.String(), nil)
	ctx := middleware.SetMemberContext(req.Context(), workspaceID.String(), db.Member{
		UserID: pgtype.UUID{Bytes: [16]byte(uuid.New()), Valid: true},
		Role:   "member",
	})
	...
	if recorder.Code != http.StatusOK {
		t.Fatalf("metric-filtered evidence response = %d, want 200: %s", recorder.Code, recorder.Body.String())
	}
	var response struct {
		Evidence []struct {
			MetricID uuid.UUID `json:"metric_id"`
	...
	if err := json.NewDecoder(recorder.Body).Decode(&response); err != nil {
		t.Fatalf("decode metric-filtered evidence response: %v", err)
	}
	if len(response.Evidence) != 1 || response.Evidence[0].MetricID != selectedMetricID {
		t.Fatalf("metric-filtered evidence response = %+v, want only selected metric evidence", response.Evidence)
	}
}

func TestWorkspaceEvidenceReadModelFiltersByGuardrail(t *testing.T) {
	workspaceID := uuid.New()
	functionID := uuid.New()
	operatingLoopID := uuid.New()
	guardrailMetricID := uuid.New()
	regularMetricID := uuid.New()
	periodStart := time.Date(2026, time.July, 1, 0, 0, 0, 0, time.UTC)
	periodEnd := periodStart.Add(24 * time.Hour)
	store := &recordingObservationStore{
		functions: []Function{
		...
		metrics: []Metric{
			{ID: guardrailMetricID, WorkspaceID: workspaceID, OperatingLoopID: operatingLoopID, Name: "Reopen rate", Family: FamilyQuality, Unit: "percent", Direction: DirectionDecrease, Source: "support", Guardrail: true, Active: true},
			{ID: regularMetricID, WorkspaceID: workspaceID, OperatingLoopID: operatingLoopID, Name: "Resolved needs", Family: FamilyOutcome, Unit: "needs", Direction: DirectionIncrease, Source: "support", Active: true},
		},
		workspaceObservations: []Observation{
			{ID: uuid.New(), MetricID: guardrailMetricID, PeriodStart: periodStart, PeriodEnd: periodEnd, Value: 0.04, EvidenceStatus: EvidenceMeasured, Confidence: 0.9, Source: "support", Method: "audited rate", CreatedAt: periodEnd},
			{ID: uuid.New(), MetricID: regularMetricID, PeriodStart: periodStart, PeriodEnd: periodEnd, Value: 15, EvidenceStatus: EvidenceMeasured, Confidence: 0.9, Source: "support", Method: "audited count", CreatedAt: periodEnd},
		},
	}
	handler := NewHandler(NewService(store))
	router := chi.NewRouter()
	...
	req := httptest.NewRequest(http.MethodGet, "/api/cerebro/ai-impact/evidence?guardrail=true", nil)
	ctx := middleware.SetMemberContext(req.Context(), workspaceID.String(), db.Member{
		UserID: pgtype.UUID{Bytes: [16]byte(uuid.New()), Valid: true},
		Role:   "member",
	})
	...
	if recorder.Code != http.StatusOK {
		t.Fatalf("guardrail-filtered evidence response = %d, want 200: %s", recorder.Code, recorder.Body.String())
	}
	var response struct {
		Evidence []struct {
			MetricID uuid.UUID `json:"metric_id"`
	...
	if err := json.NewDecoder(recorder.Body).Decode(&response); err != nil {
		t.Fatalf("decode guardrail-filtered evidence response: %v", err)
	}
	if len(response.Evidence) != 1 || response.Evidence[0].MetricID != guardrailMetricID {
		t.Fatalf("guardrail-filtered evidence response = %+v, want only guardrail evidence", response.Evidence)
	}
}
