...
func TestParseIssueCommand(t *testing.T) {
	cases := []struct {
		...
		{"slash-joined token", "/issue/issue", false, "", ""},
		{"/new after /issue is a literal title", "/issue /new x", true, "/new x", ""},
		{"bare command with description lines", "/issue\nonly description", true, "", "only description"},
