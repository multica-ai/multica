...
func (r *Router) processClaimed(ctx context.Context, set ResolverSet, msg channel.InboundMessage, inst ResolvedInstallation, claimToken pgtype.UUID) (Result, dedupFinalize, error) {
	...
	// 6. A bare fresh-session command (ForceFresh with an empty body, e.g. a lone
	//    "/new") is a pure reset: EnsureSession has already rotated to a brand-new
	...
	//    EnsureSessionInput.Fresh (dingtalk + lark resolvers).
	bareReset := msg.ForceFresh && strings.TrimSpace(msg.Text) == ""
	if bareReset {
		return Result{
