...
func (r *OutboundReplier) Reply(ctx context.Context, inst engine.ResolvedInstallation, msg channel.InboundMessage, res engine.Result) {
	...
	case engine.OutcomeIngested:
		// A /issue-created message and a bare fresh-session reset each warrant a
		// short confirmation; a plain chat message stays silent (the agent's own
		// reply lands via EventChatDone).
		switch {
		case res.IssueID.Valid:
		...
		case res.FreshReset:
			if err := r.post(ctx, inst, msg, freshResetText); err != nil {
				r.logger.WarnContext(ctx, "dingtalk replier: fresh-reset confirmation failed",
					"installation_id", util.UUIDToString(inst.ID), "error", err)
			}
		}
	}
