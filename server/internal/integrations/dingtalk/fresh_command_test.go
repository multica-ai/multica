...
func TestParseFreshSessionCommand(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "slash-joined token rejected",
			body:      "/new/new",
			wantMatch: false,
		},
		{
			name:      "second /new stays literal (no recursive stripping)",
			body:      "/new /new /issue",
			wantMatch: true,
			wantBody:  "/new /issue",
		},
		{
			name:      "issue command on the next line survives stripping",
			body:      "/new\n/issue fix login",
			wantMatch: true,
			wantBody:  "/issue fix login",
		},
		{
