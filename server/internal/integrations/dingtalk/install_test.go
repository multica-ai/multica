...
type fakeInstallQueries struct {
	...
	// violation on the (channel_type, app_id) routing index — i.e. a LIVE owner
	// raced in after the reclaim probe.
	appIDTaken bool
	// reclaimOwner, when set, is the current owner of the robot's app_id returned
	// by GetChannelInstallationReclaimByAppID; nil returns pgx.ErrNoRows (free).
	reclaimOwner  *db.GetChannelInstallationReclaimByAppIDRow
	reclaimCalled bool
...
func (f *fakeInstallQueries) GetChannelInstallationReclaimByAppID(_ context.Context, _ db.GetChannelInstallationReclaimByAppIDParams) (db.GetChannelInstallationReclaimByAppIDRow, error) {
	f.reclaimCalled = true
	if f.reclaimOwner == nil {
		return db.GetChannelInstallationReclaimByAppIDRow{}, pgx.ErrNoRows
	}
	return *f.reclaimOwner, nil
}

func (f *fakeInstallQueries) DeleteChannelInstallation(_ context.Context, id pgtype.UUID) error {
	f.deleteCalled = true
	f.deletedID = id
	// A reclaimed dead row frees the routing key, so a later upsert no longer
	// collides.
	f.appIDTaken = false
	return nil
}
