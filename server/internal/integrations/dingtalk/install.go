...
var (
	...
	// ErrAppOwnedByAnotherWorkspace is returned when the pasted DingTalk robot is
	// already connected to a DIFFERENT agent or Multica workspace — it would
	// collide with the (channel_type, app_id) routing index. A DingTalk robot is
	// one bot identity and maps to one agent; reusing it elsewhere requires
	// disconnecting it there first.
	ErrAppOwnedByAnotherWorkspace = errors.New("dingtalk: this DingTalk robot is already connected to another agent or Multica workspace")
)
...
// the pasted DingTalk robot is already connected to a DIFFERENT agent or Multica
// workspace — refuse it (ErrAppOwnedByAnotherWorkspace) rather than steal it.
func (s *InstallService) persistInstall(ctx context.Context, p installPersist) (db.ChannelInstallation, error) {
	...
	if err != nil {
		...
		if errors.As(err, &pgErr) && pgErr.Code == pgUniqueViolation {
			return db.ChannelInstallation{}, ErrAppOwnedByAnotherWorkspace
		}
