...
// the row by (workspace, agent) and refuses the pair if that AppKey is already
// connected to another agent/workspace. The corp_id is left empty here and
...
func (s *InstallService) RegisterBYO(ctx context.Context, p RegisterBYOParams) (db.ChannelInstallation, error) {
	...
	// stored config carries the AppKey for inbound routing; persistInstall refuses
	// the pair if that AppKey is already connected to another agent/workspace.
