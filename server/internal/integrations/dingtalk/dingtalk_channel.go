...
func (c *dingtalkChannel) onMessage(ctx context.Context, data *botCallbackData) error {
	...
	if !ok {
		// The message never reaches the engine (non-text type, or no sender staff
		// id), so no channel_inbound_audit row is written for it. Log the drop so a
		// "the bot ignored my image/voice message" report is diagnosable instead of
		// vanishing silently. DingTalk only supports text (Capabilities is CapText).
		if data != nil {
	...
	//
	// A `/new` prefix consumes the command slot (inboundFromCallback stripped it
	// and set ForceFresh, leaving "/issue …" as the body), so the original first
	// line was "/new …", not "/issue …". Diverting here would drop ForceFresh and
	// leave the session un-rotated; instead let it reach the engine, which rotates
	// the session AND still creates the issue on its own /issue path.
	if c.issueCmd != nil && msg.AddressedToBot && !msg.ForceFresh {
