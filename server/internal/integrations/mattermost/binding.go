...
// This file is the Mattermost user-binding token flow, mirroring
// slack/binding.go and telegram/binding.go on the generic channel_* queries
// with channel_type='mattermost': an unbound Mattermost user who messages the
// bot gets a "link your account" prompt, clicks through to the in-product
// redeem page, and their Mattermost user id is bound to their Multica account.
...
// Mint creates a single-use binding token for (installation, mattermostUserID).
func (s *BindingTokenService) Mint(ctx context.Context, workspaceID, installationID pgtype.UUID, mattermostUserID string) (BindingToken, error) {
	...
	if _, err := s.q.CreateChannelBindingToken(ctx, db.CreateChannelBindingTokenParams{
		...
		ChannelUserID:  mattermostUserID,
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, multicaUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	if row.ChannelType != string(TypeMattermost) {
		// The token table is shared across channel adapters. Keep a token from
		// another adapter from being redeemed through Mattermost; returning
		// here rolls the consume back with the surrounding transaction.
		return RedeemedBindingToken{}, ErrBindingTokenInvalid
	}
