...
func TestSendOutcome_TellsARefusalFromAMissingAnswer(t *testing.T) {
	...
	cases := []struct {
		...
		{"a write that did not go out whole", fmt.Errorf("%w: %w", errFrameNotOnTheWire, &net.OpError{Op: "write", Err: errors.New("broken pipe")}), deliveryDefinitelyFailed, "one net.Conn.Write carries the whole frame and only errors when it could not write all of it, so what the peer holds is a truncated frame its application never receives"},
		{"wrapped write that did not go out whole", fmt.Errorf("send: %w", fmt.Errorf("%w: %w", errFrameNotOnTheWire, errors.New("broken pipe"))), deliveryDefinitelyFailed, "the classification must survive being wrapped on the way up"},
...
func TestSendMedia_AnAttemptedWriteIsNotProofOfNonDelivery(t *testing.T) {
	...
	if !errors.Is(err, errFrameNotOnTheWire) {
		t.Errorf("error does not carry errFrameNotOnTheWire, so callers cannot tell the stage: %v", err)
	}
	if got := sendOutcome(err); got != deliveryDefinitelyFailed {
		t.Errorf("sendOutcome = %s, want %s — the frame did not go out whole, so the peer's application never saw one",
			got, deliveryDefinitelyFailed)
	}
}
