...
// write the wecom_user_binding row.
...
// record the installer's wecom_user_binding row in the same business
...
//
// Implementations MUST be idempotent on (installation_id, wecom_userid):
...
type InstallerBindParams struct {
	...
	WecomUserid     Userid      // the installer's per-installation open_id
}
...
// and inserting the wecom_user_binding row commit together, so a
...
// to the intended recipient — WeCom DMs are encrypted in transit by
...
func (s *BindingTokenService) Mint(ctx context.Context, workspaceID, installationID pgtype.UUID, openID Userid) (BindingToken, error) {
	...
	if err := s.queries.CreateWecomBindingToken(ctx, db.CreateWecomBindingTokenParams{
		TokenHash:      hash,
		WorkspaceID:    workspaceID,
		InstallationID: installationID,
		WecomUserid:     string(openID),
...
// wecom_user_binding row in a single DB transaction. The redeemer's
...
// a WeCom open_id to an attacker's account.
//
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, multicaUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	row, err := qtx.ConsumeWecomBindingToken(ctx, hashToken(raw))
	if err != nil {
	...
	_, err = qtx.CreateWecomUserBinding(ctx, db.CreateWecomUserBindingParams{
	...
	if err != nil {
		...
		// comment on CreateWecomUserBinding in queries/lark.sql.
		if errors.Is(err, pgx.ErrNoRows) {
			return RedeemedBindingToken{}, ErrBindingAlreadyAssigned
		}
		// 23503 is foreign_key_violation. The relevant FK here is
		// wecom_user_binding_member_fk (workspace_id, multica_user_id)
	...
	return RedeemedBindingToken{
		WorkspaceID:    row.WorkspaceID,
		InstallationID: row.InstallationID,
		WecomUserid:     Userid(row.WecomUserid),
	}, nil
}
...
//
// The underlying CreateWecomUserBinding query is idempotent on
// (installation_id, wecom_userid) when multica_user_id matches (the
...
// frontend surfaces it as "this WeCom account is bound elsewhere",
...
func (s *BindingTokenService) BindInstallerTx(ctx context.Context, qtx *db.Queries, p InstallerBindParams) error {
	...
	_, err := q.CreateWecomUserBinding(ctx, db.CreateWecomUserBindingParams{
		WorkspaceID:    p.WorkspaceID,
		MulticaUserID:  p.MulticaUserID,
		InstallationID: p.InstallationID,
		WecomUserid:     string(p.WecomUserid),
	})
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
...
// wecom_user_binding row already exists for the (installation,
...
var ErrBindingAlreadyAssigned = errors.New("wecom userid is already bound to a different user")
