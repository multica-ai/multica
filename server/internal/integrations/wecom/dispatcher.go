...
// WeCom event JSON object looks like. This keeps event-schema changes
...
type InboundMessage struct {
	...
	// MessageType is the raw WeCom msg_type ("text", "post",
	...
	// milliseconds, as WeCom sends it). The enricher uses it to anchor the
	...
	// anchor WeCom also reports; we keep it for completeness but the
...
const (
	...
	// the "click here to bind" card. DispatchResult.SenderUserid and
...
type DispatchResult struct {
	...
	IssueTitle string
	// BindingLink is a one-shot Multica bind URL for OutcomeNeedsBinding.
	// Populated synchronously by the Hub before the WS adapter replies.
	BindingLink string
}
...
//
//   - ClaimWecomInboundDedup mints a fresh claim_token UUID on insert
//     and on stale-reclaim re-take. The token is the dispatcher's
//     ownership receipt for the row.
//
//   - MarkWecomInboundDedupProcessed and ReleaseWecomInboundDedup are
...
//     commit and Mark" window. See wecom_inbound_message_dedup comment
//     in 109_lark_integration.up.sql for the full invariant set.
type DispatcherQueries interface {
	GetWecomInstallationByBotID(ctx context.Context, botID string) (db.WecomInstallation, error)
	GetWecomUserBindingByUserid(ctx context.Context, arg db.GetWecomUserBindingByUseridParams) (db.WecomUserBinding, error)
	GetChatSession(ctx context.Context, id pgtype.UUID) (db.ChatSession, error)
	ClaimWecomInboundDedup(ctx context.Context, arg db.ClaimWecomInboundDedupParams) (db.WecomInboundMessageDedup, error)
	MarkWecomInboundDedupProcessed(ctx context.Context, arg db.MarkWecomInboundDedupProcessedParams) (int64, error)
	ReleaseWecomInboundDedup(ctx context.Context, arg db.ReleaseWecomInboundDedupParams) (int64, error)
	...
	AttachWecomOutboundStreamTask(ctx context.Context, arg db.AttachWecomOutboundStreamTaskParams) (int64, error)
}
...
type FlushReplyFunc func(ctx context.Context, inst db.WecomInstallation, msg InboundMessage, res DispatchResult)
...
// Handle processes one inbound WeCom message end-to-end. It never
...
//
//   - MarkWecomInboundDedupProcessed — a durable outcome was reached
//     (audit drop row persisted, OR chat_message + session touched).
//     Future replays of this message_id are dropped as duplicates.
//
//   - ReleaseWecomInboundDedup — an infra error occurred BEFORE any
...
func (d *Dispatcher) Handle(ctx context.Context, msg InboundMessage) (DispatchResult, error) {
	// 1. Route to installation. The bot_id is the only identifier
	...
	//    wecom_inbound_audit allowing a NULL installation_id.
	inst, err := d.Queries.GetWecomInstallationByBotID(ctx, msg.BotID)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			_ = d.Audit.RecordDrop(ctx, AuditDropParams{
				EventType:      msg.EventType,
				WecomMessageID: msg.MessageID,
				ChatID:         msg.ChatID,
	...
	//
	//    Empty MessageID means the event has no WeCom message_id at all
	...
	claimed := false
	if msg.MessageID != "" {
		claim, err := d.Queries.ClaimWecomInboundDedup(ctx, db.ClaimWecomInboundDedupParams{
...
func (d *Dispatcher) processClaimed(ctx context.Context, msg InboundMessage, inst db.WecomInstallation, claimToken pgtype.UUID) (DispatchResult, dedupFinalize, error) {
	...
	// 4. Identity check. A row in wecom_user_binding means the open_id
	//    maps to a current workspace member (the composite FK to
	//    member cascades the binding away on membership revocation).
	binding, err := d.Queries.GetWecomUserBindingByUserid(ctx, db.GetWecomUserBindingByUseridParams{
		InstallationID: inst.ID,
		WecomUserid:     string(msg.SenderUserid),
	})
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			_ = d.Audit.RecordDrop(ctx, AuditDropParams{
	...
	if err != nil {
		// chat_session create + wecom_chat_session_binding create are
...
func (d *Dispatcher) scheduleRun(inst db.WecomInstallation, msg InboundMessage, sessionID pgtype.UUID, initiatorUserID pgtype.UUID) {
...
func (d *Dispatcher) flushChatRun(inst db.WecomInstallation, msg InboundMessage, sessionID pgtype.UUID, initiatorUserID pgtype.UUID) {
	...
	task, err := d.TaskService.EnqueueChatTask(ctx, session, initiatorUserID)
	if err != nil {
		switch {
		...
		default:
			...
			d.logger().Error("wecom dispatcher: flush enqueue chat task failed",
				"chat_session_id", uuidString(sessionID),
				"err", err.Error(),
			)
		}
	...
	if d.Queries != nil {
		if _, attachErr := d.Queries.AttachWecomOutboundStreamTask(ctx, db.AttachWecomOutboundStreamTaskParams{
			ChatSessionID: sessionID,
			TaskID:        task.ID,
		}); attachErr != nil {
			d.logger().Warn("wecom dispatcher: attach outbound stream task failed",
				"chat_session_id", uuidString(sessionID),
...
func (d *Dispatcher) emitFlushReply(ctx context.Context, inst db.WecomInstallation, msg InboundMessage, sessionID pgtype.UUID, outcome Outcome) {
	if d.FlushReply == nil {
		return
	}
...
// ClaimWecomInboundDedup re-takes on retry. zero-rows-affected is the
...
func (d *Dispatcher) applyFinalize(ctx context.Context, installationID pgtype.UUID, messageID string, claimToken pgtype.UUID, action dedupFinalize) {
	switch action {
	case finalizeMark:
		_, _ = d.Queries.MarkWecomInboundDedupProcessed(ctx, db.MarkWecomInboundDedupProcessedParams{
			InstallationID: installationID,
			MessageID:      messageID,
			ClaimToken:     claimToken,
		})
	case finalizeRelease:
		_, _ = d.Queries.ReleaseWecomInboundDedup(ctx, db.ReleaseWecomInboundDedupParams{
			InstallationID: installationID,
			MessageID:      messageID,
			ClaimToken:     claimToken,
		})
...
func (d *Dispatcher) createIssueFromCommand(
	...
	params := service.IssueCreateParams{
		...
		OriginType:   pgtype.Text{String: originWecomChat, Valid: true},
		OriginID:     sessionID,
	}
	return d.IssueService.Create(ctx, params, service.IssueCreateOpts{})
}

// originWecomChat is the issue.origin_type label written for issues
// created via the WeCom `/issue` command. The analytics classifier in
