...
// finalizeSuccess runs the finalize transaction (spec §4). Seal happens
// outside the transaction so a slow DB pool never sits on the AES round
// keys under a row lock (mirrors lark finishSuccess). No BindInstallerTx
// call — WeCom scan-code does not carry a per-user identity.
func (w *InstallWorker) finalizeSuccess(ctx context.Context, row db.WecomInstallSession, leaseToken string, bot *BotInfo) error {
	...
	sealed, err := w.svc.cfg.Box.Seal([]byte(bot.Secret))
	if err != nil {
		w.failSession(ctx, row.ID, InstallErrorInternalError,
			"Failed to seal WeCom bot secret")
		return nil
	}
	sealedB64 := base64.StdEncoding.EncodeToString(sealed)

	tx, err := w.svc.tx.Begin(ctx)
	if err != nil {
	...
	if err != nil {
		...
		// aborted tx, using the base store).
		w.failSession(ctx, row.ID, InstallErrorInstallationConflict,
			"Could not finalize WeCom install (conflict)")
		return nil
	}

	rows, err := qtx.CompleteWecomInstallSession(ctx, db.CompleteWecomInstallSessionParams{
	...
	if rows == 0 {
		// Lost the lease — another worker took over. Roll back the upsert
		// via defer; nothing to publish.
		return nil
	}
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("finalize: commit: %w", err)
	}
	// Publish + supervisor notify AFTER commit so a rollback does not fire
	// a phantom installation created event.
	if w.bus != nil {
	...
	w.metrics.RecordInstallSessionTerminal(installResultSucceeded)
	w.svc.cfg.Logger.Info("wecom install: complete",
...
func (w *InstallWorker) failSession(ctx context.Context, id pgtype.UUID, reason, message string) {
	w.metrics.RecordInstallSessionTerminal(reason)
