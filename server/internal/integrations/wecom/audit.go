...
// wecom_inbound_audit. It deliberately holds no caches and no
// indirection — the RecordWecomInboundDrop query rejects any column
...
// wecom_inbound_audit table. The interface signature does not accept a
...
func (l *dbAuditLogger) RecordDrop(ctx context.Context, p AuditDropParams) error {
	return l.queries.RecordWecomInboundDrop(ctx, db.RecordWecomInboundDropParams{
		EventType:      p.EventType,
		DropReason:     string(p.Reason),
		InstallationID: p.InstallationID,
		WecomChatID:    textIfNonEmpty(string(p.ChatID)),
		WecomMessageID: textIfNonEmpty(p.WecomMessageID),
	})
}
