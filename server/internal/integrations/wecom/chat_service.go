...
// sole writer for this WeCom message_id.
var ErrClaimLost = errors.New("wecom dedup claim lost to a concurrent reclaim")

// isUniqueViolation reports whether err is a Postgres unique-violation
// (SQLSTATE 23505). The wecom_chat_session_binding
// UNIQUE (installation_id, wecom_chat_id) constraint surfaces this
// code when two concurrent first messages on the same WeCom chat race
...
//     (ClaimWecomInboundDedup + Mark/Release) BEFORE AppendUserMessage
...
// WeCom chat. The implementation is the two-phase find-or-create
// expected by the interface contract:
//
//  1. Look up the existing wecom_chat_session_binding.
//  2. If found, return its chat_session_id.
//  3. Otherwise, in one transaction: create chat_session +
//     wecom_chat_session_binding. Commit.
//
// The race between two concurrent first messages on the same Lark
// chat is resolved by the UNIQUE (installation_id, wecom_chat_id)
// constraint on wecom_chat_session_binding: the loser of the race
...
func (s *chatSessionService) EnsureChatSession(ctx context.Context, p EnsureChatSessionParams) (pgtype.UUID, error) {
	...
	existing, err := s.queries.GetWecomChatSessionBinding(ctx, db.GetWecomChatSessionBindingParams{
		InstallationID: p.InstallationID,
		WecomChatID:     string(p.ChatID),
	})
	if err == nil {
	...
	if isUniqueViolation(err) {
		existing, lookupErr := s.queries.GetWecomChatSessionBinding(ctx, db.GetWecomChatSessionBindingParams{
			InstallationID: p.InstallationID,
			WecomChatID:     string(p.ChatID),
		})
		if lookupErr == nil {
...
func (s *chatSessionService) createSessionAndBinding(ctx context.Context, p EnsureChatSessionParams) (pgtype.UUID, error) {
	...
	if _, err := qtx.CreateWecomChatSessionBinding(ctx, db.CreateWecomChatSessionBindingParams{
		ChatSessionID:  session.ID,
		InstallationID: p.InstallationID,
		WecomChatID:     string(p.ChatID),
		WecomChatType:   string(p.ChatType),
	}); err != nil {
		return pgtype.UUID{}, err
	}
...
// ClaimWecomInboundDedup gate:
//
//  1. BEFORE this method is called, the Dispatcher claims an in-flight
//     row in wecom_inbound_message_dedup and gets back a `claim_token`.
...
//     ClaimToken is supplied, we run MarkWecomInboundDedupProcessed
...
//     lands for the same WeCom message_id.
//
...
func (s *chatSessionService) AppendUserMessage(ctx context.Context, p AppendUserMessageParams) (AppendResult, error) {
	...
	markedInTx := false
	if p.ClaimToken.Valid && p.WecomMessageID != "" {
		rows, err := qtx.MarkWecomInboundDedupProcessed(ctx, db.MarkWecomInboundDedupProcessedParams{
			InstallationID: p.InstallationID,
			MessageID:      p.WecomMessageID,
...
func defaultSessionTitle(t ChatType) string {
	switch t {
	case ChatTypeGroup:
		return "WeCom group chat"
	case ChatTypeSingle:
		return "WeCom direct message"
	default:
		return "WeCom chat"
	}
}
