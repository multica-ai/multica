...
type HubQueries interface {
	ListActiveWecomInstallations(ctx context.Context) ([]db.WecomInstallation, error)
	AcquireWecomWSLease(ctx context.Context, arg db.AcquireWecomWSLeaseParams) (db.WecomInstallation, error)
	ReleaseWecomWSLease(ctx context.Context, arg db.ReleaseWecomWSLeaseParams) error
}
...
//     to send the respective copy as a WeCom card; the chat_message
...
// connector owns the actual wire protocol — opening the WeCom long
...
type EventConnector interface {
	Run(ctx context.Context, inst db.WecomInstallation, emit EventEmitter) error
}
...
type ConnectorFactory func(inst db.WecomInstallation) (EventConnector, error)
...
type HubConfig struct {
	// LeaseTTL is how long a successful AcquireWecomWSLease grant is
	...
	// LeaseReleaseTimeout caps how long a single ReleaseWecomWSLease
...
// long-running WeCom connection per active installation. It enforces
...
type Hub struct {
	...
	// bindingSvc + publicURL mint the one-shot bind link synchronously
	// for OutcomeNeedsBinding so the WS adapter can include it in the
	// stream reply before the async OutcomeReplier runs.
	bindingSvc  *BindingTokenService
	publicURL   string
	bindingPath string

	// nodeID is the per-process lease ownership token. The CAS
	// predicate on AcquireWecomWSLease treats matching tokens as
	...
	// row changes (re-scan creates a fresh bot → new bot_id / new
	...
	// against the OLD bot's bot_id and the new bot silently goes dark.
	...
	// hung outbound WeCom HTTP call cannot block shutdown beyond the
	// timeout.
	replyWg sync.WaitGroup

	// activeStreams maps installation id → the connector currently
	// holding the WS lease on this replica. The outbound Patcher sends
	// agent replies through this map.
	streamMu      sync.RWMutex
	activeStreams map[string]StreamMessenger
}
...
// bot_id / app_secret) and force a restart; gen is a monotonic
...
func NewHub(queries HubQueries, factory ConnectorFactory, dispatcher *Dispatcher, cfg HubConfig) *Hub {
	...
	return &Hub{
		...
		activeStreams: make(map[string]StreamMessenger),
	}
}
...
// SetBindingPromptConfig wires the token minter and public URL used to
// enrich OutcomeNeedsBinding with a BindingLink before the WS adapter
// replies. Must be called before Run.
func (h *Hub) SetBindingPromptConfig(svc *BindingTokenService, publicURL, bindingPath string) {
	h.bindingSvc = svc
	h.publicURL = strings.TrimRight(strings.TrimSpace(publicURL), "/")
	h.bindingPath = strings.TrimSpace(bindingPath)
}

func (h *Hub) enrichBindingLink(ctx context.Context, inst db.WecomInstallation, msg InboundMessage, res DispatchResult, log *slog.Logger) DispatchResult {
	if h.bindingSvc == nil || h.publicURL == "" {
		log.Warn("wecom hub: binding link skipped (binding service or PUBLIC_URL unset)")
		return res
	}
	path := h.bindingPath
	if path == "" {
	...
	tok, err := h.bindingSvc.Mint(ctx, inst.WorkspaceID, inst.ID, msg.SenderUserid)
	if err != nil {
		log.Error("wecom hub: mint binding token failed", "error", err)
		return res
	}
	res.BindingLink = fmt.Sprintf("%s%s?token=%s", h.publicURL, path, tok.Raw)
	log.Info("wecom hub: binding link minted",
		"installation_id", uuidString(inst.ID),
		"wecom_userid", string(msg.SenderUserid),
		"bind_url", res.BindingLink,
	)
	return res
}
...
// device flow creates a fresh bot_id / app_secret) are cancelled and
...
func (h *Hub) sweep(ctx context.Context) {
	rows, err := h.queries.ListActiveWecomInstallations(ctx)
	if err != nil {
...
// own clock.
func (h *Hub) maybeRestartOnRotation(id string, row db.WecomInstallation) {
...
func (h *Hub) startSupervisor(parent context.Context, inst db.WecomInstallation) {
...
// include bot_id (which WeCom rotates on every device-flow registration),
// bot_open_id (sanity — protects against an bot_id reuse with a
...
func installationFingerprint(inst db.WecomInstallation) string {
	sum := sha256.Sum256(inst.BotSecretEncrypted)
	corpSum := sha256.Sum256(inst.CorpSecretEncrypted)
	return inst.BotID + "|" + inst.CorpID + "|" + hex.EncodeToString(sum[:]) + "|" + hex.EncodeToString(corpSum[:])
}
...
func (h *Hub) supervise(ctx context.Context, inst db.WecomInstallation, id string, gen uint64) {
	defer h.wg.Done()
	defer func() {
...
func (h *Hub) acquireLease(ctx context.Context, instID pgtype.UUID, token string) (bool, error) {
	...
	_, err := h.queries.AcquireWecomWSLease(ctx, db.AcquireWecomWSLeaseParams{
		ID:           instID,
...
// blocked on its wire I/O and continue consuming WeCom events until
...
func (h *Hub) releaseLease(instID pgtype.UUID, token string) {
	...
	defer cancel()
	if err := h.queries.ReleaseWecomWSLease(ctx, db.ReleaseWecomWSLeaseParams{
		ID:           instID,
...
// can do real WeCom HTTP calls — token mint, card send — each with
...
// for >3s WeCom treats the event as un-ACKed and re-pushes it; by then
...
func (h *Hub) handleEvent(ctx context.Context, inst db.WecomInstallation, log *slog.Logger, msg InboundMessage) (DispatchResult, error) {
	if h.dispatcher == nil {
	...
	if err != nil {
		log.Error("wecom hub: dispatcher error",
			"event_id", msg.EventID,
			"error", err,
		)
		return res, err
	...
	if res.Outcome == OutcomeNeedsBinding {
		res = h.enrichBindingLink(ctx, inst, msg, res, log)
	}
	log.Debug("wecom hub: dispatch outcome",
		"event_id", msg.EventID,
...
func (h *Hub) scheduleReply(inst db.WecomInstallation, msg InboundMessage, res DispatchResult, log *slog.Logger) {
	r := h.replier
	if r == nil {
		return
	}
...
var ErrDispatcherNotConfigured = errors.New("wecom hub: dispatcher not configured")
...
// AcquireWecomWSLease surfaces "lease held by someone else" as a
...
// HubStreamRegistry forwards StreamRegistry calls to a Hub pointer
// assigned after construction (the factory needs the registry before
// the Hub exists).
type HubStreamRegistry struct {
	Hub *Hub
}

func (r *HubStreamRegistry) RegisterStreamMessenger(instID pgtype.UUID, m StreamMessenger) {
	if r != nil && r.Hub != nil {
		r.Hub.RegisterStreamMessenger(instID, m)
	}
}

func (r *HubStreamRegistry) UnregisterStreamMessenger(instID pgtype.UUID, m StreamMessenger) {
	if r != nil && r.Hub != nil {
		r.Hub.UnregisterStreamMessenger(instID, m)
	}
}

// RegisterStreamMessenger records the active WS connector for an
// installation. Called by WSConnector on Run entry; unregistered on exit.
func (h *Hub) RegisterStreamMessenger(instID pgtype.UUID, m StreamMessenger) {
	if h == nil || m == nil {
		return
	}
	id := uuidString(instID)
	h.streamMu.Lock()
	h.activeStreams[id] = m
	h.streamMu.Unlock()
}

// UnregisterStreamMessenger removes a connector if it is still the
// registered messenger for the installation.
func (h *Hub) UnregisterStreamMessenger(instID pgtype.UUID, m StreamMessenger) {
	if h == nil || m == nil {
		return
	}
	id := uuidString(instID)
	h.streamMu.Lock()
	if h.activeStreams[id] == m {
		delete(h.activeStreams, id)
	}
	h.streamMu.Unlock()
}

// SendStreamReply implements StreamSender. Delivers a stream frame on
// the installation's active long connection, if this replica holds it.
func (h *Hub) SendStreamReply(_ context.Context, instID pgtype.UUID, reqID, streamID, content string, finish bool) error {
	if h == nil {
		return ErrNoActiveWecomConnection
	}
	h.streamMu.RLock()
	m := h.activeStreams[uuidString(instID)]
	h.streamMu.RUnlock()
	if m == nil {
		return ErrNoActiveWecomConnection
	}
	if err := m.SendStreamUpdate(reqID, streamID, content, finish); err != nil {
		return err
	}
	return nil
}
