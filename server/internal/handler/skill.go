...
func skillToResponse(s db.Skill) SkillResponse {
	...
	return SkillResponse{
		...
		// CEREBRO-PATCH(skill-ownership): ownership / approval / versioning.
...
func (h *Handler) CreateSkill(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := h.createSkillWithFiles(r.Context(), skillCreateInput{
		...
		Config:      req.Config,
		Files:       req.Files,
		// CEREBRO-PATCH(skill-create-owner): owner defaults to creator inside
		// createSkillWithFiles when unset.
		OwnerID: creatorUUID,
	})
	if err != nil {
...
func (h *Handler) UpdateSkill(w http.ResponseWriter, r *http.Request) {
	...
	// CEREBRO-PATCH(skill-metadata-sync-update): TECH-3077 — sync metadata on content change.
	if req.Content != nil {
		h.syncSkillMetadataAsync(skill.ID, skill.Content)
	}
