...
func TestBatchUpdateNoMutationReturnsZero(t *testing.T) {
	...
	a := createTestIssueBatch(t, "BU-no-mut A", "todo", "low")
	b := createTestIssueBatch(t, "BU-no-mut B", "todo", "low")
...
func TestBatchUpdateValidUpdatesPersistAndCount(t *testing.T) {
	a := createTestIssueBatch(t, "BU-ok A", "todo", "low")
	b := createTestIssueBatch(t, "BU-ok B", "todo", "low")
	t.Cleanup(func() { deleteTestIssue(t, a) })
	t.Cleanup(func() { deleteTestIssue(t, b) })
...
// createTestIssueBatch is a small helper to keep the table-driven cases clean.
// Returns the new issue's id; caller is responsible for cleanup.
func createTestIssueBatch(t *testing.T, title, status, priority string) string {
	t.Helper()
	w := httptest.NewRecorder()
