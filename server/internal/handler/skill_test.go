...
func TestFetchFromSkillsSh_UsesEntryURLForNestedDirectories(t *testing.T) {
	...
	if !skillContainsString(*requests, "api.github.com /repos/acme/skills/contents/skills/pptx/scripts?ref=main") {
		t.Fatalf("expected scripts directory to be fetched via entry.URL, got requests %v", *requests)
	}
	if skillContainsString(*requests, "api.github.com /repos/acme/skills/contents/skills/pptx?ref=main/scripts") {
...
func TestFetchFromSkillsSh_FallbackDoesNotDoubleEscapeDirectoryNames(t *testing.T) {
	...
	if !skillContainsString(*requests, "api.github.com /repos/acme/skills/contents/skills/pptx/my%20dir?ref=main") {
...
func TestFetchFromSkillsSh_ResolvesAliasedSkillNamesViaFrontmatter(t *testing.T) {
	...
	if !skillContainsString(*requests, "api.github.com /repos/vercel-labs/agent-skills/git/trees/main?recursive=1") {
...
func TestFetchFromSkillsSh_ResolvesRootLevelSkillMd(t *testing.T) {
	...
	if !skillContainsString(*requests, "api.github.com /repos/alchaincyf/huashu-design/contents?ref=master") {
...
func TestFetchFromSkillsSh_RootSkillMdFastPathSkipsFrontmatterMismatch(t *testing.T) {
	...
	if !skillContainsString(*requests, "api.github.com /repos/acme/multi/git/trees/main?recursive=1") {
...
func TestFetchFromSkillsSh_ReturnsActionableErrorForTruncatedTrees(t *testing.T) {
	...
	if !skillContainsString(*requests, "api.github.com /repos/acme/skills/contents/skills?ref=main") {
...
func TestFetchFromSkillsSh_AnthropicPptxIntegration(t *testing.T) {
	...
	for _, want := range []string{
		...
		if !skillContainsString(gotPaths, want) {
