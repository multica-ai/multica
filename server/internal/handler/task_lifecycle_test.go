...
func TestRecoverOrphanedTasks_RequeuesDeadSession(t *testing.T) {
	...
	runtimeID := createClaimReclaimRuntime(t, ctx, "Recover orphan runtime")
	agentID, issueID := createClaimReclaimAgentAndIssue(t, ctx, runtimeID, "Recover orphan agent")
	...
	if err := testPool.QueryRow(ctx, `SELECT status FROM agent WHERE id = $1`, agentID).Scan(&beforeAgent); err != nil {
		t.Fatalf("read agent before: %v", err)
	}
	if err := testPool.QueryRow(ctx, `SELECT status FROM issue WHERE id = $1`, issueID).Scan(&beforeIssue); err != nil {
		t.Fatalf("read issue before: %v", err)
	}
	if err := testPool.QueryRow(ctx, `SELECT status FROM agent_task_queue WHERE id = $1`, taskID).Scan(&beforeTask); err != nil {
	...
	`, issueID).Scan(&beforeActive); err != nil {
	...
	req := newDaemonTokenRequest("POST", "/api/daemon/runtimes/"+runtimeID+"/recover-orphans", nil, testWorkspaceID, "recover-orphans-test")
	...
	t.Logf("after recovery: agent=%s issue=%s task=%s failure=%s active=%d queued=%d failed=%d",
		afterAgent, afterIssue, afterTask, afterFailure, afterActive, afterQueued, afterFailed)
...
func TestRecoverOrphanedTasks_ConcurrentRequestsStayAtMostOnce(t *testing.T) {
	...
	ctx := context.Background()
	runtimeID := createClaimReclaimRuntime(t, ctx, "Recover concurrent runtime")
	agentID, issueID := createClaimReclaimAgentAndIssue(t, ctx, runtimeID, "Recover concurrent agent")
	if _, err := testPool.Exec(ctx, `UPDATE agent_runtime SET last_seen_at = now() - interval '10 minutes' WHERE id = $1`, runtimeID); err != nil {
		t.Fatalf("age runtime: %v", err)
	}
	t.Cleanup(func() {
	...
	for i := 0; i < 2; i++ {
		...
		go func() {
			...
			<-start
			w := httptest.NewRecorder()
			req := newDaemonTokenRequest("POST", "/api/daemon/runtimes/"+runtimeID+"/recover-orphans", nil, testWorkspaceID, "recover-orphans-concurrent")
			req = withURLParams(req, "runtimeId", runtimeID)
			testHandler.RecoverOrphanedTasks(w, req)
			var resp struct {
				Orphaned int `json:"orphaned"`
	...
	if err := testPool.QueryRow(ctx, `
		...
		FROM agent_task_queue
		WHERE issue_id = $1 AND status = 'failed'
	`, issueID).Scan(&failedCount); err != nil {
	...
	if err := testPool.QueryRow(ctx, `
		...
		FROM agent_task_queue
		WHERE issue_id = $1 AND status = 'queued'
	`, issueID).Scan(&queuedCount); err != nil {
