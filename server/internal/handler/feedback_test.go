...
func TestCreateFeedbackRateLimit(t *testing.T) {
	...
	for i := 0; i < feedbackHourlyRateLimit; i++ {
		req := newRequest("POST", "/api/feedback", CreateFeedbackRequest{Message: "feedback #" + strconv.Itoa(i)})
		w := httptest.NewRecorder()
		testHandler.CreateFeedback(w, req)
		if w.Code != http.StatusCreated {
...
func clearFeedbackForTestUser(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = testPool.Exec(context.Background(), `DELETE FROM feedback WHERE user_id = $1`, parseUUID(testUserID))
	})
}
