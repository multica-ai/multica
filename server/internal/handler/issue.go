...
// GET request line. The body uses the same string encodings as query params.
...
func (h *Handler) ListIssues(w http.ResponseWriter, r *http.Request) {
	...
	// CEREBRO-PATCH(nested-projects): project filters expand to descendants when enabled.
	...
	if p := r.URL.Query().Get("project_id"); p != "" {
		...
		projectIDsFilter = []pgtype.UUID{id}
		if row, err := h.Queries.GetProjectNesting(ctx, id); err != nil || row.ShowDescendants {
			if ids, err := h.Queries.ListProjectAndDescendantIDs(ctx, id); err == nil && len(ids) > 0 {
				projectIDsFilter = ids
			}
		}
	}
	...
	where := []string{"i.workspace_id = $1", "i.kind = 'issue'"}
	args := []any{wsUUID}
	...
	// CEREBRO-PATCH(list-issues-access-filter): privacy + project visibility
	// (JEH-1749 / JEH-1009). Mirrors sqlc ListIssues' access predicate so the
	// server-side-sort rewrite cannot leak private or restricted-project
	// issues. Admins bypass; a standalone private issue is creator-only; a
	// project issue needs workspace access, project membership, or a cerebro
	// project-group grant.
	if !isWorkspaceAdmin(member) {
		userRef := addArg(member.UserID)
		where = append(where, fmt.Sprintf(`(
    (i.project_id IS NULL AND (
        i.is_private = FALSE
        OR (i.creator_type = 'member' AND i.creator_id = %[1]s::uuid)
    ))
    OR (i.project_id IS NOT NULL AND (
        p.access = 'workspace'
        OR EXISTS (SELECT 1 FROM project_member pm WHERE pm.project_id = p.id AND pm.user_id = %[1]s::uuid)
        OR EXISTS (SELECT 1 FROM cerebro_project_group_member pgm JOIN cerebro_group_member gm ON gm.group_id = pgm.group_id WHERE pgm.project_id = p.id AND gm.user_id = %[1]s::uuid)
    ))
)`, userRef))
	}
	...
	// CEREBRO-PATCH(issue-reference-filter): cerebro_issue_reference edge filter.
	if referenceObjectFilter.Valid {
		where = append(where, fmt.Sprintf(
...
LIMIT %s OFFSET %s`, whereSql, orderBy, limitRef, offsetRef)
	...
	// CEREBRO-PATCH(count-issues-access-filter): count reuses the list query's
	// WHERE (privacy + project visibility, JEH-1749) and therefore the same
	// project LEFT JOIN.
	countQuery := fmt.Sprintf(`SELECT COUNT(*) FROM issue i LEFT JOIN project p ON p.id = i.project_id WHERE %s`, whereSql)
...
func (h *Handler) CreateIssue(w http.ResponseWriter, r *http.Request) {
	...
	analyticsPlatform, _, _ := middleware.ClientMetadataFromContext(r.Context())
...
func (h *Handler) UpdateIssue(w http.ResponseWriter, r *http.Request) {
	...
	// CEREBRO-PATCH(issue-update-privacy): standalone-issue privacy. Only
	// set when caller has access. The loadIssueForUser check above already
	// gates this — non-creator, non-admin members get 404 before reaching
	// here.
	if req.IsPrivate != nil {
		params.IsPrivate = pgtype.Bool{Bool: *req.IsPrivate, Valid: true}
	}

	// CEREBRO-PATCH(issue-update-agent-visibility): enforce agent visibility
	// — private agents can only be assigned by owner/admin.
...
func (h *Handler) validateAssigneePair(ctx context.Context, r *http.Request, workspaceID string, assigneeType pgtype.Text, assigneeID pgtype.UUID) (int, string) {
	...
	case "squad":
		...
		if status, msg := h.cerebroAgentAccessAsValidatorError(ctx, r, workspaceID, leader.ID, leader.OwnerID); status != 0 {
			return status, msg
		}
		actorType, actorID := h.resolveActor(r, requestUserID(r), workspaceID)
...
func (h *Handler) DeleteIssue(w http.ResponseWriter, r *http.Request) {
	...
	// CEREBRO-PATCH(issue-delete-audience): restrict fan-out to project
	// members for private projects.
	resolvedID := uuidToString(issue.ID)
	h.publishToAudience(protocol.EventIssueDeleted, uuidToString(issue.WorkspaceID), actorType, actorID, map[string]any{"issue_id": resolvedID}, h.audienceForIssue(r.Context(), issue))
