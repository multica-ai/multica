...
type IssueResponse struct {
	...
	TimeTracking  *IssueTimerSummaryResponse `json:"time_tracking,omitempty"`
...
func (h *Handler) GetIssue(w http.ResponseWriter, r *http.Request) {
	...
	if userID := requestUserID(r); userID != "" {
		actorType, actorIDString := h.resolveActor(r, userID, uuidToString(issue.WorkspaceID))
		if actorID, err := util.ParseUUID(actorIDString); err == nil {
			summary := h.issueTimerSummary(r, issue, actorType, actorID)
			resp.TimeTracking = &summary
		}
	}
