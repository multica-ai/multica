...
// the agent's creator plus a short greeting, then enqueues a real agent run so
// the agent introduces itself — the intro is LLM-generated by the agent, not a
// static template. Best effort: any failure is logged and never blocks the
// (already-committed) agent creation.
func (h *Handler) sendAgentWelcomeChat(ctx context.Context, agent db.Agent, creatorID, workspaceID string) {
	...
	const welcomePrompt = "👋 Hi! Please introduce yourself — who you are, what you're good at, and how I can work with you."
	...
	if err != nil {
		slog.Warn("agent welcome: create message failed", "chat_session_id", uuidToString(session.ID), "error", err)
		return
	}

	task, err := h.TaskService.EnqueueChatTask(ctx, session, parseUUID(creatorID), false)
	if err != nil {
		slog.Warn("agent welcome: enqueue task failed", "chat_session_id", uuidToString(session.ID), "error", err)
		return
	}
	if err := h.Queries.LinkChatMessageToTask(ctx, db.LinkChatMessageToTaskParams{ID: msg.ID, TaskID: task.ID}); err != nil {
		slog.Warn("agent welcome: link message to task failed", "chat_session_id", uuidToString(session.ID), "error", err)
	}

	sid := uuidToString(session.ID)
	h.publishChat(protocol.EventChatMessage, workspaceID, "member", creatorID, sid, protocol.ChatMessagePayload{
		ChatSessionID: sid,
