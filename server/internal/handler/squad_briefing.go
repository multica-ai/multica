...
// CEREBRO-PATCH(squad-parent-status): MUL-5156 — the operating protocol is split
// so responsibility 6 (parent issue status) can vary by whether this squad owns
// the issue. Header + hard rules are unchanged upstream text; squadParentStatus*
// and squadOperatingProtocolFor are the ported addition. See docs/cerebro-patches.md.
//
// squadOperatingProtocolHeader is the hard-coded system-level briefing prepended
// to every squad-leader claim. It explains the leader's coordinator role, the
...
// CEREBRO-PATCH(squad-parent-status): MUL-5156 — squadParentStatusOwned is
// responsibility 6 for the case where the issue this leader was woken on is
// assigned to THIS squad. Only then does the leader own the parent's status arc.
//
...
// name — keep the heading text in sync with cerebroCommentStatusStep.
...
// CEREBRO-PATCH(squad-parent-status): MUL-5156 — squadParentStatusNotOwned is
// responsibility 6 for every other leader path: an @squad mention on an issue
// owned by someone else (MUL-3724), and quick-create, where no issue exists yet
// on this turn. Granting status ownership there would let a squad that was
// merely pulled in to answer a question push another assignee's in-flight issue
// to in_review.
...
// CEREBRO-PATCH(squad-parent-status): MUL-5156 — hard rules split out of the
// header const so responsibility 6 can be injected between responsibility 5 and
// here by squadOperatingProtocolFor.
...
// CEREBRO-PATCH(squad-parent-status): MUL-5156 — squadOperatingProtocolFor
// assembles the protocol, selecting the parent-status responsibility that
// matches this leader's actual authority over the issue.
...
func buildSquadLeaderBriefing(ctx context.Context, q *db.Queries, squad db.Squad, ownsIssueStatus bool) string { // CEREBRO-PATCH(squad-parent-status): MUL-5156 thread ownsIssueStatus through
	var sb strings.Builder
	sb.WriteString(squadOperatingProtocolFor(ownsIssueStatus)) // CEREBRO-PATCH(squad-parent-status): MUL-5156 select status responsibility
	sb.WriteString("\n\n")
