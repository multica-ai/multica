...
// isTextPreviewable is the whitelist for the text preview proxy. Its lookup
// tables and the client-side mirror are generated from text_preview_types.json.
// Edit that source and run `pnpm generate:text-preview-types`.
//
...
func isTextPreviewable(contentType, filename string) bool {
	...
	if _, ok := textPreviewContentTypes[ct]; ok {
		return true
	}

	ext := strings.TrimPrefix(strings.ToLower(path.Ext(filename)), ".")
	if _, ok := textPreviewExtensions[ext]; ok {
		return true
	}

	base := strings.ToLower(path.Base(filename))
	_, ok := textPreviewBasenames[base]
	return ok
}
