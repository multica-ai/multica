...
func TestListTasksByIssue_ReturnsExecutionHistoryAcrossOwners(t *testing.T) {
	...
	// Create a second workspace member to simulate "other owner".
	otherUserID := createWorkspaceMemberUser(t, "Other Owner", "other-owner-filter@test.multica.ai")

	// Create an issue in the test workspace.
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ($1, 'other-owner-agent', '', 'cloud', '{}'::jsonb, $2, 'private', 1, $3, '', '{}'::jsonb, '[]'::jsonb, NULL)
		RETURNING id
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO agent (workspace_id, name, description, runtime_mode, runtime_config,
			runtime_id, visibility, max_concurrent_tasks, owner_id,
			instructions, custom_env, custom_args, mcp_config)
		VALUES ($1, 'workspace-agent', '', 'cloud', '{}'::jsonb, $2, 'private', 1, NULL, '', '{}'::jsonb, '[]'::jsonb, NULL)
		RETURNING id
	`, testWorkspaceID, runtimeID).Scan(&wsAgentID); err != nil {
		t.Fatalf("create workspace agent: %v", err)
	}
	t.Cleanup(func() { testPool.Exec(ctx, `DELETE FROM agent WHERE id = $1`, wsAgentID) })

	wsTaskID := createHandlerTestTaskForAgentOnIssue(t, wsAgentID, issue.ID)
	...
	// -- Call the handler as testUserID --
	w := httptest.NewRecorder()
	req := newRequest("GET", "/api/issues/"+issue.ID+"/task-runs", nil)
...
func TestFailTask_CommentTriggered_CreatesSystemCommentUnderTrigger(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO issue (workspace_id, title, status, priority, creator_id, creator_type, number, position)
		VALUES ($1, 'fail task system comment fixture', 'in_progress', 'medium', $2, 'member', 92002, 0)
		RETURNING id
	`, testWorkspaceID, testUserID).Scan(&issueID); err != nil {
		t.Fatalf("setup: create issue: %v", err)
	}
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO comment (issue_id, workspace_id, author_type, author_id, content, type)
		VALUES ($1, $2, 'member', $3, 'trigger for fail task', 'comment')
		RETURNING id
	`, issueID, testWorkspaceID, testUserID).Scan(&triggerCommentID); err != nil {
		t.Fatalf("setup: create trigger comment: %v", err)
	}
	...
	const failMsg = "private agent execution denied: only the agent owner can run this task"

	w := httptest.NewRecorder()
	req := newDaemonTokenRequest("POST", "/api/daemon/tasks/"+taskID+"/fail",
		map[string]any{"error": failMsg},
		testWorkspaceID, "legit-daemon")
	...
	if err := testPool.QueryRow(ctx, `
		SELECT status, error FROM agent_task_queue WHERE id = $1
	`, taskID).Scan(&status, &storedError); err != nil {
	...
	if err := testPool.QueryRow(ctx, `
		SELECT content, type, parent_id::text
		FROM comment
		WHERE issue_id = $1 AND author_type = 'agent' AND author_id = $2
		ORDER BY created_at DESC
		LIMIT 1
	`, issueID, agentID).Scan(&commentContent, &commentType, &parentID); err != nil {
