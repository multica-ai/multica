...
// badge with no reply.
// CEREBRO-PATCH(orphan-task-test): Regression test for JEH-357: a task whose
// parent (chat_session, autopilot_run, or workspace) was deleted ends up
// with every FK NULL thanks to ON DELETE SET NULL. The handler must:
//  1. Return 200 with task=null (so the daemon polls for the next task).
//  2. Mark the orphan as 'failed' inline with a clear error so the queue
//     is not blocked and the operator can see why.
func TestClaimTask_OrphanTask_FailsAndReturnsNull(t *testing.T) {
	...
	var orphanAgentID, orphanRuntimeID string
	if err := testPool.QueryRow(ctx, `
		SELECT a.id, a.runtime_id FROM agent a WHERE a.workspace_id = $1 LIMIT 1
	`, testWorkspaceID).Scan(&orphanAgentID, &orphanRuntimeID); err != nil {
		t.Fatalf("setup: get agent: %v", err)
	}
	...
	if err := testPool.QueryRow(ctx, `
		...
		RETURNING id
	`, orphanAgentID, orphanRuntimeID).Scan(&orphanTaskID); err != nil {
		t.Fatalf("setup: create orphan task: %v", err)
	}
	defer testPool.Exec(ctx, `DELETE FROM agent_task_queue WHERE id = $1`, orphanTaskID)

	w := httptest.NewRecorder()
	req := newDaemonTokenRequest("POST", "/api/daemon/runtimes/"+orphanRuntimeID+"/claim", nil,
		testWorkspaceID, "test-daemon-orphan")
	rctx := chi.NewRouteContext()
	rctx.URLParams.Add("runtimeId", orphanRuntimeID)
	req = req.WithContext(context.WithValue(req.Context(), chi.RouteCtxKey, rctx))

	testHandler.ClaimTaskByRuntime(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("ClaimTaskByRuntime (orphan): expected 200, got %d: %s", w.Code, w.Body.String())
	}

	var orphanResp struct {
		Task *struct {
			ID        string `json:"id"`
			TaskToken string `json:"task_token"`
		} `json:"task"`
	}
	if err := json.NewDecoder(w.Body).Decode(&orphanResp); err != nil {
		t.Fatalf("decode response: %v", err)
	}
	if orphanResp.Task != nil {
		t.Fatalf("orphan task: expected task=nil in response, got %+v", orphanResp.Task)
	}

	// The orphan must be marked failed so the daemon doesn't keep claiming
	// it on every poll cycle.
	var orphanStatus, orphanErrMsg string
	if err := testPool.QueryRow(ctx,
		`SELECT status, COALESCE(error, '') FROM agent_task_queue WHERE id = $1`,
		orphanTaskID,
	).Scan(&orphanStatus, &orphanErrMsg); err != nil {
		t.Fatalf("post-check: read task: %v", err)
	}
	if orphanStatus != "failed" {
		t.Fatalf("expected orphan task status 'failed', got %q (error=%q)", orphanStatus, orphanErrMsg)
	}
	if !strings.Contains(orphanErrMsg, "orphan") {
		t.Fatalf("expected orphan error message to mention 'orphan', got %q", orphanErrMsg)
	}
	...
	if err := testPool.QueryRow(ctx,
		...
		orphanTaskID,
	).Scan(&orphanTokenCount); err != nil {
		t.Fatalf("post-check: count task_token rows: %v", err)
	}
	if orphanTokenCount != 0 {
		t.Fatalf("expected 0 task_token rows for orphan task, got %d", orphanTokenCount)
	}
}
