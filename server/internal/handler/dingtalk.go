...
func dingtalkInstallationToResponse(row db.ChannelInstallation) DingTalkInstallationResponse {
	info := dingtalk.DecodePublicConfig(row.Config)
...
func (h *Handler) RegisterDingTalkBYO(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, dingtalk.ErrAppOwnedByAnotherAgent):
			// The owner may live in a workspace this user cannot see; say so
			// without naming it, so a cross-workspace conflict is not a dead end
			// yet no other workspace's usage is disclosed.
			writeError(w, http.StatusConflict, "this DingTalk robot is already connected to another agent; if that agent is in a different workspace, disconnect it there first")
...
// cannot bind a DingTalk id to an attacker's account. Failure modes map to
