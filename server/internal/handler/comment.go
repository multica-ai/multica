...
func (h *Handler) resolveCommentTargetAgentIDs(ctx context.Context, issue db.Issue, content string, parentComment *db.Comment, authorType, authorID string) []string {
	...
	if authorType == "member" && h.shouldEnqueueOnComment(ctx, issue, authorType, authorID, commentTriggerComputeOptions{}) &&
...
func (h *Handler) CreateComment(w http.ResponseWriter, r *http.Request) {
	...
	h.triggerTasksForComment(r.Context(), issue, comment, parentComment, authorType, authorID, suppressAgentIDs, taskContext)
...
func (h *Handler) triggerTasksForComment(ctx context.Context, issue db.Issue, comment db.Comment, parentComment *db.Comment, actorType, actorID string, suppressAgentIDs []pgtype.UUID, taskContext []byte) {
	...
	h.enqueueCommentAgentTriggers(ctx, issue, comment.ID, triggers, taskContext)
...
func (h *Handler) enqueueCommentAgentTriggers(ctx context.Context, issue db.Issue, triggerCommentID pgtype.UUID, triggers []commentAgentTrigger, taskContext []byte) {
	for _, trigger := range triggers {
		switch trigger.Source {
		case commentTriggerSourceIssueAssignee:
			if trigger.Squad != nil {
				if _, err := h.TaskService.EnqueueTaskForSquadLeaderWithContext(ctx, issue, trigger.Agent.ID, triggerCommentID, taskContext); err != nil {
			...
			if _, err := h.TaskService.EnqueueTaskForIssueWithContext(ctx, issue, taskContext, triggerCommentID); err != nil {
				slog.Warn("enqueue agent task on comment failed", "issue_id", uuidToString(issue.ID), "error", err)
			}
		case commentTriggerSourceMentionSquadLeader:
			squadContext := taskContext
			if trigger.Squad != nil {
				squadContext = taskContextForSquadMention(taskContext, *trigger.Squad)
			}
			if _, err := h.TaskService.EnqueueTaskForSquadLeaderWithContext(ctx, issue, trigger.Agent.ID, triggerCommentID, squadContext); err != nil {
		...
		case commentTriggerSourceMentionAgent:
			if _, err := h.TaskService.EnqueueTaskForMentionWithContext(ctx, issue, trigger.Agent.ID, triggerCommentID, taskContext); err != nil {
...
func (h *Handler) computeAssignedSquadLeaderCommentTrigger(ctx context.Context, issue db.Issue, content, authorType, authorID string, opts commentTriggerComputeOptions) (commentAgentTrigger, bool) {
	...
	if !h.canDispatchToPrivateAgent(ctx, agent, authorType, authorID, uuidToString(issue.WorkspaceID)) {
		return commentAgentTrigger{}, false
	}
...
func (h *Handler) computeMentionedAgentCommentTriggers(ctx context.Context, issue db.Issue, content string, parentComment *db.Comment, authorType, authorID string, opts commentTriggerComputeOptions) []commentAgentTrigger {
	mentions := util.ParseMentions(content)
	triggers := make([]commentAgentTrigger, 0, len(mentions)+1)
	...
	if shouldTargetParentAgentReply(parentComment, mentions, authorType) {
		...
		if err == nil && agent.RuntimeID.Valid && !agent.ArchivedAt.Valid &&
			h.canTriggerPrivateAgent(ctx, agent, authorType, authorID) {
			hasPending, err := h.Queries.HasPendingTaskForIssueAndAgent(ctx, db.HasPendingTaskForIssueAndAgentParams{
				IssueID: issue.ID,
...
func (h *Handler) UpdateComment(w http.ResponseWriter, r *http.Request) {
	...
	if oldContent != comment.Content {
		...
		if err != nil {
			...
			h.triggerTasksForComment(r.Context(), issue, comment, parentComment, actorType, actorID, suppressAgentIDs, nil)
		}
	}
