...
func (h *Handler) CreateComment(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	transactionalWrite := idempotencyKey != "" || (authorType == "agent" && parentID.Valid) || len(attachmentIDs) > 0
	if transactionalWrite {
		...
		if err == nil && len(attachmentIDs) > 0 {
			// Keep the visible comment and its attachments atomic. The issue lock
			// above precedes the attachment locks, matching the delete/update lock
			// order; a rejected or disappearing attachment therefore cannot leave
			// an orphaned comment behind.
			missing, lockErr := lockCommentAttachments(r.Context(), qtx, issue.WorkspaceID, issue.ID, attachmentIDs)
			if lockErr != nil {
				err = lockErr
			} else if missing.Valid {
				writeError(w, http.StatusConflict, "attachment "+uuidToString(missing)+" is no longer available")
				return
			} else {
				_, err = qtx.LinkAttachmentsToComment(r.Context(), db.LinkAttachmentsToCommentParams{
