...
func externalPullRequestToResponse(p db.ExternalPullRequest) GitHubPullRequestResponse {
	...
	// Link-only record: no live status is fetched, so the card degrades to
	// the "external, status unknown" display via SnapshotAvailable=false.
...
func (h *Handler) autoLinkCodePullRequestsFromText(ctx context.Context, issue db.Issue, actorType, actorID string, texts ...string) error {
	...
	// Publish realtime events for whatever was linked before surfacing a
	// partial failure — otherwise rows created before the error would sit in
	// the DB without any notification.
...
// codeDiscoveryGate throttles the Code-link backfill scan in
// ListPullRequestsForIssue to at most one run per issue per cooldown. Fresh
// links normally arrive through the comment/metadata write paths; the scan
// exists for historical content. It is held by the Handler as a pointer so
// the shallow Handler copy in beginIssueTableSnapshot never copies the lock.
type codeDiscoveryGate struct {
	mu sync.Mutex
	at map[string]time.Time
}

func newCodeDiscoveryGate() *codeDiscoveryGate {
	return &codeDiscoveryGate{at: make(map[string]time.Time)}
}

func (g *codeDiscoveryGate) allow(issueKey string) bool {
	g.mu.Lock()
	defer g.mu.Unlock()
	if last, ok := g.at[issueKey]; ok && time.Since(last) < codeDiscoveryCooldown {
		return false
	}
	g.at[issueKey] = time.Now()
	if len(g.at) > 4096 {
		for k, t := range g.at {
...
func (h *Handler) shouldRunCodeDiscovery(issue db.Issue) bool {
	return h.codeDiscovery.allow(uuidToString(issue.ID))
}
