...
func TestSquadOperatingProtocolWarnsAgainstDualTrigger(t *testing.T) {
	protocol := squadOperatingProtocolFor(true) // CEREBRO-PATCH(squad-parent-status): MUL-5156 protocol assembled via ownsIssueStatus selector
...
// contract (MUL-5156): first dispatch moves todo→in_progress and stays there;
// only a later confirmation of overall completion may advance to in_review;
// done is left to humans / integrations.
...
func TestSquadOperatingProtocolScopesParentStatusOwnership(t *testing.T) {
	...
	// The header the daemon keys IsSquadLeader off must survive both ways, or a
	// guest leader silently loses its no_action / silent-exit behavior.
...
func TestBuildSquadLeaderBriefing_FullSquad(t *testing.T) {
	...
	out := buildSquadLeaderBriefing(ctx, testHandler.Queries, squad, true) // CEREBRO-PATCH(squad-parent-status): MUL-5156
