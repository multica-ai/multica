...
func claimAndDecodeAgent(t *testing.T, runtimeID string) *TaskAgentData {
	agent, _ := claimAndDecodeTask(t, runtimeID)
	return agent
}

func claimAndDecodeTask(t *testing.T, runtimeID string) (*TaskAgentData, AgentTaskResponse) {
	t.Helper()
	...
	return resp.Task.Agent, *resp.Task
}
...
func TestClaimTask_LeaderGetsBriefingFromTaskContext(t *testing.T) {
	if testHandler == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	squad := seedSquadForBriefing(t, leaderID, "Context Briefing Squad", "Use context squad instructions.")
	helper := createHandlerTestAgent(t, "Context Briefing Helper", []byte("[]"))
	addAgentMember(t, squad.ID, helper, "implementer")
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO issue (
			workspace_id, title, status, priority, creator_id, creator_type,
			assignee_type, assignee_id, number, position
		) VALUES ($1, 'Squad context briefing claim test', 'todo', 'medium', $2, 'member',
			'member', $2, 95003, 0)
		RETURNING id
	`, testWorkspaceID, testUserID).Scan(&issueID); err != nil {
		t.Fatalf("create member-assigned issue: %v", err)
	}
	t.Cleanup(func() { testPool.Exec(ctx, `DELETE FROM issue WHERE id = $1`, issueID) })

	contextJSON, err := json.Marshal(service.TaskContext{
		SquadID:          util.UUIDToString(squad.ID),
		SquadName:        squad.Name,
		LeaderTaskSource: service.LeaderTaskSourceSquadMention,
	})
	if err != nil {
	...
	if err := testPool.QueryRow(ctx, `
		...
		RETURNING id
	`, leaderID, runtimeID, issueID, contextJSON).Scan(&taskID); err != nil {
		t.Fatalf("queue task: %v", err)
	}
	t.Cleanup(func() { testPool.Exec(ctx, `DELETE FROM agent_task_queue WHERE id = $1`, taskID) })
	...
	if task.SquadID != util.UUIDToString(squad.ID) {
		t.Fatalf("claim response squad_id = %q, want %q", task.SquadID, util.UUIDToString(squad.ID))
	}
	if task.SquadName != "Context Briefing Squad" {
		t.Fatalf("claim response squad_name = %q", task.SquadName)
	}
	for _, want := range []string{
		"## Squad Operating Protocol",
		"## Squad Roster",
		"## Squad Instructions (Context Briefing Squad)",
		"Use context squad instructions.",
		"`[@Context Briefing Helper](mention://agent/" + helper + ")`",
	} {
		if !strings.Contains(agent.Instructions, want) {
			t.Errorf("expected agent.instructions to contain %q\n--- instructions ---\n%s", want, agent.Instructions)
		}
