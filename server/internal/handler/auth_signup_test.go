...
func TestFindOrCreateUserGating(t *testing.T) {
	t.Run("new_user_blocked", func(t *testing.T) {
		...
		_, isNew, _, err := h.findOrCreateUser(context.Background(), "new@blocked.com")
		if err == nil {
	...
	t.Run("existing_user_allowed", func(t *testing.T) {
		...
		_, isNew, _, err := h.findOrCreateUser(context.Background(), "existing@test.com")
		if err != nil {
	...
	t.Run("whitelisted_user_allowed", func(t *testing.T) {
		...
		_, _, _, err := h.findOrCreateUser(context.Background(), "whitelisted@test.com")
