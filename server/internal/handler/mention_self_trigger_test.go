...
func TestEnqueueMentionedAgentTasks_SelfMentionCrossIssueEnqueues(t *testing.T) {
	...
	// CEREBRO-PATCH(private-autopilot-member-agent-push): pass empty private autopilot owner for normal self-mention tests.
	testHandler.enqueueMentionedAgentTasks(ctx, nil, fx.IssueB, fx.CommentB, nil, "agent", fx.JID, service.TaskDelegationContext{}, nil, pgtype.UUID{})
...
func TestEnqueueMentionedAgentTasks_SelfMentionWhileRunningQueuesFollowup(t *testing.T) {
	...
	// CEREBRO-PATCH(private-autopilot-member-agent-push): pass empty private autopilot owner for normal self-mention tests.
	testHandler.enqueueMentionedAgentTasks(ctx, nil, fx.IssueA, fx.CommentA, nil, "agent", fx.JID, service.TaskDelegationContext{}, nil, pgtype.UUID{})
