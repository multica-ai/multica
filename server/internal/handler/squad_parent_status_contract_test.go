...
// CEREBRO-PATCH(squad-parent-status): MUL-5156 — net-new composition tests for
// the ported squad parent-status contract (delete on the next upstream sync).

// The two tests below are composition tests, not text-presence tests. The
// parent-status contract (MUL-5156) is split across two systems that never see
// each other: the squad briefing (server-side, appended to the leader's
...
// TestGuestLeaderCannotChangeStatusOnCommentTurn is the other half of the scope
// fix (MUL-3724 path): the issue belongs to a plain agent and this squad was
// only @mentioned for help. The briefing still gets injected — the leader needs
// its roster — but no combination of the two halves may authorize a status
// change on someone else's issue.
