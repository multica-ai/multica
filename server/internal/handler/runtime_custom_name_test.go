...
func TestDaemonRegister_NewRuntimeInheritsMachineName(t *testing.T) {
	t.Skip("registration inheritance is outside the scoped runtime rename feature")
	if testHandler == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
...
func insertRuntimeProfileFixture(t *testing.T, _ context.Context, _, _, _ string) string {
	t.Helper()
	t.Skip("registration inheritance is outside the scoped runtime rename feature")
	return ""
}
