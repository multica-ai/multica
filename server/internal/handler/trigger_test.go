...
func TestIsNoteComment(t *testing.T) {
	tests := []struct {
		...
		{"explicit Chinese note skips", "仅记录，无需回复", true},
		{"explicit Chinese note with body skips", "仅记录，无需回复\n这是补充背景", true},
		{"explicit Chinese note with punctuation skips", "仅记录，无需回复：这是补充背景", true},
		{"leading whitespace before Chinese note tolerated", "  仅记录，无需回复", true},
		{"Chinese note phrase later does not skip", "这句话仅记录，无需回复", false},
