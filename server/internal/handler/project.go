...
func (h *Handler) CreateProject(w http.ResponseWriter, r *http.Request) {
	...
	// also enforce one row per (daemon_id, agent_id) within the batch. An empty
	// agent_id is the daemon default; exact agent bindings may coexist with it.
	...
	for i, res := range req.Resources {
		...
		if res.ResourceType == "local_directory" {
			...
			bindingKey := ld.DaemonID + "\x00" + ld.AgentID
			if prev, ok := localDirSeen[bindingKey]; ok {
				writeError(w, http.StatusBadRequest, "resources["+strconv.Itoa(i)+"]: duplicate local_directory for daemon and agent binding (already at index "+strconv.Itoa(prev)+")")
				return
			}
			localDirSeen[bindingKey] = i
