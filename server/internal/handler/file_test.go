...
func TestGetAttachmentContent_StreamsPDF(t *testing.T) {
	store := &mockStorage{}
	origStorage := testHandler.Storage
	testHandler.Storage = store
	defer func() { testHandler.Storage = origStorage }()
	...
	body := []byte("%PDF-1.4\n")
	id := seedPreviewAttachment(t, store, "pdf-key.pdf", "manual.pdf", "application/pdf", body)
	...
	if got := w.Header().Get("Content-Disposition"); got != `inline; filename="manual.pdf"` {
