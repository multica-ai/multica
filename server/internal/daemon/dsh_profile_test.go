...
func TestProvisionDshMulticaProfile_ReportsEveryCandidateFailure(t *testing.T) {
	...
	// registry errors surface, so it has to ride along.
	if !strings.Contains(err.Error(), "FAILME-two") || !strings.Contains(err.Error(), "registry unreachable") {
		t.Fatalf("error = %q, want the failing spec and dsh's output", err)
	}
}
