...
var probeAgentCLIs = func() map[string]AgentEntry {
	...
	if e, ok := probe("codearts", "MULTICA_CODEARTS_PATH", "codearts", "MULTICA_CODEARTS_MODEL"); ok {
		agents["codearts"] = e
	...
	// The descriptor ID doubles as the managed-runtime provider key, so a fork
	// that Multica can install is discovered through the same fallback.
	for _, desc := range agent.BuiltinRuntimes {
		pathEnv := desc.EnvPrefix + "_PATH"
		modelEnv := desc.EnvPrefix + "_MODEL"
		if e, ok := probe(desc.ID, pathEnv, desc.DefaultCommand, modelEnv); ok {
			agents[desc.ID] = e
		}
	}
	...
	if e, ok := probe("dsh", "MULTICA_DSH_PATH", "dsh", "MULTICA_DSH_MODEL"); ok && probeDshMulticaProfile(e.Path) {
		agents["dsh"] = e
	}
	...
	if e, ok := probe("qwenpaw", "MULTICA_QWENPAW_PATH", "qwenpaw", ""); ok {
		agents["qwenpaw"] = e
	}
	...
	if e, ok := probe("dim", "MULTICA_DIM_PATH", "dim", "MULTICA_DIM_MODEL"); ok {
		agents["dim"] = e
	}
	...
	if e, ok := probe("mcode", "MULTICA_MCODE_PATH", "mcode", ""); ok {
		agents["mcode"] = e
	}
	...
	if e, ok := probe("zeroclaw", "MULTICA_ZEROCLAW_PATH", "zeroclaw", ""); ok {
		agents["zeroclaw"] = e
	}
	return agents
}
