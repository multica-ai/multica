...
func TestEnsureDaemonID_Persists(t *testing.T) {
	...
	path := filepath.Join(home, ".wallts", "daemon.id")
...
func TestEnsureDaemonID_SharedAcrossProfiles(t *testing.T) {
	...
	// only source of truth is ~/.wallts/daemon.id.
	profileFile := filepath.Join(home, ".wallts", "profiles", "staging", "daemon.id")
...
func TestEnsureDaemonID_PromotesPreChangeProfileFile(t *testing.T) {
	...
	profileDir := filepath.Join(home, ".wallts", "profiles", "staging")
	...
	data, err := os.ReadFile(filepath.Join(home, ".wallts", "daemon.id"))
	if err != nil {
...
func TestLegacyDaemonUUIDs_ScansProfileDirs(t *testing.T) {
	...
	for name, id := range map[string]string{"prod": uuidA, "desktop-wallts": uuidB} {
		dir := filepath.Join(home, ".wallts", "profiles", name)
		if err := os.MkdirAll(dir, 0o755); err != nil {
	...
	corruptDir := filepath.Join(home, ".wallts", "profiles", "corrupt")
