...
type Task struct {
	...
	IssueStateDeltaKnown          bool                   `json:"issue_state_delta_known,omitempty"`          // MUL-7344: the server compared the issue's title/description/status/assignee/priority against the snapshot taken at this agent's previous run on this issue. Same contract as NewCommentsDeltaKnown — absent means NOT compared (cold start, no prior snapshot, read error, old server), and the prompt must then keep telling the agent to read the issue
	IssueChangedFields            []string               `json:"issue_changed_fields,omitempty"`             // subset of title,description,status,assignee,priority in that order; empty alongside IssueStateDeltaKnown means unchanged. Fields outside that set are not compared and must never be reported as checked
