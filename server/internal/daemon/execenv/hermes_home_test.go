...
// TestPrepareHermesNoSkillsGetsIsolatedHome proves that all Hermes tasks,
// including those without bound skills, use task-local memory and SQLite state.
func TestPrepareHermesNoSkillsGetsIsolatedHome(t *testing.T) {
	t.Parallel()
	sharedHome := t.TempDir()
	mustWrite(t, filepath.Join(sharedHome, "config.yaml"), "model: hermes-4\n")
	mustWrite(t, filepath.Join(sharedHome, "memories", "MEMORY.md"), "HOST MEMORY")
	mustWrite(t, filepath.Join(sharedHome, "state.db"), "HOST DB")
	...
	if env.HermesHome == "" {
		t.Fatal("skill-less Hermes task must receive an isolated HERMES_HOME")
	}
	if _, err := os.Stat(filepath.Join(env.HermesHome, "memories", "MEMORY.md")); !os.IsNotExist(err) {
		t.Errorf("host memory leaked into a skill-less task: %v", err)
	}
	if _, err := os.Stat(filepath.Join(env.HermesHome, "state.db")); !os.IsNotExist(err) {
		t.Errorf("shared state database leaked into a skill-less task: %v", err)
	}
}

func TestConcurrentHermesTasksHaveIsolatedHomes(t *testing.T) {
	t.Parallel()
	sharedHome := t.TempDir()
	mustWrite(t, filepath.Join(sharedHome, "config.yaml"), "model: hermes-4\n")
	mustWrite(t, filepath.Join(sharedHome, "state.db"), "HOST DB")
	...
	tasks := []struct {
	...
	}{
		{
			...
			skillContent: "alpha instructions",
	...
	results := make(chan prepareResult, len(tasks))
	for i, task := range tasks {
		i, task := i, task
		go func() {
			var skills []SkillContextForEnv
			if task.skillName != "" {
			...
			env, err := Prepare(PrepareParams{
				...
				WorkspaceID:      "ws-hermes-concurrent",
				TaskID:           task.taskID,
				Provider:         "hermes",
				HermesSourceHome: sharedHome,
			...
			results <- prepareResult{index: i, env: env, err: err}
		}()
	}

	envs := make([]*Environment, len(tasks))
	for range tasks {
		result := <-results
		if result.err != nil {
			t.Fatalf("concurrent Prepare failed: %v", result.err)
		}
		envs[result.index] = result.env
		env := result.env
		t.Cleanup(func() { env.Cleanup(true) })
	}
	if envs[0].HermesHome == envs[1].HermesHome {
		t.Fatalf("concurrent tasks share HERMES_HOME: %s", envs[0].HermesHome)
	}

	for i, task := range tasks {
		home := envs[i].HermesHome
		if task.skillSlug != "" {
			body, err := os.ReadFile(filepath.Join(home, "skills", task.skillSlug, "SKILL.md"))
			if err != nil {
				t.Fatalf("task %s bound skill missing: %v", task.issueID, err)
			}
			if !strings.Contains(string(body), task.skillContent) {
				t.Errorf("task %s skill content = %q", task.issueID, body)
			}
			for _, other := range tasks {
				if other.skillSlug == "" || other.skillSlug == task.skillSlug {
					continue
				}
				if _, err := os.Stat(filepath.Join(home, "skills", other.skillSlug)); !os.IsNotExist(err) {
					t.Errorf("task %s can see task %s skill", task.issueID, other.issueID)
				}
			}
		} else if _, err := os.ReadDir(filepath.Join(home, "skills")); err != nil {
			t.Fatalf("task %s skills dir missing: %v", task.issueID, err)
		}
		mustWrite(t, filepath.Join(home, "memories", "MEMORY.md"), task.issueID+" memory")
	}

	// Run two Hermes-shaped child processes at the same time. Each process opens
	// the state.db selected by its HERMES_HOME, holds an IMMEDIATE SQLite write
	// transaction until the other process has done the same, then records the
	// owners visible in its database. If the homes ever collapse to one shared
	// database, the second writer cannot reach the barrier (or both owners leak
	// into one result), so this test fails instead of merely checking filenames.
	python, err := exec.LookPath("python3")
	if err != nil {
		t.Fatalf("python3 is required to exercise Hermes SQLite state: %v", err)
	}
	barrierDir := t.TempDir()
	script := `
import os, pathlib, sqlite3, sys, time

label, own_ready, peer_ready, result_path = sys.argv[1:]
db_path = os.path.join(os.environ["HERMES_HOME"], "state.db")
db = sqlite3.connect(db_path, timeout=0.5, isolation_level=None)
db.execute("CREATE TABLE IF NOT EXISTS task_state (owner TEXT NOT NULL)")
db.execute("BEGIN IMMEDIATE")
db.execute("INSERT INTO task_state(owner) VALUES (?)", (label,))
pathlib.Path(own_ready).touch()
deadline = time.monotonic() + 5
while not os.path.exists(peer_ready):
    if time.monotonic() >= deadline:
        raise TimeoutError("peer Hermes SQLite writer never reached the barrier")
    time.sleep(0.01)
time.sleep(0.2)
db.commit()
owners = [row[0] for row in db.execute("SELECT owner FROM task_state ORDER BY owner")]
...
`
	...
	runs := make([]sqliteRun, len(tasks))
	for i, task := range tasks {
		ownReady := filepath.Join(barrierDir, task.issueID+".ready")
		peerReady := filepath.Join(barrierDir, tasks[1-i].issueID+".ready")
		resultPath := filepath.Join(barrierDir, task.issueID+".result")
		cmd := exec.Command(python, "-c", script, task.issueID, ownReady, peerReady, resultPath)
		cmd.Env = append(os.Environ(), "HERMES_HOME="+envs[i].HermesHome)
		runs[i].cmd = cmd
		cmd.Stdout = &runs[i].output
		cmd.Stderr = &runs[i].output
		if err := cmd.Start(); err != nil {
			t.Fatalf("start task %s SQLite process: %v", task.issueID, err)
		}
	}
	for i, task := range tasks {
		if err := runs[i].cmd.Wait(); err != nil {
			t.Fatalf("task %s SQLite process failed: %v\n%s", task.issueID, err, runs[i].output.String())
		}
		result, err := os.ReadFile(filepath.Join(barrierDir, task.issueID+".result"))
		if err != nil {
			t.Fatalf("read task %s SQLite result: %v", task.issueID, err)
		}
		if got, want := string(result), task.issueID; got != want {
			t.Fatalf("task %s SQLite owners = %q, want only %q", task.issueID, got, want)
		}
	}

	for i, task := range tasks {
		home := envs[i].HermesHome
		if data, err := os.ReadFile(filepath.Join(home, "memories", "MEMORY.md")); err != nil {
			t.Fatalf("read task %s memory: %v", task.issueID, err)
		} else if got, want := string(data), task.issueID+" memory"; got != want {
			t.Errorf("task %s memory = %q, want %q", task.issueID, got, want)
		}
		if info, err := os.Stat(filepath.Join(home, "state.db")); err != nil {
			t.Fatalf("task %s SQLite database missing: %v", task.issueID, err)
		} else if info.Size() == 0 {
			t.Errorf("task %s SQLite database is empty", task.issueID)
		}
	}
	if data, err := os.ReadFile(filepath.Join(sharedHome, "state.db")); err != nil {
		t.Fatalf("read shared database: %v", err)
	} else if got := string(data); got != "HOST DB" {
		t.Errorf("shared database changed during concurrent task setup: %q", got)
	}
}

// TestReuseHermesKeepsIsolationWhenSkillsRemoved covers the resume path: a task
// that loses its last skill remains isolated while its local skills directory
// is refreshed to empty.
func TestReuseHermesKeepsIsolationWhenSkillsRemoved(t *testing.T) {
	t.Parallel()
	sharedHome := t.TempDir()
	mustWrite(t, filepath.Join(sharedHome, "config.yaml"), "model: hermes-4\n")
	...
	if reused.HermesHome != overlayDir {
		t.Errorf("removing the last skill must keep the isolated HERMES_HOME, got %q", reused.HermesHome)
	}
	entries, err := os.ReadDir(filepath.Join(overlayDir, "skills"))
	if err != nil {
		t.Fatalf("read refreshed skills dir: %v", err)
	}
	if len(entries) != 0 {
		t.Errorf("removing the last skill should leave an empty local skills dir, got %v", entries)
	}
}
