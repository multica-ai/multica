...
// equally valid.
func storeOpenclawDiscoveryCache(cachePath, bin string, env map[string]string, activeConfigPath string, agentsList []any, agentsSource openclawAgentsSource, now time.Time) error {
	if cachePath == "" {
		return nil
	}
	if activeConfigPath == "" {
