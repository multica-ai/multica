...
func TestCommentTriggeredProtocolDoesNotForceInReview(t *testing.T) {
	...
	// carve-out (MUL-5156) must not leak into this path.
...
// CEREBRO-PATCH(squad-parent-status): MUL-5156 — squad-leader status-ownership
// carve-out tests for the fork's inlined comment/assignment workflows.
//
// TestCommentTriggeredSquadLeaderDefersToStatusOwnershipGrant (MUL-5156): a
// squad leader on a comment-triggered turn gets the same guardrail plus a named
// exception. Without it the guardrail and the Squad Operating Protocol's "Own
// the parent issue status" responsibility contradict each other on the
// @mention-dispatch shape, where the member's delivery comment never asks for a
// status change and no child-done system comment exists to ask on its behalf —
// so the parent would sit in in_progress forever.
...
// TestSquadLeaderAssignmentProtocolKeepsParentInProgress (MUL-5156):
