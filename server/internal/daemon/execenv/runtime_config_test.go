	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, want := range []string{
				...
				// GitHub issue #6417 gap 1: a recurring ask must be routed to an
				// autopilot before the agent reaches for `issue create` — a sub-issue
				// is a single run, not a schedule. Pointer here, full playbook in
				// references/issues.md (TestPlatformSkillCoversRecurringWorkRouting).
