...
// tests that don't care about platform-aware sandbox configuration. It assumes
// a Linux environment (danger-full-access with the real HOME, see #6218).
...
func prepareCodexHomeWithOpts(codexHome string, opts CodexHomeOptions, logger *slog.Logger) error {
	...
	// danger-full-access with the real HOME (#6218), macOS falls back to
	// danger-full-access because of openai/codex#10390, and Windows defaults to
	// danger-full-access unless the user opted into a native windows.sandbox;
	// see codex_sandbox.go for the full rationale. On Windows, resolve the
	// native-sandbox state across the copied config and the effective custom
	// args so an explicit user opt-in is honored and an undecidable config
	// fails closed instead of loosening.
