...
// For Multica:   writes {workDir}/multica.md  (Multica agents read multica.md as primary context; skills discovered natively from .agent_context/skills/)
...
func runtimeConfigPath(workDir, provider string) string {
	...
	case "multica":
		return filepath.Join(workDir, "multica.md")
	case "gemini":
...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if len(ctx.AgentSkills) > 0 {
		...
		switch provider {
		case "claude", "multica":
			// Claude discovers skills natively from .claude/skills/; Multica discovers
