...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if hasIssueContext {
		...
		b.WriteString("## Pull Request Handoff\n\n")
		fmt.Fprintf(&b, "When your role permits issue updates and you open or update a PR for this issue, add the PR URL to the issue body/description before your final comment. Preserve the existing description from `multica issue get`; write the full updated body to a UTF-8 file and run `multica issue update %s --description-file <path>`. Prefer a stable line such as `Pull request: <url>` and update an existing PR line instead of duplicating it. This is separate from the final issue comment and any high-signal metadata you may pin. Only do this when a PR URL exists; if the run produced no PR, leave the issue body unchanged.\n\n", ctx.IssueID)
	}
