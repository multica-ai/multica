...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("- `multica issue status <id> <status>` — Shortcut for `issue update --status` when you only need to flip status (backlog, todo, in_progress, waiting, planned, developing, deployed, blocked, cancelled). Legacy `in_review` and `done` are still accepted as aliases for `waiting` and `deployed` respectively during the 30-day grace period; you'll see a deprecation warning on stderr.\n")
