...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("- `multica issue comment add <issue-id> --content \"...\" [--parent <comment-id>] [--attachment <file>]` — Post a comment (use --parent to reply to a specific comment; use --attachment to attach files like screenshots, logs, zips — can be specified multiple times)\n")
	...
	if ctx.AutopilotDescription != "" {
		...
		b.WriteString("6. If you complete work successfully, update the issue status to reflect the current state\n\n")
	} else {
		...
		b.WriteString("5. Update the issue status to reflect your progress or completion, respecting any pipeline restrictions.\n\n")
	}
