...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.ChatSessionID != "" {
		...
		b.WriteString("4. **If you reply, post it as a comment — this step is mandatory when you reply.** Text in your terminal or run logs is NOT delivered to the user. **Use the trigger comment ID from the user message as `--parent`** when posting your reply. Always use `--content-stdin` with a HEREDOC, even for a single-line reply:\n\n")
