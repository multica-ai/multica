...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if len(ctx.AgentSkills) > 0 {
		...
		case "codex", "copilot", "opencode", "openclaw", "pi", "cursor", "kimi", "kiro", "DeepSeek-TUI":
			// Codex, Copilot, OpenCode, OpenClaw, Pi, Cursor, Kimi, Kiro, and DeepSeek
			// discover skills natively from their respective paths. For OpenClaw, the
			// daemon also writes a per-task openclaw-config.json (exported via
			// OPENCLAW_CONFIG_PATH) that pins agents.defaults.workspace to the task
			// workdir so the CLI's scanner picks up {workDir}/skills/.
			b.WriteString("You have the following skills installed (discovered automatically):\n\n")
		case "gemini", "hermes":
