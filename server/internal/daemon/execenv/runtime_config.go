...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("- `multica issue create --title \"...\" [--description \"...\" | --description-file <path> | --description-stdin] [--priority X] [--status X] [--assignee X | --assignee-id <uuid>] [--parent <issue-id>] [--stage N] [--space <space-id-or-key>] [--project <project-id>] [--due-date <RFC3339>] [--attachment <path>]` — Create a new issue; `--attachment` may be repeated. Omitting `--space` falls back to the workspace's default Space. `--stage N` (N ≥ 1) groups a sub-issue into an ordered barrier group under its parent so the parent wakes per stage, not per child. For agent-authored long descriptions, prefer `--description-file <path>` — flags after a HEREDOC terminator can be silently swallowed (#4182).\n")
	b.WriteString("- `multica issue update <id> [--title X] [--description X | --description-file <path> | --description-stdin] [--priority X] [--status X] [--assignee X | --assignee-id <uuid>] [--parent <issue-id>] [--stage N] [--space <space-id-or-key>] [--project <project-id>] [--due-date <RFC3339>]` — Update issue fields; `--space` moves the issue to another Space (renumbers it; the old identifier keeps resolving); use `--parent \"\"` to clear parent. For agent-authored long descriptions, prefer `--description-file <path>` over stdin (#4182).\n")
	...
	// Inject Space context. Unlike Project (optional, many-to-many), every
	// issue-bound task has exactly one Space — it owns the issue's identifier
	// namespace (SPACE_KEY-NUMBER) — so this fires whenever ctx carries one.
	if ctx.SpaceID != "" {
