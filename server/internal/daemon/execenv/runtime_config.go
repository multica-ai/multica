...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.ChatSessionID != "" {
		...
		b.WriteString(commentThreadStep(ctx.IssueID, ctx.TriggerCommentID, ctx.IssueSnapshotInlined, ctx.BundleContextHint)) // CEREBRO-PATCH(runtime-config-snapshot): skip/replace the comment-list step when a fewer-calls saving is on (FIR-2384). Upstream's new BuildNewCommentsHint / BuildResumedCommentsHint / BuildColdCommentsHint helpers (MUL-2785/2809/2805) are TODO: integrate inside commentThreadStep so the FIR-2384 bundling and the upstream since-delta hints coexist.
