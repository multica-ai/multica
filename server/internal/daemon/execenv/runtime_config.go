...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("- `multica project list --output json` — List projects in workspace (use BEFORE creating an issue to find the right `--project <id>`)\n")
	b.WriteString("- `multica project get <id> --output json` — Get project details\n")
	...
	b.WriteString("### Write\n")
	b.WriteString("- `multica issue create --title \"...\" [--description \"...\"] [--project <project-id>] [--priority X] [--assignee X] [--parent <issue-id>] [--status X]` — Create a new issue. **Always pick a `--project` if one fits**: run `multica project list --output json` first and match the issue's topic to an existing project. Only omit `--project` if no project fits (e.g. one-off scratch tasks, generic workspace admin).\n")
	...
	b.WriteString("- `multica issue update <id> [--title X] [--description X] [--priority X] [--project <project-id>]` — Update issue fields (including moving to a project)\n")
	b.WriteString("- `multica autopilot create --title \"...\" --agent <name> --mode create_issue [--description \"...\"] [--project <project-id>]` — Create an autopilot. Set `--project` so issues created by the autopilot inherit it.\n")
	b.WriteString("- `multica autopilot update <id> [--title X] [--description X] [--status active|paused] [--project <project-id>]` — Update an autopilot (use empty string for `--project` to clear)\n")
