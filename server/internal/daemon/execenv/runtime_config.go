...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if hasIssueContext {
		...
		b.WriteString("- **Recommended keys** (reuse these names so queries stay consistent across the workspace; coin a new key only when none fits): `pr_url`, `pr_number`, `pipeline_status`, `artifact_summary`, `deploy_url`, `external_issue_url`, `waiting_on`, `blocked_reason`, `decision`. Use snake_case ASCII. The list is short on purpose — most issues only need 1-2 of these pinned, not the full set.\n\n")
		b.WriteString("## Artifact Durability\n\n")
		b.WriteString("If this run creates or changes a durable deliverable (code, docs, reports, generated files, research notes, diagrams, exported data, or any other artifact the user needs after the run), do not rely on the task workdir as the deliverable. Workdirs are execution scratch space.\n\n")
		b.WriteString("Before moving the issue to `in_review`, leave at least one durable evidence path in the final issue comment:\n")
		b.WriteString("- PR URL or commit URL for repo-backed work.\n")
		b.WriteString("- A Multica comment attachment for generated standalone files, posted with `multica issue comment add <issue-id> --attachment <path>`.\n")
		b.WriteString("- The artifact body pasted into the final comment when it is short enough to be useful inline.\n\n")
		b.WriteString("Repo-bound tasks require a usable repo checkout before file work. Use project resources or `multica repo checkout <url>` first. If the needed repo is missing or checkout fails and the user did not explicitly ask for scratch-only output, post a blocked comment explaining the missing repo/resource and move the issue to `blocked` instead of producing scratch files. If scratch-only output is explicitly acceptable, attach or paste every generated deliverable before review.\n\n")
	}
	...
	if ctx.ChatSessionID != "" {
		...
		b.WriteString("5. Follow your Skills and Agent Identity to complete the task (write code, investigate, etc.). If the task requires repo-backed file work, confirm a usable repo checkout before creating deliverables; missing repo access is a blocker unless the user explicitly requested scratch-only output.\n")
		if ctx.IsSquadLeader {
		...
		fmt.Fprintf(&b, "8. When done, run `multica issue status %s in_review` only after the final comment includes durable evidence for any created or changed artifact as described in ## Artifact Durability.\n", ctx.IssueID)
