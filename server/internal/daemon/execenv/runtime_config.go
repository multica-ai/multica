...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.ChatSessionID != "" {
		...
		b.WriteString("\n")
		b.WriteString("**Long-running external jobs:** If your task dispatches an external job that will take minutes to hours (deployments, EC2 builds, CI pipelines, etc.) and you post a progress update with an estimated completion time, you MUST also schedule a follow-up check-in so the issue doesn't go stale. Use the harness `ScheduleWakeup` tool to wake up after the estimated completion time — when the wakeup fires you'll run again on this same issue to verify the job's actual status and post an update. Without this, other agents and users will see your stale estimate as current status (see GitHub issue #4093). If the job has already completed by the time you post the initial update, you can skip the wakeup and exit normally.\n")
		if ctx.IsSquadLeader {
