...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.ChatSessionID != "" {
		...
		b.WriteString("\n")
		b.WriteString("**Long-running external jobs:** If your task dispatches an external job that will take minutes to hours (deployments, EC2 builds, CI pipelines, etc.) and you post a progress update with an estimated completion time, you MUST also schedule a follow-up check-in so the issue doesn't go stale. Use the harness `ScheduleWakeup` tool to wake up after the estimated completion time. When calling `ScheduleWakeup`, set the `prompt` parameter to something specific: include what job you dispatched, which issue you're on, and what you expect to check. For example: \"Check whether the EC2 deployment for this issue completed and post an update.\" A vague prompt like \"check status\" will leave the wakeup run without enough context to be useful.\n")
		b.WriteString("\n")
		b.WriteString("After scheduling the wakeup, your remaining steps (6-9) still apply — the \"final result\" for this run is the progress update you already posted, not the job's outcome (the job is still running). Post a brief comment confirming the wakeup is scheduled, then exit. The wakeup run will handle checking the actual job status. If the job has already completed by the time you post the initial update, skip the wakeup and exit normally.\n")
		if ctx.IsSquadLeader {
