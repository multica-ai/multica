...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.ChatSessionID != "" {
		...
		b.WriteString(commentThreadStep(ctx.IssueID, ctx.TriggerCommentID, ctx.IssueSnapshotInlined, ctx.BundleContextHint, ctx.PriorSessionResumed)) // CEREBRO-PATCH(runtime-config-snapshot): skip/replace the comment-list step when a fewer-calls saving is on (FIR-2384)
