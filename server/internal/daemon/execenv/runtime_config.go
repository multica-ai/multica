...
func buildCodeTaskPreflightSection() string {
	var b strings.Builder
	b.WriteString("## Code Task Preflight\n\n")
	b.WriteString("For issue work that may produce a code diff, run a lightweight delivery preflight before editing, then refresh it before crossing each delivery boundary. This is a staged check, not a blanket blocker: local investigation and narrow diffs may continue when the task and repo are clear, but do not claim commit/push/MR readiness until the matching stage below is true.\n\n")
	b.WriteString("- `local_edit` (本地可改): issue/Jira/task key is identified or the issue explicitly says `local-only`; the target repo is known and checked out or checkout is available; the requested change and local validation path are clear. If a delivery task has no repo or task key, ask one concrete question or mark the blocker before editing.\n")
	b.WriteString("- `commit_ready` (可提交): `local_edit` is true, a real Jira/task key is available for the commit message, author/committer are known from the request/issue or verified with `git config user.name` and `git config user.email`, and the diff is scoped to the requested repo.\n")
	b.WriteString("- `push_ready` (可推送): `commit_ready` is true, a source branch and remote are chosen, the target/base branch is known, `git remote -v` succeeds, and push credentials are available or a dry-run/first push verifies the path. If auth or branch protection is unknown, report `push_ready=false` instead of waiting until handoff.\n")
	b.WriteString("- `mr_ready` (可建 MR): `push_ready` is true, the MR/PR target branch is confirmed, and a working MR/PR creation channel is available (`gh`, GitHub/GitLab UI/API/CLI, or the repo's documented flow). If the task is local-only, say `mr_ready=false (local-only)` explicitly.\n\n")
	b.WriteString("When reporting delivery state, use these four labels so downstream agents can distinguish local diff, commit, push, and MR readiness.\n\n")
	return b.String()
}
...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if hasIssueContext {
		...
		b.WriteString(buildCodeTaskPreflightSection())
	}
	...
	if ctx.ChatSessionID != "" {
		...
		b.WriteString("5. If the request may change code, run the `## Code Task Preflight` before editing. Keep the four-stage state (`local_edit`, `commit_ready`, `push_ready`, `mr_ready`) explicit in your plan or handoff, and stop at the earliest blocked delivery stage instead of discovering it after implementation.\n")
		...
		b.WriteString("10. Do NOT change the issue status unless the comment explicitly asks for it\n\n")
	} else {
		...
		b.WriteString("4. If the issue may change code, run the `## Code Task Preflight` before editing. Keep the four-stage state (`local_edit`, `commit_ready`, `push_ready`, `mr_ready`) explicit in your plan or handoff, and stop at the earliest blocked delivery stage instead of discovering it after implementation.\n")
		fmt.Fprintf(&b, "5. Run `multica issue status %s in_progress` unless your Agent Identity forbids issue status changes; if it does, skip this step.\n", ctx.IssueID)
		b.WriteString("6. Complete the task within your Agent Identity boundaries. Do not investigate, implement, create issues, update issues, or delegate if your Agent Identity forbids that action; if your role is delegation-only, perform the allowed delegation work and stop once that outcome is delivered.\n")
		if ctx.IsSquadLeader {
			fmt.Fprintf(&b, "7. **Post your final results as a comment** (unless your outcome is `no_action` — in that case, calling `multica squad activity %s no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment announcing no_action or saying you are exiting silently): post it with `multica issue comment add %s` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n", ctx.IssueID, ctx.IssueID)
		} else {
			fmt.Fprintf(&b, "7. **Post your final results as a comment — this step is mandatory**: post it with `multica issue comment add %s` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n", ctx.IssueID)
		}
		b.WriteString("8. Before exiting: only if this run produced a fact that clears the high bar (important AND likely to be re-read by future runs on this same issue, e.g. a new PR URL or deploy URL), or you noticed a metadata key from entry that is now stale, pin or clear it via `multica issue metadata set`/`delete`. Most runs write nothing here — that is the expected outcome, not a gap. When in doubt, do not write. See the `## Issue Metadata` section above for the full bar.\n")
		fmt.Fprintf(&b, "9. When done, run `multica issue status %s in_review` unless your Agent Identity forbids issue status changes; if it does, skip this step.\n", ctx.IssueID)
		fmt.Fprintf(&b, "10. If blocked, run `multica issue status %s blocked` unless your Agent Identity forbids issue status changes. Post a comment explaining the blocker unless your Agent Identity forbids issue comments.\n\n", ctx.IssueID)
	}
