...
// Trae CLI: skills → {workDir}/.trae/skills/{name}/SKILL.md  (native discovery)
...
func resolveSkillsDir(workDir, provider string) (string, error) {
	...
	case "traecli":
		// Trae CLI auto-discovers project-level skills from .trae/skills/
		// in the workdir.
		skillsDir = filepath.Join(workDir, ".trae", "skills")
	default:
