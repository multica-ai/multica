...
func TestEnsureCodexSandboxConfig_WindowsDangerFullAccessStripsWindowsSection(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	configPath := filepath.Join(dir, "config.toml")
...
trust = "always"`
	...
	// Verify that the [windows] section header and sandbox key were stripped
	if strings.Contains(s, "[windows]") {
		t.Error("expected [windows] section header to be stripped, but it was found")
	}
	if strings.Contains(s, `sandbox = "unelevated"`) {
		t.Error("expected legacy sandbox setting to be stripped, but it was found")
	}

	// Verify the managed block contains the disabled sandbox setting
	expectedManaged := `# BEGIN multica-managed (do not edit; regenerated by daemon)
sandbox_mode = "danger-full-access"
windows.sandbox = "disabled"
# END multica-managed`

	if !strings.Contains(s, expectedManaged) {
		t.Errorf("expected managed block to contain windows.sandbox = \"disabled\", got:\n%s", s)
	}

	// Verify unrelated user settings are preserved
	if !strings.Contains(s, `model = "gpt-5.5"`) {
		t.Error("lost user model configuration")
	}
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions configuration")
	}
}
