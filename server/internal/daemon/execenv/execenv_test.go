...
func TestPrepareWithProjectResources(t *testing.T) {
		...
		ProjectResources: []ProjectResourceForEnv{
			{
				...
				ResourceRef:  json.RawMessage(`{"url":"https://github.com/dwickyfp/wallts","default_branch_hint":"main"}`),
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	resourcesPath := filepath.Join(env.WorkDir, ".wallts", "project", "resources.json")
...
func TestWriteProjectResourcesSkippedWhenNone(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".wallts", "project", "resources.json")); !os.IsNotExist(err) {
...
func TestWriteContextFilesAutopilotRunOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"wallts autopilot get autopilot-1 --output json",
		"no assigned issue",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "Run `wallts issue get") {
...
func TestInjectRuntimeConfigClaude(t *testing.T) {
	...
	s := string(content)
	for _, want := range []string{
		"Wallts Agent Runtime",
		"wallts issue get",
		"wallts issue comment list",
		"Go Conventions",
		"PR Review",
...
func TestInjectRuntimeConfigAvailableCommandsCoreOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"wallts issue create --title",
		"wallts issue update <id>",
		"--description-file <path>",
		"--parent \"\"",
		"wallts repo checkout <url>",
		"wallts issue status <id> <status>",
		"wallts issue comment add <issue-id>",
		"wallts issue comment add --help",
		"wallts squad member set-role <squad-id>",
	} {
		if !strings.Contains(s, want) {
	...
	for _, banned := range []string{
		"wallts issue list [--status",
		"wallts issue label list",
		"wallts issue subscriber list",
		"wallts label list",
		"wallts workspace member list",
		"wallts agent list",
		"wallts squad list",
		"wallts issue runs",
		"wallts issue run-messages",
		"wallts attachment download",
		"wallts autopilot list",
		"wallts autopilot create",
		"wallts autopilot update",
		"wallts autopilot trigger",
		"wallts autopilot delete",
		"wallts project get",
		"wallts project resource list",
		"wallts issue assign",
		"wallts issue label add",
		"wallts issue label remove",
		"wallts issue subscriber add",
		"wallts issue subscriber remove",
		"wallts issue comment delete",
		"wallts label create",
	} {
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Wallts Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "wallts issue get") {
		t.Error("should reference wallts CLI even without skills")
	}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `wallts issue comment add`
			// invocation for this issue — not just a prose mention of posting.
			mustContain := []string{
				"wallts issue comment add issue-1",
				"mandatory",
			}
			...
			for _, want := range []string{
				"Final results MUST be delivered via `wallts issue comment add`",
...
// as `?` before reaching `wallts.exe` (#2198 / #2236 / #2376).
//
// Not parallel: mutates the package-level runtimeGOOS.
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, walltsManagedBeginMarker) || !strings.Contains(s, walltsManagedEndMarker) {
	...
	// inherit from any surrounding table scope. See upsertWalltsManagedBlock
	// for why.
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), walltsManagedBeginMarker); n != 1 {
...
func TestEnsureCodexSandboxConfigHoistsAboveUserTables(t *testing.T) {
	...
	// permissions.wallts.sandbox_mode and Codex would never see it — see
...
`
	...
	beginIdx := strings.Index(s, walltsManagedBeginMarker)
	endIdx := strings.Index(s, walltsManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.wallts]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.wallts]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.wallts content")
	}
	...
	if n := strings.Count(string(data2), walltsManagedBeginMarker); n != 1 {
...
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.wallts], got:\n%s", s)
	}
	if strings.Count(s, walltsManagedBeginMarker) != 1 {
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if !strings.Contains(string(data), walltsManagedBeginMarker) {
		t.Error("reused config.toml missing wallts-managed block")
	}
}
...
// inside a Wallts task, despite the daemon redirecting CODEX_HOME to a
// per-task directory.
...
func TestInjectRuntimeConfigCommentTriggerColdStartRead(t *testing.T) {
	...
	for _, want := range []string{
		...
		"wallts issue comment list " + issueID + " --thread " + triggerID + " --tail 30 --output json",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "wallts issue comment list "+issueID+" --thread "+triggerID+" --output json") {
...
func TestInjectRuntimeConfigAssignmentTriggerMentionsRecent(t *testing.T) {
	...
	for _, want := range []string{
		"wallts issue comment list issue-1 --output json",
...
func TestInjectRuntimeConfigIssueMetadataSectionScope(t *testing.T) {
	...
	coreDiscoveryLines := []string{
		...
		"wallts issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]",
		"wallts issue metadata delete <issue-id> --key <k>",
	}
	...
	withoutSection := wantSection{
		// We can't simply require `wallts issue metadata list` absent
	...
	cases := []struct {
		...
		},
		{
			...
			filename: "CLAUDE.md",
			workflowStepPresent: []string{
				"wallts issue metadata list issue-md-2 --output json",
				"See the `## Issue Metadata` section above",
				"wallts issue metadata set",
				"wallts issue metadata delete",
				"Before exiting",
...
func TestInjectRuntimeConfigIssueMetadataCodexFormattingUnchanged(t *testing.T) {
	...
	t.Run("linux_heredoc", func(t *testing.T) {
		...
		if !strings.Contains(s, "wallts issue metadata list issue-md-codex --output json") {
