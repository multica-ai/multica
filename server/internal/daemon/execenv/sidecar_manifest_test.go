...
// nothing related to Wallts before a task ran must contain nothing
// related to Wallts after the task finishes — no .agent_context/,
// no .claude/skills/, no .wallts/, no stub directories. The test
...
// must still exist and the Wallts-written sibling must be gone.
...
// round-trip to the .wallts/project/resources.json branch — a separate
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .wallts/sidecar.txt and created
	// .wallts/ + .wallts/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .wallts/.
	managedDir := filepath.Join(workDir, ".wallts")
	...
	// .wallts still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-wallts/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug Wallts would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a Wallts skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the Wallts
// sibling (which lives at `<slug>-wallts`) must be fully removed by
// CleanupSidecars.
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			ctx := TaskContextForEnv{
				...
				AgentSkills: []SkillContextForEnv{
					{
						Name:        "Issue Review",
						Description: "Wallts's version",
						Content:     "---\nname: issue-review\n---\n\nWallts skill content.\n",
						Files: []SkillFileContextForEnv{
							{Path: "templates/checklist.md", Content: "- [ ] check"},
						},
					},
...
// — a Wallts-only namespace file. If the user already has a file at
...
// is the matching byte-exact matrix for `.wallts/project/
// resources.json` — the other Wallts-only namespace file. Same
...
func TestPrepareThenCleanupSidecarsProjectResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".wallts", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".wallts", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
// first try the natural slug, then `-wallts`, then `-wallts-2`,
// `-wallts-3`, … The PR-review concern is "Wallts skill must still
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-wallts`.
	...
	if slug != "issue-review-wallts" {
		t.Errorf("second allocation should bump to `-wallts`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-wallts") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-wallts"), 0o755); err != nil {
	...
	if slug != "issue-review-wallts-2" {
		t.Errorf("third allocation should be `-wallts-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-wallts-2") {
...
// `Issue Review` skill, the Wallts sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// Wallts sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the Wallts skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Wallts's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	walltsDir := filepath.Join(workDir, ".claude", "skills", "issue-review-wallts")
	if _, err := os.Stat(filepath.Join(walltsDir, "SKILL.md")); err != nil {
		t.Errorf("Wallts sibling skill should exist at %s: %v", walltsDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// Wallts sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(walltsDir); !os.IsNotExist(err) {
		t.Errorf("Wallts sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
