...
// — users cannot audit or edit the contents from any Wallts UI.
//
// This conflicts with the Wallts daemon's context model. Wallts
...
//     unrelated host-project context into Wallts tasks. The reproduction
//     in github.com/dwickyfp/wallts#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Wallts issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Wallts task
// (and accept the leak risk) can keep the feature enabled by setting
// `WALLTS_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Wallts-owned, user-visible,
...
// WalltsCodexMemoryEnv is the env var users can set to keep Codex
...
const WalltsCodexMemoryEnv = "WALLTS_CODEX_MEMORY"

const (
	walltsMemoryFeatureBeginMarker = "# BEGIN wallts-managed memory-feature (do not edit; regenerated by daemon)"
	walltsMemoryFeatureEndMarker   = "# END wallts-managed memory-feature"
	walltsMemoryConfigBeginMarker  = "# BEGIN wallts-managed memory-config (do not edit; regenerated by daemon)"
	walltsMemoryConfigEndMarker    = "# END wallts-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(walltsMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(walltsMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(walltsMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(walltsMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(WalltsCodexMemoryEnv))
...
// renderWalltsMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderWalltsMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(walltsMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(walltsMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderWalltsMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderWalltsMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(walltsMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(walltsMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderWalltsMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderWalltsMemoryConfigBlock(true))
}
...
//
// When WALLTS_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per WALLTS_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderWalltsMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderWalltsMemoryConfigBlock(false))
	}
