...
func writeAvailableCommandsQuickCreate(b *strings.Builder) {
	...
	b.WriteString("`--output json` writes JSON to stdout; confirmations and warnings go to stderr. Do not merge them (`2>&1`) into anything that parses the output — that makes a write that SUCCEEDED look like it failed and invites a duplicate retry.\n\n")
	b.WriteString("### Core\n")
	b.WriteString("- `multica issue create --title \"...\" [--description \"...\" | --description-file <path> | --description-stdin] [--priority X] [--status X] [--assignee X | --assignee-id <uuid>] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <YYYY-MM-DD>] [--attachment <path>]` — Create a new issue; `--attachment` may be repeated. Inline `--description \"...\"` is only for a short single-line body with no code, quotes, backticks or `$()`. Anything multi-line, or carrying code snippets / file paths / quotes / backticks / `$()` — which quick-create descriptions usually are — MUST go to a file, because the shell rewrites or truncates rich text passed inline (MUL-2904). Prefer `--description-file <path>` over `--description-stdin` (flags after a HEREDOC terminator can be silently swallowed, #4182). Write that file inside your working directory (e.g. `./description.md`), never `/tmp` or shared paths, and use a permitted file-write method — never run `--description-file` against a file whose write did not succeed. The CLI rejects a path outside the workdir so a stale file from another run can't leak in (MUL-4252).\n\n")
	if runtimeGOOS == "windows" {
		b.WriteString("On Windows, prefer your file-write tool to create the UTF-8 description. PowerShell may run in `ConstrainedLanguage`, which blocks .NET file APIs such as `[System.IO.File]::WriteAllText` and constructing `System.Text.UTF8Encoding`. If you must use PowerShell, use `Set-Content -LiteralPath ./description.md -Value $description -Encoding utf8 -ErrorAction Stop`, where `$description` contains literal text (for example, a single-quoted here-string whose closing delimiter does not occur in the content). If a write method is denied, retry the write with your file-write tool or this compatible cmdlet; do not disable the sandbox or elevate just to write the description. Continue with `issue create --description-file` only after the replacement write succeeds. If no permitted writer succeeds, report the failure without creating an issue from a stale or partial file.\n\n")
	}
}
