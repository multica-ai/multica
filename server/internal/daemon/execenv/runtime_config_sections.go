...
func writeCommentFormatting(b *strings.Builder) {
	...
	if runtimeGOOS == "windows" {
		b.WriteString("On Windows, **always write the comment body to a UTF-8 file with your file-write tool first, then post it with `--content-file <path>`** — do NOT pipe via `--content-stdin` (Windows PowerShell 5.1's `$OutputEncoding` may replace non-ASCII characters with `?`). When you read or write text files from PowerShell 5.1, specify UTF-8 explicitly: `Get-Content -Raw -Encoding UTF8` and `Set-Content -Encoding UTF8`; do not rely on BOM-less file defaults, which can use the ANSI code page (for example cp936/GBK on zh-CN Windows). Never use inline `--content` for agent-authored comments. Write the file inside your working directory, never `/tmp` or shared paths (MUL-4252). Keep the same `--parent` value from the trigger comment when replying. Delete the temp file (`Remove-Item ./reply.md`) after posting; do not rely on `\\n` escapes.\n\n")
		return
	}
