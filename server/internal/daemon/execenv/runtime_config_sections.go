...
func writeAvailableCommands(b *strings.Builder, ctx TaskContextForEnv) {
	...
	// Keep assignment in the core brief so agents can distinguish a handoff
	// that starts the new owner from an ownership-only change for an assignee
	// already handling the issue.
	b.WriteString("- `multica issue assign <id> (--to X | --to-id <uuid> | --unassign) [--no-start]` — change ownership. On assign/update/status, `--no-start` records the change without starting another run — use it when the target assignee is already handling this issue from another run. To hand the issue to another agent, reassign without `--no-start`; this starts a new run for the new owner.\n")
