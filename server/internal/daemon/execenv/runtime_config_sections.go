...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("- The platform now auto-syncs the two lifecycle boundaries, so a missed manual write cannot leave the board inverted: when your run moves to `running` an unstarted issue becomes `in_progress` (and an issue wrongly left in `in_review` is pulled back to `in_progress`); when a direct delivery's run moves to `completed` an `in_progress`/unstarted issue becomes `in_review`. Squad leader dispatch turns are exempt from the completion sync — a leader keeps the parent `in_progress` while members work.\n")
