...
func writeBackgroundTaskSafetySlim(b *strings.Builder, interactive bool) {
	...
	if interactive {
		...
		b.WriteString("When the work in your current run is actually finished, first collect all tool results and update the issue status as appropriate, then call `multica issue finish-run`. This requests completion of only your current run, even if another agent is assigned to the issue; it does not finish other agents' runs or change the issue status. After it succeeds, deliver your final reply and yield normally: Multica completes this run only after that reply finishes. Do not call it for greetings, progress updates, questions, or while awaiting review/results. Changing an issue status alone does not finish the run. Human input or interruption cancels the completion request; finish any newly requested work and request completion again.\n\n")
	} else {
