...
func cerebroNoBusyWaitRule() string {
	...
	add("- **Any other external state you would otherwise wait on:** report what you are waiting on (in a comment or issue metadata) and exit, instead of polling. But \"exit\" is not the same as \"I'll be re-run for free\": the platform only re-runs you on a concrete event — a new comment, a status change, or a CI/deploy update. **Waiting on the clock is not an event.** If you need to resume after some elapsed time (\"check back in 2 hours\"), you MUST schedule a wakeup first (see \"Wakeup before waiting\" below) — otherwise nothing re-runs you and the issue stalls silently. A stopped run costs nothing; a polling run costs the whole budget.\n")
