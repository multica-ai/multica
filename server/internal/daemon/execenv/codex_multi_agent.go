...
// subagents. The Wallts daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Wallts runtime
...
//
// Users who explicitly want Codex native subagents inside a Wallts task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `WALLTS_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// WalltsCodexMultiAgentEnv is the env var users can set to keep Codex
...
const WalltsCodexMultiAgentEnv = "WALLTS_CODEX_MULTI_AGENT"

// walltsMultiAgentBeginMarker / walltsMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	walltsMultiAgentBeginMarker = "# BEGIN wallts-managed multi-agent (do not edit; regenerated by daemon)"
	walltsMultiAgentEndMarker   = "# END wallts-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(walltsMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(walltsMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(WalltsCodexMultiAgentEnv))
...
// renderWalltsMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderWalltsMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(walltsMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(walltsMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderWalltsMultiAgentBlock(true)
...
//
// When WALLTS_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per WALLTS_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderWalltsMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
