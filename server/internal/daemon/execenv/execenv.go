...
func Reuse(params ReuseParams, logger *slog.Logger) *Environment {
	...
	// from the retired per-task HOME redirection (#6218); it is inert — the
	// daemon no longer injects HOME/XDG overrides, and the managed config
	// rewrite below drops its stale writable_roots grant.
	if params.Provider == "codex" {
