...
func cerebroToolsBrief(entries []ToolBriefEntry) string { // CEREBRO-PATCH(cerebro-tools-brief): FIR-2312 dynamic per-permission tools section for Available Commands
	...
	b.WriteString("A **connection** is an external API or MCP server a workspace admin wired into this workspace (for example a customer-service backend or a data registry). Its tools show up below and you call them like any other tool. To learn a tool's exact arguments, read its tool schema (MCP tools are self-describing) or call it once and read the error — the same way you run `--help` for a `multica` command. Tools marked `(ask)` pause for human approval when you call them.\n\n")
