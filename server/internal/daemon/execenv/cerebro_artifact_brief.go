...
**Show a document on a comment / chat / DM / channel — use the white card, never a grey ` + "`--attachment`" + `.** When your titled output (` + "`.md`" + ` report, plan, analysis, note) is meant to appear in a message, create it with ` + "`multica artifact create`" + ` / ` + "`multica document create`" + ` and then reference it with the ` + "`--artifact <id>`" + ` flag (preferred) OR, if you hand-write it, as a **real markdown link** ` + "`[document](mention://artifact/<id>)`" + ` inside the message body. The bare string ` + "`mention://artifact/<id>`" + ` is NOT autolinked — it renders as plain text with no card, on both web and mobile. The platform renders the markdown link as a compact **white card**; clicking it opens the full note editor. This is the rule on all four message surfaces:
- **Issue comment** — pass ` + "`--artifact <id>`" + ` to ` + "`multica issue comment add`" + `, or write ` + "`[document](mention://artifact/<id>)`" + ` in the ` + "`--content`" + ` body (HEREDOC over ` + "`--content-stdin`" + `, per ## Comment Formatting).
- **Chat session** — pass ` + "`--artifact <id>`" + ` to ` + "`multica chat session send`" + `, or write ` + "`[document](mention://artifact/<id>)`" + ` in ` + "`--content`" + `.
- **DM** and **channel** — same: write ` + "`[document](mention://artifact/<id>)`" + ` (a real markdown link) in the message text.
...
> The first-class ` + "`--artifact <id>`" + ` flag for ` + "`issue comment add`" + ` / ` + "`chat session send`" + ` (FIR-1904) is now LIVE — it injects the white-card mention link for you, so prefer it. If you ever hand-write the link, it MUST be a markdown link ` + "`[document](mention://artifact/<id>)`" + `; the bare string ` + "`mention://artifact/<id>`" + ` is not autolinked and shows as plain text.
