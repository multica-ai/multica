...
// `no such host` errors when calling out (for example, `wallts issue get`
// hitting the Wallts API). See upstream issue openai/codex#10390.
//
...
// actually reach the Wallts API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the Wallts CLI can reach the API.
...
// walltsManagedBeginMarker / walltsManagedEndMarker delimit the block the
...
const (
	walltsManagedBeginMarker = "# BEGIN wallts-managed (do not edit; regenerated by daemon)"
	walltsManagedEndMarker   = "# END wallts-managed"
)

// renderWalltsManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.wallts]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertWalltsManagedBlock), avoids both traps.
func renderWalltsManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(walltsManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(walltsManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(walltsManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(walltsManagedEndMarker) + `\n*`)

// upsertWalltsManagedBlock returns the config content with the wallts-managed
...
// `[permissions.wallts]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderWalltsManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertWalltsManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderWalltsManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the wallts-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertWalltsManagedBlock(existing, policy)
