...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" {
		loginHint := "'forge login'"
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'forge login --profile %s'", d.cfg.Profile)
		}
...
// `forge repo checkout` accepts project-only URLs without an extra round
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by Forge Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// For brew installs, it keeps the symlink path (e.g. /opt/homebrew/bin/forge)
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (TaskResult, error) {
	...
	// `forge repo checkout` would reject project-only URLs that aren't also
	...
	// can call the Forge API and the local daemon (e.g. `forge repo checkout`).
	...
	// Quick-create marker — when set, the forge CLI's `issue create`
	...
	// Ensure the forge CLI is on PATH inside the agent's environment.
	// Some runtimes (e.g. Codex) run in an isolated sandbox that may not
	// inherit the daemon's PATH. Prepend the directory of the running
	// forge binary so that `forge` commands in the agent always resolve.
