...
type Config struct {
	...
	Agents                         map[string]AgentEntry // keyed by provider: claude, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor, kimi, kiro, antigravity, warp
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	if e, ok := probe("MULTICA_WARP_PATH", "oz", "MULTICA_WARP_MODEL"); ok {
		agents["warp"] = e
	}
	if len(agents) == 0 {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor-agent, kimi, kiro-cli, agy, or oz and ensure it is on PATH")
	}
...
var defaultAgentCommandNames = []string{
	...
	"gemini", "pi", "cursor-agent", "copilot", "kimi", "kiro-cli", "agy", "oz",
}
