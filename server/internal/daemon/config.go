...
type Config struct {
	...
	Agents                         map[string]AgentEntry // keyed by provider: claude, codebuddy, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor, kimi, kiro, DeepSeek-TUI, antigravity, qoderclicn
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	if e, ok := probe("MULTICA_CODEBUDDY_PATH", "cbc", "MULTICA_CODEBUDDY_MODEL"); ok {
		agents["codebuddy"] = e
	}
	...
	if len(agents) == 0 {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, cbc (codebuddy), codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor-agent, kimi, kiro-cli, DeepSeek-TUI (deepseek-tui), agy, or qoderclicn and ensure it is on PATH")
	}
	...
	// Workspaces root: override > env > default (~/multica_workspaces or
	// profile/configPath-specific instance state dir).
	workspacesRoot, err := ResolveWorkspacesRoot(profile, configPath, overrides.WorkspacesRoot)
	if err != nil {
		return Config{}, err
	}
...
// MULTICA_WORKSPACES_ROOT env > explicit config-path state dir > default
...
// directly so they pick the same directory the running daemon would have
// picked.
func ResolveWorkspacesRoot(profile, configPath, override string) (string, error) {
	...
	if root == "" {
		if configPath != "" {
			...
			if err != nil {
				return "", fmt.Errorf("resolve state dir for workspaces root: %w", err)
			}
			root = filepath.Join(stateDir, "workspaces")
		} else {
