...
type Config struct {
	...
	Agents                         map[string]AgentEntry // keyed by provider: claude, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor, kimi, kiro, devin
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	devinPath := envOrDefault("MULTICA_DEVIN_PATH", "devin")
	if _, err := exec.LookPath(devinPath); err == nil {
		agents["devin"] = AgentEntry{
			Path:  devinPath,
			Model: strings.TrimSpace(os.Getenv("MULTICA_DEVIN_MODEL")),
		}
	}
	if len(agents) == 0 {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor-agent, kimi, kiro-cli, or devin and ensure it is on PATH")
	}
