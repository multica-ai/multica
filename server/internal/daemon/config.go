...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	if e, ok := probe("MULTICA_GEMINI_PATH", "agy", "MULTICA_GEMINI_MODEL"); ok {
		agents["gemini"] = e
	}
	...
	if len(agents) == 0 {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, codex, copilot, opencode, openclaw, hermes, agy, pi, cursor-agent, kimi, or kiro-cli and ensure it is on PATH")
	}
...
var defaultAgentCommandNames = []string{
	...
	"agy", "pi", "cursor-agent", "copilot", "kimi", "kiro-cli",
}
