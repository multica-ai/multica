...
func buildQuickCreatePrompt(task Task) string {
	...
	case agentID != "":
		fmt.Fprintf(&b, "    - When the user did NOT name an assignee, default to the CORE SQUAD for core-development / architecture / security / complex tasks: check the workspace squads with `multica squad list --output json` for `pge-core-squad` (or the squad named in the workspace's `default_core_squad_id` setting) and pass `--assignee-id <that squad's id>`. This mirrors the platform's own default for such tasks. For ops / inspection / light tasks, default to YOURSELF: pass `--assignee-id %q` (your agent UUID). The picker agent is the expected owner for light work, and the squad is the expected owner for core work — never leave the issue unassigned, and use the UUID flag, not `--assignee <name>`, so the assignment is unambiguous even when other agents share part of your name.\n\n", agentID)
	case agentName != "":
		fmt.Fprintf(&b, "    - When the user did NOT name an assignee, default to the CORE SQUAD for core-development / architecture / security / complex tasks: check the workspace squads with `multica squad list --output json` for `pge-core-squad` (or the squad named in the workspace's `default_core_squad_id` setting) and pass `--assignee-id <that squad's id>`. This mirrors the platform's own default for such tasks. For ops / inspection / light tasks, default to YOURSELF: pass `--assignee %q`. The picker agent is the expected owner for light work, and the squad is the expected owner for core work — never leave the issue unassigned.\n\n", agentName)
	default:
		b.WriteString("    - When the user did NOT name an assignee, default to the CORE SQUAD for core-development / architecture / security / complex tasks: check the workspace squads with `multica squad list --output json` for `pge-core-squad` (or the squad named in the workspace's `default_core_squad_id` setting) and pass `--assignee-id <that squad's id>`. This mirrors the platform's own default for such tasks. For ops / inspection / light tasks, default to YOURSELF (the picker agent): pass `--assignee-id <your agent UUID>` (preferred) or `--assignee <your agent name>`. Never leave the issue unassigned.\n\n")
	}
