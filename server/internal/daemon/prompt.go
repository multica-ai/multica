...
func BuildPrompt(task Task, provider string) string {
	...
	// CEREBRO-PATCH(rounds-answer-snapshots): FIR-3179 — no release-time Round prompt; member replies retain the standard comment prompt.
	...
	fmt.Fprintf(&b, "For comment history, follow the rule in your runtime workflow file (assignment-triggered tasks treat the read as mandatory). Start with `multica issue comment list %s --recent 20 --output json` to read the 20 most recently active threads, then page older threads via the stderr `Next thread cursor: ...` line and the matching `--before` / `--before-id` until you have enough history. Resolved threads come back folded — `--full` to expand. `--since <RFC3339>` is still available for incremental polling and may combine with `--recent`.\n", task.IssueID)
	return b.String()
}

func buildWakeupPrompt(task Task, provider string) string {
	var b strings.Builder
	b.WriteString("You are running as a local coding agent for a Multica workspace.\n\n")
	fmt.Fprintf(&b, "Your assigned issue ID is: %s\n\n", task.IssueID)
	...
	if task.WakeupTriggerType == "approval" {
		...
		// CEREBRO-PATCH(wakeup-task-mandate): TECH-3548 a scheduled wakeup note is a real task — execute it; never classify it as a stale re-fire without re-checking live state.
		b.WriteString("✅ The wakeup note below is a REAL task you (or an owner) scheduled for this exact moment. Your job is to EXECUTE it now. Do NOT classify it as a stale re-fire, duplicate trigger, or already-handled based on reading the thread — actively re-verify the current live state (open the actual site, run the command, check the deploy) BEFORE concluding the work is done. Misreading a real verify/deploy task as 'nothing to do' is the failure this prevents.\n\n")
	...
	if bundle, ok := bundleContextInstruction(task); ok {
		b.WriteString(bundle)
		if task.TriggerCommentID != "" {
			b.WriteString(execenv.BuildCommentReplyInstructions(provider, task.IssueID, task.TriggerCommentID))
		}
	...
	fmt.Fprintf(&b, "Start by running `multica issue context %s --output json` to understand the issue context and recent thread.\n\n", task.IssueID)
	if task.TriggerCommentID != "" {
		b.WriteString(execenv.BuildCommentReplyInstructions(provider, task.IssueID, task.TriggerCommentID))
	} else {
		fmt.Fprintf(&b, "If you reply, post it with `multica issue comment add %s` using `--content-stdin`.\n\n", task.IssueID)
	}
	return b.String()
}
...
func buildCommentPrompt(task Task, provider string) string {
	...
	if task.TriggerCommentContent != "" {
		...
		if task.TriggerCommentCreatedAt != "" {
			fmt.Fprintf(&b, "Sent at: %s\n\n", task.TriggerCommentCreatedAt)
		}
		if task.TriggerAuthorType == "agent" {
