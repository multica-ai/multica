...
func buildQuickCreatePrompt(task Task) string {
	...
	b.WriteString("- priority: one of `urgent`, `high`, `medium`, `low`, or omit. Map P0/P1 → urgent/high; \"asap\"/\"urgent\" (in any language) → urgent; \"low priority\" (in any language) → low. If unspecified, omit.\n")
	b.WriteString("- assignee:\n")
	b.WriteString("    - When the user names someone (\"assign to X\" / \"@X\" or equivalent in any language), call `multica workspace members --output json` and find the matching member by display name (case-insensitive substring match is fine). On a clean match, pass `--assignee <name>`. On no match or ambiguous match, do NOT pass `--assignee` — instead append a final line to the description: `Unrecognized assignee: X`.\n")
	agentName := ""
	if task.Agent != nil {
