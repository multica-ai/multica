...
func buildQuickCreatePrompt(task Task) string {
	...
	b.WriteString("     Inline images: keep every inline image reference exactly where it appears in the input — the interleaving of text and images is meaningful (a sentence may refer to the image directly above or below it); never move, reorder, drop, or duplicate them.\n\n")
	...
	b.WriteString("- On CLI error or JSON parse error, exit with a single line explaining why the issue was not created, as the only output. The platform delivers that line back to the requester in chat, so phrase it for them: plain language, no CLI flags, commands, or retry-with-flag suggestions — they cannot run commands from chat. For a duplicate-issue error, name the existing issue's identifier and title, e.g. `An active issue with the same title already exists: ND-2 — What's up (in review)`.\n")
	return b.String()
}
