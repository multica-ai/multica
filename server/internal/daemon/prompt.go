...
func buildQuickCreatePrompt(task Task) string {
	...
	b.WriteString("  1. **User request** — Faithfully restate what the user wants done, in their own terms. Preserve the user's phrasing, tone, and scope. Do NOT paraphrase substantive content into generic language (e.g. don't turn \"把这个按钮改成红色\" into \"UI improvement needed\"). Do NOT add implementation plans, acceptance criteria, design decisions, or constraints the user did not express.\n\n")
	b.WriteString("     **Strip these BEFORE writing the User request — removing non-spec material is NOT paraphrasing:**\n")
	b.WriteString("     - **Routing meta-instructions to the issue-creating agent itself.** Verbal wrappers about how to create the issue: \"create an issue / ticket\", \"open a ticket\", \"记一下\", \"帮我建个 issue\", \"assign to X\", \"分配给 X\". The user's underlying intent stays, only the verbal routing wrapper goes.\n")
	b.WriteString("     - **Pure conversational fillers with no information.** Discourse markers like \"对吧？\", \"嗯\", \"你看\", \"那个…\", or \"比如\" used as filler (NOT \"比如\" introducing a concrete example). When unsure whether a phrase carries information, keep it.\n\n")
	b.WriteString("     **CC mentions are special — strip the verbal \"cc\" wrapper, but PRESERVE the mention link.** `multica issue create` has no `--subscriber` / `--cc` flag; the platform auto-subscribes members whose `[@Name](mention://member/<uuid>)` link appears anywhere in the description body. So when the user wrote \"cc @Y\" / \"另外 cc @Y\" / \"通知 @Y\", drop the verbal \"cc\" wrapper from the User request, then append a single trailing line `CC: [@Y](mention://member/<uuid>)` to the description (multiple cc'd members on the same line, space-separated). Without this line, the cc routing has nowhere to land and @Y will not be subscribed.\n")
	b.WriteString("     For the **assignee** mention (\"assign to @X\" / \"分配给 X\"), the `--assignee-id` flag IS the routing channel — drop the `@X` from the description body and rely on the flag; do not also add an assignee mention to the body.\n\n")
	b.WriteString("  2. **Context** — include this section ONLY when the input cited external references AND you successfully fetched them AND they yielded verifiable factual content worth recording. Fetch referenced resources (PRs, issues, docs) and summarize the relevant factual content. Keep summaries to verifiable facts only (e.g. \"PR #45 changes the auth middleware to use JWT\" not \"this suggests we should also update the tests\"). Preserve any image URLs or markdown image references inline.\n\n")
	b.WriteString("     **Do NOT emit a Context section to:**\n")
	b.WriteString("     - Apologize for resources you could not fetch (e.g. an image attachment that was not piped through to this run). Stay silent — the executing agent can ask the user for the original if needed.\n")
	b.WriteString("     - Restate facts already expressed in the User request.\n")
	b.WriteString("     - Add your own commentary, design suggestions, or \"reference implementations\" the user did not cite.\n\n")
	b.WriteString("     If there is nothing factual to add, omit the Context section entirely. An empty or apologetic Context section is worse than none.\n\n")
	...
	b.WriteString("  - NEVER reduce the user's multi-sentence input to a single vague sentence. If the user wrote three sentences of substantive content, the description should carry at least that much information.\n")
