...
func buildCommentPrompt(task Task, provider string) string {
	...
	if task.TriggerCommentContent != "" {
		...
		if task.Agent != nil && strings.Contains(task.Agent.Instructions, "## Squad Operating Protocol") {
			fmt.Fprintf(&b, "⚠️ **Squad leader no_action rule:** If you decide no action is needed, call `multica squad activity %s no_action --reason \"...\"` and EXIT. DO NOT post any comment — not even one that says \"no action needed\" or \"exiting silently\". The squad activity call records your decision; a comment is redundant noise. If you delegated to multiple members and need all of their results before deciding, call `multica squad await %s --members <name-a>,<name-b>` first, then `multica squad activity %s no_action --reason \"awaiting members\"` and EXIT.\n\n", task.IssueID, task.IssueID, task.IssueID)
		}
	}
