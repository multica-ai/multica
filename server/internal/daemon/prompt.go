...
func buildQuickCreatePrompt(task Task) string {
	...
	b.WriteString("     Every uploaded image or file Markdown reference in the user input is durable issue context: copy each reference verbatim into the description. Even when rewriting the surrounding prose, never summarize it away or remove it as routing or conversational material.\n\n")
