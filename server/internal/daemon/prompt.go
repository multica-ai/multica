...
func buildChannelMentionPrompt(task Task) string {
	...
	if task.ChannelThreadRootMsgID != "" {
		fmt.Fprintf(&b, "When you need to share a final result, reply in the same thread (the user @-mentioned you from the replies panel): `multica channel message reply %s %s --content \"...\"`. Do NOT reply to the triggering message directly (that would create a nested thread). Use `multica channel message send %s --content \"...\"` only when the triggering message or recent conversation explicitly asks for a main-timeline answer. If no visible reply is warranted, finish silently after doing the required work.\n", task.ChannelID, task.ChannelThreadRootMsgID, task.ChannelID)
	} else {
		fmt.Fprintf(&b, "When you share a final result, post on the main channel timeline (the user @-mentioned you from the main message area): `multica channel message send %s --content \"...\"`. Do NOT use `message reply` unless the triggering message or recent conversation explicitly asks you to answer in a thread instead. If no visible reply is warranted, finish silently after doing the required work.\n", task.ChannelID)
	}
	return b.String()
}
