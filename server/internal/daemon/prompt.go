...
func buildCommentPrompt(task Task, provider string) string {
	...
	// Comment-reading pointer. Warm path with new comments: since-delta. Warm
	// resumed path with no new comments: the trigger is already injected, so don't
	// force a duplicate thread read. Cold path: read the triggering thread, not
	// the flat timeline. Final fallback (no trigger id, shouldn't happen here):
	// plain read.
