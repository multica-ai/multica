...
func buildQuickCreatePrompt(task Task) string {
	...
	b.WriteString("  Passing the description: prefer inline `--description \"...\"`. If you instead write the body to a file for `--description-file`, that file MUST live inside your current working directory (e.g. `./description.md`) — never `/tmp` or any machine-shared path, where a different run may have left a stale file that would silently become this issue's description. If the file write fails for any reason, stop and fix it; never run `--description-file` against a file whose write did not succeed.\n\n")
