...
func (s *TaskService) ClaimTask(ctx context.Context, agentID pgtype.UUID) (*db.AgentTaskQueue, error) {
	...
	// CEREBRO-PATCH(task-claim-workspace-pause-guard): defense-in-depth —
	// even if a task survived the bulk-cancel during pause (e.g. created in
	// the brief window between toggle and cancel), don't claim it. The
	// enqueue gate is the primary guard; this is the safety net.
...
func (s *TaskService) CompleteTask(ctx context.Context, taskID pgtype.UUID, result []byte, sessionID, workDir string) (*db.AgentTaskQueue, error) {
	...
	if task.ChatSessionID.Valid {
		...
		if err := json.Unmarshal(result, &payload); err == nil && payload.Output != "" {
			row, err := s.Queries.GetAssistantChatMessageByTaskID(ctx, task.ID)
			if err != nil {
				slog.Error("failed to load assistant chat message", "task_id", util.UUIDToString(task.ID), "error", err)
			} else {
				assistantMsg = &row
			}
		}
