...
// TestWebhookRepairLeavesRunUnboundOnPendingCollision covers the rarest race: the
// agent already holds its one pending task per (issue, agent) — an unstamped comment
// task — so a fresh dispatched enqueue would violate the constraint. The repair must
// neither crash-loop the webhook worker nor bind the run to that pending comment
// task; it leaves the run unbound for the stuck-run monitor to reconcile.
func TestWebhookRepairLeavesRunUnboundOnPendingCollision(t *testing.T) {
	ctx := context.Background()
	svc, agentID, run, _, insertTask := newCreateIssueRunFixture(t)
	ap, err := svc.Queries.GetAutopilot(ctx, run.AutopilotID)
	if err != nil {
	...
	insertTask(agentID, 0, "queued", pgtype.UUID{}) // pending unstamped comment task holds the slot
	...
	if got.TaskID.Valid {
		t.Fatalf("webhook repair bound the run despite the pending-task collision: task_id=%x", got.TaskID.Bytes)
	}
}
