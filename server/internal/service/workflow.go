...
func (s *WorkflowService) StartWorkflow(ctx context.Context, workflowID pgtype.UUID, triggerType string, triggerID pgtype.UUID) (*db.WorkflowRun, error) {
	...
	if workflow.Mode == "orchestrated" {
		return s.startOrchestratedWorkflow(ctx, workflow, triggerType, triggerID)
	}
...
func (s *WorkflowService) startOrchestratedWorkflow(ctx context.Context, workflow db.Workflow, triggerType string, triggerID pgtype.UUID) (*db.WorkflowRun, error) {
	...
	planningComment := fmt.Sprintf("**Workflow Planning**\n\n%s\n\n---\nYou are the planner agent for this workflow. Your job is to create an execution plan.\n\n"+
		...
		prompt, util.UUIDToString(run.ID))
...
func (s *WorkflowService) StartCEOCommand(
	...
	run, err := s.startOrchestratedWorkflow(ctx, workflow, "command", userID)
	if err != nil {
