...
func (s *IssueService) Create(ctx context.Context, p IssueCreateParams, opts IssueCreateOpts) (IssueCreateResult, error) {
	...
	labels, err := validateIssueLabels(ctx, qtx, p.WorkspaceID, projectID, p.LabelIDs)
	if err != nil {
...
var ErrIssueLabelNotFound = errors.New("label not found")
var ErrIssueLabelProjectMismatch = errors.New("label is not available for this issue's project")

func validateIssueLabels(ctx context.Context, q *db.Queries, workspaceID pgtype.UUID, projectID pgtype.UUID, labelIDs []pgtype.UUID) ([]db.IssueLabel, error) {
	if len(labelIDs) == 0 {
		return nil, nil
	}
	labels := make([]db.IssueLabel, 0, len(labelIDs))
	for _, labelID := range labelIDs {
		...
		if label.ProjectID.Valid && (!projectID.Valid || label.ProjectID != projectID) {
			return nil, ErrIssueLabelProjectMismatch
		}
