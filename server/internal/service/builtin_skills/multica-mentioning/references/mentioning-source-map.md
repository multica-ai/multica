...
| authority lineage is persisted per-action: an agent editing its own comment re-stamps `source_task_id` to the current editing task (issue-scoped, like create), so a cross-issue edit clears it and every authority/originator read fails closed — preview, save, and reconcile agree (MUL-4857) | `server/internal/handler/comment.go` (search `commentSourceTaskIDForIssue` and its use in `UpdateComment`) |
