...
| `custom_args` | `agent.custom_args` (JSON array) | JSON shape checked CLI-side; Windows Codex runtimes use the managed `-c`, `windows.sandbox="unelevated"` prefix unless copied config, fixed runtime/profile arguments, or an explicit per-agent override already own the setting. `is_codex_windows_sandbox_arg_managed` stores provenance separately so an identical user-authored pair is never removed; runtime-only edits add or remove only the proven managed prefix as the platform changes | daemon (extra CLI switches); defaults to `[]` outside Windows Codex |
