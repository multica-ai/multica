...
| Managed Windows Codex prefix provenance | `runAgentCopy`, `copiedAgentCustomArgs` | A source prefix is removed only when `is_codex_windows_sandbox_arg_managed` proves platform ownership; create re-derives the target default. An identical user-owned pair and explicit `--custom-args` remain authoritative | focused `cmd_agent_copy_test.go` cases |
...
| Windows Codex sandbox argv default | migration `327`; `cmd/multica/cmd_agent{,_copy}.go`; `internal/handler/{agent,agent_custom_args,daemon,mika_agent,onboarding_shim}.go`; `internal/daemon/{daemon,execenv/{execenv,codex_home,codex_sandbox}}.go`; `pkg/agent/{codex,codex_windows_sandbox_args}.go`; `packages/core/{agents,api}`; `packages/views/agents/components/tabs/custom-args-tab.tsx` | Registration records OS/fixed/shared ownership only as a persistence and settings-preview hint. Prepare copies the current config and exposes task-scoped ownership; one task policy then drives effective argv and final ExecOptions, so add/change preserves explicit/elevated precedence and removal restores the exact managed `-c`, `windows.sandbox="unelevated"` pair without `danger-full-access`. Runtime-only updates re-read/normalize under a row lock; migration 327 down removes one proven leading managed pair before dropping provenance. Explicit overrides win without duplication, copy/web/CLI ownership remains provenance-based, and non-Windows transitions remove only a proven managed prefix. |
