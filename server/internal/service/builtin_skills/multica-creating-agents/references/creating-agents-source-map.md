...
| Workspace MCP library + assignment API | `internal/handler/workspace_mcp_api.go` | `GET /api/workspaces/{id}/mcp-servers` returns name / transport / last_probe only, never the entry, for any role; `POST`/`PUT`/`DELETE` on the library are owner/admin; `GET`/`POST`/`PUT .../enabled`/`DELETE /api/agents/{id}/mcp-servers` manage one agent's assignments and admit the agent owner or a workspace owner/admin. Every write refuses agent actors. Deleting a library entry sweeps its bindings in the same transaction (no FK) |
| Workspace MCP daemon probe | `internal/handler/workspace_mcp_probe.go`; `internal/daemon/mcp_probe.go`; CLI `cmd_workspace.go` `workspace mcp probe` | Owner/admin `POST .../mcp-servers/{id}/probe` enqueues a pending-work handshake on a chosen online runtime that advertises `mcp-probe-v1`. The daemon fetches the write-only config, runs initialize + tools/list, reports a redacted result, and the server persists `last_probe`. Members may see last_probe; they cannot trigger a spawn. |
