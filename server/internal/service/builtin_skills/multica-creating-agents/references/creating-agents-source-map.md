...
| Windows Codex sandbox argv default | `internal/handler/{agent,agent_custom_args,daemon,onboarding_shim}.go`; `internal/daemon/daemon.go`; `pkg/agent/{codex,codex_windows_sandbox_args}.go`; `packages/core/agents/codex-windows-sandbox.ts`; `packages/views/agents/components/tabs/custom-args-tab.tsx` | Daemon registration records the OS and whether fixed args own the setting; every agent-create path plus runtime-only updates normalize the exact managed `-c`, `windows.sandbox="unelevated"` prefix; the Codex spawn seam independently enforces it from GOOS/effective argv; explicit overrides win without duplication and non-Windows transitions remove only the managed prefix |
