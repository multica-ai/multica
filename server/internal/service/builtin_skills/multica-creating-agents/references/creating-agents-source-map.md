...
| Windows Codex sandbox argv default | migration `327`; `cmd/multica/cmd_agent{,_copy}.go`; `internal/handler/{agent,agent_custom_args,daemon,mika_agent,onboarding_shim}.go`; `internal/daemon/{daemon,execenv/codex_sandbox}.go`; `pkg/agent/{codex,codex_windows_sandbox_args}.go`; `packages/core/{agents,api}`; `packages/views/agents/components/tabs/custom-args-tab.tsx` | Daemon registration records the OS and whether fixed args or the shared copied config own the setting; every user-visible agent-create path plus runtime-only updates normalize the exact managed `-c`, `windows.sandbox="unelevated"` prefix and persist a separate provenance bit; web/CLI writes declare explicit replacements user-owned and copy flows remove only a proven managed prefix; the Codex spawn seam independently enforces precedence from GOOS/effective argv/copied config; explicit overrides win without duplication and non-Windows transitions remove only a proven managed prefix |
