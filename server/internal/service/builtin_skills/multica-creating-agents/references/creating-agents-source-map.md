...
| Windows Codex sandbox argv default | `internal/handler/{agent,agent_custom_args,daemon}.go`; `internal/daemon/daemon.go`; `pkg/agent/codex_windows_sandbox_args.go`; `packages/core/agents/codex-windows-sandbox.ts`; `packages/views/agents/components/tabs/custom-args-tab.tsx` | Daemon registration records the OS hint; create/update persistence and preview append the exact `-c`, `windows.sandbox="unelevated"` pair; launch independently enforces it from the daemon's GOOS; explicit overrides and non-Windows runtimes remain unchanged |
