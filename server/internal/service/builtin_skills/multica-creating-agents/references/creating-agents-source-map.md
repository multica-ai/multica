...
| Managed Windows Codex prefix provenance | `runAgentCopy`, `copiedAgentCustomArgs` | A source prefix is removed only when `codex_windows_sandbox_arg_managed` proves platform ownership; create re-derives the target default. An identical user-owned pair and explicit `--custom-args` remain authoritative | focused `cmd_agent_copy_test.go` cases |
