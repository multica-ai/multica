...
| Child → terminal (`done`/`blocked`/`cancelled`) notifies + wakes the resolved parent coordinator, gated by the stage barrier; blocked → done is a distinct completion wake. The system comment is a durable leased outbox with persisted target/origin/retry state; deferred batches recover only as complete groups, barrier generations deduplicate independent groups, and task creation is idempotent by comment ID. Unassigned mention-started squad flows atomically recheck parent assignment + active/current leader, and assignment/leader races are re-resolved by the worker | `server/internal/handler/issue_child_done.go` (`notifyParentOfChildDone`, `resolveChildDoneDispatchTarget`, `triggerChildDoneSquad`); `server/internal/handler/child_done_dispatch_worker.go`; `server/internal/service/task.go` (`EnqueueTaskForChildDone*`, `createAgentTaskWithSquadGuard`); `server/pkg/db/queries/{child_done_transition,comment,issue,squad,agent}.sql`; migrations `251`–`259` | parent-assignee-only wake |
