...
(`SPACE_KEY-NUMBER`, e.g. `MUL-2759`). The prefix is the issue's Space key — a space
owns its issue-number namespace, and each workspace has a default space whose key is
...
but see the reference-only rule below: a key that appears **only** as a bare mention
in the body is linked yet hidden from that list.
