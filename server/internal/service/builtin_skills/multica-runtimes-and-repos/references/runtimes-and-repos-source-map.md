...
- `server/internal/daemon/local_repo.go` canonicalizes and validates daemon-local `file://` sources before they enter the normal cache/checkout path; the resulting checkout keeps the local repository as `origin`, so task branches can be pushed back normally.
