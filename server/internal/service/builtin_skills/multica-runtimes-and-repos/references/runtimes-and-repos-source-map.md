...
- `server/internal/daemon/local_repo.go` canonicalizes and validates daemon-local `file://` sources before they enter the normal cache/checkout path; `server/internal/daemon/repocache/cache.go` disables push URLs for those sources.
