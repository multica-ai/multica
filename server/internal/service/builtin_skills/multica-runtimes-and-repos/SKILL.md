...
Host operators can create a custom runtime profile with `--activation-mode local` when the executable is intentionally available on only selected computers. Each selected computer must bind that profile with `runtime profile set-path`; daemons without a local binding skip it instead of publishing a failed runtime. `workspace` remains the default activation mode for commands that are expected to exist on every daemon.
