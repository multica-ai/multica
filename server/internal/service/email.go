...
func NewEmailService() *EmailService {
	...
	if from == "" {
		from = "noreply@tracker.3jtech.app"
	}
	...
	default:
		fmt.Println("EmailService: DEV mode — codes printed to stdout (set TRACKER_DEV_VERIFICATION_CODE in .env for a fixed local code)")
	}
...
func (s *EmailService) SendVerificationCode(to, code string) error {
	...
	if s.smtpHost != "" {
		return s.sendSMTP(to, "Your 3J Tracker verification code", body)
	}
	if s.client == nil {
	...
	params := &resend.SendEmailRequest{
		From:    s.fromEmail,
		To:      []string{to},
		Subject: "Your 3J Tracker verification code",
		Html:    body,
	}
...
func (s *EmailService) SendInvitationEmail(to, inviterName, workspaceName, invitationID string) error {
	...
	if appURL == "" {
		appURL = "https://tracker-v1.3jtech.app"
	}
...
func buildInvitationParams(from, to, inviterName, workspaceName, inviteURL string) *resend.SendEmailRequest {
	...
	return &resend.SendEmailRequest{
		From:    from,
		To:      []string{to},
		Subject: fmt.Sprintf("%s invited you to %s on 3J Tracker", subjectInviter, subjectWorkspace),
		Html: fmt.Sprintf(
			`<div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
				<h2>You're invited to join %s</h2>
				<p><strong>%s</strong> invited you to collaborate in the <strong>%s</strong> workspace on 3J Tracker.</p>
