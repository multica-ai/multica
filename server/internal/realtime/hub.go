...
// CEREBRO-PATCH(realtime-broadcast-to-workspace-users): sends a message only
// to clients in the given workspace whose userID is present in audience. When
// audience is nil, it behaves the same as BroadcastToWorkspace. Used for
// project-restricted / standalone-private events where only an explicit subset
// of members is allowed to see the update.
func (h *Hub) BroadcastToWorkspaceUsers(workspaceID string, audience map[string]bool, message []byte) {
	...
	key := sk(ScopeWorkspace, workspaceID)
	h.mu.RLock()
	clients := h.rooms[key]
	var slow []*Client
	var sent int64
