...
func (c *APIClient) UploadAttachmentTo(ctx context.Context, fileData []byte, filename string, target AttachmentTarget) (map[string]any, error) {
	...
	if resp.StatusCode >= 400 {
		respData, _ := io.ReadAll(io.LimitReader(resp.Body, 4096))
		return nil, fmt.Errorf("upload file returned %d: %s", resp.StatusCode, strings.TrimSpace(string(respData)))
	}
...
//
// CEREBRO-PATCH(cli-attachments): same-origin handling for relative
// downloadURLs (originally a cerebro fork-only feature) is now upstream's
// default behavior. The auth-header gate below uses upstream's broader
// isRelative test, which covers cerebro's previous "/"-prefixed paths plus
// other non-http(s) URLs.
