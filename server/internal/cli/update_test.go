...
func TestReleaseAssetCandidates(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "darwin versioned candidate",
...
func TestFindReleaseAsset(t *testing.T) {
	t.Run("finds the versioned asset", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "checksums.txt", BrowserDownloadURL: "ignore"},
			{Name: "cs-workflow-cli-1.2.3-darwin-amd64.tar.gz", BrowserDownloadURL: "new"},
		}
		...
		if got.Name != "cs-workflow-cli-1.2.3-darwin-amd64.tar.gz" {
			t.Fatalf("asset mismatch: got %q", got.Name)
		}
	})

	t.Run("returns error when versioned asset is absent", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "cs-workflow_linux_amd64.tar.gz", BrowserDownloadURL: "old"},
		}

		if _, err := findReleaseAsset(assets, "1.2.3", "linux", "amd64"); err == nil {
			t.Fatal("expected error when only a legacy-named asset exists")
		}
	})
...
func TestFindChecksumManifestAsset(t *testing.T) {
	t.Run("finds checksums.txt among assets", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "cs-workflow-cli-1.2.3-darwin-arm64.tar.gz"},
			{Name: "checksums.txt", BrowserDownloadURL: "https://example/checksums.txt"},
			{Name: "cs-workflow-cli-1.2.3-linux-amd64.tar.gz"},
		}
...
func TestParseChecksumManifest(t *testing.T) {
	manifest := []byte(strings.Join([]string{
		...
		"",
		"aaaa1111  cs-workflow-cli-1.2.3-darwin-arm64.tar.gz",
		"bbbb2222  cs-workflow-cli-1.2.3-darwin-amd64.tar.gz",
		"cccc3333\tmulti-tab-separator.tar.gz",
		"DDDD4444  cs-workflow_linux_amd64.tar.gz",
	}, "\n"))

	t.Run("returns lowercase sha for matched entry", func(t *testing.T) {
		got, err := parseChecksumManifest(manifest, "cs-workflow-cli-1.2.3-darwin-arm64.tar.gz")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if got != "aaaa1111" {
	...
	t.Run("downcases an uppercase entry", func(t *testing.T) {
		got, err := parseChecksumManifest(manifest, "cs-workflow_linux_amd64.tar.gz")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if got != "dddd4444" {
	...
	t.Run("skips blank lines and comments", func(t *testing.T) {
		...
		if _, err := parseChecksumManifest(manifest, "cs-workflow-cli-1.2.3-darwin-arm64.tar.gz"); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
}
