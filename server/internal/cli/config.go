...
const defaultCLIConfigPath = ".wallts/config.json"
...
// An empty profile returns the default path (~/.wallts/config.json).
// A named profile returns ~/.wallts/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".wallts", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.wallts/. A named profile returns ~/.wallts/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		return filepath.Join(home, ".wallts"), nil
	}
	return filepath.Join(home, ".wallts", "profiles", profile), nil
}
