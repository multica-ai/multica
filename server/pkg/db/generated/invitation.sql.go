...
RETURNING id, workspace_id, inviter_id, invitee_email, invitee_user_id, role, status, created_at, updated_at, expires_at, max_uses, use_count
`
...
SELECT wi.id, wi.workspace_id, wi.inviter_id, wi.invitee_email, wi.invitee_user_id, wi.role, wi.status, wi.created_at, wi.updated_at, wi.expires_at, wi.max_uses, wi.use_count,
...
const redeemShareableInvitation = `-- name: RedeemShareableInvitation :one
UPDATE workspace_invitation
...
func (q *Queries) RedeemShareableInvitation(ctx context.Context, id pgtype.UUID) (WorkspaceInvitation, error) {
	row := q.db.QueryRow(ctx, redeemShareableInvitation, id)
	var i WorkspaceInvitation
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
