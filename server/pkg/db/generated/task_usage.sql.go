...
const listIssueTaskUsageForTasks = `-- name: ListIssueTaskUsageForTasks :many
SELECT
    tu.task_id,
    tu.provider,
    tu.model,
...
type ListIssueTaskUsageForTasksParams struct {
	IssueID pgtype.UUID   `json:"issue_id"`
	TaskIds []pgtype.UUID `json:"task_ids"`
}

type ListIssueTaskUsageForTasksRow struct {
	TaskID           pgtype.UUID `json:"task_id"`
	Provider         string      `json:"provider"`
	Model            string      `json:"model"`
	InputTokens      int64       `json:"input_tokens"`
...
// pass. Uses idx_agent_task_queue_issue_id (migration 035) + the task_usage
// task_id index (migration 032).
func (q *Queries) ListIssueTaskUsageForTasks(ctx context.Context, arg ListIssueTaskUsageForTasksParams) ([]ListIssueTaskUsageForTasksRow, error) {
	rows, err := q.db.Query(ctx, listIssueTaskUsageForTasks, arg.IssueID, arg.TaskIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListIssueTaskUsageForTasksRow{}
	for rows.Next() {
		var i ListIssueTaskUsageForTasksRow
		if err := rows.Scan(
			&i.TaskID,
			&i.Provider,
			&i.Model,
