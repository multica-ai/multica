...
const lockDingTalkConnectorDeleteWriters = `-- name: LockDingTalkConnectorDeleteWriters :exec
...
func (q *Queries) LockDingTalkConnectorDeleteWriters(ctx context.Context, workspaceID pgtype.UUID) error {
	_, err := q.db.Exec(ctx, lockDingTalkConnectorDeleteWriters, workspaceID)
	return err
}

const lockDingTalkConnectorsForWorkspaceDelete = `-- name: LockDingTalkConnectorsForWorkspaceDelete :many
SELECT c.id
FROM dingtalk_connector c
JOIN dingtalk_workspace_grant g ON g.connector_id = c.id
WHERE g.workspace_id = $1
...
func (q *Queries) LockDingTalkConnectorsForWorkspaceDelete(ctx context.Context, workspaceID pgtype.UUID) ([]pgtype.UUID, error) {
	rows, err := q.db.Query(ctx, lockDingTalkConnectorsForWorkspaceDelete, workspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
