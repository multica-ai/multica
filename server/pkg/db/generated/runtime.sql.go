...
SELECT id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, visibility, profile_id, claim_window_start, claim_window_timezone FROM agent_runtime
...
const updateAgentRuntimeEditable = `-- name: UpdateAgentRuntimeEditable :one
UPDATE agent_runtime
SET visibility = CASE
    ...
    claim_window_start = CASE
        WHEN $3::boolean THEN $4::time
        ELSE claim_window_start
    END,
    claim_window_timezone = CASE
        WHEN $3::boolean THEN $5::text
...
type UpdateAgentRuntimeEditableParams struct {
	SetVisibility       bool        `json:"set_visibility"`
	Visibility          string      `json:"visibility"`
	SetClaimWindow      bool        `json:"set_claim_window"`
	ClaimWindowStart    pgtype.Time `json:"claim_window_start"`
	ClaimWindowTimezone pgtype.Text `json:"claim_window_timezone"`
	ID                  pgtype.UUID `json:"id"`
}

func (q *Queries) UpdateAgentRuntimeEditable(ctx context.Context, arg UpdateAgentRuntimeEditableParams) (AgentRuntime, error) {
	row := q.db.QueryRow(ctx, updateAgentRuntimeEditable,
		arg.SetVisibility,
		arg.Visibility,
		arg.SetClaimWindow,
		arg.ClaimWindowStart,
		arg.ClaimWindowTimezone,
		arg.ID,
	)
	var i AgentRuntime
	err := row.Scan(
...
DO UPDATE SET
    ...
    last_seen_at = now(),
    updated_at = now()
RETURNING id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, visibility, profile_id, claim_window_start, claim_window_timezone, (xmax = 0) AS inserted
`
