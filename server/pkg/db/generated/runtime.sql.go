...
const updateAgentRuntimeCheapModel = `-- name: UpdateAgentRuntimeCheapModel :one
UPDATE agent_runtime
...
type UpdateAgentRuntimeCheapModelParams struct {
	ID         pgtype.UUID `json:"id"`
...
func (q *Queries) UpdateAgentRuntimeCheapModel(ctx context.Context, arg UpdateAgentRuntimeCheapModelParams) (AgentRuntime, error) {
	row := q.db.QueryRow(ctx, updateAgentRuntimeCheapModel, arg.ID, arg.CheapModel)
	var i AgentRuntime
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const updateAgentRuntimeSandboxPolicy = `-- name: UpdateAgentRuntimeSandboxPolicy :one
UPDATE agent_runtime
...
func (q *Queries) UpdateAgentRuntimeSandboxPolicy(ctx context.Context, arg UpdateAgentRuntimeSandboxPolicyParams) (AgentRuntime, error) {
	row := q.db.QueryRow(ctx, updateAgentRuntimeSandboxPolicy, arg.ID, arg.SandboxPolicy)
	var i AgentRuntime
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const updateAgentRuntimeToolsConfig = `-- name: UpdateAgentRuntimeToolsConfig :one
UPDATE agent_runtime
...
// configuration.
func (q *Queries) UpdateAgentRuntimeToolsConfig(ctx context.Context, arg UpdateAgentRuntimeToolsConfigParams) (AgentRuntime, error) {
	row := q.db.QueryRow(ctx, updateAgentRuntimeToolsConfig, arg.ID, arg.ToolsConfig)
	var i AgentRuntime
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
DO UPDATE SET
    ...
    last_seen_at = now(),
    updated_at = now()
RETURNING id, workspace_id, daemon_id, name, runtime_mode, provider, status, device_info, metadata, last_seen_at, created_at, updated_at, owner_id, legacy_daemon_id, sandbox_enabled, visibility, profile_id, paused_at, unpause_at, pause_reason, current_account_id, presentation_mode, capabilities, cli_version, tools_config, sandbox_policy, auto_pause_count, cheap_model, (xmax = 0) AS inserted
`
