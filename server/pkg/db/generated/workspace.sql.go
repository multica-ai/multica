...
VALUES ($1, $2, $3, $4, $5)
RETURNING id, name, slug, description, settings, created_at, updated_at, context, repos, issue_prefix, issue_counter, avatar_url, scout_agent_id
`
...
SELECT id, name, slug, description, settings, created_at, updated_at, context, repos, issue_prefix, issue_counter, avatar_url, scout_agent_id FROM workspace
WHERE id = $1
`
...
const setWorkspaceScoutAgent = `-- name: SetWorkspaceScoutAgent :one
UPDATE workspace SET scout_agent_id = $2, updated_at = now()
WHERE id = $1
RETURNING id, name, slug, description, settings, created_at, updated_at, context, repos, issue_prefix, issue_counter, avatar_url, scout_agent_id
`

type SetWorkspaceScoutAgentParams struct {
	ID           pgtype.UUID `json:"id"`
	ScoutAgentID pgtype.UUID `json:"scout_agent_id"`
}

func (q *Queries) SetWorkspaceScoutAgent(ctx context.Context, arg SetWorkspaceScoutAgentParams) (Workspace, error) {
	row := q.db.QueryRow(ctx, setWorkspaceScoutAgent, arg.ID, arg.ScoutAgentID)
	var i Workspace
	err := row.Scan(
		&i.ID,
		&i.Name,
...
const clearWorkspaceScoutAgent = `-- name: ClearWorkspaceScoutAgent :one
UPDATE workspace SET scout_agent_id = NULL, updated_at = now()
WHERE id = $1
RETURNING id, name, slug, description, settings, created_at, updated_at, context, repos, issue_prefix, issue_counter, avatar_url, scout_agent_id
`

func (q *Queries) ClearWorkspaceScoutAgent(ctx context.Context, id pgtype.UUID) (Workspace, error) {
	row := q.db.QueryRow(ctx, clearWorkspaceScoutAgent, id)
	var i Workspace
	err := row.Scan(
		&i.ID,
		&i.Name,
