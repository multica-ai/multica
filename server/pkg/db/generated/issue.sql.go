...
const countUnresolvedBlockedByDependencies = `-- name: CountUnresolvedBlockedByDependencies :one
...
func (q *Queries) CountUnresolvedBlockedByDependencies(ctx context.Context, issueID pgtype.UUID) (int32, error) {
	row := q.db.QueryRow(ctx, countUnresolvedBlockedByDependencies, issueID)
	var blocked_by_count int32
	err := row.Scan(&blocked_by_count)
	return blocked_by_count, err
}
...
const listIssuesBlockedByIssue = `-- name: ListIssuesBlockedByIssue :many
SELECT blocked.id, blocked.workspace_id, blocked.title, blocked.description, blocked.status, blocked.priority, blocked.assignee_type, blocked.assignee_id, blocked.creator_type, blocked.creator_id, blocked.parent_issue_id, blocked.acceptance_criteria, blocked.context_refs, blocked.position, blocked.due_date, blocked.created_at, blocked.updated_at, blocked.number, blocked.project_id
FROM issue_dependency dep
...
func (q *Queries) ListIssuesBlockedByIssue(ctx context.Context, dependsOnIssueID pgtype.UUID) ([]Issue, error) {
	rows, err := q.db.Query(ctx, listIssuesBlockedByIssue, dependsOnIssueID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listUnresolvedBlockedByCounts = `-- name: ListUnresolvedBlockedByCounts :many
...
type ListUnresolvedBlockedByCountsRow struct {
	IssueID        pgtype.UUID `json:"issue_id"`
	BlockedByCount int32       `json:"blocked_by_count"`
}

func (q *Queries) ListUnresolvedBlockedByCounts(ctx context.Context, workspaceID pgtype.UUID) ([]ListUnresolvedBlockedByCountsRow, error) {
	rows, err := q.db.Query(ctx, listUnresolvedBlockedByCounts, workspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListUnresolvedBlockedByCountsRow{}
	for rows.Next() {
		var i ListUnresolvedBlockedByCountsRow
		if err := rows.Scan(&i.IssueID, &i.BlockedByCount); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
