...
const getProjectIDsByIssues = `-- name: GetProjectIDsByIssues :many
...
type GetProjectIDsByIssuesRow struct {
...
func (q *Queries) GetProjectIDsByIssues(ctx context.Context, issueIds []pgtype.UUID) ([]GetProjectIDsByIssuesRow, error) {
	rows, err := q.db.Query(ctx, getProjectIDsByIssues, issueIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetProjectIDsByIssuesRow{}
	for rows.Next() {
		var i GetProjectIDsByIssuesRow
		if err := rows.Scan(&i.IssueID, &i.ProjectID); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
