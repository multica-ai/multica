...
RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, archived_at, archived_by, origin_type, origin_id, first_executed_at, start_date, metadata
`
...
const countArchivedIssues = `-- name: CountArchivedIssues :one
...
WHERE i.workspace_id = $1
  ...
  AND ($16::uuid IS NULL OR i.archived_by = $16)
`

type CountArchivedIssuesParams struct {
	WorkspaceID       pgtype.UUID   `json:"workspace_id"`
...
func (q *Queries) CountArchivedIssues(ctx context.Context, arg CountArchivedIssuesParams) (int64, error) {
	row := q.db.QueryRow(ctx, countArchivedIssues,
		arg.WorkspaceID,
		arg.Status,
		arg.Priority,
		arg.Priorities,
...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, archived_at, archived_by, origin_type, origin_id, first_executed_at, start_date, metadata FROM issue
WHERE workspace_id = $1
...
WHERE i.workspace_id = $1
  ...
  AND ($18::uuid IS NULL OR i.archived_by = $18)
...
const unarchiveIssue = `-- name: UnarchiveIssue :one
UPDATE issue SET
    archived_at = NULL,
    archived_by = NULL
WHERE id = $1 AND workspace_id = $2
...
func (q *Queries) UnarchiveIssue(ctx context.Context, arg UnarchiveIssueParams) (Issue, error) {
	row := q.db.QueryRow(ctx, unarchiveIssue, arg.ID, arg.WorkspaceID)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
