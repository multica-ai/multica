...
const failStaleAutopilotRuns = `-- name: FailStaleAutopilotRuns :many
...
type FailStaleAutopilotRunsRow struct {
	ID          pgtype.UUID `json:"id"`
	AutopilotID pgtype.UUID `json:"autopilot_id"`
}
...
func (q *Queries) FailStaleAutopilotRuns(ctx context.Context, timeoutSecs float64) ([]FailStaleAutopilotRunsRow, error) {
	rows, err := q.db.Query(ctx, failStaleAutopilotRuns, timeoutSecs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []FailStaleAutopilotRunsRow{}
	for rows.Next() {
		var i FailStaleAutopilotRunsRow
		if err := rows.Scan(&i.ID, &i.AutopilotID); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
