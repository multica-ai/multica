...
INSERT INTO autopilot_run (
    ...
    $6, $7,
    $8, $9
) RETURNING id, autopilot_id, trigger_id, source, status, issue_id, task_id, triggered_at, completed_at, failure_reason, trigger_payload, result, created_at, squad_id, planned_at, webhook_delivery_id, scope_claim
`
...
type CreateAutopilotRunParams struct {
	...
	ScopeClaim        pgtype.Text        `json:"scope_claim"`
}
...
const getActiveAutopilotRunByScopeClaim = `-- name: GetActiveAutopilotRunByScopeClaim :one
SELECT id, autopilot_id, trigger_id, source, status, issue_id, task_id, triggered_at, completed_at, failure_reason, trigger_payload, result, created_at, squad_id, planned_at, webhook_delivery_id, scope_claim FROM autopilot_run
WHERE autopilot_id = $1
  AND scope_claim = $2
  AND source = 'webhook'
...
type GetActiveAutopilotRunByScopeClaimParams struct {
	AutopilotID pgtype.UUID `json:"autopilot_id"`
	ScopeClaim  pgtype.Text `json:"scope_claim"`
}

// Returns the in-flight webhook run holding a caller-supplied scope claim.
func (q *Queries) GetActiveAutopilotRunByScopeClaim(ctx context.Context, arg GetActiveAutopilotRunByScopeClaimParams) (AutopilotRun, error) {
	row := q.db.QueryRow(ctx, getActiveAutopilotRunByScopeClaim, arg.AutopilotID, arg.ScopeClaim)
	var i AutopilotRun
	err := row.Scan(
		&i.ID,
		&i.AutopilotID,
...
const updateAutopilotRunTriggerPayload = `-- name: UpdateAutopilotRunTriggerPayload :one
UPDATE autopilot_run
...
type UpdateAutopilotRunTriggerPayloadParams struct {
	ID             pgtype.UUID `json:"id"`
	TriggerPayload []byte      `json:"trigger_payload"`
}

func (q *Queries) UpdateAutopilotRunTriggerPayload(ctx context.Context, arg UpdateAutopilotRunTriggerPayloadParams) (AutopilotRun, error) {
	row := q.db.QueryRow(ctx, updateAutopilotRunTriggerPayload, arg.ID, arg.TriggerPayload)
	var i AutopilotRun
	err := row.Scan(
		&i.ID,
		&i.AutopilotID,
