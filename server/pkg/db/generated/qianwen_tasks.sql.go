...
const listQianwenVisibleCurrentTasks = `-- name: ListQianwenVisibleCurrentTasks :many
...
WITH RECURSIVE caller AS MATERIALIZED (
    ...
    WHERE installation.id = $1::uuid
      ...
      AND binding.multica_user_id = $4::uuid
      AND binding.channel_user_id = $5::text
      AND binding.config ->> 'open_uuid' = $6::text
...
type ListQianwenVisibleCurrentTasksParams struct {
	...
	CursorValid     bool               `json:"cursor_valid"`
	CursorCreatedAt pgtype.Timestamptz `json:"cursor_created_at"`
	CursorID        pgtype.UUID        `json:"cursor_id"`
	PageSize        int32              `json:"page_size"`
}

type ListQianwenVisibleCurrentTasksRow struct {
...
func (q *Queries) ListQianwenVisibleCurrentTasks(ctx context.Context, arg ListQianwenVisibleCurrentTasksParams) ([]ListQianwenVisibleCurrentTasksRow, error) {
	rows, err := q.db.Query(ctx, listQianwenVisibleCurrentTasks,
		arg.InstallationID,
		arg.ConnectionID,
		arg.AccessTokenHash,
		arg.MulticaUserID,
	...
	defer rows.Close()
	items := []ListQianwenVisibleCurrentTasksRow{}
	for rows.Next() {
		var i ListQianwenVisibleCurrentTasksRow
		if err := rows.Scan(
			&i.HasTask,
			&i.TaskID,
			&i.RequestID,
