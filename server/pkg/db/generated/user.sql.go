...
VALUES ($1, $2, $3)
RETURNING id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, account_status
`
...
SELECT id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, account_status FROM "user"
WHERE id = $1
`
...
const setUserAccountStatus = `-- name: SetUserAccountStatus :one
UPDATE "user" SET
    account_status = $2,
    updated_at = now()
WHERE id = $1
...
func (q *Queries) SetUserAccountStatus(ctx context.Context, arg SetUserAccountStatusParams) (User, error) {
	row := q.db.QueryRow(ctx, setUserAccountStatus, arg.ID, arg.AccountStatus)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Name,
