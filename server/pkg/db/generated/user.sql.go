...
VALUES ($1, $2, $3)
RETURNING id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, language, profile_description, timezone, pushover_user_key, pushover_login_codes_enabled
`
...
func (q *Queries) CreateUser(ctx context.Context, arg CreateUserParams) (User, error) {
	...
	err := row.Scan(
		...
		&i.Timezone,
		&i.PushoverUserKey,
		&i.PushoverLoginCodesEnabled,
	)
	return i, err
}

const getUser = `-- name: GetUser :one
SELECT id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, language, profile_description, timezone, pushover_user_key, pushover_login_codes_enabled FROM "user"
WHERE id = $1
`
...
const updateUserPushoverSettings = `-- name: UpdateUserPushoverSettings :one
UPDATE "user" SET
    pushover_user_key = $2,
...
type UpdateUserPushoverSettingsParams struct {
	ID                        pgtype.UUID `json:"id"`
...
func (q *Queries) UpdateUserPushoverSettings(ctx context.Context, arg UpdateUserPushoverSettingsParams) (User, error) {
	row := q.db.QueryRow(ctx, updateUserPushoverSettings, arg.ID, arg.PushoverUserKey, arg.PushoverLoginCodesEnabled)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Name,
