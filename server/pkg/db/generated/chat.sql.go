...
const getActiveIssueChatSessionByCreator = `-- name: GetActiveIssueChatSessionByCreator :one
SELECT id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, issue_id FROM chat_session
...
type GetActiveIssueChatSessionByCreatorParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	CreatorID   pgtype.UUID `json:"creator_id"`
	IssueID     pgtype.UUID `json:"issue_id"`
}

func (q *Queries) GetActiveIssueChatSessionByCreator(ctx context.Context, arg GetActiveIssueChatSessionByCreatorParams) (ChatSession, error) {
	row := q.db.QueryRow(ctx, getActiveIssueChatSessionByCreator, arg.WorkspaceID, arg.CreatorID, arg.IssueID)
	var i ChatSession
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
