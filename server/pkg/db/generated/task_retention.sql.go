...
const countCompletedTaskRetentionCandidates = `-- name: CountCompletedTaskRetentionCandidates :one
...
type CountCompletedTaskRetentionCandidatesParams struct {
...
func (q *Queries) CountCompletedTaskRetentionCandidates(ctx context.Context, arg CountCompletedTaskRetentionCandidatesParams) (int64, error) {
	row := q.db.QueryRow(ctx, countCompletedTaskRetentionCandidates, arg.AsOf, arg.CompletedDays)
	var candidate_rows int64
	err := row.Scan(&candidate_rows)
	return candidate_rows, err
}

const countFailedTaskRetentionCandidates = `-- name: CountFailedTaskRetentionCandidates :one
SELECT count(*) AS candidate_rows
FROM agent_task_queue t
...
type CountFailedTaskRetentionCandidatesParams struct {
	AsOf       pgtype.Timestamptz `json:"as_of"`
	FailedDays int32              `json:"failed_days"`
}
...
func (q *Queries) CountFailedTaskRetentionCandidates(ctx context.Context, arg CountFailedTaskRetentionCandidatesParams) (int64, error) {
	row := q.db.QueryRow(ctx, countFailedTaskRetentionCandidates, arg.AsOf, arg.FailedDays)
	var candidate_rows int64
	err := row.Scan(&candidate_rows)
	return candidate_rows, err
}

const deleteCompletedTaskRetentionBatch = `-- name: DeleteCompletedTaskRetentionBatch :many
WITH victims AS (
    SELECT t.id
    FROM agent_task_queue t
    WHERE t.status = 'completed'
...
WHERE d.id = v.id
  ...
  AND d.completed_at < ($1::timestamptz) - make_interval(days => $2::int)
  AND NOT EXISTS (
      SELECT 1 FROM autopilot_run ar
...
type DeleteCompletedTaskRetentionBatchParams struct {
	AsOf          pgtype.Timestamptz `json:"as_of"`
	CompletedDays int32              `json:"completed_days"`
...
func (q *Queries) DeleteCompletedTaskRetentionBatch(ctx context.Context, arg DeleteCompletedTaskRetentionBatchParams) ([]pgtype.UUID, error) {
	rows, err := q.db.Query(ctx, deleteCompletedTaskRetentionBatch, arg.AsOf, arg.CompletedDays, arg.BatchSize)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const deleteFailedTaskRetentionBatch = `-- name: DeleteFailedTaskRetentionBatch :many
WITH victims AS (
    SELECT t.id
    FROM agent_task_queue t
    WHERE t.status = 'failed'
...
type DeleteFailedTaskRetentionBatchParams struct {
	AsOf       pgtype.Timestamptz `json:"as_of"`
	FailedDays int32              `json:"failed_days"`
	BatchSize  int32              `json:"batch_size"`
}
...
func (q *Queries) DeleteFailedTaskRetentionBatch(ctx context.Context, arg DeleteFailedTaskRetentionBatchParams) ([]pgtype.UUID, error) {
	rows, err := q.db.Query(ctx, deleteFailedTaskRetentionBatch, arg.AsOf, arg.FailedDays, arg.BatchSize)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getTaskRetentionAsOf = `-- name: GetTaskRetentionAsOf :one
...
func (q *Queries) GetTaskRetentionAsOf(ctx context.Context) (pgtype.Timestamptz, error) {
	row := q.db.QueryRow(ctx, getTaskRetentionAsOf)
