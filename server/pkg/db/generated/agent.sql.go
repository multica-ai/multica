...
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, verify_agent_id, starter_prompts
`
...
RETURNING task.id, task.agent_id, task.issue_id, task.status, task.priority, task.dispatched_at, task.started_at, task.completed_at, task.result, task.error, task.created_at, task.context, task.runtime_id, task.session_id, task.work_dir, task.trigger_comment_id, task.chat_session_id, task.autopilot_run_id, task.attempt, task.max_attempts, task.parent_task_id, task.failure_reason, task.trigger_summary, task.force_fresh_session, task.is_leader_task, task.wait_reason, task.initiator_user_id, task.handoff_note, task.prepare_lease_expires_at, task.squad_id, task.runtime_mcp_overlay, task.escalation_for_task_id, task.fire_at, task.originator_user_id, task.runtime_connected_apps, task.coalesced_comment_ids, task.delivered_comment_ids, task.chat_input_task_id, task.chat_finalize_deferred_at, task.originator_source, task.delegated_from_task_id, task.retry_of_task_id, task.rerun_of_task_id, task.rule_version_id, task.trigger_evidence_kind, task.trigger_evidence_ref_id, task.accountable_user_id, task.session_rollout_missing, task.retired_session_id, task.quick_actions_disabled, task.regenerate_quick_actions_for, task.branch_name, task.durable_work_dir, task.channel_context_revision, task.concrete_model, task.requested_concrete_model, task.auto_rerun_count, task.help_signal
`
...
const clearAgentVerifyAgent = `-- name: ClearAgentVerifyAgent :one
...
func (q *Queries) ClearAgentVerifyAgent(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentVerifyAgent, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
SELECT
    ...
    p.channel_context_revision, COALESCE($6::text, p.concrete_model), COALESCE($7::text, p.requested_concrete_model),
    COALESCE($8::int, p.auto_rerun_count),
...
type CreateRetryTaskParams struct {
	...
	ConcreteModel          pgtype.Text        `json:"concrete_model"`
	RequestedConcreteModel pgtype.Text        `json:"requested_concrete_model"`
	AutoRerunCount         pgtype.Int4        `json:"auto_rerun_count"`
	NewTaskID              pgtype.UUID        `json:"new_task_id"`
}
...
const createVerifierTask = `-- name: CreateVerifierTask :one
INSERT INTO agent_task_queue (
...
type CreateVerifierTaskParams struct {
	AgentID              pgtype.UUID `json:"agent_id"`
	RuntimeID            pgtype.UUID `json:"runtime_id"`
	IssueID              pgtype.UUID `json:"issue_id"`
	Priority             int32       `json:"priority"`
...
func (q *Queries) CreateVerifierTask(ctx context.Context, arg CreateVerifierTaskParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, createVerifierTask,
		arg.AgentID,
		arg.RuntimeID,
		arg.IssueID,
		arg.Priority,
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, verify_agent_id, starter_prompts FROM agent
WHERE id = $1
`
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, squad_id, runtime_mcp_overlay, escalation_for_task_id, fire_at, originator_user_id, runtime_connected_apps, coalesced_comment_ids, delivered_comment_ids, chat_input_task_id, chat_finalize_deferred_at, originator_source, delegated_from_task_id, retry_of_task_id, rerun_of_task_id, rule_version_id, trigger_evidence_kind, trigger_evidence_ref_id, accountable_user_id, session_rollout_missing, retired_session_id, quick_actions_disabled, regenerate_quick_actions_for, branch_name, durable_work_dir, channel_context_revision, concrete_model, requested_concrete_model, auto_rerun_count, help_signal FROM agent_task_queue
WHERE id = $1
`
...
const setAgentTaskHelpSignal = `-- name: SetAgentTaskHelpSignal :exec
UPDATE agent_task_queue
SET help_signal = $2
WHERE id = $1
`

type SetAgentTaskHelpSignalParams struct {
	ID         pgtype.UUID `json:"id"`
	HelpSignal []byte      `json:"help_signal"`
}
...
func (q *Queries) SetAgentTaskHelpSignal(ctx context.Context, arg SetAgentTaskHelpSignalParams) error {
	_, err := q.db.Exec(ctx, setAgentTaskHelpSignal, arg.ID, arg.HelpSignal)
	return err
}
...
UPDATE agent SET
    ...
    verify_agent_id = COALESCE($21, verify_agent_id),
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, verify_agent_id, starter_prompts
`
