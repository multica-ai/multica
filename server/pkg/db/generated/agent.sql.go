...
func (q *Queries) ArchiveAgent(ctx context.Context, arg ArchiveAgentParams) (Agent, error) {
	...
	err := row.Scan(
		...
		&i.ServiceTier,
		&i.CodexWindowsSandboxArgManaged,
	)
	return i, err
}
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, codex_windows_sandbox_arg_managed FROM agent
WHERE id = $1
`
...
UPDATE agent SET
    ...
    codex_windows_sandbox_arg_managed = COALESCE($19, codex_windows_sandbox_arg_managed),
...
type UpdateAgentParams struct {
	...
	ThinkingLevel                 pgtype.Text `json:"thinking_level"`
	ServiceTier                   pgtype.Text `json:"service_tier"`
	CodexWindowsSandboxArgManaged pgtype.Bool `json:"codex_windows_sandbox_arg_managed"`
	ComposioToolkitAllowlist      []string    `json:"composio_toolkit_allowlist"`
}
