...
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, starter_prompts, task_token_templates
`
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, starter_prompts, task_token_templates FROM agent
WHERE id = $1
`
...
const updateAgentTaskTokenTemplates = `-- name: UpdateAgentTaskTokenTemplates :one
UPDATE agent
...
type UpdateAgentTaskTokenTemplatesParams struct {
	ID                 pgtype.UUID `json:"id"`
...
func (q *Queries) UpdateAgentTaskTokenTemplates(ctx context.Context, arg UpdateAgentTaskTokenTemplatesParams) (Agent, error) {
	row := q.db.QueryRow(ctx, updateAgentTaskTokenTemplates, arg.ID, arg.TaskTokenTemplates)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
