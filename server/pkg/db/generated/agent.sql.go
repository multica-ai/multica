...
const listDueDeferredRuntimeRestoreTasks = `-- name: ListDueDeferredRuntimeRestoreTasks :many
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, not_before, squad_id, escalation_for_task_id, fire_at FROM agent_task_queue
WHERE runtime_id = $1
  AND status = 'deferred'
  AND fire_at <= now()
...
func (q *Queries) ListDueDeferredRuntimeRestoreTasks(ctx context.Context, runtimeID pgtype.UUID) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, listDueDeferredRuntimeRestoreTasks, runtimeID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const cancelDeferredRuntimeRestoreForAgent = `-- name: CancelDeferredRuntimeRestoreForAgent :many
UPDATE agent_task_queue
SET status = 'cancelled', completed_at = now(), prepare_lease_expires_at = NULL
WHERE agent_id = $1
  AND status = 'deferred'
...
func (q *Queries) CancelDeferredRuntimeRestoreForAgent(ctx context.Context, agentID pgtype.UUID) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, cancelDeferredRuntimeRestoreForAgent, agentID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const createDeferredRuntimeRestoreTask = `-- name: CreateDeferredRuntimeRestoreTask :one
INSERT INTO agent_task_queue (
    agent_id, runtime_id, issue_id, status, priority, context, fire_at
)
VALUES (
    $1, $2, $3, 'deferred', 0, $4, $5
)
RETURNING id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, not_before, squad_id, escalation_for_task_id, fire_at
`

type CreateDeferredRuntimeRestoreTaskParams struct {
	AgentID   pgtype.UUID        `json:"agent_id"`
	RuntimeID pgtype.UUID        `json:"runtime_id"`
	IssueID   pgtype.UUID        `json:"issue_id"`
	Context   []byte             `json:"context"`
...
func (q *Queries) CreateDeferredRuntimeRestoreTask(ctx context.Context, arg CreateDeferredRuntimeRestoreTaskParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, createDeferredRuntimeRestoreTask,
		arg.AgentID,
		arg.RuntimeID,
		arg.IssueID,
		arg.Context,
...
const completeRuntimeRestoreTask = `-- name: CompleteRuntimeRestoreTask :one
UPDATE agent_task_queue
...
func (q *Queries) CompleteRuntimeRestoreTask(ctx context.Context, id pgtype.UUID) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, completeRuntimeRestoreTask, id)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
...
const updateAgentTaskRuntime = `-- name: UpdateAgentTaskRuntime :exec
UPDATE agent_task_queue
SET runtime_id = $2
WHERE id = $1
`

type UpdateAgentTaskRuntimeParams struct {
	ID        pgtype.UUID `json:"id"`
	RuntimeID pgtype.UUID `json:"runtime_id"`
}

func (q *Queries) UpdateAgentTaskRuntime(ctx context.Context, arg UpdateAgentTaskRuntimeParams) error {
	_, err := q.db.Exec(ctx, updateAgentTaskRuntime, arg.ID, arg.RuntimeID)
	return err
}
