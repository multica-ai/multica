...
RETURNING t.id, t.agent_id, t.issue_id, t.status, t.priority, t.dispatched_at, t.started_at, t.completed_at, t.result, t.error, t.created_at, t.context, t.runtime_id, t.session_id, t.work_dir, t.trigger_comment_id, t.chat_session_id, t.autopilot_run_id, t.attempt, t.max_attempts, t.parent_task_id, t.failure_reason, t.trigger_summary, t.force_fresh_session, t.is_leader_task, t.wait_reason, t.channel_id, t.channel_message_id, t.channel_thread_id, t.channel_reply_to_id, t.initiator_user_id
`
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, custom_env_copied_pending, thinking_level, service_tier, system_template_id, personal_template_id, skip_system_template, skip_personal_template FROM agent
WHERE id = $1
`
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, channel_id, channel_message_id, channel_thread_id, channel_reply_to_id, initiator_user_id FROM agent_task_queue
WHERE id = $1
`
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, custom_env_copied_pending, thinking_level, service_tier FROM agent
WHERE id = ANY($1::uuid[])
`
...
func (q *Queries) GetAgentsByIDs(ctx context.Context, dollar_1 []pgtype.UUID) ([]Agent, error) {
	rows, err := q.db.Query(ctx, getAgentsByIDs, dollar_1)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const hasChannelMentionTaskForMessageAndAgent = `-- name: HasChannelMentionTaskForMessageAndAgent :one
SELECT count(*) > 0 AS has_task FROM agent_task_queue
WHERE channel_message_id = $1 AND agent_id = $2
`

type HasChannelMentionTaskForMessageAndAgentParams struct {
	ChannelMessageID pgtype.UUID `json:"channel_message_id"`
	AgentID          pgtype.UUID `json:"agent_id"`
}

func (q *Queries) HasChannelMentionTaskForMessageAndAgent(ctx context.Context, arg HasChannelMentionTaskForMessageAndAgentParams) (bool, error) {
	row := q.db.QueryRow(ctx, hasChannelMentionTaskForMessageAndAgent, arg.ChannelMessageID, arg.AgentID)
	var has_task bool
...
func (q *Queries) ListAgentsByOwner(ctx context.Context, arg ListAgentsByOwnerParams) ([]Agent, error) {
	rows, err := q.db.Query(ctx, listAgentsByOwner, arg.WorkspaceID, arg.OwnerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listChannelMentionTasksForMessages = `-- name: ListChannelMentionTasksForMessages :many
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, channel_id, channel_message_id, channel_thread_id, channel_reply_to_id, initiator_user_id FROM agent_task_queue
...
func (q *Queries) ListChannelMentionTasksForMessages(ctx context.Context, messageIds []pgtype.UUID) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, listChannelMentionTasksForMessages, messageIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listTasksByIssueForOwner = `-- name: ListTasksByIssueForOwner :many
SELECT atq.id, atq.agent_id, atq.issue_id, atq.status, atq.priority, atq.dispatched_at, atq.started_at, atq.completed_at, atq.result, atq.error, atq.created_at, atq.context, atq.runtime_id, atq.session_id, atq.work_dir, atq.trigger_comment_id, atq.chat_session_id, atq.autopilot_run_id, atq.attempt, atq.max_attempts, atq.parent_task_id, atq.failure_reason, atq.trigger_summary, atq.force_fresh_session, atq.is_leader_task, atq.wait_reason, atq.channel_id, atq.channel_message_id, atq.channel_thread_id, atq.channel_reply_to_id, atq.initiator_user_id FROM agent_task_queue atq
JOIN agent a ON a.id = atq.agent_id
...
type ListTasksByIssueForOwnerParams struct {
	IssueID pgtype.UUID `json:"issue_id"`
	OwnerID pgtype.UUID `json:"owner_id"`
}

func (q *Queries) ListTasksByIssueForOwner(ctx context.Context, arg ListTasksByIssueForOwnerParams) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, listTasksByIssueForOwner, arg.IssueID, arg.OwnerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
UPDATE agent SET
    ...
    service_tier = COALESCE($17, service_tier),
    custom_env_copied_pending = COALESCE($18, custom_env_copied_pending),
    skip_system_template = COALESCE($19, skip_system_template),
    skip_personal_template = COALESCE($20, skip_personal_template),
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, custom_env_copied_pending, thinking_level, service_tier, system_template_id, personal_template_id, skip_system_template, skip_personal_template
`
...
type UpdateAgentParams struct {
	...
	SkipSystemTemplate     pgtype.Bool `json:"skip_system_template"`
	SkipPersonalTemplate   pgtype.Bool `json:"skip_personal_template"`
}
...
func (q *Queries) UpdateAgent(ctx context.Context, arg UpdateAgentParams) (Agent, error) {
	row := q.db.QueryRow(ctx, updateAgent,
		...
		arg.ServiceTier,
		arg.CustomEnvCopiedPending,
		arg.SkipSystemTemplate,
		arg.SkipPersonalTemplate,
	)
	var i Agent
	err := row.Scan(
		&i.ID,
