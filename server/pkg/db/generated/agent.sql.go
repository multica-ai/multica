...
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, hermes_profile
`
...
const clearAgentHermesProfile = `-- name: ClearAgentHermesProfile :one
UPDATE agent SET hermes_profile = NULL, updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, hermes_profile
`

func (q *Queries) ClearAgentHermesProfile(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentHermesProfile, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
type CreateAgentParams struct {
	...
	HermesProfile      pgtype.Text `json:"hermes_profile"`
}
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, hermes_profile FROM agent
WHERE id = $1
`
...
UPDATE agent SET
    ...
    hermes_profile = COALESCE($16, hermes_profile),
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, hermes_profile
`
