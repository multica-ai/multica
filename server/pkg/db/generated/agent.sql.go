...
RETURNING id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, last_heartbeat_at, idempotency_key, scheduled_at
`
...
INSERT INTO agent_task_queue (agent_id, runtime_id, issue_id, status, priority, trigger_comment_id, idempotency_key)
...
func (q *Queries) CreateRetryTask(ctx context.Context, arg CreateRetryTaskParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, createRetryTask, arg.ID, arg.ScheduledAt)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, last_heartbeat_at, idempotency_key, scheduled_at FROM agent_task_queue
WHERE id = $1
`
...
const getCompletedTaskByIdempotencyKey = `-- name: GetCompletedTaskByIdempotencyKey :one
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, last_heartbeat_at, idempotency_key, scheduled_at FROM agent_task_queue
WHERE idempotency_key = $1 AND status = 'completed'
LIMIT 1
`

func (q *Queries) GetCompletedTaskByIdempotencyKey(ctx context.Context, idempotencyKey pgtype.UUID) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, getCompletedTaskByIdempotencyKey, idempotencyKey)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
...
SELECT atq.id, atq.agent_id, atq.issue_id, atq.status, atq.priority, atq.dispatched_at, atq.started_at, atq.completed_at, atq.result, atq.error, atq.created_at, atq.context, atq.runtime_id, atq.session_id, atq.work_dir, atq.trigger_comment_id, atq.chat_session_id, atq.autopilot_run_id, atq.attempt, atq.max_attempts, atq.parent_task_id, atq.failure_reason, atq.last_heartbeat_at, atq.idempotency_key, atq.scheduled_at FROM agent_task_queue atq
JOIN agent a ON a.id = atq.agent_id
WHERE a.workspace_id = $1
