...
const listLocalPathDaemonIDsByProjects = `-- name: ListLocalPathDaemonIDsByProjects :many
...
type ListLocalPathDaemonIDsByProjectsRow struct {
	ProjectID pgtype.UUID `json:"project_id"`
...
func (q *Queries) ListLocalPathDaemonIDsByProjects(ctx context.Context, projectIds []pgtype.UUID) ([]ListLocalPathDaemonIDsByProjectsRow, error) {
	rows, err := q.db.Query(ctx, listLocalPathDaemonIDsByProjects, projectIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListLocalPathDaemonIDsByProjectsRow{}
	for rows.Next() {
		var i ListLocalPathDaemonIDsByProjectsRow
		if err := rows.Scan(&i.ProjectID, &i.DaemonID); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
