...
WHERE atq.runtime_id = r.id
  ...
  AND r.last_seen_at < now() - make_interval(secs => @runtime_stale_secs::double precision)
  AND (
    (r.provider = 'codex'  AND atq.started_at < now() - make_interval(secs => @codex_liveness_secs::double precision))
    OR
    (r.provider = 'claude' AND atq.started_at < now() - make_interval(secs => @claude_liveness_secs::double precision))
  )
