...
INSERT INTO agent (
    ...
    instructions, custom_env, custom_args, mcp_config, model, hermes_profile
...
UPDATE agent SET
    ...
    hermes_profile = COALESCE(sqlc.narg('hermes_profile'), hermes_profile),
    updated_at = now()
WHERE id = $1
RETURNING *;
...
-- name: ClearAgentHermesProfile :one
UPDATE agent SET hermes_profile = NULL, updated_at = now()
WHERE id = $1
RETURNING *;
