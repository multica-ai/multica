...
-- property/status definitions, and quick actions carry NO FK to workspace, so —
-- unlike the CASCADE-backed tables the DELETE below sweeps — they are not
-- cleaned up implicitly. Remove their workspace-owned rows here so they commit
-- or roll back atomically with the workspace row.
