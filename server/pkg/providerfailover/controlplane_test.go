...
func TestTaskSpawnTargetIsStableAcrossFormatting(t *testing.T) {
	t.Parallel()
	a := TaskSpawnTarget(" parent ", " Build   API\nworker ", "agent", "agent-1", "project-1", 2)
	b := TaskSpawnTarget("parent", "Build API worker", "agent", "agent-1", "project-1", 2)
	if a == "" || a != b {
		t.Fatalf("stable child target mismatch: %q != %q", a, b)
	}
	if a == TaskSpawnTarget("parent", "Build UI worker", "agent", "agent-1", "project-1", 2) {
		t.Fatal("distinct sibling titles must not collapse")
	}
	if got := TaskSpawnTarget("", "Build API worker", "agent", "agent-1", "project-1", 2); got != "" {
		t.Fatalf("missing parent must fail closed, got %q", got)
	}
}

func TestStagePromotionTargetIsPerChild(t *testing.T) {
	t.Parallel()
	a := StagePromotionTarget("parent", "child-a", 2)
	if a == "" || a != StagePromotionTarget(" parent ", " child-a ", 2) {
		t.Fatalf("stage target must be stable, got %q", a)
	}
	if a == StagePromotionTarget("parent", "child-b", 2) {
		t.Fatal("two children in the same stage must remain distinct")
	}
}
