	...
	go func() {
		...
		for scanner.Scan() {
			...
			case "result":
				...
				if evt.Stats != nil {
					b.accumulateUsage(usage, evt.Stats, geminiFallbackUsageModel(evt.Model, opts.Model))
				}
			}
		}
...
func (b *geminiBackend) accumulateUsage(usage map[string]TokenUsage, stats *geminiStreamStats, fallbackModel string) {
	...
	if len(stats.Models) == 0 && (stats.InputTokens > 0 || stats.OutputTokens > 0) {
		u := usage[fallbackModel]
		u.InputTokens += int64(stats.InputTokens)
		u.OutputTokens += int64(stats.OutputTokens)
		usage[fallbackModel] = u
	}
}

func geminiFallbackUsageModel(eventModel, optsModel string) string {
	if eventModel != "" {
		return eventModel
	}
	if optsModel != "" {
