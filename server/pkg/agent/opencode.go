...
var opencodeBlockedArgs = map[string]blockedArgMode{
	...
	"--auto":    blockedStandalone, // CEREBRO-PATCH(opencode-permission-flag): daemon manages non-interactive permission prompts using the installed CLI contract.
}
...
func (b *opencodeBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	args := []string{"run", "--format", "json", "--auto"} // CEREBRO-PATCH(opencode-permission-flag): use the installed runtime's real non-interactive flag.
