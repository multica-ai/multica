...
// daemon↔QwenPaw communication contract.
var qwenpawBlockedArgs = map[string]blockedArgMode{
	"acp": blockedStandalone,
}

// qwenpawBackend implements Backend by spawning `qwenpaw acp` and communicating
// via the standard ACP JSON-RPC 2.0 transport over stdin/stdout.
// QwenPaw 1.1.3+ exposes a full ACP server (`qwenpaw acp`) that supports
// initialize, session/new, session/load, session/prompt with streaming updates,
// session/set_model, and cancel — the same protocol surface as hermes/kiro.
...
func (b *qwenpawBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// Forward stderr to the daemon log *and* sniff provider-level errors so
	// we can surface them in the task result — same pattern as hermes/kiro.
