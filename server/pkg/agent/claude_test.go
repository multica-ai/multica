...
func TestClaudeExecuteSurfacesStderrWhenChildExitsEarly(t *testing.T) {
	...
	// Fake claude binary: writes a canonical V8-abort line to stderr, then
	// exits non-zero before emitting any stream-json to stdout. This is the
	// exact failure mode that motivated PR #1674 — without sampling
	// stderrBuf.Tail() after cmd.Wait() returns, Result.Error would be a
	// useless "exit status 3".
...
func TestClaudeExecuteResumeStdinWriteTimeout(t *testing.T) {
	...
	fakePath := filepath.Join(t.TempDir(), "claude")
	// Replace the shell with sleep so CommandContext cancellation kills the
	// process holding the stdio pipes instead of leaving a child sleep behind.
	script := "#!/bin/sh\nexec sleep 5\n"
	writeTestExecutable(t, fakePath, []byte(script))
	...
	defer cancel()
	session, err := backend.Execute(ctx, strings.Repeat("x", 2*1024*1024), ExecOptions{
	...
	case result := <-session.Result:
		...
		if !strings.Contains(result.Error, "timed out waiting for claude stdin write") {
			t.Fatalf("expected stdin write timeout error, got %q", result.Error)
		}
	case <-time.After(3 * time.Second):
		t.Fatal("timeout waiting for stdin write failure result")
	}
}
