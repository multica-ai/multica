...
while IFS= read -r line; do
  ...
  case "$line" in
    *'"method":"initialize"'*)
      ...
      ;;
    *'"method":"session/new"'*)
      printf '{"jsonrpc":"2.0","id":%s,"result":{"sessionId":"ses_new","models":{"availableModels":[{"modelId":"grok-build","name":"Grok Build","description":""},{"modelId":"grok-4","name":"Grok 4","description":""}]}}}\n' "$id"
      ;;
    *'"method":"session/load"'*)
...
func TestGrokBlockedArgsFiltering(t *testing.T) {
	...
	session, err := backend.Execute(ctx, "task", ExecOptions{
		...
		CustomArgs: []string{"agent", "stdio", "--always-approve", "--yolo", "headless", "-p", "--output-format", "json", "--permission-mode", "default", "--rules", "extra"},
	})
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
	...
	wantPrefix := []string{"agent", "--always-approve", "stdio"}
	...
	for _, once := range wantPrefix {
		if c := countTokens(lines, once); c != 1 {
			t.Errorf("expected exactly one %q, got %d (full: %q)", once, c, joined)
		}
	}
	for _, blocked := range []string{"headless", "-p", "--output-format", "json", "--permission-mode", "default", "--yolo"} {
	...
	if !strings.Contains(joined, "--rules extra") {
