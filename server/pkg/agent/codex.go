...
const (
	walltsCodexMcpBeginMarker = "# BEGIN wallts-managed mcp_servers (do not edit; regenerated by daemon)"
	walltsCodexMcpEndMarker   = "# END wallts-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(walltsCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(walltsCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = walltsCodexMcpBeginMarker + "\n" + walltsCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(walltsCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(walltsCodexMcpEndMarker)
	sb.WriteString("\n")
	...
	go func() {
		...
		_, err := c.request(runCtx, "initialize", map[string]any{
			"clientInfo": map[string]any{
				"name":    "wallts-agent-sdk",
				"title":   "Wallts Agent SDK",
				"version": "0.2.0",
			},
