...
// ACP transport; `--always-approve` (alias `--yolo`) is daemon-owned so
// headless Multica runs do not block on interactive permission prompts.
// Switching into headless/serve/leader/print modes would break the
// daemon↔grok ACP contract.
...
// grokBackend implements Backend by spawning `grok agent --always-approve stdio`
// and communicating via the standard ACP (Agent Client Protocol) JSON-RPC 2.0
// transport over stdin/stdout.
//
// This targets xAI's Grok Build CLI (https://x.ai / the `grok` binary). Grok
// Build exposes ACP via `grok agent stdio` (flags such as --always-approve and
// --model belong on the `agent` command, before the transport subcommand).
// We reuse the existing Hermes/Kimi/Kiro/Qoder/Trae ACP client (hermesClient)
// with provider-specific launch args and tool-name normalization.
//
// Capability notes are intentionally conservative: we attempt session/load on
// resume and session/set_model when a model is requested; if a particular Grok
// Build version lacks those methods, the run fails with a clear error rather
// than silently continuing on the wrong session/model.
...
func (b *grokBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// Custom args are appended after the fixed prefix and filtered so they
	// cannot strip ACP mode or disable auto-approval.
	grokArgs := append(
		[]string{"agent", "--always-approve", "stdio"},
