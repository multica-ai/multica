...
// Gemini, Pi, Cursor, Kimi, Kiro, Csc). It mirrors the happy-cli AgentBackend
...
type Config struct {
	ExecutablePath string            // path to CLI binary (claude, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor, kimi, kiro-cli, csc)
	Env            map[string]string // extra environment variables
	Logger         *slog.Logger
}

// New creates a Backend for the given agent type.
// Supported types: "claude", "codex", "copilot", "opencode", "openclaw", "hermes", "gemini", "pi", "cursor", "kimi", "kiro", "csc".
func New(agentType string, cfg Config) (Backend, error) {
	...
	switch agentType {
	case "claude":
		return &claudeBackend{streamJSONBackend{cfg: cfg, name: "claude", defaultBin: "claude"}}, nil
	case "codex":
	...
	case "csc":
		return &cscBackend{streamJSONBackend{cfg: cfg, name: "csc", defaultBin: "csc"}}, nil
	default:
		return nil, fmt.Errorf("unknown agent type: %q (supported: claude, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor, kimi, kiro, csc)", agentType)
	}
}
