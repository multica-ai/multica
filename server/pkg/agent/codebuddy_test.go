...
// CodeBuddy carries the same content-block shape as Claude and was, for a
// while, the adapter that had neither the unwrapping nor the image marker —
// the helper was right and this caller simply never used it. These assertions
// exist so that cannot recur silently.
func TestCodebuddyToolResultOutputWiring(t *testing.T) {
	t.Parallel()

	imageBlock := []map[string]any{{
		"type": "image",
		"source": map[string]any{
			"type": "base64", "media_type": "image/png", "data": "CCCCDDDD",
		},
	}}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			b.handleUser(codebuddySDKMessage{
				Type: "user",
				Message: mustMarshal(t, codebuddyMessageContent{
					Role: "user",
					Content: []codebuddyContentBlock{{
			...
			case m := <-ch:
				...
				if tc.wantImage && !strings.Contains(m.Output, "CCCCDDDD") {
