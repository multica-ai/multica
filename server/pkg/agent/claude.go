...
func buildClaudeArgs(prompt string, opts ExecOptions, logger *slog.Logger) ([]string, claudePromptTransport) {
	...
	args := []string{"-p"}
	// Single-shot prompts under the argv budget are passed as a positional
	// argument so the daemon never writes task prompt bytes to stdin during
	// Claude startup. Resume and very large prompts use stream-json stdin.
...
// placeholder so daemon.log stays scannable. Stdin runs do not embed the prompt
// in argv, so the original args are returned.
func claudeArgsForLog(args []string, promptTransport claudePromptTransport) []string {
	...
	copy(out, args)
	out[1] = fmt.Sprintf("<prompt:%d bytes>", len([]byte(args[1])))
	return out
}
