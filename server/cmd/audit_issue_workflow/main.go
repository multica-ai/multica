// audit_issue_workflow verifies the additive workflow projection after a
...
func run() error {
	...
	rows, err := db.New(pool).ListIssueWorkflowConsistency(ctx)
	if err != nil {
		return fmt.Errorf("audit issue workflow: %w", err)
	}
	...
	if !consistent {
		return errors.New("issue workflow consistency check failed")
	}
	return nil
}
