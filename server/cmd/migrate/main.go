...
var preMigrationHooks = map[string]preMigrationHook{
	...
	"274_agent_task_retry_of_unique":                        runRetryOfUniqueIndexHook,
...
// retryOfUniqueIndexName is the unique index migration 274 builds; CreateRetryTask's
// ON CONFLICT (retry_of_task_id) resolves against it.
const retryOfUniqueIndexName = "idx_agent_task_retry_of_task_id_unique"

// runRetryOfUniqueIndexHook makes migration 274 safe to (re)run (MUL-4809 §4.1 P0-2).
// 274 builds a UNIQUE index CONCURRENTLY on agent_task_queue.retry_of_task_id, and the
...
//
//  1. Nothing constrained the column before 274, so pre-existing duplicate
...
//     274 as applied while every auto-retry still fails with 42P10 (no unique or
//     exclusion constraint matching the ON CONFLICT specification).
//
// This hook runs before every attempt at 274 — a migration that fails is never recorded,
...
func runRetryOfUniqueIndexHook(ctx context.Context, pool *pgxpool.Pool) error {
	...
	if err := pool.QueryRow(ctx, `
		...
		return fmt.Errorf("retry_of unique pre-274 hook: inspect index: %w", err)
	}
	if invalid {
		...
		if _, err := pool.Exec(ctx, `DROP INDEX IF EXISTS `+retryOfUniqueIndexName); err != nil {
			return fmt.Errorf("retry_of unique pre-274 hook: drop invalid index: %w", err)
		}
	...
	if err := pool.QueryRow(ctx, `
		...
		return fmt.Errorf("retry_of unique pre-274 hook: duplicate scan: %w", err)
	}
	if dupes > 0 {
		return fmt.Errorf("retry_of unique pre-274 hook: %d parent task(s) have more than one "+
