...
var preMigrationHooks = map[string]preMigrationHook{
	...
	"211_agent_task_retry_of_unique":                        runRetryOfUniqueIndexHook,
}

// retryOfUniqueIndexName is the unique index migration 211 builds; CreateRetryTask's
// ON CONFLICT (retry_of_task_id) resolves against it.
const retryOfUniqueIndexName = "idx_agent_task_retry_of_task_id_unique"

// runRetryOfUniqueIndexHook makes migration 211 safe to (re)run (MUL-4809 §4.1 P0-2).
// 211 builds a UNIQUE index CONCURRENTLY on agent_task_queue.retry_of_task_id, and the
...
//
//  1. Nothing constrained the column before 211, so pre-existing duplicate
...
//     211 as applied while every auto-retry still fails with 42P10 (no unique or
//     exclusion constraint matching the ON CONFLICT specification).
//
// This hook runs before every attempt at 211 — a migration that fails is never recorded,
...
func runRetryOfUniqueIndexHook(ctx context.Context, pool *pgxpool.Pool) error {
	// to_regclass resolves through the same search_path the DROP below uses, so we can
	// never inspect one schema's index and drop another's.
	var invalid bool
	if err := pool.QueryRow(ctx, `
		...
		return fmt.Errorf("retry_of unique pre-211 hook: inspect index: %w", err)
	}
	if invalid {
		...
		if _, err := pool.Exec(ctx, `DROP INDEX IF EXISTS `+retryOfUniqueIndexName); err != nil {
			return fmt.Errorf("retry_of unique pre-211 hook: drop invalid index: %w", err)
		}
	...
	if err := pool.QueryRow(ctx, `
		SELECT count(*) FROM (
			SELECT retry_of_task_id
			FROM agent_task_queue
			WHERE retry_of_task_id IS NOT NULL
			GROUP BY retry_of_task_id
			HAVING count(*) > 1
			LIMIT 100
		) d`).Scan(&dupes); err != nil {
		return fmt.Errorf("retry_of unique pre-211 hook: duplicate scan: %w", err)
	}
	if dupes > 0 {
		return fmt.Errorf("retry_of unique pre-211 hook: %d parent task(s) have more than one "+
