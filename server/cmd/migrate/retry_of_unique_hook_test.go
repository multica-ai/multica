...
// MUL-4809 §4.1 P0-2 — migration 274 re-run safety.
//
// 274 builds a UNIQUE index CONCURRENTLY on agent_task_queue.retry_of_task_id, and
...
// would record 274 as applied while every auto-retry still fails with 42P10. These tests
...
// retryOfMigrationOpts writes the real 274 body to a temp dir and wires runOptions to the
// sandbox bookkeeping table plus the PRODUCTION hook map.
func retryOfMigrationOpts(t *testing.T, schema string) runOptions {
	t.Helper()
	dir := t.TempDir()
	path := filepath.Join(dir, "274_agent_task_retry_of_unique.up.sql")
	...
	if err := os.WriteFile(path, []byte(body), 0o600); err != nil {
		t.Fatalf("write 274 migration: %v", err)
	}
	return runOptions{
		Direction:             "up",
		Files:                 []string{path},
...
func retryOfAppliedCount(t *testing.T, pool *pgxpool.Pool, table string) int {
	...
	if err := pool.QueryRow(context.Background(),
		...
		"274_agent_task_retry_of_unique").Scan(&n); err != nil {
...
// before 274, so a rolling deploy could carry duplicates. The hook must hard-fail the run
