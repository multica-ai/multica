...
func NewRouter(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, pushSvc *service.PushService) chi.Router {
	r, _ := NewRouterWithOptions(pool, hub, bus, analyticsClient, rdb, pushSvc, RouterOptions{})
	return r
}
...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, pushSvc *service.PushService, opts RouterOptions) (chi.Router, *handler.Handler) {
	queries := db.New(pool)
	...
	cerebroEvalsHandler := cerebroevals.NewHandler(pool).WithRunExecutor(evalExecutor).WithBlockingGateAuthorizer(evalBlockingGateAdapter{svc: cerebroGroupPermissionsHandler.Service}) // CEREBRO-PATCH(cerebro-evals-routes): FIR-3308/FIR-3493 eval catalog + server runner. CEREBRO-PATCH(cerebro-evals-blocking-gate): FIR-3496 restrict blocking bindings.
	...
	// CEREBRO-PATCH(workflow-session-stamper-wire): FIR-2283 followup point b — deterministic phase badge/rename on run_skill completion; nil-safe seam.
	h.TaskService.WorkflowSessionStamper = cerebroworkflows.NewSessionPhaseStamper(pool)
	...
	// CEREBRO-PATCH(cerebro-mini-app-runtime-ingress): FIR-3172 signed service-only bundle and deployment callbacks.
	...
	r.Group(func(r chi.Router) {
		...
		r.Route("/api/workspaces", func(r chi.Router) {
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				r.Group(func(r chi.Router) {
					...
					// CEREBRO-PATCH(cerebro-tool-policy-changes): FIR-3091 punkt 8 fase 2 per-permission change log (admin/owner-gated in handler).
					r.Get("/tool-policy/changes", cerebroToolPolicyHandler.Changes)
					// CEREBRO-PATCH(cerebro-tool-policy-usage): FIR-3091 punkt 8 fase 3 per-permission usage log (admin/owner-gated in handler).
					r.Get("/tool-policy/usage", cerebroToolPolicyHandler.Usage)
		...
		r.Group(func(r chi.Router) {
			...
			r.Route("/api/agents", func(r chi.Router) {
				...
				// CEREBRO-PATCH(cerebro-agent-office-observability-routes): FIR-1775 Phase 4 —
				// workspace-wide context observability overview (change frequency, approvers, drift).
				r.Get("/context/observability", cerebroAgentOfficeHandler.ObservabilityWorkspace)
			...
			mountCerebroModelRegistryRoutes(r, cerebroModelRegistryHandler) // CEREBRO-PATCH(cerebro-model-registry-routes): FIR-2698 model-registry governance routes (wiring in cerebro_model_registry_routes.go).
			...
			r.Route("/api/runtimes", func(r chi.Router) {
				...
				r.Get("/latest-version", h.GetLatestRuntimeVersion) // CEREBRO-PATCH(runtime-distribution-version): FIR-3064 server-owned fork release version.
			...
			// CEREBRO-PATCH(channel-message-search-mine): FIR-407 — per-user search across the caller's own channels/DMs (global Cmd+K).
			r.Get("/api/cerebro/my/channel-messages/search", h.SearchMyChannelMessages)
			...
			r.Route("/api/cerebro/apps", func(r chi.Router) {
				...
				// CEREBRO-PATCH(cerebro-mini-app-detail-route): FIR-3172 app detail screen reads one app and its versions.
				r.Get("/{id}", cerebroAppsHandler.Get)
				r.With(cerebroAppsHandler.RequireCapability("apps.manage")).Get("/{id}/versions/{version}/files", cerebroAppsHandler.VersionFiles)
				...
				r.With(cerebroAppsHandler.RequireCapability("apps.manage")).Post("/{id}/retry-deployment", cerebroAppsHandler.RetryDeployment)
				r.With(cerebroAppsHandler.RequireCapability("apps.manage")).Post("/{id}/rollback", cerebroAppsHandler.Rollback)
				r.With(cerebroAppsHandler.RequireCapability("apps.manage")).Post("/{id}/disable", cerebroAppsHandler.Disable)
				...
				r.Post("/{id}/invoke", cerebroAppsHandler.Invoke)
			...
			// CEREBRO-PATCH(cerebro-folder-grants-routes): FIR-1590 Collections per-folder access grants (Valgt her/Arvet).
			r.Route("/api/cerebro/folder-grants", func(r chi.Router) {
