...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	// Azure DevOps webhook — authenticated via ?secret=<auto-generated> query param.
	r.Post("/api/webhooks/azuredevops", h.HandleADOWebhook)
		...
		r.Route("/api/workspaces", func(r chi.Router) {
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				r.Group(func(r chi.Router) {
					...
					// ADO: list is member-visible (same pattern as GitHub).
					r.Get("/azuredevops/installations", h.ListADOInstallations)
				})
				// Admin-level access
				r.Group(func(r chi.Router) {
				...
				// Azure DevOps integration — create/delete require admin+;
				// the list endpoint is in the member-level group above.
				r.Group(func(r chi.Router) {
					r.Use(middleware.RequireWorkspaceRoleFromURL(queries, "id", "owner", "admin"))
					r.Post("/azuredevops/installations", h.CreateADOInstallation)
					r.Delete("/azuredevops/installations/{installationId}", h.DeleteADOInstallation)
				})
			})
		})
		...
		r.Group(func(r chi.Router) {
			...
			r.Route("/api/issues", func(r chi.Router) {
				...
				r.Route("/{id}", func(r chi.Router) {
					...
					r.Get("/ado-pull-requests", h.ListADOPullRequestsForIssue)
				})
			})
