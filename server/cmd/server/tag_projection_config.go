...
func tagHTTPAssertionVerifierFromEnv() (*tagaccess.HTTPAssertionVerifier, error) {
	...
	if keyID == "" || encodedKey == "" {
		return nil, errors.New("Tag HTTP assertion authentication configuration is incomplete")
	}
	key, err := base64.StdEncoding.Strict().DecodeString(encodedKey)
	if err != nil || len(key) < sha256.Size {
		return nil, errors.New("Tag HTTP assertion authentication key is invalid")
	}
