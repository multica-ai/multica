...
func TestRerunIssueSetsForceFreshSession(t *testing.T) {
	...
	task, err := taskService.RerunIssue(ctx, pgtype.UUID{Bytes: parseUUIDBytes(issueID), Valid: true}, pgtype.UUID{}, pgtype.UUID{}, false) // CEREBRO-PATCH(resume-failed-run): FIR-3901
	if err != nil {
...
func TestRerunIssueTargetsSourceTaskAgent(t *testing.T) {
	...
	task, err := taskService.RerunIssue(
		...
		pgtype.UUID{},
		false, // CEREBRO-PATCH(resume-failed-run): FIR-3901 — rerun takes a resume flag.
	)
	if err != nil {
		t.Fatalf("RerunIssue failed: %v", err)
	}
