...
func init() {
	...
	projectResourceAddCmd.Flags().String("execution-mode", "", "Shortcut: how runs share the directory — in_place (default, one run at a time), shared (concurrent in the original directory), or worktree (isolated git worktrees) (only used when --type local_directory)")
	...
	projectResourceUpdateCmd.Flags().String("execution-mode", "", "Shortcut: new execution mode — in_place, shared, or worktree (local_directory)")
