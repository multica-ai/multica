...
func tryResolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"WALLTS_APP_URL", "FRONTEND_ORIGIN"} {
...
var loginCmd = &cobra.Command{
	...
	Long:  "Log in to Wallts, then automatically discover and watch all your workspaces.",
	// Up to one positional is accepted so `--token mul_...`
...
// "prompt me interactively", preserving the legacy `wallts login --token`
// no-value form alongside the documented `--token mul_...`
// value form.
const tokenPromptSentinel = "\x00prompt"

func init() {
	loginCmd.Flags().String("token", "", "Authenticate using a personal access token (`mul_...` user PAT). Pass `--token mul_...` to supply it inline, or `--token` alone to be prompted interactively.")
	// NoOptDefVal lets `--token` (no value) keep its old prompt-mode behavior
	// while `--token mul_...` and the `=value` form
	// consume the value normally.
	loginCmd.Flags().Lookup("token").NoOptDefVal = tokenPromptSentinel
	loginCmd.Flags().String(callbackHostFlag, "", "Host the auth callback URL points at (auto-detected from the server's route when empty). Use this for reverse-proxy / FQDN setups where auto-detection picks the wrong interface.")
}
...
func runLogin(cmd *cobra.Command, args []string) error {
	...
	if err := autoWatchWorkspaces(cmd); err != nil {
		...
		fmt.Fprintf(os.Stderr, "Run 'wallts workspace list' and 'wallts workspace watch <id>' to set up manually.\n")
		return nil
	}

	fmt.Fprintf(os.Stderr, "\n→ Run 'wallts daemon start' to start your local agent runtime.\n")
	return nil
}
...
func autoWatchWorkspaces(cmd *cobra.Command) error {
	...
	if len(workspaces) > 1 {
		fmt.Fprintln(os.Stderr, "\nUse 'wallts workspace switch <id|slug>' to change the default workspace.")
	}
...
func waitForWorkspaceCreation(cmd *cobra.Command, client *cli.APIClient) ([]struct {
	...
	if appURL == "" {
		...
		fmt.Fprintln(os.Stderr, "\nNo workspaces found.")
		fmt.Fprintln(os.Stderr, "Create a workspace in the web dashboard, then run 'wallts login' again.")
		return nil, nil
	}
