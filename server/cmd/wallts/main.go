...
var rootCmd = &cobra.Command{
	Use:   "wallts",
	Short: "Wallts CLI — local agent runtime and management tool",
	Long:  "Work seamlessly with Wallts from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("wallts {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Wallts server URL (env: WALLTS_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: WALLTS_WORKSPACE_ID)")
	...
	sessionCmd.GroupID = groupAdditional
	...
	rootCmd.AddCommand(failoverGroupCmd)
