...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate wallts with Wallts",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("WALLTS_TOKEN")); v != "" {
		return v
	}
...
func resolveAppURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'wallts setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'wallts login' to authenticate.")
		return nil
	}
...
<title>Wallts — Authenticated</title>
<style>
