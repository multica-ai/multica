...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("WALLTS_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("WALLTS_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("WALLTS_WORKSPACE_ID")
	agentName := os.Getenv("WALLTS_AGENT_NAME")
	taskID := os.Getenv("WALLTS_TASK_ID")
