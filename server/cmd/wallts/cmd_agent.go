...
// custom_env values — `wallts agent list / get / update` no longer
...
func init() {
	...
	// `wallts agent env set <id>` — that path is owner/admin-only,
...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, WALLTS_SERVER_URL env, or 'wallts config set server_url <url>'")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("WALLTS_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("WALLTS_TASK_ID"); taskID != "" {
...
func resolveServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "server-url", "WALLTS_SERVER_URL", "")
	if val != "" {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'wallts setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
// inside a daemon-managed agent task (daemon sets WALLTS_AGENT_ID and
// WALLTS_TASK_ID in the agent env). In that context the workspace must be
// provided explicitly by the daemon — falling back to user-global
// ~/.wallts/config.json would let the agent act on whatever workspace the
...
func inAgentExecutionContext() bool {
	return os.Getenv("WALLTS_AGENT_ID") != "" || os.Getenv("WALLTS_TASK_ID") != ""
}

func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "WALLTS_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		if inAgentExecutionContext() {
			return "", fmt.Errorf("workspace_id is required: WALLTS_WORKSPACE_ID must be set by the daemon in agent execution context (no fallback to user config)")
		}
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set WALLTS_WORKSPACE_ID env, or run 'wallts config set workspace_id <id>'")
	}
	return id, nil
}
...
func runAgentCreateFromTemplate(cmd *cobra.Command, client *cli.APIClient, name, runtimeID, slug string) error {
	...
	// Matches the timeout used by `wallts skill import` (cmd_skill.go).
...
func runAgentUpdate(cmd *cobra.Command, args []string) error {
	...
	if len(body) == 0 {
		return fmt.Errorf("no fields to update; use --name, --description, --instructions, --runtime-id, --runtime-config, --model, --custom-args, --visibility, --status, or --max-concurrent-tasks (env vars now live behind `wallts agent env set <id>`)")
	}
