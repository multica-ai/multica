...
- **`docs/agents/permission-architecture.md`** — the *structural* companion to `permission-system.md`: which permission **models** actually exist (HTTP role/membership middleware, the tool-policy chain "5 interfaces", group capabilities, and per-feature stores + the grant resolver), the **complete register of every gate that lives only in code** and is NOT in the 5 interfaces, and the **decision to remove Persona** (with the death-plan ordering: service now, grant table is a migration target). **Read this before adding a permission interface, moving a gate into the tool-policy chain, or touching Persona / the grant store.** Do not assume the 5 settable interfaces are the whole story — most access control is still code-only. Update it in the same PR if you change the permission model.
