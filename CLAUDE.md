...
- **`docs/agents/permission-architecture.md`** — the *structural* companion to `permission-system.md`: which permission **models** actually exist (HTTP role/membership middleware, the tool-policy chain "5 interfaces", group capabilities, and per-feature stores + the grant resolver), the **complete register of every gate that lives only in code** and is NOT in the 5 interfaces, and **§5: the capability engine (grant resolver + permgate, FIR-2193) documented as a usable function** — Persona-the-service is removed, but the engine is live and load-bearing (credentials/repo/approval-inbox) and consolidates onto the tool-policy chain via the engine-flip FIR-1512, never an ad-hoc drop. **Read this before adding a permission interface, moving a gate into the tool-policy chain, or touching the grant store / capability engine.** Do not assume the 5 settable interfaces are the whole story — most access control is still code-only. Update it in the same PR if you change the permission model.
...
- **`docs/agents/agent-detail-tabs.md`** — how a tab gets onto the agent detail page: the two pages behind `cerebro_agent_page_redesign`, the single tab registry in `packages/cerebro-agent-tabs`, `?tab=` deep links, and where an agent's runtime is changed when `cerebro_agent_setup_capabilities` is on. **Read this before adding, moving, or removing an agent detail tab** — registering a tab anywhere else is how the two pages drifted apart (FIR-4840).
- **`docs/agents/inbox-message-types.md`** — what shows up in the inbox: the 3 row kinds (issue notifications, channels/DM, agent chat) and the much richer `InboxItemType` notification taxonomy underneath (mentions, reminders, agent activity, …), plus how per-row actions stay identical across kinds (desktop 3-dot menu + shared `MobileRowActions` swipe). **Read this before touching the inbox list, a `Cerebro*RowActions` component, or adding a notification type.** Covers the reminder family (`reminder` / `due_date_reminder` / `start_date_reminder`) and how note @-mentions surface as `mentioned` items (via `details.note_id`). If you add a type, row kind, or change the row-action contract, update this doc in the same PR.
